# coding=utf-8
"""
Qwen3 MOE model identical to qwen.py except the post-attention RMSNorm is
replaced by the rmsnorm_hbm NKI kernel (kernels/moe_fused_tkg/rmsnorm_nki.py).

Everything else (attention, MoE module, CTE/TKG branching, compiler args,
state-dict conversion, env vars) matches qwen.py.

Notes on the kernel swap:
- The kernel hard-codes _H=2048 and _EPS=1e-6. Hidden size must be 2048
  (Qwen3-30B-A3B at TP=4) and rms_norm_eps must equal 1e-6 (which Qwen3
  uses). The kernel applies (x * rsqrt(mean(x^2) + eps)) * gamma in fp32.
- We always apply post_attention_layernorm externally via the NKI kernel,
  so the moe_fused_nki_kernel_enabled path that passes rmsnorm into the
  fused MoE TKG kernel is disabled (to make the swap meaningful).
"""
...
# Hidden dim is pinned by the NKI rmsnorm kernel (rmsnorm_nki.py uses _H=2048).
_MOE_H = 2048
# LNC=2 grid for rmsnorm_hbm on trn3. The kernel runs identical work on both
# cores; only prg_id==0 writes the output.
_LNC = 2
...
def _post_attn_rmsnorm_nki(layernorm_module, hidden_states):
    """
    Replacement for `self.post_attention_layernorm(hidden_states)` that uses
    the rmsnorm_hbm NKI kernel. Reads gamma from the layernorm module's
    `weight` parameter so the trace still pulls from the same state-dict
    entry — no state-dict conversion changes needed.
    """
    orig_shape = hidden_states.shape
    inp_2d = hidden_states.reshape(-1, _MOE_H).to(torch.bfloat16).contiguous()
    gamma = layernorm_module.weight.reshape(1, _MOE_H).to(torch.bfloat16).contiguous()
    out_2d = rmsnorm_hbm[_LNC](inp_2d, gamma)
    return out_2d.reshape(orig_shape)


class NeuronQwen3MoeDecoderLayer(nn.Module):
    """
    Identical to qwen.py's NeuronQwen3MoeDecoderLayer except the
    post-attention RMSNorm call goes through the rmsnorm_hbm NKI kernel.

    The post_attention_layernorm module itself is still constructed (its
    `weight` parameter is what holds gamma in the state dict and is what
    we pass to the kernel), but its forward is never invoked.
    """
    ...
    def __init__(self, config: Qwen3MoeInferenceConfig, layer_idx: int):
        ...
        # Use NxDI default MoE module (no init_tkg_module) so the post-attn
        # rmsnorm is always applied externally — that's what we replace
        # with the NKI kernel.
    ...
    def forward(
        ...
        **kwargs,
        ...
        # MoE
        residual = hidden_states
        # Replace post_attention_layernorm with the rmsnorm_hbm NKI kernel.
        hidden_states = _post_attn_rmsnorm_nki(self.post_attention_layernorm, hidden_states)
