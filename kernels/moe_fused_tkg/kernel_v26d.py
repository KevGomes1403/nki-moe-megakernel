...
kernel_v26d — Plan D: Direct PSUM read for up + Fused affinity scale
=============================================================
Based on v25e (Plan E: Combined micro-optimizations).

Changes vs v25e:
  Change 1 (Plan D): Read up directly from PSUM in tensor_tensor.
    Instead of: PSUM→SBUF(up_sb) + tensor_tensor(inter_f32) + activation(inter_bf16)
    Do: tensor_tensor(inter_bf16, silu_res, gate_up_psum[up_slice], multiply)
    Saves 2 instructions per expert (16 total across K=8).

  Change 2 (Plan D): Fuse PSUM flush + affinity scale.
    Instead of: activation(copy, PSUM→SBUF) + tensor_scalar(scale)
    Do: activation(copy, PSUM→SBUF, scale=aff_bcast)
    Saves 1 instruction per expert (8 total across K=8).
...
def qwen3_moe_fused_tkg(
    ...
    # -----------------------------------------------------------------------
    # Stage 4: Selective-Expert MLP — Plan D optimizations
    #
    ...
    #          Plan D Change 1: tensor_tensor reads up directly from PSUM.
    #          Plan D Change 2: activation(scale=...) fuses PSUM flush + affinity.
    # -----------------------------------------------------------------------
    ...
    for t in nl.static_range(T):
        ...
        for k in nl.static_range(K):
            ...
            # Plan D Change 1: SiLU on gate directly from PSUM (ScalarE)
            ...
            # SiLU(gate) * up — read up DIRECTLY from PSUM, output bf16
            # On NeuronCore-v3, VectorE can access PSUM in parallel with ScalarE.
            # tensor_tensor reads up from PSUM (no separate copy needed) and outputs bf16.
            inter_bf16 = nl.ndarray((_PMAX, I_tiles), dtype=inp.dtype, buffer=nl.sbuf)
            nisa.tensor_tensor(inter_bf16, silu_res, gate_up_psum[0:_PMAX, gu_base + I_tiles:gu_base + 2 * I_tiles], nl.multiply)
            ...
            # Plan D Change 2: Fused PSUM flush + affinity scale in one activation call
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v26d with native weight layouts — no preprocessing required.
