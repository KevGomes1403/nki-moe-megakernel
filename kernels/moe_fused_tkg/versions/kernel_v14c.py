...
kernel_v14c — Plan C: Fused Single-Pass K Loop
===============================================

Changes from v10b:
  1. Merges v10b's three separate K-loops (Pass 1 gate/up, Pass 2 down,
     Pass 3 accumulate) into a single static_range(K) loop per token.
     For each expert k in the fused loop:
       a. DMA gate/up weights → fused_tile_3d (3D DMA, same as v10b Pass 1)
       b. DMA down weights → down_tile_3d (3D DMA, fired right after; both
          DMA engines run concurrently to different destinations)
       c. Compute gate/up matmul → PSUM (no SBUF store to inter_buf)
       d. Compute SiLU × up → inter_bf16_k in SBUF
       e. Compute down matmul using inter_bf16_k → down_psum
       f. Scale by aff_bcast[:, k], accumulate into output_temp
          (copy for k=0, add for k>0)

  2. Eliminates:
       - inter_buf [_PMAX, I_tiles, K] bf16  — 4 KB SBUF saved
       - down_buf  [_PMAX, H_free_shard, K] fp32 — 32 KB SBUF saved
       - Separate Pass 2 and Pass 3 loops

  3. The K loop uses static_range because output_temp has a carried
     read-modify-write dependency across k iterations (k=1 reads
     output_temp written by k=0). Inner loops remain affine_range.
...
v10b optimizations preserved:
  ...
  4. Router weight batching: 4 tiles per DMA (v10b)
...
SBUF budget estimate (per core):
  ...
  fused_tile_3d:       [128, 16, 512] bf16    = 16 KB/partition  (reused per k)
  down_tile_3d:        [128, 2, 1024] bf16    =  4 KB/partition  (reused per k)
  inter_f32:           [128, 2] fp32          =  1 KB
  inter_bf16_k:        [128, 2] bf16          =  0.5 KB
  down_result_sb:      [128, 8] fp32          =  4 KB
  down_result_scaled:  [128, 8] fp32          =  4 KB
  aff_bcast:           [128, 8] fp32          =  4 KB
  router_w_wide_sb:    [128, 512] bf16        = 128 KB  (4 tiles × 128 cols, reused)
  out_sb:              [1, 1024] bf16         =  2 KB
  Peak (fused_tile + down_tile coexist within k iteration): ~20 KB/partition
...
def qwen3_moe_fused_tkg(
    ...
    #
    # Router weight batching — 4 tiles per DMA (from v10b).
    ...
    # -----------------------------------------------------------------------
    # Stage 4: Selective-Expert MLP — Fused Single-Pass K Loop (Plan C)
    #
    # v10b had 3 separate K-loops:
    #   Pass 1: gate/up for all K experts → inter_buf
    #   Pass 2: down for all K experts    → down_buf
    #   Pass 3: accumulate down_buf       → output_temp
    #
    # v14c merges all 3 into a single static_range(K) loop.
    # For each expert k: DMA gate/up + DMA down (concurrent), compute gate/up,
    # compute SiLU×up, compute down matmul, scale+accumulate into output_temp.
    # Eliminates inter_buf (4 KB) and down_buf (32 KB).
    #
    ...
    # Declared BEFORE the k loop — accumulates across k iterations.
    ...
    for t in nl.static_range(T):
        ...
        # ── FUSED K LOOP: gate/up + down + accumulate in a single pass ──────────
        # static_range because output_temp has a carried read-modify-write
        # dependency across k iterations (k=1 reads output_temp written by k=0).
        for k in nl.static_range(K):
            expert_id = top8_idx.ap(pattern=[[K, 1], [1, 1]], offset=t * K + k)

            # ── DMA gate/up (3D DMA, same as Pass 1 in v10b) ─────────────────
            # gate_up_w_fused shape: [E=128, H=2048, I*2=512]
            # Target: fused_tile_3d[p, h1, col] = gate_up_w_fused[expert_id, h1*128+p, col]
            # AP pattern (3D):
            ...
            # ── DMA down (3D DMA, same as Pass 2 in v10b, H_shard offset) ────
            # Fire right after gate/up DMA — both DMA engines run concurrently
            # since transfers are to different destinations.
            ...
            # ── Gate/up matmul (PSUM accumulation, same as Pass 1 in v10b) ───
            ...
            # ── SiLU × up (fused into inter_f32 in SBUF, no K-slot store) ───
            inter_f32 = nl.ndarray((_PMAX, I_tiles), dtype=nl.float32, buffer=nl.sbuf)
            nisa.activation(inter_f32, op=nl.silu, data=gate_psum)
            nisa.tensor_tensor(inter_f32, inter_f32, up_psum, op=nl.multiply)
            ...
            # ── Down matmul (same as Pass 2 in v10b) ─────────────────────────
            # Each core loads only H_shard columns → H_free_shard output tiles.
            ...
            # ── Scale by affinity and accumulate directly into output_temp ───
            # For k=0: copy (initialise output_temp); for k>0: fused multiply-add.
            # The static `if k == 0 / else` is valid because k is a static_range index.
