...
kernel_v29k — Plan K: Single-wave 8-expert processing
======================================================
Based on v28f (Plan F: ROUTER_BATCH=16).

Changes vs v28f (Plan K: Single wave of 8 experts):
  K1: Remove _K_WAVE=4 constant, use K=8 directly for a single wave
  K2: Allocate 8 named SBUF buffers (instead of 4 reused across 2 waves)
  K3: PSUM sizes doubled: gate_up_psum [128, 32], down_psum [128, 64]
  K4: Issue ALL 24 DMAs in a single Phase 1 loop (k=0..7)
  K5: Single Phase 2 loop (k=0..7) — no inter-wave PSUM memsets
  K6: aff_bcast index is k directly (0..7), no separate kk variable

SBUF Budget (per partition):
  8 x gate_up [16 x 384 x 2] = 96 KiB
  8 x down_full0 [1024 x 2]  = 16 KiB
  8 x down_full1 [1024 x 2]  = 16 KiB
  gate_t1 + up_t1             =  8 KiB
  misc                        = ~4 KiB
  Total: ~140 KiB < 224 KiB ✓
...
def qwen3_moe_fused_tkg(
    ...
    # -----------------------------------------------------------------------
    # Stage 4: Selective-Expert MLP — Plan K: Single-wave 8-expert processing
    #
    # Single wave: Load & compute all 8 experts at once
    # -----------------------------------------------------------------------
    ...
    for t in nl.static_range(T):
        ...
        # ------------------------------------------------------------------
        # Allocate 8 named SBUF buffers (one per expert)
        # ------------------------------------------------------------------
        ...
        gate_up_bufs = [
            gate_up_buf0, gate_up_buf1, gate_up_buf2, gate_up_buf3,
        ...
        down_full0_bufs = [
            down_full0_buf0, down_full0_buf1, down_full0_buf2, down_full0_buf3,
        ...
        down_full1_bufs = [
            down_full1_buf0, down_full1_buf1, down_full1_buf2, down_full1_buf3,
            down_full1_buf4, down_full1_buf5, down_full1_buf6, down_full1_buf7,
        ]

        # ------------------------------------------------------------------
        # Zero pad region (rows I1:I0 = 64:128) for all 8 down_full1 buffers
        # ------------------------------------------------------------------
        for k_pad in range(8):
        ...
        # ==================================================================
        # SINGLE WAVE: All 8 Experts
        # ==================================================================

        # Phase 1: Issue ALL 24 DMAs (all 8 experts)
        ...
        # ------------------------------------------------------------------
        # Compute norm_weights (after Phase 1 DMAs issued — overlaps with
        # in-flight DMAs, same position as v28f)
        # ------------------------------------------------------------------
        ...
        # PSUM allocation for single wave (8-expert capacity)
        ...
        # Phase 2: Compute all 8 experts in one loop
        for k in nl.static_range(K):
            gu_base = k * 2 * I_tiles
            ...
            nisa.tensor_scalar(
                ...
                data=down_result_sb,
                op0=nl.multiply,
                operand0=aff_bcast[0:_PMAX, k:k + 1],  # k=0..7
            )
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v29k with native weight layouts — no preprocessing required.
