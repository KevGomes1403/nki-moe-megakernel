...
kernel_v28c — Plan C: Issue Phase 1a DMAs BEFORE pre-init memsets
=================================================================
Based on v27d.

Changes vs v27d:
  Move Phase 1a expert DMAs to execute IMMEDIATELY after buffer allocations,
  BEFORE the 6 blocking memsets (4 down_full1 + gate_t1_128 + up_t1_128).
  Memsets then run concurrently with in-flight DMAs.

  Safety: Phase 1a DMAs write to down_full1_bufs[k][0:64, :] only.
  Post-DMA memsets write to down_full1_bufs[k][64:128, :] and
  gate_t1_128/up_t1_128[..., 64:128] — zero conflicts.
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        # ------------------------------------------------------------------
        # Plan C change: Phase 1a DMAs issued FIRST, immediately after
        # buffer allocations — before any memsets.
        # DMAs write to: gate_up_bufs[k], down_full0_bufs[k],
        #                down_full1_bufs[k][0:I1, :]
        # No conflict with subsequent memsets which write to:
        #   down_full1_bufs[k][I1:, :], gate_t1_128[..., I1:], up_t1_128[..., I1:]
        # ------------------------------------------------------------------

        # Phase 1a: Load experts 0-3 (12 DMAs) — BEFORE memsets
        ...
        # ------------------------------------------------------------------
        # Memsets run concurrently with in-flight DMAs (VectorE engine)
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v28c with native weight layouts — no preprocessing required.
