...
kernel_v32a — Plan A: Pipelined K=8 loop via interleave_degree=2 section rotation
===================================================================================
Extends v30b_gu_shard with a single unified K=8 expert loop wrapped in an
open_scope(interleave_degree=2) "expert_pipe" scope. Each iteration opens an
inner per-expert scope, allocates weight buffers via alloc_stack (which rotate
between two SBUF sections), performs DMA → matmul → allreduce → SiLU → down
projection, then calls close_scope() + increment_section() to rotate the section.

Key changes from v30b:
  - Collapse 2-wave structure (K_WAVE=4 × 2) into single for k in static_range(8)
  - Wrap loop in sbm.open_scope(interleave_degree=2, name="expert_pipe")
  - Per-iteration: open inner expert scope, alloc gate_up_buf/down_full0_buf/
    down_full1_buf/gate_t1_128/up_t1_128 (one of each, rotated via sections)
  - Ping-pong PSUM: gate_up_psum_a/b and down_psum_a/b allocated outside loop,
    selected by k%2 each iteration
  - Per-expert sendrecv with pipe_id=k (0..7) for correct LNC allreduce
  - norm_weights/aff_bcast computed before the expert loop (shared across all 8)
  - No mid-loop memset fence (nisa.memset(gate_up_psum) at line 468 removed)
"""
...
def _qwen3_moe_sbuf_in_sbuf_out(
    ...
    # -----------------------------------------------------------------------
    # Stage 4: Selective-Expert MLP — Pipelined K=8 loop (Plan A / v32a)
    # -----------------------------------------------------------------------
    sbm.open_scope(name="expert_loop_outer")

    # output_temp allocated in outer scope — must persist across all K=8 iterations
    ...
    for t in nl.static_range(T):
        ...
        # ------------------------------------------------------------------
        # Compute norm_weights and aff_bcast BEFORE the expert loop.
        # These are shared across all K=8 experts — no per-expert dependency.
        # v32a: moved out of per-expert scope (no kk distinction needed).
        # ------------------------------------------------------------------
        ...
        # aff_bcast: broadcast ALL K=8 affinities into partition dim
        ...
        # ------------------------------------------------------------------
        # Ping-pong PSUM tiles: allocated in token scope (before expert_pipe).
        # gate_up_psum_a/b, down_psum_a/b — pick by k%2 each iteration.
        # Small free-dim (2*I_tiles=4) so PSUM headroom is fine.
        # ------------------------------------------------------------------
        gate_up_psum_a = nl.ndarray((_PMAX, 2 * I_tiles), dtype=nl.float32, buffer=nl.psum)
        gate_up_psum_b = nl.ndarray((_PMAX, 2 * I_tiles), dtype=nl.float32, buffer=nl.psum)
        down_psum_a = nl.ndarray((_PMAX, H_free_shard), dtype=nl.float32, buffer=nl.psum)
        down_psum_b = nl.ndarray((_PMAX, H_free_shard), dtype=nl.float32, buffer=nl.psum)

        # ==================================================================
        # Plan A: Single K=8 loop with interleave_degree=2 section rotation
        # The compiler can schedule DMA of iteration k+1 during compute of k,
        # hiding DMA latency. Section rotation means alloc_stack inside the
        # inner scope uses two alternating SBUF addresses (ping-pong for
        # gate_up_buf, down_full0_buf, down_full1_buf, gate_t1_128, up_t1_128).
        # ==================================================================
        sbm.open_scope(interleave_degree=2, name=f"expert_pipe_t{t}")

        for k in nl.static_range(_K):
            # ------------------------------------------------------------------
            # Inner per-expert scope: alloc_stack rotates between 2 sections.
            # Section 0 (k=0,2,4,6): uses SBUF addresses A.
            # Section 1 (k=1,3,5,7): uses SBUF addresses B.
            # After close_scope + increment_section, the next iteration reuses
            # the address set of the just-freed section.
            # ------------------------------------------------------------------
            sbm.open_scope(name=f"expert_k{k}_t{t}")

            # Weight buffers for this expert — rotated via section ping-pong
            # v32a: only ONE of each (not 4 as in v30b), addresses alternate A/B
            gate_up_buf = sbm.alloc_stack((_PMAX, H_free_shard, _GU_FLAT), gate_up_w.dtype, buffer=nl.sbuf, name=f"gate_up_buf_k{k}_t{t}")
            down_full0_buf = sbm.alloc_stack((_PMAX, H_shard), down_w.dtype, buffer=nl.sbuf, name=f"down_full0_buf_k{k}_t{t}")
            down_full1_buf = sbm.alloc_stack((_PMAX, H_shard), down_w.dtype, buffer=nl.sbuf, name=f"down_full1_buf_k{k}_t{t}")

            # Zero pad region (rows I1:I0 = 64:128) for down_full1
            nisa.memset(down_full1_buf[nl.ds(I1, I1), 0:H_shard], value=0.0)

            # gate_t1_128/up_t1_128: tile-1 tail buffers with zero padding
            # v32a: one per rotation section (H_free_shard slices only)
            gate_t1_128 = sbm.alloc_stack((_PMAX, H_free_shard, I0), gate_up_w.dtype, buffer=nl.sbuf, name=f"gate_t1_128_k{k}_t{t}")
            up_t1_128   = sbm.alloc_stack((_PMAX, H_free_shard, I0), gate_up_w.dtype, buffer=nl.sbuf, name=f"up_t1_128_k{k}_t{t}")
            ...
            # Per-expert allreduce SBUF buffers (inside scope → freed per-expert)
            gu_send = sbm.alloc_stack((_PMAX, 2 * I_tiles), nl.float32, buffer=nl.sbuf, name=f"gu_send_k{k}_t{t}")
            gu_recv = sbm.alloc_stack((_PMAX, 2 * I_tiles), nl.float32, buffer=nl.sbuf, name=f"gu_recv_k{k}_t{t}")
            gu_full = sbm.alloc_stack((_PMAX, 2 * I_tiles), nl.float32, buffer=nl.sbuf, name=f"gu_full_k{k}_t{t}")

            silu_res         = sbm.alloc_stack((_PMAX, I_tiles), nl.float32, buffer=nl.sbuf, name=f"silu_res_k{k}_t{t}")
            up_sb            = sbm.alloc_stack((_PMAX, I_tiles), nl.float32, buffer=nl.sbuf, name=f"up_sb_k{k}_t{t}")
            inter_f32        = sbm.alloc_stack((_PMAX, I_tiles), nl.float32, buffer=nl.sbuf, name=f"inter_f32_k{k}_t{t}")
            inter_bf16       = sbm.alloc_stack((_PMAX, I_tiles), dtype,       buffer=nl.sbuf, name=f"inter_bf16_k{k}_t{t}")
            down_result_sb   = sbm.alloc_stack((_PMAX, H_free_shard), nl.float32, buffer=nl.sbuf, name=f"down_result_sb_k{k}_t{t}")
            down_result_scaled = sbm.alloc_stack((_PMAX, H_free_shard), nl.float32, buffer=nl.sbuf, name=f"down_result_scaled_k{k}_t{t}")

            # Select ping-pong PSUM tile
            gate_up_psum = gate_up_psum_a if k % 2 == 0 else gate_up_psum_b
            down_psum    = down_psum_a    if k % 2 == 0 else down_psum_b

            # Memset chosen PSUM at the top of each iteration (fresh start per expert)
            nisa.memset(gate_up_psum, value=0.0)
            nisa.memset(down_psum, value=0.0)

            # ------------------------------------------------------------------
            # Step a: Issue 3 DMAs for this expert's weights
            # v32a: offset patterns identical to v30b; expert_id from top8_idx
            # ------------------------------------------------------------------
            expert_id = top8_idx.ap(pattern=[[K, 1], [1, 1]], offset=t * K + k)

            # gate_up_w: [E, H=2048, GU_FLAT=384] — load H_free_shard tiles from prg_id shard
            nisa.dma_copy(
                dst=gate_up_buf,
                src=gate_up_w.ap(
            ...
            # down_w: [E, I=192, H=2048] — load I0=128 rows for this core's H_shard
            nisa.dma_copy(
                dst=down_full0_buf,
                src=down_w.ap(
            ...
            # down_w: partial I1=64 rows (rows I0:I0+I1) for this core's H_shard
            nisa.dma_copy(
                dst=down_full1_buf[0:I1, 0:H_shard],
                src=down_w.ap(
            ...
            # ------------------------------------------------------------------
            # Step b: Tensor-copy tile-1 tails (gate/up second I-tile)
            # Pad cols [I1:I0] to 0 (already done by memset above).
            # ------------------------------------------------------------------
            nisa.tensor_copy(
                dst=gate_t1_128[0:_PMAX, 0:H_free_shard, 0:I1],
                src=gate_up_buf[0:_PMAX, 0:H_free_shard, nl.ds(I0, I1)],
            )
            nisa.tensor_copy(
                dst=up_t1_128[0:_PMAX, 0:H_free_shard, 0:I1],
                src=gate_up_buf[0:_PMAX, 0:H_free_shard, nl.ds(I + I0, I1)],
            )

            # ------------------------------------------------------------------
            # Step c: Gate/Up matmul — loop over H_free_shard × I_tiles
            # Moving operand: rmsnorm_normed_bf16 column for global h index
            # (prg_id * H_free_shard + h1) * T
            # ------------------------------------------------------------------
            for h1 in nl.affine_range(H_free_shard):
                for i_tile in nl.static_range(I_tiles):
                    if i_tile == 0:
                        g_stat = gate_up_buf[0:_PMAX, h1, nl.ds(0, I0)]
                        u_stat = gate_up_buf[0:_PMAX, h1, nl.ds(I, I0)]
                    else:
            ...
            # ------------------------------------------------------------------
            # Step d: Allreduce gate_up_psum across LNC cores
            # pipe_id=k (0..7) — both cores execute same static loop, same pipe_id
            # ------------------------------------------------------------------
            # Copy PSUM → SBUF (sendrecv requires src in SBUF)
            nisa.activation(gu_send, op=nl.copy, data=gate_up_psum[0:_PMAX, 0:2 * I_tiles])
            ...
            # Step e: Reduce local + received = full gate_up activation
            nisa.tensor_tensor(gu_full, gu_send, gu_recv, nl.add)

            # ------------------------------------------------------------------
            # Step f: SiLU on gate half, copy up half, element-wise multiply
            # ------------------------------------------------------------------
            nisa.activation(silu_res, op=nl.silu, data=gu_full[0:_PMAX, 0:I_tiles])
            nisa.activation(up_sb, op=nl.copy, data=gu_full[0:_PMAX, I_tiles:2 * I_tiles])
            nisa.tensor_tensor(inter_f32, silu_res, up_sb, nl.multiply)
            nisa.activation(inter_bf16, op=nl.copy, data=inter_f32)

            # ------------------------------------------------------------------
            # Step g: Down projection matmul
            # ------------------------------------------------------------------
            for h1_out in nl.affine_range(H_free_shard):
                nisa.nc_matmul(
                    dst=down_psum[0:_PMAX, h1_out:h1_out + 1],
                    stationary=down_full0_buf[0:_PMAX, nl.ds(h1_out * _PMAX, _PMAX)],
                    moving=inter_bf16[0:_PMAX, 0:1],
                )
                nisa.nc_matmul(
                    dst=down_psum[0:_PMAX, h1_out:h1_out + 1],
                    stationary=down_full1_buf[0:_PMAX, nl.ds(h1_out * _PMAX, _PMAX)],
                    moving=inter_bf16[0:_PMAX, 1:2],
                )

            # ------------------------------------------------------------------
            # Step h: Copy PSUM → SBUF, scale by affinity weight for this expert
            # aff_bcast[:, k:k+1] gives the broadcast affinity for expert k (0..7)
            # ------------------------------------------------------------------
            nisa.activation(
            ...
            # ------------------------------------------------------------------
            # Step i: Accumulate into output_temp
            # k==0: tensor_copy (initialize). k>0: tensor_tensor add (accumulate).
            # output_temp lives in outer scope — unaffected by section rotation.
            # ------------------------------------------------------------------
            if k == 0:
                nisa.tensor_copy(
            ...
            # Step j: Close inner expert scope, rotate SBUF section
            sbm.close_scope()       # frees all per-expert alloc_stack tensors
            sbm.increment_section() # rotates to next SBUF section (A→B→A→B→...)

        sbm.close_scope()  # expert_pipe_t{t}
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v32a with native weight layouts — no preprocessing required.
