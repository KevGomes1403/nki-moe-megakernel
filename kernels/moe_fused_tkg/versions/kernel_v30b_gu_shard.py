...
kernel_v30b_gu_shard — extends v30a_sbuf_io with H-sharded gate/up projection
==============================================================================
Like v30a, the sub-kernel _qwen3_moe_sbuf_in_sbuf_out:
...
New in v30b (Plan A):
  - Gate/up projection is now H-sharded across LNC cores, matching down projection.
  - Core 0 loads gate_up_w[e, 0:H_shard=1024, :], Core 1 loads [e, H_shard:H, :]
  - Partial gate+up PSUMs from each H_shard are all-reduced via nisa.sendrecv
  - After all-reduce, both cores have the full gate/up activation → SiLU → multiply
  - ~12 MB/token reduction in HBM traffic for gate/up weights
"""
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v30b_gu_shard with native weight layouts — no preprocessing required.
