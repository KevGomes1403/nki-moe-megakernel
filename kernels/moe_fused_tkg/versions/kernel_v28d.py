...
kernel_v28d — v27d with pre-init SBUF memsets hoisted before Stage 1
=====================================================================
Based on v27d (Plan D: hoist pre-init memsets).

Changes vs v27d:
  Hoist ALL buffer allocations and pre-init memsets (down_full1 pad zeros,
  gate_t1_128/up_t1_128 pad zeros, PSUM wave-0 zeros) to execute BEFORE
  Stage 1 (before RMSNorm DMA). They run concurrently with input loading
  and the full router stage (~20 μs of work), completing long before TopK.
  After TopK, expert DMAs can fire immediately without pre-work in the t-loop.
...
def qwen3_moe_fused_tkg(
    ...
    # -----------------------------------------------------------------------
    # HOISTED: Buffer allocations and pre-init memsets
    # These execute BEFORE Stage 1 (before RMSNorm DMA), running concurrently
    # with input loading + full router stage (~20 μs), completing before TopK.
    # After TopK, expert DMAs can fire immediately.
    # -----------------------------------------------------------------------
    ...
    # PSUM allocation and wave-0 zero-init (hoisted — already zeroed when t-loop starts)
    ...
    #
    # NOTE: All pre-init memsets and PSUM wave-0 zeros are hoisted above.
    # The t-loop starts directly with Phase 1a DMAs.
    # -----------------------------------------------------------------------
    ...
    for t in nl.static_range(T):
        ...
        # Phase 1a: Load experts 0-3 (12 DMAs — immediately after TopK, no pre-work)
        ...
        # ------------------------------------------------------------------
        # Compute norm_weights HERE (after Phase 1a DMAs issued)
        ...
        # Phase 2a: Compute experts 0-3 (PSUM already zeroed before Stage 1)
        for k in nl.static_range(_K_WAVE):
            gu_base = k * 2 * I_tiles
        ...
        # rows I1:I0 remain zeroed from initial memset hoisted before Stage 1.

        # PSUM memset for wave 1 (inter-wave reset stays here)
        nisa.memset(gate_up_psum, value=0.0)
        nisa.memset(down_psum, value=0.0)
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v28d with native weight layouts — no preprocessing required.
