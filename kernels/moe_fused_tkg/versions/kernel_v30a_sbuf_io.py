...
kernel_v30a_sbuf_io — v30_sbuf_io with column-major output layout
==================================================================
Sub-kernel _qwen3_moe_sbuf_in_sbuf_out:
  ...
  - Returns out_sb ([PMAX=128, H_free_shard*T=8] bf16) in SBUF — column-major,
    partition-first. No transpose needed vs v30's row-major [T, H_shard] output.
Wrapper qwen3_moe_fused_tkg_sbuf_io: loads inp from HBM, calls sub-kernel,
  then transposes and DMAs out_sb to HBM in row-major [T, H_shard] format.
  (The 8 transposes in the wrapper are test-only overhead; in production,
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v30a_sbuf_io with native weight layouts — no preprocessing required.
