...
kernel_v31a_gu_relayout — extends v30b_gu_shard with improved DMA bandwidth for gate/up weights
================================================================================================
...
New in v31a (Plan B1 — gate/up weight HBM relayout):
  - gate_up_w HBM layout changed from [E=128, H=2048, GU_FLAT=384]
    to [E=128, GU_FLAT=384, H=2048] (transposed last two dims).
  - In the old layout, partition-dim stride = 384 elements (768 B) —
    each partition read 8 non-contiguous 768-byte runs, giving poor DMA packet size.
  - In the new layout, H is contiguous in HBM. Each partition now holds one full
    row of H=2048 bf16 = 4 KiB contiguous. 16 DMA engines × 8 partitions × 4 KiB
    = 32 KiB per engine, matching ideal NKI DMA size.
  - The matmul math is unchanged: stationary operand still has partition=contraction (H
    sub-tile) and free=output columns (I sub-tile). Only the HBM access pattern changes.
  - The run() function transposes gate_up_w offline before kernel invocation:
      gate_up_w = gate_up_w.transpose(-2, -1).contiguous()  # [E, GU_FLAT, H]
  - All other optimizations from v30b are preserved: SBUF hoisting, 2-wave scheme,
    sendrecv all-reduce, I1 zero-padding, stream_shuffle_broadcast for aff/norm,
    LNC H-sharding for both gate/up and down projections.
"""
...
def _qwen3_moe_sbuf_in_sbuf_out(
    ...
    gate_up_w,  # [E=128, GU_FLAT=384, H=2048] bf16 — HBM (v31a relayout)
    ...
    v31a: gate_up_w is now [E, GU_FLAT=384, H=2048] (transposed from v30b).
    The DMA AP pattern uses stride=1 along H (contiguous) for the partition dimension,
    yielding 4 KiB contiguous reads per partition instead of 768 B scattered reads.
    """
    B = T
    ...
    for t in nl.static_range(T):
        ...
        # Shape is unchanged from v30b: [PMAX, H_free_shard, GU_FLAT]
        # ------------------------------------------------------------------
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # v31a: gate_up_w layout is now [E, GU_FLAT=384, H=2048].
            # We want SBUF[p, h1, gu] = HBM[expert, gu, prg_id*H_shard + h1*PMAX + p]
            # AP pattern:
            #   partition dim: stride=1 (H is contiguous), count=PMAX=128
            #   free dim 1 (h1): stride=PMAX=128 (next 128-block of H), count=H_free_shard=8
            #   free dim 2 (gu): stride=H=2048 (jump to next GU row), count=GU_FLAT=384
            # offset: start at prg_id's H shard: prg_id * H_SHARD = prg_id * 1024
            nisa.dma_copy(
                dst=gate_up_bufs[k],
                src=gate_up_w.ap(
                    pattern=[[1, _PMAX],            # partition: H stride=1, count=128
                              [_PMAX, H_free_shard], # free dim 1: h1, step PMAX in H
                              [_H, _GU_FLAT]],       # free dim 2: gu, step H=2048 between GU rows
                    offset=prg_id * _H_SHARD,        # H offset for this core's shard
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # v31a: same contiguous H pattern as wave 0
            nisa.dma_copy(
                dst=gate_up_bufs[k],
                src=gate_up_w.ap(
                    pattern=[[1, _PMAX],            # partition: H stride=1, count=128
...
def qwen3_moe_fused_tkg_sbuf_io(inp, gamma, router_w, gate_up_w, down_w):
    ...
    v31a: gate_up_w must be pre-transposed to [E, GU_FLAT=384, H=2048] by run().
    """
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v31a_gu_relayout with native weight layouts.
    ...
    v31a change: gate_up_w is transposed offline to [E, GU_FLAT=384, H=2048]
    for contiguous partition-dim DMA reads.
    ...
    # v31a: offline relayout to [E, GU_FLAT, H] for contiguous partition-dim DMA
    gate_up_w = gate_up_w.transpose(-2, -1).contiguous()

    assert gate_up_w.shape == (
        _E, _GU_FLAT, _H
    ), f"gate_up_w shape {gate_up_w.shape} != ({_E}, {_GU_FLAT}, {_H})"
    assert down_w.shape == (
        _E, _I, _H
