...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        for k in nl.static_range(K):
            ...
            # ---- Gate/Up Projection: H-SHARD hidden @ gate_up_w -> gate_out, up_out ----
            # Change B: Each core loads only H_free_shard=8 H-tiles (half of H=2048).
            # Change A: Declare persistent PSUMs outside h1 loop for hardware accumulation.

            # H-sharded weight view: each core accesses only its half of H dimension.
            # core 0: H rows [0:1024], core 1: H rows [1024:2048]
            fused_w_view_shard = (
                TensorView(gate_up_w_fused)
                .select(dim=0, index=expert_id)                                          # [H=2048, 2*I=512]
                .slice(dim=0, start=prg_id * H_shard, end=(prg_id + 1) * H_shard)       # [H_shard=1024, 512]
                .reshape_dim(dim=0, shape=[H_free_shard, _PMAX])                         # [H_free_shard=8, 128, 512]
            )

            # Change A: Persistent PSUMs for gate and up — declared outside h1 loop so the compiler
            # sees all nc_matmul iterations writing to the same tensor -> hardware accumulation mode.
            # affine_range outer loop preserves loop structure; compiler does NOT flush between iterations.
            ...
            # Change A: affine_range preserves loop structure -> compiler uses hardware accumulation.
            # Change B: H_free_shard (not H_free) — only 8 tiles per core instead of 16.
            for h1 in nl.affine_range(H_free_shard):
                ...
                nisa.dma_copy(dst=fused_tile, src=fused_h_view.get_view(), dge_mode=_adaptive_dge(fused_h_view))

                for i_tile in nl.static_range(I_tiles):   # stays static — accumulates per column
                    nisa.nc_matmul(
                        dst=gate_psum[0:_PMAX, i_tile:i_tile+1],
                        stationary=fused_tile[0:_PMAX, nl.ds(i_tile * I0, I0)],
                        moving=rmsnorm_normed_bf16[0:_PMAX, 0:T, prg_id * H_free_shard + h1],  # H_shard slice
                    )
                    nisa.nc_matmul(
                    ...
                    # NO per-iteration PSUM flush, NO g_contrib/u_contrib, NO tensor_tensor add

            # Change A: Single flush each — was H_free * I_tiles * 2 = 64 flushes per expert in v1b
            ...
            # Change B: SBUF-to-SBUF all-reduce — exchange partial gate/up with the other LNC core.
            # nisa.sendrecv uses NeuronLink directly — no HBM roundtrip.
            # K=8 experts × 2 sendrecvs × [128,2] × 4B = 16KB total — ~0.05µs at 307 GB/s.
            ...
            nisa.sendrecv(src=gate_result_sb, dst=recv_gate,
                          send_to_rank=other, recv_from_rank=other, pipe_id=k * 2)
            nisa.sendrecv(src=up_result_sb,   dst=recv_up,
                          send_to_rank=other, recv_from_rank=other, pipe_id=k * 2 + 1)

            # Add the two H-shard partial sums to get the full intermediate
            ...
            nisa.tensor_tensor(full_gate, gate_result_sb, recv_gate, nl.add)
            nisa.tensor_tensor(full_up,   up_result_sb,   recv_up,   nl.add)

            # SiLU(gate) * up -> inter [128, I_tiles] fp32
            silu_result = nl.ndarray((_PMAX, I_tiles), dtype=nl.float32, buffer=nl.sbuf)
            nisa.activation(silu_result, op=nl.silu, data=full_gate)      # ← was gate_result_sb
            inter_f32 = nl.ndarray((_PMAX, I_tiles), dtype=nl.float32, buffer=nl.sbuf)
            nisa.tensor_tensor(inter_f32, silu_result, full_up, nl.multiply)  # ← was up_result_sb
            ...
            # ---- Down Projection: persistent PSUM + whole-shard weight tile ----
            #
            # perBankT = _PSUM_FREE // T = 512 for T=1.
            # H_free_shard=8 output columns all fit in one psum[128, 512] bank
            # at column offsets 0..7. I_tiles=2 iterations both write to the same
            # column offsets -> hardware PSUM accumulation across affine_range(I_tiles).
            # Result: 1 PSUM flush per expert (was 16). 2 DMA loads of [128,1024]
            # per expert (was 16 × [128,128]).

            # Build down weight TensorView for this expert's H-shard (done once per expert)
            down_w_view = (
                TensorView(down_w)
                .select(dim=0, index=expert_id)
                .slice(dim=1, start=prg_id * H_shard, end=(prg_id + 1) * H_shard)
            )
            # shape after view: [I=256, H_shard=1024]

            # Single PSUM bank: [_PMAX=128, _PSUM_FREE=512].
            # Only columns 0..H_free_shard-1 will be written; rest stay zero from memset.
            ...
            for i_tile2 in nl.affine_range(I_tiles):
                # Load [I0=128, H_shard=1024] — entire H-shard for this I-tile.
                # One 256 KB DMA replaces 8 × 32 KB DMA loads. affine_range lets
                # DMA for i_tile2+1 overlap with matmuls for i_tile2.
                ...
                for h1_out in nl.static_range(H_free_shard):
                    # Writes to psum column h1_out (one T=1-wide slot).
                    # For i_tile2=0 and i_tile2=1, same column index -> compiler
                    # emits hardware accumulation (no flush between iterations).
                    nisa.nc_matmul(
            ...
            # Single flush for entire H_free_shard output (was 16 flushes per expert)
