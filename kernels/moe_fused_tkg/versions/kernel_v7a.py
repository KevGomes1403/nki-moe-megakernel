...
v7a changes vs v5a — Plan C analysis and outcome:

  Root cause: v5a issues 80 DGE calls per invocation:
    - Gate/Up: 8 h1-tile DMAs × 8 experts = 64 DGE calls in Pass 1
    - Down:    2 i-tile DMAs × 8 experts  = 16 DGE calls in Pass 2
  Each DGE call has setup overhead (~1 µs each).

  Plan C attempted: single bulk DMA per expert via reshape_dim.
    - Gate/Up: TensorView.reshape_dim(dim=0, shape=[_PMAX, H_free_shard]) on
      [H_shard=1024, I*2=512] produces row-major layout:
      fused_all[p, h1, col] = weight[H_start + p*H_free_shard + h1, col]
    - This is INCOMPATIBLE with v5a's column-major input convention:
      rmsnorm_normed_bf16[p, h1] = input[h1*128 + p]
    - The nc_matmul would accumulate over wrong (p,h1) weight-input pairings.
    - Only correct reshape_dim is [H_free_shard, _PMAX] (column-major), but
      that gives SBUF partition=H_free_shard=8 (invalid, must be 128).
    - Down: same issue — I-dimension convention mismatch.

  Result: Plan C bulk load is architecturally infeasible with v5a's column-major
  input/weight convention. Changing to row-major would also require rewriting
  router_w stationary loading (which requires column-major H tile grouping in HBM).

  v7a is therefore identical to v5a in behavior (same DGE call count = 80).
  This file documents the failed Plan C attempt and serves as a correctness baseline.

  Everything identical to v5a:
    - 3-pass structure (static K Pass 1, affine K Pass 2, static K Pass 3)
    - Gate/Up: per-h1 DMA, 8 DMAs per expert × 8 experts = 64 DGE calls
    - Down: per-i_tile DMA, 2 DMAs per expert × 8 experts = 16 DGE calls
    - inter_buf [128, I_tiles, K] and down_buf [128, H_free_shard, K] unchanged
    - aff_bcast pre-broadcast unchanged
    - sendrecv pipe_ids unchanged (k*2 and k*2+1)
    - All RMSNorm, Router, TopK, output store unchanged
"""
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        # Plan C Fix 1: Load all H_shard gate/up weights in ONE DMA per expert
        # instead of 8 separate h1-tile DMAs.
        #
        # TensorView reshaping:
        #   gate_up_w_fused[expert_id] has shape [H=2048, I*2=512]
        #   Slice H_shard: [H_shard=1024, I*2=512]
        #   reshape_dim(dim=0, shape=[_PMAX=128, H_free_shard=8]):
        #     → [_PMAX=128, H_free_shard=8, I*2=512]
        #
        # SBUF shape: [_PMAX=128, H_free_shard=8, I*2=512]
        # Partition dim = 128 (first dim), free dim = H_free_shard * I*2 = 8*512 = 4096
        # Total size: 128 × 4096 × 2 bytes (bf16) = 1 MB per expert → fits in SBUF
        #
        ...
        for k in nl.static_range(K):
            ...
            # Gate/up projection: per-h1 tile DMA approach (v5a pattern).
            # Note: bulk single-DMA approach (reshape_dim [_PMAX, H_free_shard]) produces
            # row-major weight layout fused_all[p,h1,col]=weight[p*8+h1,col] which is
            # incompatible with v5a's column-major input rmsnorm_normed_bf16[p,h1]=input[h1*128+p].
            # The per-h1 DMA approach is the only correct approach with this input convention.
            fused_w_view_shard = (
        ...
        # Uses v5a's per-I-tile DMA approach (2 DMAs per expert) — same as v5a for correctness.
        # The bulk down approach (1 DMA per expert with reshape_dim) produces wrong results
        # due to I-dimension mapping issues; using the known-correct v5a pattern instead.
