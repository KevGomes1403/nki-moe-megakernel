...
kernel_v28e — v27d with router DMA hw_dge + batch=8
====================================================
Based on v27d (Plan D: remove redundant inter-wave memsets).

Changes vs v27d (Plan E: Router DMA optimization):
  E1: Router DMA dge_mode changed from 0 to 3 (hw_dge, compile-time offset
      h_chunk * _ROUTER_BATCH * _PMAX * E is linear in h_chunk).
  E2: _ROUTER_BATCH increased from 4 to 8, halving router DMA calls from
      4 to 2 (inner loop runs 8 matmul iterations per DMA call).
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v28e with native weight layouts — no preprocessing required.
