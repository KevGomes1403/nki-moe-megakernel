...
kernel_v28a — Plan A: Direct K=64 matmul for I1-tile
=====================================================
Based on v27d.

Changes vs v27d:
  Eliminate gate_t1_128 and up_t1_128 buffers entirely.
  Instead of zero-padding to K=128 and copying slices into them, use
  direct slices of gate_up_bufs[k] with K=64 for the second I-tile.

  Removed:
    - gate_t1_128 = nl.ndarray(...)  allocation
    - up_t1_128   = nl.ndarray(...)  allocation
    - nisa.memset(gate_t1_128[...], ...)
    - nisa.memset(up_t1_128[...], ...)
    - nisa.tensor_copy into gate_t1_128 (per expert, both waves)
    - nisa.tensor_copy into up_t1_128   (per expert, both waves)

  Modified inner loop (both wave 0 and wave 1):
    i_tile == 0: g_stat = gate_up_bufs[k][..., nl.ds(0,  I0)]   (K=128)
    i_tile == 1: g_stat = gate_up_bufs[k][..., nl.ds(I0, I1)]   (K=64)
    i_tile == 0: u_stat = gate_up_bufs[k][..., nl.ds(I,  I0)]   (K=128)
    i_tile == 1: u_stat = gate_up_bufs[k][..., nl.ds(I+I0, I1)] (K=64)

  Saves ~8 KiB SBUF + 2 memset + 16 tensor_copy instructions.
...
SBUF Budget (per partition):
  ...
  misc                        = ~2 KiB
  Total: ~66 KiB << 224 KiB (8 KiB freed vs v27d)
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        # gate_t1_128/up_t1_128: single pair of reused buffers (no H_free dim)
        # Plan A: reduced from [_PMAX, H_free, I0] to [_PMAX, I0] to save SBUF.
        # tensor_copy now happens per-h1 inside the matmul loop.
        gate_t1_128 = nl.ndarray((_PMAX, I0), dtype=gate_up_w.dtype, buffer=nl.sbuf)
        up_t1_128   = nl.ndarray((_PMAX, I0), dtype=gate_up_w.dtype, buffer=nl.sbuf)
        nisa.memset(gate_t1_128[0:_PMAX, nl.ds(I1, I1)], value=0.0)
        nisa.memset(up_t1_128[0:_PMAX, nl.ds(I1, I1)],   value=0.0)
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # Gate/Up matmul (Plan A: per-h1 copy into smaller [_PMAX, I0] buffer)
            for h1 in nl.affine_range(H_free):
                # Copy K=64 valid rows for this h1 into the reused single-slot buffers
                nisa.tensor_copy(
                    dst=gate_t1_128[0:_PMAX, 0:I1],
                    src=gate_up_bufs[k][0:_PMAX, h1, nl.ds(I0, I1)],
                )
                nisa.tensor_copy(
                    dst=up_t1_128[0:_PMAX, 0:I1],
                    src=gate_up_bufs[k][0:_PMAX, h1, nl.ds(I + I0, I1)],
                )
                for i_tile in nl.static_range(I_tiles):
                    if i_tile == 0:
                        ...
                        g_stat = gate_t1_128[0:_PMAX, 0:I0]
                        u_stat = up_t1_128[0:_PMAX, 0:I0]
                    nisa.nc_matmul(
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v26g with native weight layouts — no preprocessing required.
