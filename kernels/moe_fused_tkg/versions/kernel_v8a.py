...
v4a changes vs v3b — Plan A: Batch RMSNorm Input/Gamma Loads (32 tiny DMAs -> 2):
  Root cause: inp/gamma tile loops each issue H_free=16 DMA calls of [128, 1] = 256 bytes.
  At 256B each, these are far below the 32KB DMA efficiency threshold.

  1. inp load: Replace the 16-iteration loop with a single large DMA.
     Reshape inp_2d[T, H] in HBM to [H_free*T, _PMAX] and load the entire input
     in one transpose DMA into inp_all_sb[_PMAX, H_free*T].
     Layout: inp_all_sb[p, t*H_free + h1] = inp_2d[t, h1*128 + p]
     For T=1: one 4KB DMA replaces 16 x 256B calls.

  2. gamma load: Replace the 16-iteration loop with a single DMA.
     Reshape gamma_1d[H] to [H_free, _PMAX] in HBM and load into
     gamma_sb[_PMAX, H_free] in one 4KB DMA replacing 16 x 256B calls.

  3. Downstream adaptation:
     - rmsnorm_out is now [_PMAX, H_free*T] (2D, not 3D)
     - T=1 (TKG) assumption: H_free*T = H_free, so rmsnorm_out is [128, 16]
     - All downstream tensors using rmsnorm_out updated accordingly.

v5a changes vs v4a — Pre-broadcast Affinity Weights (ported from kernel_v4b.py):
  Combines v4a batch loads with v4b pre-broadcast affinity optimization.

  Root cause: v4a still performs per-expert affinity setup inside Pass 2's affine_range(K)
  loop: 1 memset + 1 tensor_copy + 4 nc_stream_shuffles = 6 instructions x K=8 experts
  = 48 instructions per token, all to broadcast a single scalar to 128 partitions.

  Fix: Allocate aff_bcast [128, K] once per token (before Pass 1), copy all K affinity
  weights from norm_weights[t, 0:K] (partition 0) into row 0, then broadcast to all
  128 partitions with 4 nc_stream_shuffles — 6 instructions total, regardless of K.
  In Pass 2, replace the per-expert aff_sb block with a direct slice aff_bcast[:, k:k+1].

  Instruction reduction: 48 per token -> 6 per token (8x fewer broadcast instructions).

v8a changes vs v5a — Plan D: Bulk 3D DMA for Gate/Up and Down Weights:
  Bottleneck targeted: gpsimd_engine_active_time (DGE overhead from 80 dynamic DMA calls).

  Root cause: v5a issues 8 separate DMA calls per expert for gate/up (one per h1 tile)
  and 2 for down. Each DGE call has fixed per-call overhead regardless of transfer size.

  Fix:
    1. Gate/Up: Replace per-h1 DMA loop with ONE 3D DMA per expert loading
       fused_tile_3d[_PMAX=128, H_free_shard=8, I*2=512] via AP pattern.
    2. Down: Replace per-i_tile DMA loop with ONE 3D DMA per expert loading
       down_tile_3d[I0=128, I_tiles=2, H_shard=1024] via AP pattern.

  DGE calls reduced: 80 -> 16 (5x reduction).
  Expected effect: gpsimd_engine_active_time drops from ~69 µs to ~14-25 µs.
"""
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        for k in nl.static_range(K):
            ...
            # Plan D: ONE 3D DMA per expert for gate/up (replaces H_free_shard=8 per-h1 DMAs)
            # gate_up_w_fused shape: [E=128, H=2048, I*2=512]
            # Target: fused_tile_3d[p, h1, col] = gate_up_w_fused[expert_id, H_shard_start + h1*128 + p, col]
            # HBM offset (within expert, from H_shard_start): (h1*128 + p) * (I*2) + col
            # AP pattern dims:
            #   dim 0 (p):   stride = I*2 = 512, count = _PMAX = 128
            #   dim 1 (h1):  stride = _PMAX*I*2 = 65536, count = H_free_shard = 8
            #   dim 2 (col): stride = 1, count = I*2 = 512
            # offset = prg_id * H_shard * (I*2) skips to the H-shard start within the expert
            fused_tile_3d = nl.ndarray((_PMAX, H_free_shard, I * 2), dtype=gate_up_w.dtype, buffer=nl.sbuf)
            nisa.dma_copy(
                dst=fused_tile_3d,
                src=gate_up_w_fused.ap(
                    pattern=[[I * 2, _PMAX], [_PMAX * I * 2, H_free_shard], [1, I * 2]],
                    offset=prg_id * H_shard * (I * 2),
            ...
            # Matmul loop — no DMA inside, reads from pre-loaded 3D tile
        ...
        for k in nl.affine_range(K):
            ...
            # Plan D: ONE 3D DMA per expert for down (replaces I_tiles=2 per-i_tile DMAs)
            # down_w shape: [E=128, I=256, H=2048]
            # Target: down_tile_3d[i0, i1, h] = down_w[expert_id, i1*I0 + i0, H_shard_start + h]
            # HBM offset (within expert): (i1*I0 + i0)*H + (H_shard_start + h)
            # AP pattern dims (relative to start of expert's [I, H] block, H_shard slice):
            #   dim 0 (i0): stride = H = 2048, count = I0 = 128
            #   dim 1 (i1): stride = I0*H = 131072 (step I_tiles=2 rows of I0), wait...
            #
            # Actually down_w is [E, I, H], so for element [e, row, col]:
            #   flat offset within expert = row * H + col
            # down_tile_3d[i0, i1, h] points to [e, i1*I0 + i0, H_shard_start + h]
            #   HBM offset within expert = (i1*I0 + i0) * H + H_shard_start + h
            # AP pattern (3D, dims indexed as [i0, i1, h]):
            #   i0: HBM stride = H = 2048, count = I0 = 128
            #   i1: HBM stride = I0 * H = 32768, count = I_tiles = 2
            #   h:  HBM stride = 1, count = H_shard = 1024
            # offset = H_shard_start = prg_id * H_shard (within expert's row of H columns)
