...
kernel_v28g — v28e + remove memset(aff_bcast) + eliminate sum_reduced_sb
=========================================================================
Based on v28e (Plan E: router DMA hw_dge + batch=8).

Changes vs v28e (Plan G):
  ...
  G2: Eliminate sum_reduced_sb intermediate buffer. tensor_partition_reduce
      now writes directly into norm_sum_sb[0:1, 0:T], removing one
      buffer allocation and one tensor_copy.
...
# Router DMA batching: 8 tiles per DMA (Plan E2: doubled from 4)
_ROUTER_BATCH = 8
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v28g with native weight layouts — no preprocessing required.
