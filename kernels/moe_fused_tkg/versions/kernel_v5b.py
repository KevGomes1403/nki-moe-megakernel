...
v5b changes vs v4a — Plan B: Pre-scale inter by affinity in Pass 1 to relieve Pass 2 critical path:
  Root cause: Pass 2 (affine_range K) has a VE scaling step (aff_sb lookup + tensor_scalar)
  on the critical path after the TE matmul, blocking DMA/TE overlap for the next expert.

  Algebraic identity: w_k * (inter[k] @ down_w[k]) = (w_k * inter[k]) @ down_w[k]
  Scaling inter before the down matmul is numerically equivalent to scaling after.

  Changes:
  1. aff_bcast pre-computation: Before Pass 1, allocate aff_bcast [128, K] and broadcast
     all K affinity weights once per token using 4 nc_stream_shuffles total
     (vs K×4=32 per-expert shuffles in the original v4a Pass 2).
  2. Pass 1 (static_range K, already serialized): After SiLU, pre-scale inter_f32 by
     aff_bcast[:, k] before storing to inter_buf. Moves VE scaling off the Pass 2
     critical path and into the already-serialized Pass 1 loop.
  3. Pass 2 (affine_range K): Remove entire per-expert aff_sb block (memset + tensor_copy
     + 4 shuffles + tensor_scalar). Replace with direct tensor_copy of down_result_sb
     to down_buf — the result already incorporates affinity via pre-scaled inter.
  Pass 3: No changes — accumulation of pre-scaled down_buf is algebraically correct.
"""
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        # Pre-broadcast all K affinity weights to [128, K] once per token.
        # Used in Pass 1 to pre-scale inter, eliminating per-expert scaling from Pass 2.
        # 4 shuffles total (vs K×4=32 per-expert shuffles in original v4a Pass 2).
        aff_bcast = nl.ndarray((_PMAX, K), dtype=nl.float32, buffer=nl.sbuf)
        nisa.memset(aff_bcast, value=0.0)
        # norm_weights[t, 0:K] lives in partition t; copy K weights into partition 0
        # of aff_bcast so the shuffles below can replicate them across all 128 partitions.
        nisa.tensor_copy(dst=aff_bcast[0:T, 0:K], src=norm_weights[0:T, 0:K])
        # Each nc_stream_shuffle group broadcasts partition 0's K values to 32 partitions
        # (shuffle_mask=[0]*32 means every dest partition reads from src partition 0).
        # 4 groups × 32 partitions = 128 partitions total — all holding the same K weights.
        ...
        for k in nl.static_range(K):
            ...
            # Pre-scale inter by affinity weight k before storing.
            # Mathematical identity: w_k * (inter @ down_w) = (w_k * inter) @ down_w
            # This moves per-expert scaling from Pass 2 (affine_range, critical path) to
            # Pass 1 (static_range, already serialized) — relieves Pass 2 VE pressure.
            inter_scaled_f32 = nl.ndarray((_PMAX, I_tiles), dtype=nl.float32, buffer=nl.sbuf)
            nisa.tensor_scalar(
                inter_scaled_f32,
                data=inter_f32,
                op0=nl.multiply,
                operand0=aff_bcast[0:_PMAX, k:k + 1],  # pre-broadcast weight for expert k
            )
            # Store pre-scaled inter (cast to bf16 for matmul efficiency).
            # nisa.activation(op=nl.copy) handles fp32->bf16 cast transparently.
            nisa.activation(
                inter_buf[0:_PMAX, 0:I_tiles, k:k + 1],
                op=nl.copy,
                data=inter_scaled_f32,  # store w_k*inter instead of raw inter
            )
        ...
        for k in nl.affine_range(K):
            ...
            # down_result_sb already incorporates affinity weight (inter was pre-scaled in Pass 1).
            # No per-expert scaling needed here — Pass 2 affine_range critical path is now clean.
            nisa.tensor_copy(
