"""
Test harness for kernel_v31a_gu_relayout.py
Compares v31a (gate/up HBM relayout [E, GU_FLAT, H]) against a pure PyTorch fp32-promoted reference.
Tolerance: atol=1e-3, rtol=1e-2 (fp32-promoted reference per CLAUDE.md).
Also benchmarks v31a.
"""
import sys
import os
...
os.environ["NEURON_RT_INSPECT_OUTPUT_DIR"] = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "_bench_out_v31a"
)
...
from kernel_v31a_gu_relayout import run as run_v31a
from kernel_v31a_gu_relayout import qwen3_moe_fused_tkg_sbuf_io
...
# Note: gate_up_w is provided in NATIVE layout [E, H, 2*I]; run() will transpose offline
inp       = make_tensor((B, 1, _H))           # [B, 1, H] bf16
gamma     = make_tensor((1, _H))              # [1, H] bf16
router_w  = make_tensor((_H, _E))             # [H, E] bf16
gate_up_w = make_tensor((_E, _H, 2 * _I))     # [E, H, 2*I] bf16 — native layout
...
print("Running v31a gu_relayout kernel...")
out_v31a = run_v31a(inp, gamma, router_w, gate_up_w, down_w)
xm.mark_step()

# Transfer to CPU for comparison
out_v31a_cpu = out_v31a.cpu().float().numpy()
ref_out_np   = ref_out.float().numpy()

max_diff  = np.max(np.abs(ref_out_np - out_v31a_cpu))
mean_diff = np.mean(np.abs(ref_out_np - out_v31a_cpu))

print(f"\n--- Comparison Results ---")
print(f"ref   output shape : {ref_out_np.shape}")
print(f"v31a  output shape : {out_v31a_cpu.shape}")
print(f"max_diff           : {max_diff:.6f}")
print(f"mean_diff          : {mean_diff:.6f}")

try:
    np.testing.assert_allclose(ref_out_np, out_v31a_cpu, rtol=1e-2, atol=1e-3)
    print("\nPASS — v31a matches PyTorch reference within atol=1e-3, rtol=1e-2")
except AssertionError as e:
    print(f"\nFAIL — {e}")
    # Extra diagnostics
    diff = np.abs(ref_out_np - out_v31a_cpu)
    failing = np.argwhere(diff > 1e-3 + 1e-2 * np.abs(ref_out_np))
    print(f"Number of failing elements: {len(failing)}")
    if len(failing) > 0:
        idx = failing[0]
        print(f"First failing element idx={tuple(idx)}: ref={ref_out_np[tuple(idx)]:.6f}, v31a={out_v31a_cpu[tuple(idx)]:.6f}")
    sys.exit(1)
...
# gate_up_w must already be in relayout [E, GU_FLAT, H] for direct kernel invocation
...
# Pre-transpose gate_up_w for direct kernel invocation in benchmark
gate_up_w_b_native = make2((_E, _H, 2 * _I))
gate_up_w_b = gate_up_w_b_native.transpose(-2, -1).contiguous()   # [E, GU_FLAT, H]
...
if ntff_path and neff_path:
    ...
    _print_profile_results(prof, "qwen3_moe_fused_tkg_sbuf_io (v31a)")
else:
...
if bench_neff:
    _pending_bench.append({
        "neff": bench_neff, "warmup": 5, "iters": 50, "name": "qwen3_moe_fused_tkg_sbuf_io_v31a"
    })

r = BenchmarkResult(prof, bench_neff or neff_path or "", ntff_path)
print(f"\n--- v31a Benchmark Metrics ---")
