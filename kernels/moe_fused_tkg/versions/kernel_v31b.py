...
kernel_v31b — extends v30b_gu_shard with Plan "C3-on-v4": exact tile sizing
=============================================================================
...
New in v31b (Plan C3-on-v4):
  - Eliminates gate_t1_128/up_t1_128 padding buffers and associated memsets.
  - Eliminates down_full1 zero-pad memset for bottom 64 rows.
  - Gate/up matmul for i_tile==1 uses tile_size=(_PMAX, I1) column-tiling so
    only 64 PE columns participate (exactly the I1=64 real columns).
  - Down matmul for i_tile==1 (second accumulation) uses tile_size=(I1, _PMAX)
    row-tiling so only 64 PE rows participate (exactly the I1=64 real rows),
    with explicit accumulate=True.
  - Requires NeuronCore-v4 (trn3) tile-size/tile-position matmul features.
"""
...
_I1 = 64     # second tile (partial: 64 valid rows)
...
def _qwen3_moe_sbuf_in_sbuf_out(
    ...
    for t in nl.static_range(T):
        ...
        # v31b: down_full1_bufs keep full (_PMAX, H_shard) allocation but bottom
        # 64 rows are garbage — row-tiling in the matmul ensures we only read top I1=64 rows.
        ...
        # v31b: NO down_full1 pad memset (removed — row-tiling handles partial rows)
        # v31b: NO gate_t1_128/up_t1_128 padding buffers (removed — column-tiling handles partial cols)
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # v31b: NO tensor_copy to gate_t1_128/up_t1_128 (buffers removed)

            # Gate/Up matmul — v31b: use column-tiling for i_tile==1 (I1=64 real cols)
            ...
            for h1 in nl.affine_range(H_free_shard):
                for i_tile in nl.static_range(I_tiles):
                    if i_tile == 0:
                        g_stat = gate_up_bufs[k][0:_PMAX, h1, nl.ds(0, I0)]       # [128, 128]
                        u_stat = gate_up_bufs[k][0:_PMAX, h1, nl.ds(I, I0)]        # [128, 128]
                        # full stationary — use default tile_size (no tiling args needed)
                        nisa.nc_matmul(
                    ...
                    else:  # i_tile == 1: only I1=64 real cols in stationary free dim
                        g_stat = gate_up_bufs[k][0:_PMAX, h1, nl.ds(I0, I1)]      # [128, 64]
                        u_stat = gate_up_bufs[k][0:_PMAX, h1, nl.ds(I + I0, I1)]  # [128, 64]
                        # dst must match stationary free = I1=64 (dst partition = stationary free)
                        # gate_up_psum[I1:128, gu_base+1] stays zero from the memset above
                        nisa.nc_matmul(
                            dst=gate_up_psum[0:I1, gu_base + i_tile:gu_base + i_tile + 1],
                            stationary=g_stat,
                            moving=rmsnorm_normed_bf16[0:_PMAX, nl.ds((prg_id * H_free_shard + h1) * T, T)],
                            tile_position=(0, 0),
                            tile_size=(_PMAX, I1),   # use only left 64 PE columns
                        )
                        nisa.nc_matmul(
                            dst=gate_up_psum[0:I1, gu_base + I_tiles + i_tile:gu_base + I_tiles + i_tile + 1],
                            stationary=u_stat,
                            moving=rmsnorm_normed_bf16[0:_PMAX, nl.ds((prg_id * H_free_shard + h1) * T, T)],
                            tile_position=(0, 0),
                            tile_size=(_PMAX, I1),   # use only left 64 PE columns
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # Down matmul — v31b: row-tiling for i_tile==1 (I1=64 real rows)
            for h1_out in nl.affine_range(H_free_shard):
                nisa.nc_matmul(                                          # i_tile=0: full 128 contraction
                ...
                nisa.nc_matmul(                                          # i_tile=1: only 64 real I rows
                    dst=down_psum[0:_PMAX, d_base + h1_out:d_base + h1_out + 1],
                    stationary=down_full1_bufs[k][0:I1, nl.ds(h1_out * _PMAX, _PMAX)],  # [64, 128]
                    moving=inter_bf16[0:I1, 1:2],                                        # [64, 1]
                    tile_position=(0, 0),
                    tile_size=(I1, _PMAX),   # use only top 64 PE rows for contraction
                )
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v31b with native weight layouts — no preprocessing required.
