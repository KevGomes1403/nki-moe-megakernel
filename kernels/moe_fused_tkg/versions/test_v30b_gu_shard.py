"""
Test harness for kernel_v30b_gu_shard.py
Compares v30b (H-sharded gate/up) against a pure PyTorch fp32-promoted reference.
Tolerance: atol=1e-3, rtol=1e-2 (fp32-promoted reference per CLAUDE.md).
Also benchmarks v30b.
"""
import sys
import os
...
from kernel_v30b_gu_shard import run as run_v30b
from kernel_v30b_gu_shard import qwen3_moe_fused_tkg_sbuf_io
...
print("Running v30b gu_shard kernel...")
out_v30b = run_v30b(inp, gamma, router_w, gate_up_w, down_w)
xm.mark_step()

# Transfer to CPU for comparison
out_v30b_cpu = out_v30b.cpu().float().numpy()
ref_out_np   = ref_out.float().numpy()

max_diff  = np.max(np.abs(ref_out_np - out_v30b_cpu))
mean_diff = np.mean(np.abs(ref_out_np - out_v30b_cpu))

print(f"\n--- Comparison Results ---")
print(f"ref  output shape : {ref_out_np.shape}")
print(f"v30b output shape : {out_v30b_cpu.shape}")
print(f"max_diff          : {max_diff:.6f}")
print(f"mean_diff         : {mean_diff:.6f}")

try:
    np.testing.assert_allclose(ref_out_np, out_v30b_cpu, rtol=1e-2, atol=1e-3)
    print("\nPASS — v30b matches PyTorch reference within atol=1e-3, rtol=1e-2")
except AssertionError as e:
    print(f"\nFAIL — {e}")
    # Extra diagnostics
    diff = np.abs(ref_out_np - out_v30b_cpu)
    failing = np.argwhere(diff > 1e-3 + 1e-2 * np.abs(ref_out_np))
    print(f"Number of failing elements: {len(failing)}")
    if len(failing) > 0:
        idx = failing[0]
        print(f"First failing element idx={tuple(idx)}: ref={ref_out_np[tuple(idx)]:.6f}, v30b={out_v30b_cpu[tuple(idx)]:.6f}")
    sys.exit(1)
...
if ntff_path and neff_path:
    ...
    _print_profile_results(prof, "qwen3_moe_fused_tkg_sbuf_io (v30b)")
else:
...
if bench_neff:
    _pending_bench.append({
        "neff": bench_neff, "warmup": 5, "iters": 50, "name": "qwen3_moe_fused_tkg_sbuf_io_v30b"
    })

r = BenchmarkResult(prof, bench_neff or neff_path or "", ntff_path)
print(f"\n--- v30b Benchmark Metrics ---")
