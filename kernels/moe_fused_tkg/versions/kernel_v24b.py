...
kernel_v24b — Plan B: DMA tile-1 in prefetch phase (eliminate 256 tensor_copy)
===============================================================================

Interface contract: identical to v20a.
  gate_up_w  [E=128, H=2048, 2*I=384] bf16
  down_w     [E=128, I=192,  H=2048]   bf16

Plan B changes vs v22b:
  Move tile-1 loading (gate_t1_128, up_t1_128) from the per-expert compute
  phase INTO the DMA prefetch phase.  Instead of 32 tensor_copy calls per
  expert (256 total for K=8), we issue 2 extra DGE indirect DMA calls per
  expert in the prefetch window.  Since DMA runs concurrently with compute,
  these calls are fully hidden behind the previous expert's compute, leaving
  the tensor/vector engines free for the matmul/SiLU/down pipeline.

  gate_t1_128 and up_t1_128 are also ping-ponged (slot0/slot1) so that
  DMA4/DMA5 writing into s_nxt does not clobber s_cur which is in active
  use by the compute phase.

  New prefetch phase (5 DMA calls per expert, targeting slot s_nxt):
    DMA1: gate_up         -> gate_up_slot[s_nxt]
    DMA2: down0           -> down0_slot[s_nxt]
    DMA3: down1           -> down1_slot[s_nxt]
    DMA4: gate tile-1     -> gate_t1_slot[s_nxt][0:_PMAX, 0:H_free, 0:I1]
    DMA5: up   tile-1     -> up_t1_slot[s_nxt][0:_PMAX, 0:H_free, 0:I1]

  Compute phase (no tensor_copy -- uses gate_t1_slot[s_cur] / up_t1_slot[s_cur]):
    gate/up matmul
    SiLU, inter
    down matmul
    flush, scale, accumulate

  Zero-padding: gate_t1_slot{0,1}[..., I1:I0] and up_t1_slot{0,1}[..., I1:I0]
  are zeroed by hoisted memsets before the k-loop. DMA4/DMA5 only write cols
  [0:I1]. Cols [I1:I0=128] remain zero. ✓

  SBUF budget vs v22b: +2 * (128*16*64*2/128) = +4 KB per slot pair = +8 KB total.
  Still well within 224 KiB SBUF limit.
"""
...
_I1 = 64     # second tile (partial: 64 valid rows, 64 zero-padded to I0=128)
...
def qwen3_moe_fused_tkg(
    ...
    #
    # All weight buffers (gate_up, down0, down1, gate_t1, up_t1) are
    # ping-ponged across two slots.  For each k:
    #   s_cur = k % 2  -> slot holding pre-fetched data for expert k
    #   s_nxt = 1-s_cur -> slot to receive prefetch for expert k+1
    #
    # Prefetch phase issues 5 DMA calls targeting s_nxt:
    #   DMA1-3: gate_up / down0 / down1
    #   DMA4:   gate tile-1 cols [I0:I0+I1] -> gate_t1_slot[s_nxt]
    #   DMA5:   up   tile-1 cols [I+I0:I+I0+I1] -> up_t1_slot[s_nxt]
    #
    # Compute phase reads from s_cur; no tensor_copy needed.
    # -----------------------------------------------------------------------
    output_temp = nl.ndarray((_PMAX, H_free_shard, T), dtype=nl.float32, buffer=nl.sbuf)

    # Ping-pong slot buffers
    ...
    # down1_slot{0,1}: rows 0:I1 valid, I1:_PMAX = 0 (hoisted memset)
    ...
    # gate_t1_slot{0,1}: [_PMAX, H_free, I0] -- DMA4 writes cols 0:I1, cols I1:I0 = 0
    gate_t1_slot0 = nl.ndarray((_PMAX, H_free, I0), dtype=gate_up_w.dtype, buffer=nl.sbuf)
    gate_t1_slot1 = nl.ndarray((_PMAX, H_free, I0), dtype=gate_up_w.dtype, buffer=nl.sbuf)
    # up_t1_slot{0,1}: same shape
    up_t1_slot0 = nl.ndarray((_PMAX, H_free, I0), dtype=gate_up_w.dtype, buffer=nl.sbuf)
    up_t1_slot1 = nl.ndarray((_PMAX, H_free, I0), dtype=gate_up_w.dtype, buffer=nl.sbuf)
    ...
    for t in nl.static_range(T):
        ...
        # ------------------------------------------------------------------
        # Zero pad rows I1:_PMAX in down1 slot buffers
        nisa.memset(down1_slot0[nl.ds(I1, I1), 0:H_shard], value=0.0)
        nisa.memset(down1_slot1[nl.ds(I1, I1), 0:H_shard], value=0.0)

        # Zero cols I1:I0 in all gate_t1 / up_t1 slots (DMA4/DMA5 only write 0:I1)
        nisa.memset(gate_t1_slot0[0:_PMAX, 0:H_free, nl.ds(I1, I1)], value=0.0)
        nisa.memset(gate_t1_slot1[0:_PMAX, 0:H_free, nl.ds(I1, I1)], value=0.0)
        nisa.memset(up_t1_slot0[0:_PMAX, 0:H_free, nl.ds(I1, I1)],   value=0.0)
        nisa.memset(up_t1_slot1[0:_PMAX, 0:H_free, nl.ds(I1, I1)],   value=0.0)
        ...
        # ------------------------------------------------------------------
        # Prolog: Issue all 5 DMA calls for expert 0 -> slot 0
        # ------------------------------------------------------------------
        ...
        # DMA4: gate tile-1 for expert 0 -> gate_t1_slot0[0:_PMAX, 0:H_free, 0:I1]
        nisa.dma_copy(
            dst=gate_t1_slot0[0:_PMAX, 0:H_free, 0:I1],
            src=gate_up_w.ap(
        ...
        # DMA5: up tile-1 for expert 0 -> up_t1_slot0[0:_PMAX, 0:H_free, 0:I1]
        nisa.dma_copy(
            dst=up_t1_slot0[0:_PMAX, 0:H_free, 0:I1],
            src=gate_up_w.ap(
                pattern=[[_GU_FLAT, _PMAX], [_PMAX * _GU_FLAT, H_free], [1, I1]],
                offset=I + I0,
                scalar_offset=expert_id_0,
        ...
        # selects between the two slot buffers statically.
        # ------------------------------------------------------------------
        for k in nl.static_range(K):
            s_cur = k % 2
            s_nxt = 1 - s_cur

            # Prefetch expert k+1 -> slot s_nxt (5 DMA calls)
            if k < K - 1:
                ...
                if s_nxt == 0:
                    ...
                    nisa.dma_copy(
                        dst=gate_t1_slot1[0:_PMAX, 0:H_free, 0:I1],
                        src=gate_up_w.ap(
                            pattern=[[_GU_FLAT, _PMAX], [_PMAX * _GU_FLAT, H_free], [1, I1]],
                            offset=I0,
                            scalar_offset=expert_id_k1,
                    ...
                    nisa.dma_copy(
                        dst=up_t1_slot1[0:_PMAX, 0:H_free, 0:I1],
                        src=gate_up_w.ap(
                            pattern=[[_GU_FLAT, _PMAX], [_PMAX * _GU_FLAT, H_free], [1, I1]],
                            offset=I + I0,
                            scalar_offset=expert_id_k1,
            ...
            # gate_t1 / up_t1 already loaded by DMA4/DMA5 into s_cur slot.
            # No tensor_copy needed -- 256 instructions eliminated.
            # ------------------------------------------------------------------

            # Select slot buffers for s_cur
            if s_cur == 0:
                gate_up_buf  = gate_up_slot0
                down0_buf    = down0_slot0
                down1_buf    = down1_slot0
                gate_t1_buf  = gate_t1_slot0
                up_t1_buf    = up_t1_slot0
            else:
                gate_up_buf  = gate_up_slot1
                down0_buf    = down0_slot1
                down1_buf    = down1_slot1
                gate_t1_buf  = gate_t1_slot1
                up_t1_buf    = up_t1_slot1
            ...
            for h1 in nl.affine_range(H_free):
                for i_tile in nl.static_range(I_tiles):
                    if i_tile == 0:
                        ...
                        g_stat = gate_t1_buf[0:_PMAX, h1, 0:I0]            # gate t1
                        u_stat = up_t1_buf[0:_PMAX, h1, 0:I0]              # up t1
                    nisa.nc_matmul(
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v24b with native weight layouts -- no preprocessing required."""
    import torch
