...
kernel_v28i — v28f with dma_transpose for inp+gamma loading
=============================================================
Based on v28f (Plan F: _ROUTER_BATCH=16 + router dge_mode=3).

Changes vs v28f (Plan I: dma_transpose):
  I1: Replace 3-instruction inp loading (dma_copy + nc_transpose + activation)
      with single nisa.dma_transpose call. Saves 2 instructions.
  I2: Replace 3-instruction gamma loading (dma_copy + nc_transpose + activation)
      with single nisa.dma_transpose call. Saves 2 instructions.
  Total: 4 instructions removed from Stage 1 hot path.
...
def qwen3_moe_fused_tkg(
    ...
    # Plan I1: dma_transpose replaces dma_copy + nc_transpose + activation (3→1 instruction)
    # dma_transpose requires 4D src. Permutation [3,1,2,0]: dst[p,0,0,f] = src[f,0,0,p].
    # inp_2d_hbm_reshaped: HBM[H_free*T=16, PMAX=128] → reshape 4D → [16, 1, 1, 128]
    # dst_4d: SBUF[PMAX=128, 1, 1, H_free*T=16] → reshape to [PMAX=128, H_free*T=16]
    inp_4d_hbm = inp_2d_hbm_reshaped.reshape((H_free * T, 1, 1, _PMAX))
    rmsnorm_out_4d = nl.ndarray((_PMAX, 1, 1, H_free * T), dtype=inp.dtype, buffer=nl.sbuf)
    nisa.dma_transpose(dst=rmsnorm_out_4d, src=inp_4d_hbm)
    rmsnorm_out = rmsnorm_out_4d.reshape((_PMAX, H_free * T))

    gamma_1d = gamma.reshape((H,))
    gamma_1d_hbm_reshaped = gamma_1d.reshape((H_free, _PMAX))
    # Plan I2: dma_transpose replaces dma_copy + nc_transpose + activation (3→1 instruction)
    # dma_transpose requires 4D src. Permutation [3,1,2,0]: dst[p,0,0,f] = src[f,0,0,p].
    # gamma_1d_hbm_reshaped: HBM[H_free=16, PMAX=128] → reshape 4D → [16, 1, 1, 128]
    # dst_4d: SBUF[PMAX=128, 1, 1, H_free=16] → reshape to [PMAX=128, H_free=16]
    gamma_4d_hbm = gamma_1d_hbm_reshaped.reshape((H_free, 1, 1, _PMAX))
    gamma_sb_4d = nl.ndarray((_PMAX, 1, 1, H_free), dtype=gamma.dtype, buffer=nl.sbuf)
    nisa.dma_transpose(dst=gamma_sb_4d, src=gamma_4d_hbm)
    gamma_sb = gamma_sb_4d.reshape((_PMAX, H_free))
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v28i with native weight layouts — no preprocessing required.
