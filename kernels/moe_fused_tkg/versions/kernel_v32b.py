...
kernel_v32b — Plan B: Staged DMA — down-weight prefetch overlapped with gate/up matmul
=======================================================================================
Based on kernel_v30b_gu_shard (H-sharded gate/up projection).

Plan B change (v32b):
  The original v30b Phase 1a issues all 12 DMAs per wave up-front (4 gate/up + 4 down_full0
  + 4 down_full1) before any matmul starts. This means the first gate/up matmul cannot
  begin until all 12 DMAs have their addresses resolved, including the last down_full1 DMA
  which is not consumed until after gate/up matmul + sendrecv + silu.

  In v32b, Phase 1a is split into:
    - Phase 1-GU:  Issue only the 4 gate_up_w DMAs (same .ap() pattern and offsets as v30b).
    - Phase 1-D:   Issue the 8 down DMAs (down_full0 + down_full1 for k=0..3), issued
                   AFTER Phase 1-GU returns but BEFORE Phase 2a's gate/up matmul.

  The down weight buffers are placed in a dedicated "down_pf" scope so the scheduler
  knows they cannot alias gate/up buffers. The scope is opened after gate_up_bufs are
  allocated (but before down bufs are allocated) and closed after the wave's down
  matmuls complete.

  Ordering invariant preserved:
    - memset(down_full1_bufs[k][I1:I0]) happens BEFORE the DMA into [0:I1] to avoid races.
    - gate_t1_128 / up_t1_128 remain in the outer token scope (not in the prefetch scope).

  Expected effect:
    - DMA engine switches from "finish all 12 DMAs, then compute" to "finish 4 gate/up
      DMAs, start compute, drain 8 down DMAs during compute".
    - Tensor engine starts sooner; DMA engine keeps busy during matmul.
    - dma_active_pct stays high but overlaps with tensor_engine_pct → should reduce idle.
"""
...
def _qwen3_moe_sbuf_in_sbuf_out(
    ...
    for t in nl.static_range(T):
        ...
        # These stay in the outer token scope (NOT in the down_pf scope) — they are
        # reused between waves, so they must remain live for both waves.
        ...
        # Phase 1-GU (wave 0): Issue only the 4 gate_up_w DMAs.
        # These are needed immediately by Phase 2a's gate/up matmul.
        ...
        # Open dedicated scope for down prefetch buffers (wave 0).
        # This advertises disjoint SBUF addresses from gate_up_bufs to the scheduler,
        # allowing down DMAs to overlap with gate/up matmul.
        sbm.open_scope(name=f"down_pf_w0_t{t}")
        ...
        # IMPORTANT: memset BEFORE the DMA to avoid racing with the DMA destination
        # ------------------------------------------------------------------
        for k_pad in range(4):
            nisa.memset(down_full1_bufs[k_pad][nl.ds(I1, I1), 0:H_shard], value=0.0)

        # Phase 1-D (wave 0): Issue the 8 down DMAs.
        # These are issued AFTER gate_up DMAs (Phase 1-GU) but BEFORE the gate/up matmul.
        # The down weights are not consumed until after gate/up matmul + sendrecv + silu,
        # so the DMA engine can fill them concurrently with tensor engine gate/up compute.
        for k in nl.static_range(_K_WAVE):
            expert_id = top8_idx.ap(pattern=[[K, 1], [1, 1]], offset=t * K + k)
        ...
        # Each core has a partial sum over its H_shard; need full sum.
        # sendrecv: each core sends its partial to the other and receives
        # the other's partial, then reduces local + received = full.
        # ------------------------------------------------------------------
        ...
        # Close the down prefetch scope for wave 0.
        # All wave 0 down matmuls are complete; the buffers are no longer needed.
        sbm.close_scope()  # down_pf_w0_t
        ...
        # Phase 1-GU (wave 1): Issue only the 4 gate_up_w DMAs for experts 4-7.
        # Reuses gate_up_bufs[0..3] — safe because wave 0 compute is complete.
        for k in nl.static_range(_K_WAVE):
        ...
        # Open dedicated scope for down prefetch buffers (wave 1).
        sbm.open_scope(name=f"down_pf_w1_t{t}")

        down_full0_buf0_w1 = sbm.alloc_stack((_PMAX, H_shard), down_w.dtype, buffer=nl.sbuf, name=f"down_full0_buf0_w1_t{t}")
        down_full0_buf1_w1 = sbm.alloc_stack((_PMAX, H_shard), down_w.dtype, buffer=nl.sbuf, name=f"down_full0_buf1_w1_t{t}")
        down_full0_buf2_w1 = sbm.alloc_stack((_PMAX, H_shard), down_w.dtype, buffer=nl.sbuf, name=f"down_full0_buf2_w1_t{t}")
        down_full0_buf3_w1 = sbm.alloc_stack((_PMAX, H_shard), down_w.dtype, buffer=nl.sbuf, name=f"down_full0_buf3_w1_t{t}")
        down_full0_bufs_w1 = [down_full0_buf0_w1, down_full0_buf1_w1, down_full0_buf2_w1, down_full0_buf3_w1]

        down_full1_buf0_w1 = sbm.alloc_stack((_PMAX, H_shard), down_w.dtype, buffer=nl.sbuf, name=f"down_full1_buf0_w1_t{t}")
        down_full1_buf1_w1 = sbm.alloc_stack((_PMAX, H_shard), down_w.dtype, buffer=nl.sbuf, name=f"down_full1_buf1_w1_t{t}")
        down_full1_buf2_w1 = sbm.alloc_stack((_PMAX, H_shard), down_w.dtype, buffer=nl.sbuf, name=f"down_full1_buf2_w1_t{t}")
        down_full1_buf3_w1 = sbm.alloc_stack((_PMAX, H_shard), down_w.dtype, buffer=nl.sbuf, name=f"down_full1_buf3_w1_t{t}")
        down_full1_bufs_w1 = [down_full1_buf0_w1, down_full1_buf1_w1, down_full1_buf2_w1, down_full1_buf3_w1]

        # Zero pad region (rows I1:I0 = 64:128) for wave 1 down_full1 buffers
        # IMPORTANT: memset BEFORE the DMA to avoid racing with the DMA destination
        for k_pad in range(4):
            nisa.memset(down_full1_bufs_w1[k_pad][nl.ds(I1, I1), 0:H_shard], value=0.0)

        # Phase 1-D (wave 1): Issue the 8 down DMAs for experts 4-7.
        # Issued AFTER gate_up DMAs but BEFORE gate/up matmul.
        for k in nl.static_range(_K_WAVE):
            kk = k + 4  # global expert index
            expert_id = top8_idx.ap(pattern=[[K, 1], [1, 1]], offset=t * K + kk)

            nisa.dma_copy(
                dst=down_full0_bufs_w1[k],
                src=down_w.ap(
                    pattern=[[H, I0], [1, H_shard]],
                    offset=prg_id * H_shard,
                    scalar_offset=expert_id,
            ...
            nisa.dma_copy(
                dst=down_full1_bufs_w1[k][0:I1, 0:H_shard],
                src=down_w.ap(
                    pattern=[[H, I1], [1, H_shard]],
                    offset=I0 * H + prg_id * H_shard,
                    scalar_offset=expert_id,
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # Down matmul (uses wave 1 down buffers from down_pf_w1 scope)
            for h1_out in nl.affine_range(H_free_shard):
                nisa.nc_matmul(
                    dst=down_psum[0:_PMAX, d_base + h1_out:d_base + h1_out + 1],
                    stationary=down_full0_bufs_w1[k][0:_PMAX, nl.ds(h1_out * _PMAX, _PMAX)],
                    moving=inter_bf16[0:_PMAX, 0:1],
                )
                nisa.nc_matmul(
                    dst=down_psum[0:_PMAX, d_base + h1_out:d_base + h1_out + 1],
                    stationary=down_full1_bufs_w1[k][0:_PMAX, nl.ds(h1_out * _PMAX, _PMAX)],
                    moving=inter_bf16[0:_PMAX, 1:2],
                )
        ...
        # Close the down prefetch scope for wave 1.
        # All wave 1 down matmuls are complete; the buffers are no longer needed.
        sbm.close_scope()  # down_pf_w1_t
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v32b with native weight layouts — no preprocessing required.
