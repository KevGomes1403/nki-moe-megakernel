...
kernel_v29j — v28f with shared_hbm output + remove dead aff_bcast memset
==========================================================================
Based on v28f (Plan F: ROUTER_BATCH=16).

Changes vs v28f (Plan J):
  J1: Fix output buffer: change `buffer=nl.hbm` → `buffer=nl.shared_hbm` in Stage 5.
  J2: Remove the dead `nisa.memset(aff_bcast, value=0.0)` call. This is dead code:
      aff_bcast rows 0-127 are ALL overwritten by the subsequent tensor_copy and
      4 nc_stream_shuffle calls covering all rows 0-127.
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v29j with native weight layouts — no preprocessing required.
