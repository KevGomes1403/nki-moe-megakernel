...
kernel_v22b — Plan B: Double-buffer expert weights (DMA-compute pipeline)
===========================================================================
...
Plan B changes vs v20a:
  Replace the flat K-slot SBUF buffers (gate_up_flat, down_full0_flat, down_full1_flat)
  with 2-slot ping-pong buffers (gate_up_slot0/1, down0_slot0/1, down1_slot0/1).

  New loop structure (inside `for t in nl.static_range(T)`):
    1. Issue DMA for expert 0 → slot 0  (prolog)
    2. for k in nl.static_range(K):
         s_cur = k % 2   (slot holding expert k)
         s_nxt = (k+1)%2 (slot to prefetch into)
         if k < K-1:
           Issue DMA for expert k+1 → slot s_nxt  (overlaps compute below)
         Compute expert k from slot s_cur
         (tile-1 prep, gate/up matmul, SiLU, down matmul, affinity scale, accumulate)

  nl.static_range unrolls loops at compile time, so Python if/else on k and
  k%2 work correctly — each iteration becomes a separate code region.

  The 2-slot buffer sizes are:
    gate_up_slot{0,1}: [_PMAX, H_free, _GU_FLAT]  = [128, 16, 384]
    down0_slot{0,1}:   [_PMAX, H_shard]            = [128, 1024]
    down1_slot{0,1}:   [_PMAX, H_shard]            = [128, 1024]

  SBUF budget for Stage 4 (per partition lane, 224 KiB limit):
    gate_up_slot0:  128 * 16 * 384 * 2 / 128 = 12288 B = 12 KB
    gate_up_slot1:  12 KB
    down0_slot0:    128 * 1024 * 2 / 128      = 2048 B  = 2 KB
    down0_slot1:    2 KB
    down1_slot0:    2 KB
    down1_slot1:    2 KB
    gate_t1_128:    128 * 16 * 128 * 2 / 128  = 4096 B  = 4 KB
    up_t1_128:      4 KB
    other (output_temp, aff_bcast, gate/up/inter/down sbufs): ~40 KB
    Stage 1-3 (rmsnorm, router): ~72 KB peak (freed before Stage 4)
    Stage 4 peak: ~80 KB << 224 KiB limit
"""
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v22b with native weight layouts — no preprocessing required."""
    import torch
