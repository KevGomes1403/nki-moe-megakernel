...
v4b changes vs v3b:
  Plan B — Pre-broadcast all K affinity weights before expert loops:
    - aff_bcast [128, K] allocated once per token (outside k loops).
    - norm_weights[t, 0:K] copied to partition 0, then 4 nc_stream_shuffles broadcast
      to all 128 partitions — 4 shuffles total vs K×4=32 shuffles in v3b.
    - In Pass 2, per-expert aff_sb block (memset + tensor_copy + 4 shuffles) removed;
      tensor_scalar now uses aff_bcast[0:_PMAX, k:k+1] directly.
    - Reduces instruction count: 6×K=48 per token -> 6 per token.
"""
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        # 4 shuffles broadcast T=1 row's K values to all 128 partitions at once.
