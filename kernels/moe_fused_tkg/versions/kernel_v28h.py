...
kernel_v28h — v28e + Plan F + Plan G (combined)
================================================
Based on v28e (Plan E: router DMA hw_dge + batch=8).

Changes vs v28e (Plans F+G combined):
  F1: _ROUTER_BATCH increased from 8 to 16, reducing router DMA calls from
      2 to 1 (inner loop runs 16 matmul iterations per DMA call).
  G1: Remove nisa.memset(aff_bcast, value=0.0) — padding region is
      immediately overwritten by tensor_copy + 4 nc_stream_shuffles.
  G2: Eliminate sum_reduced_sb intermediate buffer. tensor_partition_reduce
      now writes directly into norm_sum_sb[0:1, 0:T].
...
# Router DMA batching: 16 tiles per DMA (Plan H: F+G combined, 1 DMA call total)
_ROUTER_BATCH = 16
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        # G1: memset removed — padding rows overwritten by tensor_copy + shuffles
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v28h with native weight layouts — no preprocessing required.
