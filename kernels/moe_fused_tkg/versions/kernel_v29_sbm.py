...
kernel_v29_sbm — v28f adapted to use SbufManager for SBUF allocation
=====================================================================
All nl.ndarray(..., buffer=nl.sbuf) replaced with sbm.alloc_stack / sbm.alloc_heap.
psum tensors are unchanged.
"""
...
def _qwen3_moe_tkg_sbuf_out(
    inp,        # [B=1, 1, H=2048] bf16 — HBM
    gamma,      # [1, H=2048] bf16       — HBM
    router_w,   # [H=2048, E=128] bf16   — HBM
    gate_up_w,  # [E=128, H=2048, 2*I=384] bf16 — HBM
    ...
    rmsnorm_normed_bf16 = sbm.alloc_heap((_PMAX, H_free * T), inp.dtype, buffer=nl.sbuf, name="rmsnorm_normed_bf16")
    ...
    rmsnorm_out = sbm.alloc_stack((_PMAX, H_free * T), inp.dtype, buffer=nl.sbuf, name="rmsnorm_out")
    ...
    router_w_wide_sb = sbm.alloc_stack((_PMAX, _ROUTER_BATCH, E), inp.dtype, buffer=nl.sbuf, name="router_w_wide_sb")
    ...
    for t in nl.static_range(T):
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            inter_bf16 = sbm.alloc_stack((_PMAX, I_tiles), inp.dtype, buffer=nl.sbuf, name=f"inter_bf16_w0k{k}_t{t}")
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            inter_bf16 = sbm.alloc_stack((_PMAX, I_tiles), inp.dtype, buffer=nl.sbuf, name=f"inter_bf16_w1k{k}_t{t}")
            nisa.activation(inter_bf16, op=nl.copy, data=inter_f32)
    ...
    out_sb = sbm.alloc_stack((T, H_shard), inp.dtype, buffer=nl.sbuf, name="out_sb")
...
@nki.jit
def qwen3_moe_fused_tkg_sbm(inp, gamma, router_w, gate_up_w, down_w):
    """
    Thin wrapper: creates a SbufManager and delegates to _qwen3_moe_tkg_sbuf_out.
    Invoked as: qwen3_moe_fused_tkg_sbm[2](inp, gamma, router_w, gate_up_w, down_w)
    """
    sbm = SbufManager(0, nl.tile_size.total_available_sbuf_size, use_auto_alloc=True)
    return _qwen3_moe_tkg_sbuf_out(inp, gamma, router_w, gate_up_w, down_w, sbm=sbm)


def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v29_sbm with native weight layouts — no preprocessing required.
    ...
    outputs = qwen3_moe_fused_tkg_sbm[2](inp, gamma, router_w, gate_up_w, down_w)
