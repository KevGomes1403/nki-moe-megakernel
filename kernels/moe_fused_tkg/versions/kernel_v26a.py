...
kernel_v26a — Plan A: Pre-scale intermediate by affinity + shared down PSUM accumulation
=============================================================
Based on v25e (Plan E: Combined micro-optimizations).

Changes vs v25e:
  Plan A: Pre-scale + shared down PSUM
    1. Scale inter_f32 by affinity DURING the f32→bf16 cast via nisa.activation scale param.
       This fuses the affinity multiplication into the existing cast instruction.
    2. Use a SHARED down_psum with only H_free_shard=8 cols (instead of K*H_free_shard=64).
       All 8 experts accumulate into the SAME PSUM location via nc_matmul add-to-PSUM.
    3. After the k loop, ONE PSUM flush (instead of per-expert flush+scale+accumulate).
    4. Remove per-expert PSUM flush, affinity scale, and if/else accumulation from k loop.

  Preserved from v25e:
    Change 1: Reorder norm_weights computation after Phase 1 DMA start.
    Change 3 (gate_up only): Per-expert gate_up_psum slots [128, 32].
...
PSUM budget: gate_up_psum [128, 32] + down_psum [128, 8] = 40 cols < 512 max. ✓
"""
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        # ------------------------------------------------------------------
        # PSUM allocation:
        # gate_up_psum: per-expert [_PMAX, K * 2 * I_tiles] = [128, 32] (Change 3 from v25e)
        # down_psum:    SHARED [_PMAX, H_free_shard] = [128, 8] (Plan A: all experts accumulate here)
        # ------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        for k in nl.static_range(K):
            # Per-expert PSUM base offset for gate_up (Change 3 from v25e)
            ...
            # Plan A: Fuse affinity scale into f32→bf16 cast (single instruction)
            # nisa.activation computes: dst = op(data * scale) with scale as [N,1] vector
            inter_bf16 = nl.ndarray((_PMAX, I_tiles), dtype=inp.dtype, buffer=nl.sbuf)
            nisa.activation(inter_bf16, op=nl.copy, data=inter_f32, scale=aff_bcast[0:_PMAX, k:k + 1])

            # ----------------------------------------------------------------
            # Down matmul: [I, H] @ [I, T=1] → [H_shard, T] partial sum
            # Plan A: SHARED PSUM slot — all experts accumulate at offset 0
            # nc_matmul naturally adds to PSUM, so all K experts sum together
            # ----------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # Plan A: ONE PSUM flush after all K experts have accumulated
        # ------------------------------------------------------------------
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v26a with native weight layouts — no preprocessing required.
