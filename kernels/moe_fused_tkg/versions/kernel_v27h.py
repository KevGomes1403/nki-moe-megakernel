...
kernel_v27h — v27d + fused scale+accumulate via scalar_tensor_tensor
=====================================================================
Based on v27d (best: 103.48 μs).

Changes vs v27d:
  Replace 3-instruction flush pattern (activation + tensor_scalar + tensor_tensor/copy)
  with 2-instruction pattern per expert using nisa.scalar_tensor_tensor:
    - k==0 wave 0: activation→output_temp + tensor_scalar in-place (direct init)
    - k>0 wave 0 + all wave 1: activation→down_result_sb + scalar_tensor_tensor
      computes: output_temp = (down_result_sb * aff) + output_temp
  Saves 8 instructions total (1 per expert × 8 experts).
  Eliminates 8 down_result_scaled allocations (reduced SBUF churn).
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # Flush down PSUM -> SBUF, fused scale+accumulate (saves 1 instr/expert)
            if k == 0:
                # k=0: no prior data — write PSUM directly to output_temp, then scale
                nisa.activation(
                ...
                nisa.tensor_scalar(
                    output_temp[0:_PMAX, 0:H_free_shard, t:t + 1],
                    data=output_temp[0:_PMAX, 0:H_free_shard, t:t + 1],
                    op0=nl.multiply,
                    operand0=aff_bcast[0:_PMAX, 0:1],  # wave 0, expert 0
                )
            else:
                # k>0: fuse scale+accumulate — computes (down_result_sb * aff) + output_temp
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # Flush down PSUM -> SBUF, fused scale+accumulate
            # Wave 1 always accumulates (output_temp initialized by wave 0)
            # Computes: output_temp = (down_result_sb * aff) + output_temp
            down_result_sb = nl.ndarray((_PMAX, H_free_shard), dtype=nl.float32, buffer=nl.sbuf)
            nisa.activation(
                down_result_sb,
