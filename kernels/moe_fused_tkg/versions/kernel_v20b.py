...
kernel_v20b — Plan B: 2D coalesced gate_up DMAs + full-H down DMA
=============================================================
...
Change 1 — gate_up DMA: replace single 3D DMA per expert with H_free=16 2D DMAs per expert,
  each [_PMAX, _GU_FLAT]. Within each H-tile, all 128 partitions × 384 cols are contiguous
  (stride=384=count → 1 HW packet per H-tile). Total gate_up packets: 16 per expert × 8 = 128.
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v20b with native weight layouts — no preprocessing required.
