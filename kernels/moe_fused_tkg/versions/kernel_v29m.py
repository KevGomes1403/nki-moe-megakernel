...
kernel_v29m — v28f with merged down_w DMA (down_full0+down_full1→single DMA) + shared_hbm output
==========================================
Based on v28f (Plan F: ROUTER_BATCH=16).

Changes vs v28f (Plan M: merged down DMA):
  M1: down_w preprocessed in run() from [E, I=192, H] to [E, 256, H] (zero-padded).
  M2: down_full0_bufs + down_full1_bufs merged into down_merged_bufs [PMAX, H_shard*2].
  M3: Two per-expert DMA calls (down_full0 + down_full1) replaced with ONE merged DMA.
  M4: Output buffer uses nl.shared_hbm.
  Result: 16 indirect DMAs per token instead of 24 (−8 sw_dge descriptor chains).

Interface contract (no repack required):
  gate_up_w  [E=128, H=2048, 2*I=384] bf16
  down_w     [E=128, I=192,  H=2048]   bf16  (preprocessed to [E, 256, H] in run())

SBUF Budget (per partition):
  4 x gate_up [16 x 384 x 2] = 48 KiB
  4 x down_merged [128 x 2048]= 16 KiB
  gate_t1 + up_t1             =  8 KiB
  misc                        = ~2 KiB
  Total: ~74 KiB << 224 KiB (same as v28f)
...
# Padded I dimension for merged down DMA
_I_PAD = 256  # = I0 * 2 = 128 * 2
...
def qwen3_moe_fused_tkg(
    ...
    gate_up_w,  # [E=128, H=2048, 2*I=384] bf16
    down_w,     # [E=128, 256,  H=2048]  bf16  (zero-padded, was I=192)
):
    """
    ...
    for t in nl.static_range(T):
        ...
        # Plan M: merged down buffers [PMAX, 2, H_shard] replaces down_full0 + down_full1
        # Shape: [PMAX, 2, H_shard] — dim1=0 is rows 0:128 (was down_full0), dim1=1 is rows 128:256 (was down_full1+zeros)
        down_merged_buf0 = nl.ndarray((_PMAX, 2, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_merged_buf1 = nl.ndarray((_PMAX, 2, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_merged_buf2 = nl.ndarray((_PMAX, 2, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_merged_buf3 = nl.ndarray((_PMAX, 2, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_merged_bufs = [down_merged_buf0, down_merged_buf1, down_merged_buf2, down_merged_buf3]
        ...
        # Phase 1a: Load experts 0-3 (8 DMAs: 1 gate_up + 1 down_merged per expert)
        for k in nl.static_range(_K_WAVE):
            ...
            # Plan M: single merged DMA for down_w rows 0:128 and 128:256
            # down_w is [E, 256, H] where rows 192:256 are zero-padded
            # Pattern: [[H, I0=128], [I0*H, 2], [1, H_shard]]
            #   outer: 128 partitions (rows within each tile), step H per row
            #   middle: 2 tiles (tile0=rows 0:128, tile1=rows 128:256), step I0*H between tiles
            #   inner: H_shard contiguous columns
            nisa.dma_copy(
                dst=down_merged_bufs[k],
                src=down_w.ap(
                    pattern=[[H, I0], [I0 * H, 2], [1, H_shard]],
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # Down matmul — Plan M: use merged buffer [PMAX, 2, H_shard]
            # tile 0 (dim1=0): was down_full0 (expert rows 0:128)
            # tile 1 (dim1=1): was down_full1 (expert rows 128:192, then 192:256 are zeros from padding)
            for h1_out in nl.affine_range(H_free_shard):
                nisa.nc_matmul(
                    dst=down_psum[0:_PMAX, d_base + h1_out:d_base + h1_out + 1],
                    stationary=down_merged_bufs[k][0:_PMAX, 0, nl.ds(h1_out * _PMAX, _PMAX)],
                    moving=inter_bf16[0:_PMAX, 0:1],
                )
                nisa.nc_matmul(
                    dst=down_psum[0:_PMAX, d_base + h1_out:d_base + h1_out + 1],
                    stationary=down_merged_bufs[k][0:_PMAX, 1, nl.ds(h1_out * _PMAX, _PMAX)],
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # Plan M: single merged DMA for down_w rows 0:256 (rows 192:256 are zeros)
            nisa.dma_copy(
                dst=down_merged_bufs[k],
                src=down_w.ap(
                    pattern=[[H, I0], [I0 * H, 2], [1, H_shard]],
    ...
    # -----------------------------------------------------------------------
    # Stage 5: Transpose fp32->bf16, store to shared_hbm
    # -----------------------------------------------------------------------
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v29m with native weight layouts — preprocesses down_w to [E, 256, H].
    ...
    down_w: [E, I=192, H=2048]       — native layout (padded to [E, 256, H] here).
            [E, I_padded=256, H=2048] — already padded (used as-is).
    ...
    # Plan M: Pad down_w I dimension from 192 to 256 (add 64 zero rows)
    if down_w.shape[1] == _I:  # I=192 — needs padding
        down_w = F.pad(down_w, (0, 0, 0, 64))  # [E, 256, H]
    elif down_w.shape[1] != _I_PAD:
        # Some other size — slice to I first then pad
        down_w = down_w[:, :_I, :]
        down_w = F.pad(down_w, (0, 0, 0, 64))  # [E, 256, H]

    assert down_w.shape == (
        _E, _I_PAD, _H
    ), f"down_w shape {down_w.shape} != ({_E}, {_I_PAD}, {_H})"
