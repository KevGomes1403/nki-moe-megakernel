"""
Test harness for kernel_v32a.py (Plan A: section rotation pipelined K=8 loop)
Compares v32a against a pure PyTorch fp32-promoted reference.
Tolerance: atol=1e-3, rtol=1e-2 (fp32-promoted reference per CLAUDE.md).
Also benchmarks v32a.
"""
import sys
import os
...
from kernel_v32a import run as run_v32a
from kernel_v32a import qwen3_moe_fused_tkg_sbuf_io
...
print("Running v32a kernel...")
out_v32a = run_v32a(inp, gamma, router_w, gate_up_w, down_w)
xm.mark_step()

# Transfer to CPU for comparison
out_v32a_cpu = out_v32a.cpu().float().numpy()
ref_out_np   = ref_out.float().numpy()

max_diff  = np.max(np.abs(ref_out_np - out_v32a_cpu))
mean_diff = np.mean(np.abs(ref_out_np - out_v32a_cpu))

print(f"\n--- Comparison Results ---")
print(f"ref  output shape : {ref_out_np.shape}")
print(f"v32a output shape : {out_v32a_cpu.shape}")
print(f"max_diff          : {max_diff:.6f}")
print(f"mean_diff         : {mean_diff:.6f}")

try:
    np.testing.assert_allclose(ref_out_np, out_v32a_cpu, rtol=1e-2, atol=1e-3)
    print(f"\nPASS — v32a matches PyTorch reference within atol=1e-3, rtol=1e-2  max_diff={max_diff:.2e}")
except AssertionError as e:
    print(f"\nFAIL — {e}")
    # Extra diagnostics
    diff = np.abs(ref_out_np - out_v32a_cpu)
    failing = np.argwhere(diff > 1e-3 + 1e-2 * np.abs(ref_out_np))
    print(f"Number of failing elements: {len(failing)}")
    if len(failing) > 0:
        idx = failing[0]
        print(f"First failing element idx={tuple(idx)}: ref={ref_out_np[tuple(idx)]:.6f}, v32a={out_v32a_cpu[tuple(idx)]:.6f}")
    sys.exit(1)
...
if ntff_path and neff_path:
    ...
    _print_profile_results(prof, "qwen3_moe_fused_tkg_sbuf_io (v32a)")
else:
...
if bench_neff:
    _pending_bench.append({
        "neff": bench_neff, "warmup": 5, "iters": 50, "name": "qwen3_moe_fused_tkg_sbuf_io_v32a"
    })

r = BenchmarkResult(prof, bench_neff or neff_path or "", ntff_path)
print(f"\n--- v32a Benchmark Metrics ---")
