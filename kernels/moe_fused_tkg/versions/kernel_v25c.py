...
kernel_v25c — Plan C: Merge tile-1 copy into matmul loop for overlap
=====================================================================

Based on kernel_v20b. Original Plan C (K=64 matmul) failed: compiler either
rejects K<128 stationary or generates 2x slower code. Fallback 3: merge the
tile-1 tensor_copy operations into the same affine_range loop as the matmul,
so VectorE (copy) and TensorE (matmul) can overlap across H-tiles.

In v20b, tile-1 copies run in a separate static_range loop before the matmul
loop, serializing all 16 copies before any matmul starts. By merging them into
the affine_range matmul loop, the compiler can overlap copy[h1] with matmul[h1-1].
...
SBUF budget estimate (per partition lane, 224 KiB limit):
  ...
  other buffers:   ~46 KB (same as v20b)
...
def qwen3_moe_fused_tkg(
    ...
    # -----------------------------------------------------------------------
    # Stage 4: Selective-Expert MLP — Plan C: K=64 matmul for tile 1
    #
    # Phase 1: Issue ALL DMAs (gate_up + down) before any compute.
    # Phase 2: Serial expert compute — tile 1 uses 64-wide stationary directly
    #          from gate_up_flat, no copy to zero-padded buffer needed.
    # -----------------------------------------------------------------------
    ...
    for t in nl.static_range(T):
        ...
        # Plan C fallback 3: tile-1 copy merged into matmul loop via affine_range
        # to overlap copies with compute across H-tiles.
        ...
        for k in nl.static_range(K):  # static_range: k is compile-time constant for scalar_offset
            ...
            # down tile 0: full H width, I0=128 rows
            nisa.dma_copy(
            ...
            # down tile 1: full H width, I1=64 rows
            nisa.dma_copy(
        ...
        # Plan C fallback 3: tile-1 copy uses affine_range (not static_range)
        # to allow pipelining across H-tiles, then merged into matmul loop.
        # ------------------------------------------------------------------
        for k in nl.static_range(K):
            gate_expert_col = k * _GU_STRIDE_PER_EXPERT
            down_expert_col = k * H

            # --- Tile-1 prep using affine_range for pipelining ---
            for h1 in nl.affine_range(H_free):
                h_col = gate_expert_col + h1 * _GU_FLAT
                nisa.tensor_copy(
                    dst=gate_t1_128[0:_PMAX, h1, 0:I1],
                    src=gate_up_flat[0:_PMAX, nl.ds(h_col + I0, I1)],
                )
                nisa.tensor_copy(
                    dst=up_t1_128[0:_PMAX, h1, 0:I1],
                    src=gate_up_flat[0:_PMAX, nl.ds(h_col + I + I0, I1)],
                )
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v25c with native weight layouts — no preprocessing required.
