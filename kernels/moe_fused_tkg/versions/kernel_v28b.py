...
kernel_v28b — v27d with prologue instruction removals (Plan B)
==============================================================
Based on v27d (remove redundant inter-wave memsets).

Changes vs v27d:
  Change 1: Remove redundant nisa.memset(aff_bcast, value=0.0).
             All rows of aff_bcast are overwritten before being read.
  Change 2: Eliminate sum_reduced_sb intermediate + tensor_copy by writing
             tensor_partition_reduce directly into norm_sum_sb[0:1, :].
  Change 3: Skipped — activation_reduce causes ISA check failure in neuronx-cc.
...
def qwen3_moe_fused_tkg(
    ...
    # Change 2: Write partition_reduce directly into norm_sum_sb[0:1, :],
    #           eliminating sum_reduced_sb intermediate and tensor_copy.
    norm_sum_sb = nl.ndarray((_PMAX, T), dtype=nl.float32, buffer=nl.sbuf)
    nisa.tensor_partition_reduce(dst=norm_sum_sb[0:1, 0:T], data=rmsnorm_reduced[0:_PMAX, 0:T], op=nl.add)
    ...
    for t in nl.static_range(T):
        ...
        # Change 1: Remove redundant nisa.memset(aff_bcast, value=0.0).
        #           All 128 rows are overwritten before being read:
        #           row 0 by tensor_copy, rows 0-127 in groups of 32 by 4 shuffles.
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v28b with native weight layouts — no preprocessing required.
