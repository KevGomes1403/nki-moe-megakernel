...
kernel_v27d_fixed — v27d with gate/up PSUM cast to bf16 before SiLU
====================================================================
Based on v27d (Plan G: 2-wave, 4 buffer sets, no redundant memsets).

Root cause of v27d accuracy failures vs reference.py:
  The nc_matmul gate/up accumulates into fp32 PSUM (hardware behavior).
  The reference.py computes gate/up as bf16 @ bf16 → bf16, then .float().
  At output positions with near-zero values from cancellation of large expert
  contributions, the fp32 accumulation (kernel) and bf16 accumulation
  (reference) diverge by more than allclose(rtol=0.05, atol=0.05).

Fix: after accumulating gate/up into fp32 PSUM, cast to bf16 SBUF before
  SiLU, matching the reference's bf16 intermediate precision. This adds
  one activation(copy) cast per expert per wave but introduces no new
  SBUF buffers (reuses existing inter_f32 slot) and preserves all
  DMA/2-wave/buffer-reuse optimizations from v27d.

Changes vs v27d:
  Wave 0 and Wave 1 SiLU+inter computation:
    OLD: silu_res = silu(gate_up_psum fp32)  → inter_f32 → inter_bf16
    NEW: gate_sb = cast(gate_up_psum → bf16)
         up_sb_bf16 = cast(gate_up_psum[up cols] → bf16)
         silu_res = silu(gate_sb fp32, via cast back)  → inter_bf16 directly
  This matches: reference gate = (xt @ gate_w).float()  → bf16 gate intermediate.
...
SBUF Budget (per partition):
  ...
  gate_t1 + up_t1             =  8 KiB
  misc                        = ~2 KiB
  Total: ~74 KiB << 224 KiB (96 KiB freed vs v25e)
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # Fix: cast gate/up PSUM to bf16 first to match reference's bf16 matmul precision.
            # Reference computes: gate = (xt @ gate_w).float()  [bf16 matmul → float cast]
            # Kernel nc_matmul accumulates in fp32 PSUM (more precise than bf16 matmul).
            # At cancellation points, the fp32 accumulation diverges from bf16 accumulation.
            # Solution: cast fp32 PSUM → bf16 → fp32 before SiLU to match reference precision.
            gate_psum_bf16 = nl.ndarray((_PMAX, I_tiles), dtype=inp.dtype, buffer=nl.sbuf)
            nisa.activation(gate_psum_bf16, op=nl.copy, data=gate_up_psum[0:_PMAX, gu_base:gu_base + I_tiles])

            up_psum_bf16 = nl.ndarray((_PMAX, I_tiles), dtype=inp.dtype, buffer=nl.sbuf)
            nisa.activation(up_psum_bf16, op=nl.copy, data=gate_up_psum[0:_PMAX, gu_base + I_tiles:gu_base + 2 * I_tiles])

            # SiLU on bf16-rounded gate (fp32 computation, but input has bf16 precision)
            gate_f32 = nl.ndarray((_PMAX, I_tiles), dtype=nl.float32, buffer=nl.sbuf)
            nisa.activation(gate_f32, op=nl.copy, data=gate_psum_bf16)

            up_f32 = nl.ndarray((_PMAX, I_tiles), dtype=nl.float32, buffer=nl.sbuf)
            nisa.activation(up_f32, op=nl.copy, data=up_psum_bf16)

            silu_res = nl.ndarray((_PMAX, I_tiles), dtype=nl.float32, buffer=nl.sbuf)
            nisa.activation(silu_res, op=nl.silu, data=gate_f32)
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v27d_fixed with native weight layouts — no preprocessing required.
