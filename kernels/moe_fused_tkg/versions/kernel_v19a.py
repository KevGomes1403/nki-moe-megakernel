...
kernel_v19a — Plan E: Eliminate aff_bcast nc_stream_shuffle via direct tensor_scalar operand broadcast
=============================================================
...
Plan C optimisation (vs v16a):
  Issue down DMAs immediately after gate_up DMA within each k-loop iteration.

Plan D optimisation (vs v18a):
  Restructure Stage 4 into two phases:
  1. Down prefetch loop (nl.affine_range(K)): issues all 16 down DMAs simultaneously.
     With affine_range the compiler signals all iterations independent → DMA engine
     queues all 16 packets at once before any compute begins.
  2. Compute loop (nl.static_range(K)): reads down from pre-loaded buffers; gate_up
     uses K=2 double-buffering (next expert prefetched during current compute).

  Root cause being fixed: down DMA for expert k+1 was serialized after expert k's
  gate+down compute chain. By pre-fetching all K=8 down tiles before any compute,
  the DMA engine can pipeline all 16 down packets simultaneously. Gate_up uses
  double-buffering (1-expert lookahead) to fit within SBUF budget.

  SBUF note: gate_up_tiles[K=8] would require 96 KB, exceeding the ~66 KB remaining
  budget. Using K=2 double-buffer for gate_up (24 KB) keeps total ~172 KB < 224 KB.

Plan E optimisation (vs v18b):
  Eliminate the 4× nc_stream_shuffle used to broadcast affinity weights to all 128
  partition lanes. Instead, pass norm_weights[t:t+1, k:k+1] (shape [1,1]) directly
  as operand0 to nisa.tensor_scalar — hypothesised to broadcast across all 128 lanes.

  Removes: aff_bcast [_PMAX=128, K=8] fp32 (~4 KB SBUF) + 6 setup instructions
           (memset + tensor_copy + 4× nc_stream_shuffle) per token.
  Saves ~24.9% vector_engine_pct contribution from VectorE/GpSimdE shuffle ops.
...
SBUF budget estimate (per partition lane, 224 KiB limit):
  ...
  aff_bcast:          [_PMAX, 8]       fp32  =   4 KB  (retained — Plan E direct broadcast failed)
  gate_up_tiles:      [2,_PMAX,16,384] bf16  =  24 KB  (K=2 double-buffer)
  ...
  down_full0s:        [8,_PMAX,1024]   bf16  =  16 KB  (all K=8 experts pre-loaded)
  down_full1s:        [8,_PMAX,1024]   bf16  =  16 KB  (all K=8 experts pre-loaded)
  ...
  Total peak:        ~170 KB  — within 224 KiB limit.
"""
...
def qwen3_moe_fused_tkg(
    ...
    # -----------------------------------------------------------------------
    # Stage 4: Selective-Expert MLP — Plan D two-phase prefetch + compute
    #
    # SBUF constraint: gate_up_tiles[K=8, _PMAX, H_free, _GU_FLAT] = 96 KB exceeds
    # the remaining SBUF budget. Instead, use a K=2 double-buffer for gate_up (24 KB)
    # and full K=8 prefetch for down tiles (32 KB total, much smaller).
    #
    # Phase 1 (affine_range): pre-issue all K=8 down DMAs (16 packets) simultaneously,
    #   and issue the first 2 gate_up DMAs (slots 0,1) into the double-buffer.
    # Phase 2 (static_range): for each expert k, gate_up is read from slot k%2;
    #   the next expert's gate_up is prefetched into slot (k+1)%2 during compute.
    #
    # This approach:
    # - Frees all 16 down DMAs from compute-chain dependencies (full pipelining)
    # - Provides 1-ahead gate_up prefetch (same as Plan C's double-buffer benefit)
    # - Stays within 224 KB SBUF budget
    #
    ...
    for t in nl.static_range(T):
        ...
        # Plan E attempt: direct broadcast via norm_weights[t:t+1, k:k+1] [1,1]
        # FAILED with compiler error:
        #   "TensorScalarPtr inst.404: 1st Immediate pointer's number of partitions
        #    must match src AP"
        # operand0 partition count must equal data partition count (128), not 1.
        #
        # Fallback: keep the aff_bcast approach (memset + tensor_copy + 4× nc_stream_shuffle)
        # identical to v18b. Plan E hypothesis is disproven — the hardware verifier
        # enforces partition-count matching on tensor_scalar operands.
        ...
        # --- Allocate K+2 separate SBUF buffers ---
        # NKI DMA requires src and dst to have the same number of dimensions.
        # Separate per-expert ndarrays (vs slices of a K-dim array) ensure the DMA
        # dst is always a simple 2D (_PMAX, free) or 3D (_PMAX, h, w) buffer.
        #
        # gate_up: K=2 double-buffer as two separate 3D ndarrays (12 KB each = 24 KB).
        # down: K=8 separate 2D ndarrays per tile-type (2 KB each × 8 = 32 KB total).
        gate_up_tile_e = [
            nl.ndarray((_PMAX, H_free, _GU_FLAT), dtype=gate_up_w.dtype, buffer=nl.sbuf),
            nl.ndarray((_PMAX, H_free, _GU_FLAT), dtype=gate_up_w.dtype, buffer=nl.sbuf),
        ]
        # K=8 separate SBUF allocations per down tile type (NKI requires explicit stmts)
        down_full0_0 = nl.ndarray((_PMAX, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_full0_1 = nl.ndarray((_PMAX, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_full0_2 = nl.ndarray((_PMAX, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_full0_3 = nl.ndarray((_PMAX, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_full0_4 = nl.ndarray((_PMAX, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_full0_5 = nl.ndarray((_PMAX, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_full0_6 = nl.ndarray((_PMAX, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_full0_7 = nl.ndarray((_PMAX, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_full1_0 = nl.ndarray((_PMAX, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_full1_1 = nl.ndarray((_PMAX, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_full1_2 = nl.ndarray((_PMAX, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_full1_3 = nl.ndarray((_PMAX, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_full1_4 = nl.ndarray((_PMAX, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_full1_5 = nl.ndarray((_PMAX, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_full1_6 = nl.ndarray((_PMAX, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_full1_7 = nl.ndarray((_PMAX, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        # Python list for static indexing — safe because static_range k is a Python int
        down_full0_e = [down_full0_0, down_full0_1, down_full0_2, down_full0_3,
                        down_full0_4, down_full0_5, down_full0_6, down_full0_7]
        down_full1_e = [down_full1_0, down_full1_1, down_full1_2, down_full1_3,
                        down_full1_4, down_full1_5, down_full1_6, down_full1_7]

        # Hoist pad memsets for ALL K down_full1 tiles at once (static unroll)
        for k_pad in nl.static_range(K):
            nisa.memset(down_full1_e[k_pad][nl.ds(I1, I1), 0:H_shard], value=0.0)

        # gate_t1_128/up_t1_128 pad zeros (single buffers, reused across experts)
        ...
        # PSUM buffers (hoisted, zeroed per-expert inside compute loop)
        ...
        # --- Phase 1: Issue all K=8 down DMAs (static unroll, independent) ---
        # static_range with per-expert separate buffers allows the compiler to see
        # all K×2=16 down DMAs as independent → DMA engine pipelines all at once.
        # Each down_full0_e[k] / down_full1_e[k] is a separate SBUF allocation.
        for k in nl.static_range(K):
            expert_id = top8_idx.ap(pattern=[[K, 1], [1, 1]], offset=t * K + k)

            # down tile 0 DMA: I rows 0:128, H_shard=1024 cols only (shard-aware)
            nisa.dma_copy(
                dst=down_full0_e[k],
                src=down_w.ap(
            ...
            # down tile 1 DMA: I rows 128:192 (64 valid), H_shard=1024 cols only
            # Rows 64:128 already zeroed via k_pad loop above.
            nisa.dma_copy(
                dst=down_full1_e[k][0:I1, 0:H_shard],
                src=down_w.ap(
        ...
        # --- Prime gate_up double-buffer: load slot 0 (expert 0) ---
        # Slot 1 (expert 1) will be loaded inside the compute loop.
        expert_id_0 = top8_idx.ap(pattern=[[K, 1], [1, 1]], offset=t * K + 0)
        nisa.dma_copy(
            dst=gate_up_tile_e[0],
            src=gate_up_w.ap(
        ...
        # --- Phase 2: Compute loop with gate_up double-buffering ---
        # static_range to avoid race conditions on shared gate_t1_128/up_t1_128 buffers.
        for k in nl.static_range(K):
            buf_slot = k % 2

            # Prefetch next expert's gate_up into the alternate slot (if k < K-1)
            if k < K - 1:
                next_slot = (k + 1) % 2
                expert_id_next = top8_idx.ap(pattern=[[K, 1], [1, 1]], offset=t * K + k + 1)
                nisa.dma_copy(
                    dst=gate_up_tile_e[next_slot],
                    src=gate_up_w.ap(
                        pattern=[[_GU_FLAT, _PMAX], [_PMAX * _GU_FLAT, H_free], [1, _GU_FLAT]],
                        offset=0,
                        scalar_offset=expert_id_next,
                        indirect_dim=0,
                    ),
                    dge_mode=0,
                )
            ...
            # Pad region (cols I1:I0 = 64:128) was zeroed once above (hoisted).
            # ----------------------------------------------------------------
            nisa.tensor_copy(
                dst=gate_t1_128[0:_PMAX, 0:H_free, 0:I1],
                src=gate_up_tile_e[buf_slot][0:_PMAX, 0:H_free, nl.ds(I0, I1)],
            )
            nisa.tensor_copy(
                dst=up_t1_128[0:_PMAX, 0:H_free, 0:I1],
                src=gate_up_tile_e[buf_slot][0:_PMAX, 0:H_free, nl.ds(I + I0, I1)],
            )
            ...
            for h1 in nl.affine_range(H_free):
                for i_tile in nl.static_range(I_tiles):
                    if i_tile == 0:
                        # Tile 0: K=128 cols taken directly from the double-buffer slot
                        g_stat = gate_up_tile_e[buf_slot][0:_PMAX, h1, nl.ds(0, I0)]   # gate cols 0:128
                        u_stat = gate_up_tile_e[buf_slot][0:_PMAX, h1, nl.ds(I, I0)]   # up   cols 192:320
                    else:
            ...
            # ----------------------------------------------------------------
            # Down matmul using pre-loaded down_full0_e[k], down_full1_e[k]
            # All down tiles were pre-fetched in Phase 1 before any compute.
            # h1_out is the local H-tile index within the shard (0..H_free_shard-1).
            # ----------------------------------------------------------------
            ...
            for h1_out in nl.affine_range(H_free_shard):
                ...
                nisa.nc_matmul(
                    ...
                    stationary=down_full0_e[k][0:_PMAX, nl.ds(h1_out * _PMAX, _PMAX)],
                    moving=inter_bf16[0:_PMAX, 0:1],
                )
                # Tile 1: I rows 128:192 (64 valid, rows 64:128 zero from k_pad memset)
                nisa.nc_matmul(
                    dst=down_psum[0:_PMAX, h1_out:h1_out + 1],
                    stationary=down_full1_e[k][0:_PMAX, nl.ds(h1_out * _PMAX, _PMAX)],
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v19a with native weight layouts — no preprocessing required.
