...
kernel_v23b — Plan B: Fuse gate SiLU + down PSUM flush
=======================================================
...
Plan B changes vs v22b:
  Two independent fusions that eliminate 2 activation(copy) ops per expert
  (16 fewer ops total across K=8 experts):

  Fusion 1: gate PSUM flush + SiLU → single activation(silu) reading from PSUM
    Before: activation(gate_sb, copy, PSUM) + activation(silu_res, silu, gate_sb)
    After:  activation(silu_res, silu, PSUM)   ← direct PSUM→SiLU
    Removes gate_sb buffer allocation.

  Fusion 2: down PSUM flush + affinity scale → tensor_scalar reading from PSUM
    Before: activation(down_result_sb, copy, PSUM) + tensor_scalar(scaled, down_result_sb, ...)
    After:  tensor_scalar(down_result_scaled, PSUM, ...)   ← direct PSUM→scale
    Removes down_result_sb buffer allocation.

  If fusion 2 rejected by compiler, fallback: keep two-step (only fusion 1 applied).
"""
...
def qwen3_moe_fused_tkg(
    ...
    #          + Fusion 1 (gate SiLU) + Fusion 2 (down PSUM scale)
    # -----------------------------------------------------------------------
    ...
    for t in nl.static_range(T):
        ...
        for k in nl.static_range(K):
            ...
            # --- Fusion 1: gate PSUM → SiLU in one activation call ---
            # Before (v22b): activation(gate_sb, copy, PSUM) + activation(silu_res, silu, gate_sb)
            # After  (v23b): activation(silu_res, silu, PSUM)  ← fused flush+SiLU, saves 1 op/expert
            ...
            nisa.activation(silu_res, op=nl.silu, data=gate_up_psum[0:_PMAX, 0:I_tiles])       # fused flush+SiLU
            nisa.activation(up_sb,   op=nl.copy, data=gate_up_psum[0:_PMAX, I_tiles:2 * I_tiles])  # flush up
            ...
            # --- Fusion 2: down PSUM flush + affinity scale in one tensor_scalar call ---
            # Before (v22b): activation(down_result_sb, copy, PSUM) + tensor_scalar(scaled, down_result_sb, ...)
            # After  (v23b): tensor_scalar(down_result_scaled, PSUM, ...)  ← saves 1 op/expert
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v23b with native weight layouts — no preprocessing required."""
    import torch
