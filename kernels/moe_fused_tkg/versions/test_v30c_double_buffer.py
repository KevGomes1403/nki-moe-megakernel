"""
Test harness for kernel_v30c_double_buffer.py
Compares v30c (double-buffered H-sharded gate/up) against a pure PyTorch fp32-promoted reference.
...
from kernel_v30c_double_buffer import run as run_v30c
from kernel_v30c_double_buffer import qwen3_moe_fused_tkg_sbuf_io
...
print("Running v30c double_buffer kernel...")
...
try:
    ...
    print("\nPASS — v30c matches PyTorch reference within atol=1e-3, rtol=1e-2")
...
if ntff_path and neff_path:
    ...
    _print_profile_results(prof, "qwen3_moe_fused_tkg_sbuf_io (v30c)")
else:
...
if bench_neff:
    _pending_bench.append({
        "neff": bench_neff, "warmup": 5, "iters": 50, "name": "qwen3_moe_fused_tkg_sbuf_io_v30c"
    })

r = BenchmarkResult(prof, bench_neff or neff_path or "", ntff_path)
print(f"\n--- v30c Benchmark Metrics ---")
