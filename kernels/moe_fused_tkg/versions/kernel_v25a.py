...
kernel_v25a — Plan A: H_shard-only down DMA
=============================================================
...
Change 1 — gate_up DMA: same as v20b (H_free 2D DMAs per expert, each [_PMAX, _GU_FLAT]).

Change 2 — down DMA: load only H_shard=1024 columns (not full H=2048) per I-tile per expert.
  Each core loads its own shard via offset=prg_id*H_shard into HBM.
  DMA pattern: [[H, I_rows], [1, H_shard]] with offset including prg_id*H_shard.
  SBUF buffers are K*H_shard wide instead of K*H → saves 50% SBUF for down weights.
...
SBUF budget estimate (per partition lane, 224 KiB limit):
  ...
  down_full0_flat: K × H_shard × 2 / _PMAX = 8×1024×2/128 = 16 KB
  down_full1_flat: 16 KB
  other buffers:   ~46 KB (same as v19b)
  Stage 4 peak:    96 + 16 + 16 + 46 = 174 KB < 224 KB ✓
"""
...
def qwen3_moe_fused_tkg(
    ...
    # -----------------------------------------------------------------------
    # Stage 4: Selective-Expert MLP — Plan A: 2D coalesced gate_up + H_shard-only down
    #
    ...
    #
    # down_full0_flat / down_full1_flat [_PMAX, K * H_shard]:
    #   Expert k occupies cols [k*H_shard : (k+1)*H_shard] (H_shard=1024 columns).
    #   DMA loads only H_shard width per core → each core fetches its own shard from HBM.
    #   No prg_id offset needed at matmul time since buffer already contains only our shard.
    # -----------------------------------------------------------------------
    ...
    for t in nl.static_range(T):
        ...
        # down: load only H_shard per expert per I-tile — each core loads its own shard
        # ------------------------------------------------------------------
        gate_up_flat    = nl.ndarray((_PMAX, K * _GU_STRIDE_PER_EXPERT), dtype=gate_up_w.dtype, buffer=nl.sbuf)
        ...
        # For gate_up: H_free 2D DMAs per expert, each [_PMAX, _GU_FLAT] fully coalesced
        #   step_p=_GU_FLAT=count_col → 128 consecutive HBM rows of 384 cols = 1 HW packet per H-tile
        # For down: H_shard-only 2D DMA per expert per I-tile, each core loads its own shard
        # ------------------------------------------------------------------
        for k in nl.static_range(K):  # static_range: k is compile-time constant for scalar_offset
            ...
            # gate_up: H_free 2D DMAs — each loads one H-tile [_PMAX, _GU_FLAT] fully coalesced
            # Within H-tile h1: HBM rows h1*_PMAX to (h1+1)*_PMAX-1, all 384 cols
            # HBM layout: gate_up_w[e, h1*_PMAX:(h1+1)*_PMAX, :] = contiguous block
            for h1 in nl.static_range(H_free):
                nisa.dma_copy(
                    dst=gate_up_flat[0:_PMAX, nl.ds(gate_expert_col + h1 * _GU_FLAT, _GU_FLAT)],
                    src=gate_up_w.ap(
                        # 2D pattern: step_p=_GU_FLAT=count_col → consecutive HBM rows, FULLY COALESCED
                        # Each partition p gets row (h1*_PMAX + p) of the expert
                        pattern=[[_GU_FLAT, _PMAX], [1, _GU_FLAT]],
                        offset=h1 * _PMAX * _GU_FLAT,        # H-tile h1 base within expert
                        scalar_offset=expert_id,              # expert base address (DGE)
                        indirect_dim=0,
                    ),
                    dge_mode=0,
                )

            down_expert_col = k * H_shard   # col base in H_shard down flat bufs for expert k

            # down tile 0: H_shard width, I0=128 rows — each core loads its own shard
            # HBM: down_w[expert_id, 0:I0, prg_id*H_shard:(prg_id+1)*H_shard]
            nisa.dma_copy(
                dst=down_full0_flat[0:_PMAX, nl.ds(down_expert_col, H_shard)],
                src=down_w.ap(
                    pattern=[[H, I0], [1, H_shard]],
                    offset=prg_id * H_shard,     # start at our shard within each I-row
            ...
            # down tile 1: H_shard width, I1=64 rows — same pattern, offset to I-tile 1
            # Pad rows I1:_PMAX already zeroed by memset above
            nisa.dma_copy(
                dst=down_full1_flat[0:I1, nl.ds(down_expert_col, H_shard)],
                src=down_w.ap(
                    pattern=[[H, I1], [1, H_shard]],
                    offset=I0 * H + prg_id * H_shard,  # skip I0 rows + shard offset
                    scalar_offset=expert_id,
                    indirect_dim=0,
                ),
                dge_mode=0,
        ...
        for k in nl.static_range(K):
            gate_expert_col = k * _GU_STRIDE_PER_EXPERT
            down_expert_col = k * H_shard

            # --- Tile-1 prep (gate_t1_128, up_t1_128) ---
            for h1 in nl.static_range(H_free):
                h_col = gate_expert_col + h1 * _GU_FLAT    # compile-time constant (static h1)
                nisa.tensor_copy(
                    dst=gate_t1_128[0:_PMAX, h1, 0:I1],
                    src=gate_up_flat[0:_PMAX, nl.ds(h_col + I0, I1)],       # gate cols I0:I0+I1
                )
                nisa.tensor_copy(
                    dst=up_t1_128[0:_PMAX, h1, 0:I1],
                    src=gate_up_flat[0:_PMAX, nl.ds(h_col + I + I0, I1)],   # up cols I+I0:I+I0+I1
                )
            ...
            for h1 in nl.affine_range(H_free):
                h_col_affine = gate_expert_col + h1 * _GU_FLAT   # affine expression of h1
                for i_tile in nl.static_range(I_tiles):
                    if i_tile == 0:
                        g_stat = gate_up_flat[0:_PMAX, nl.ds(h_col_affine,       I0)]  # gate t0 [128, 128]
                        u_stat = gate_up_flat[0:_PMAX, nl.ds(h_col_affine + I,   I0)]  # up   t0 [128, 128]
                    else:
            ...
            # --- Down matmul using H_shard-only flat buffers ---
            # Buffer already contains only our shard, so offset is just expert_base + tile_offset
            nisa.memset(down_psum, value=0.0)

            for h1_out in nl.affine_range(H_free_shard):
                off = down_expert_col + h1_out * _PMAX
                nisa.nc_matmul(
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v25a with native weight layouts — no preprocessing required.
