...
kernel_v27c — Plan C: 2-Buffer 4-Wave Processing
=================================================
Based on v26g (Plan G: 2-wave, 4 buffer sets).

Changes vs v26g:
  Reduce to 2 buffer sets. Process 8 experts in 4 waves of 2.
  Wave w: experts 2*w and 2*w+1 using buffers 0 and 1.

  This further halves SBUF from v26g's ~74 KiB to ~42 KiB per partition,
  freeing even more room for compiler scheduling temporaries.

  Single-expert PSUM (12 cols) with per-expert memset.
  No redundant down_full1 re-memsets between waves.
...
# 4-wave constants: 2 experts per wave
_N_WAVES = 4
_K_WAVE = 2
...
def qwen3_moe_fused_tkg(
    ...
    # -----------------------------------------------------------------------
    # Stage 4: 4-Wave Expert Processing (2 buffer sets, 2 experts per wave)
    #
    # Wave w: experts 2*w and 2*w+1
    # Buffer 0 for expert 2*w, Buffer 1 for expert 2*w+1
    # DMA phase loads both experts, then compute phase processes both.
    # Compiler can overlap wave w DMA with wave w-1 compute.
    #
    # PSUM: single-expert capacity per-expert memset.
    # -----------------------------------------------------------------------
    ...
    for t in nl.static_range(T):
        ...
        # (DMA only writes [0:I1, :], rows 64:128 persist zeroed)
        # ------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # 4 waves, 2 experts each. Unrolled via static_range.
        # ------------------------------------------------------------------
        for wave in nl.static_range(_N_WAVES):

            # Phase 1: Load 2 experts (6 DMAs)
            for k in nl.static_range(_K_WAVE):
                kk = wave * _K_WAVE + k  # global expert index
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v27c with native weight layouts — no preprocessing required."""
    import torch
