...
kernel_v26g — Plan G: 2-Wave Expert Processing (4 Buffer Sets)
=============================================================
Based on v25e (Plan E: Combined micro-optimizations).

Changes vs v25e:
  Reduce buffer sets from 8 to 4. Process experts in 2 waves of 4.
  Wave 0: experts 0-3, Wave 1: experts 4-7.
  This frees ~96 KiB/partition of SBUF for compiler temporaries,
  potentially enabling better instruction scheduling.

  - 4 named buffers each for gate_up, down_full0, down_full1 (was 8)
  - PSUM allocation reduced to 4-expert capacity
  - gate_up_psum: [_PMAX, 4 * 2 * I_tiles] = [128, 16] (was [128, 32])
  - down_psum: [_PMAX, 4 * H_free_shard] = [128, 32] (was [128, 64])
  - Buffers reused between waves; down_full1 padding re-memset before wave 1
  - gate_t1_128/up_t1_128 zero-padded once before wave 0 (persist through both)

  Preserves v25e's Change 1 (norm_weights after DMA), Change 3 (per-expert PSUM),
  Change 4 (fused SiLU from PSUM).
