...
kernel_v10b — Router 4-tile DMA batch + Pass1 affine_range(K)
=============================================================

Changes from v9b:
  1. Router weight batching: 4 tiles per DMA (4 × 32KB = 128KB per packet).
     16 separate 32KB DMAs → 4 × 128KB DMAs. Fewer start overheads, better
     bandwidth efficiency (well above 32KB efficiency floor).
  2. Pass 1 gate/up k-loop: static_range(K) → affine_range(K), signalling
     independence to the compiler so it can overlap expert k+1's gate/up 3D
     DMA with expert k's matmul computation.

All other optimizations from v9b are preserved unchanged.

v9b optimizations preserved:
