"""
Benchmark for kernel_v19b — Plan F: K=8 full prefetch (all 24 DMAs before compute).

Plan F improvement vs v18a (Plans A+C):
  Issue ALL 24 DMAs (8×gate_up + 8×down_full0 + 8×down_full1) in a single
  prefetch loop BEFORE any compute. The DMA engine can pipeline all 24 packets
  simultaneously, so all weights arrive before the compute loop begins.
  Uses K=8 separate named SBUF buffers (Python lists) accessed by static_range.
"""
...
os.environ["NEURON_RT_INSPECT_OUTPUT_DIR"] = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "output_v19b"
)
...
spec = importlib.util.spec_from_file_location(
    "_kernel", "/home/ubuntu/nki-moe/kernels/moe_fused_tkg/kernel_v19b.py"
)
...
if r:
    print(f"\n{'='*60}")
    print("  kernel_v19b — Plan F: K=8 full prefetch")
    print(f"{'='*60}")
    ...
    print("    v18a (Plans A+C): device_time_us=120.47, spill_bytes=0")
else:
