"""Benchmark for kernel_v29m — v28f with merged down_w DMA (Plan M) + shared_hbm output."""
import os, sys
os.environ["NEURON_PLATFORM_TARGET_OVERRIDE"] = "trn2"
os.environ["NEURON_RT_INSPECT_ENABLE"] = "1"
os.environ["NEURON_RT_INSPECT_DEVICE_PROFILE"] = "1"
os.environ["NEURON_RT_INSPECT_OUTPUT_DIR"] = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "output_v29m"
)
...
spec = importlib.util.spec_from_file_location(
    "_kernel", "/home/ubuntu/nki-moe/kernels/moe_fused_tkg/kernel_v29m.py"
)
...
xm.mark_step()
# Pre-pad down_w to [E, 256, H] for kernel_v29m
import torch.nn.functional as F
down_w_raw = (torch.randn(E, I, H) * scale).to(torch.bfloat16)
down_w = F.pad(down_w_raw, (0, 0, 0, 64))  # [E, 256, H]
...
if r:
    print(f"\n{'='*60}")
    print("  kernel_v29m — v28f + merged down DMA (Plan M) + shared_hbm")
    print(f"{'='*60}")
    ...
    print(f"\n  Baseline v28f: device_time_us=98.15")
    print(f"  Target: <=95 us")
    print(f"  Expected sw_dma_packet_count: ~5296 (down from 7936)")
else:
