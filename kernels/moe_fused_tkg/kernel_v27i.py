...
kernel_v27i — v27d + prefetch wave-1 DMAs inside wave-0 compute loop
======================================================================
Based on v27d (best: 103.48 μs).

Changes vs v27d:
  Eliminate the separate Phase 1b DMA section by prefetching each wave-1
  expert's 3 DMAs at the end of computing the corresponding wave-0 expert.
  After computing expert k (which frees buf[k]), immediately issue DMAs
  for expert k+4 into buf[k]. This overlaps ~40 μs of DMA transfer time
  with the remaining wave-0 matmul compute (experts k+1..3), eliminating
  the serial DMA stall between Phase 2a and Phase 2b.

  Structure:
    Phase 1a: 12 DMAs for experts 0-3
    Phase 2a loop (k=0..3):
      compute expert k
      prefetch expert k+4 → buf[k]  ← NEW: 3 DMAs issued after each expert
    PSUM memset for wave 1
    Phase 2b loop (k=0..3): compute expert k+4 (data already in buf[k])
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        # Phase 2a: Compute experts 0-3 + prefetch experts 4-7 into buf[k]
        # After finishing expert k (freeing buf[k]), issue 3 DMAs for expert k+4.
        # This overlaps ~40 μs of DMA transfer with remaining wave-0 compute.
        for k in nl.static_range(_K_WAVE):
            gu_base = k * 2 * I_tiles
            ...
            # --- PREFETCH expert k+4 into buf[k] (buf[k] is now done) ---
            # Issued here so DMA transfer overlaps with computing experts k+1..3.
            # down_full1_bufs[k] rows [I1:I0] remain zero from initial memset.
            kk_pre = k + 4
            expert_id_pre = top8_idx.ap(pattern=[[K, 1], [1, 1]], offset=t * K + kk_pre)
            nisa.dma_copy(
                dst=gate_up_bufs[k],
                src=gate_up_w.ap(
                    pattern=[[_GU_FLAT, _PMAX], [_PMAX * _GU_FLAT, H_free], [1, _GU_FLAT]],
                    offset=0,
                    scalar_offset=expert_id_pre,
                    indirect_dim=0,
                ),
                dge_mode=0,
            )
        ...
        # ==================================================================
        # WAVE 1: Experts 4-7 (data already prefetched into buf[0..3])
        # ==================================================================

        # NOTE: No down_full1 re-memset needed — DMA only writes [0:I1, :],
        # rows I1:I0 remain zeroed from initial memset before wave 0.

        # PSUM memset for wave 1 (runs while prefetch DMAs are in-flight)
        nisa.memset(gate_up_psum, value=0.0)
        nisa.memset(down_psum, value=0.0)
