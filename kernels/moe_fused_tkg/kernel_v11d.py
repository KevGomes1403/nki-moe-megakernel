...
kernel_v11d — Gate/Up Pass 1 double-buffer prefetch
====================================================

Changes from v10b:
  1. Pass 1 gate/up DMA: software double-buffering (ping-pong) to overlap
     expert k+1 DMA with expert k matmul computation.
     - Allocate two SBUF buffers (gate_up_ping, gate_up_pong) outside the loop.
     - Pre-issue DMA for expert 0 into ping before entering the k-loop.
     - Inside k-loop (static_range): immediately issue DMA for expert k+1
       into nxt_tile, then compute on cur_tile.
     - static_range(K) required so k%2 evaluates at trace time.
  2. Remove per-iteration fused_tile_3d allocation inside the loop;
     use cur_tile (ping or pong) instead.
...
SBUF budget estimate (per core):
  ...
  gate_up_ping:        [128, 16, 512] bf16    = 16 KB   (double-buffer slot 0)
  gate_up_pong:        [128, 16, 512] bf16    = 16 KB   (double-buffer slot 1)
  ...
  Net change vs v10b: +1 × fused_tile_3d = +16 KB
  Current peak ~38 KB/partition — within 224 KB limit
"""
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        # ── PASS 1: gate/up with double-buffer prefetch ──────────────────────────
        # Allocate TWO ping-pong gate/up SBUF buffers outside the k-loop.
        # Each is [_PMAX, H_free, I*2] bf16 = 16 KB per partition.
        # With current peak ~22 KB/partition and 224 KB limit, adding 32 KB is safe.
        gate_up_ping = nl.ndarray((_PMAX, H_free, I * 2), dtype=gate_up_w.dtype, buffer=nl.sbuf)
        gate_up_pong = nl.ndarray((_PMAX, H_free, I * 2), dtype=gate_up_w.dtype, buffer=nl.sbuf)

        # Pre-issue DMA for expert 0 into ping BEFORE entering the k-loop.
        # This gives maximum time for the DMA to complete before k=0 compute starts.
        _eid_pre = top8_idx.ap(pattern=[[K, 1], [1, 1]], offset=t * K + 0)
        nisa.dma_copy(
            dst=gate_up_ping,
            src=gate_up_w_fused.ap(
                pattern=[[I * 2, _PMAX], [_PMAX * I * 2, H_free], [1, I * 2]],
                offset=0,
                scalar_offset=_eid_pre,
                indirect_dim=0,
            ),
            dge_mode=0,
        )
        ...
        for k in nl.static_range(K):
            # Select which buffer holds expert k's data (already loaded).
            # cur_tile: contains current expert k's gate/up weights (DMA already issued)
            # nxt_tile: will hold expert k+1's weights (DMA about to be issued)
            cur_tile = gate_up_ping if k % 2 == 0 else gate_up_pong
            nxt_tile = gate_up_pong if k % 2 == 0 else gate_up_ping

            # ── PREFETCH expert k+1 IMMEDIATELY (first instruction in the loop body) ──
            # Issue DMA for expert k+1 into nxt_tile NOW, before any compute on cur_tile.
            # The DMA runs concurrently with the gate/up matmul below.
            if k + 1 < K:   # compile-time check: k is static_range, k+1 < K is constant
                _eid_next = top8_idx.ap(pattern=[[K, 1], [1, 1]], offset=t * K + (k + 1))
                nisa.dma_copy(
                    dst=nxt_tile,
                    src=gate_up_w_fused.ap(
                        pattern=[[I * 2, _PMAX], [_PMAX * I * 2, H_free], [1, I * 2]],
                        offset=0,
                        scalar_offset=_eid_next,
                        indirect_dim=0,
                    ),
                    dge_mode=0,
                )
            ...
            for h1 in nl.affine_range(H_free):
                for i_tile in nl.static_range(I_tiles):
                    nisa.nc_matmul(
                        ...
                        stationary=cur_tile[0:_PMAX, h1, nl.ds(i_tile * I0, I0)],
                        moving=rmsnorm_normed_bf16[0:_PMAX, nl.ds(h1 * T, T)],
                    )
                    nisa.nc_matmul(
                        dst=up_psum[0:_PMAX, i_tile:i_tile + 1],
                        stationary=cur_tile[0:_PMAX, h1, nl.ds(I + i_tile * I0, I0)],
                        moving=rmsnorm_normed_bf16[0:_PMAX, nl.ds(h1 * T, T)],
                    )
