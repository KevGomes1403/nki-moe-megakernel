...
kernel_v27e — Plan E: 4-Buffer 4-Wave (2 experts per wave)
===========================================================
Based on v26g (Plan G: 2-wave, 4 buffer sets).

Changes vs v26g:
  Keep 4 buffer sets (~74 KiB SBUF) but process 4 waves of 2 experts.
  Wave 0: experts 0,1 (bufs 0,1). Wave 1: experts 2,3 (bufs 2,3).
  Wave 2: experts 4,5 (bufs 0,1). Wave 3: experts 6,7 (bufs 2,3).

  Hypothesis: Smaller DMA phases (6 DMAs instead of 12) allow better
  DMA-compute interleaving between waves while maintaining v26g's SBUF.
  Single-expert PSUM (12 cols) with per-expert memset.
  No redundant down_full1 re-memsets (only bufs 0,1 after wave 1; bufs 2,3 after wave 3 — but wave structure means re-memset only needed before waves 2,3).

Interface contract (no repack required):
  gate_up_w  [E=128, H=2048, 2*I=384] bf16
  down_w     [E=128, I=192,  H=2048]   bf16

SBUF Budget (per partition): ~74 KiB (same as v26g)
PSUM: 4+8 = 12 cols (minimal)
"""
...
# 4-wave: 2 experts per wave, 4 buffers rotating in pairs
_N_WAVES = 4
_K_WAVE = 2
...
def qwen3_moe_fused_tkg(
    ...
    # -----------------------------------------------------------------------
    # Stage 4: 4-Wave Expert Processing (4 buffer sets, 2 experts per wave)
    #
    # Wave 0: experts 0,1 → bufs 0,1
    # Wave 1: experts 2,3 → bufs 2,3
    # Wave 2: experts 4,5 → bufs 0,1 (reused)
    # Wave 3: experts 6,7 → bufs 2,3 (reused)
    #
    # Single-expert PSUM with per-expert memset.
    # -----------------------------------------------------------------------
    ...
    for t in nl.static_range(T):
        ...
        # Zero pad all 4 down_full1 buffers once (persists through all waves)
        ...
        # 4 waves, unrolled
        for wave in nl.static_range(_N_WAVES):
            # Buffer indices for this wave: wave 0,2 → bufs 0,1; wave 1,3 → bufs 2,3
            buf_base = (wave % 2) * 2  # 0 or 2

            # Phase 1: Load 2 experts into wave's buffer pair
            for k in nl.static_range(_K_WAVE):
                kk = wave * _K_WAVE + k
                b = buf_base + k  # buffer index (0-3)
            ...
            # Compute norm_weights once (overlap with wave 0's DMA)
            if wave == 0:
            ...
            # Phase 2: Compute 2 experts
            for k in nl.static_range(_K_WAVE):
                kk = wave * _K_WAVE + k
                b = buf_base + k

                # PSUM memset per expert
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v27e with native weight layouts."""
    import torch
    ...
    assert gate_up_w.shape == (_E, _H, _GU_FLAT)
    assert down_w.shape == (_E, _I, _H)
