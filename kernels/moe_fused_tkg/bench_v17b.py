"""
Benchmark for kernel_v17b — Plan B: prefetch all K=8 gate_up tiles before the compute loop.

Plan B change vs v17a:
  A separate prefetch loop (nl.affine_range(K)) issues all 8 gate_up DMAs before the
  compute loop, allowing the DMA engine to pipeline all 8 × 1.5 MB gate_up DMAs
  together instead of serializing them within the compute chain.
"""
...
os.environ["NEURON_RT_INSPECT_OUTPUT_DIR"] = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "output_v17b"
)
...
from kernels.moe_fused_tkg.benchmark import wrap_benchmark

import importlib.util

spec = importlib.util.spec_from_file_location(
    "_kernel_v17b", "/home/ubuntu/nki-moe/kernels/moe_fused_tkg/kernel_v17b.py"
)
mod = importlib.util.module_from_spec(spec)
spec.loader.exec_module(mod)
_kernel_v17b = mod.qwen3_moe_fused_tkg

kernel_fn = wrap_benchmark(lambda *args: _kernel_v17b[2](*args), warmup=10, iters=100)

# ---------------------------------------------------------------------------
# Create inputs — native layouts, no repack (same as bench_v16c)
# ---------------------------------------------------------------------------
...
if r:
    print(f"\n{'='*60}")
    print("  kernel_v17b — Plan B: gate_up prefetch")
    print(f"{'='*60}")
    ...
    print("    v17a (Plan A): device_time_us=124.23, dma_active_pct=61.2%")
else:
