"""
Correctness check: kernel_v18a vs kernel_v16c reference.
"""

import os
import sys

# Must be before any neuron/torch_xla import
os.environ["NEURON_PLATFORM_TARGET_OVERRIDE"] = "trn2"

sys.path.insert(0, "/home/ubuntu/nki-moe")

import torch
import torch_xla.core.xla_model as xm
import importlib.util


def load_kernel(path, name):
    spec = importlib.util.spec_from_file_location(name, path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod.qwen3_moe_fused_tkg


kernel_ref = load_kernel(
    "/home/ubuntu/nki-moe/kernels/moe_fused_tkg/kernel_v16c.py", "_kernel_v16c"
)
kernel_new = load_kernel(
    "/home/ubuntu/nki-moe/kernels/moe_fused_tkg/kernel_v18a.py", "_kernel_v18a"
)

# ---------------------------------------------------------------------------
# Create inputs — match bench_v16c.py exactly
# ---------------------------------------------------------------------------
device = xm.xla_device()
torch.manual_seed(42)

B, H, E, K = 1, 2048, 128, 8
I = 192
scale = 0.1

inp      = (torch.randn(B, 1, H) * scale).to(torch.bfloat16).to(device)
gamma    = (torch.randn(1, H) * scale + 1.0).to(torch.bfloat16).to(device)
router_w = (torch.randn(H, E) * scale).to(torch.bfloat16).to(device)

gate_up_w = torch.zeros(E, H, 2 * I, dtype=torch.bfloat16)
gate_up_w[:, :, 0:I]   = (torch.randn(E, H, I) * scale).to(torch.bfloat16)
gate_up_w[:, :, I:2*I] = (torch.randn(E, H, I) * scale).to(torch.bfloat16)
gate_up_w = gate_up_w.to(device)
xm.mark_step()

down_w = (torch.randn(E, I, H) * scale).to(torch.bfloat16).to(device)
xm.mark_step()

args = (inp, gamma, router_w, gate_up_w, down_w)

# ---------------------------------------------------------------------------
# Run both kernels
# ---------------------------------------------------------------------------
out_ref = kernel_ref[2](*args)
xm.mark_step()

out_new = kernel_new[2](*args)
xm.mark_step()

# Move to CPU for comparison
out_ref_cpu = out_ref.cpu()
out_new_cpu = out_new.cpu()

max_diff = (out_ref_cpu - out_new_cpu).abs().max().item()
print(f"max_diff = {max_diff:.2e}")

try:
    torch.testing.assert_close(out_new_cpu, out_ref_cpu, rtol=1e-2, atol=1e-2)
    print(f"Correctness: PASS  max_diff={max_diff:.2e}")
except AssertionError as e:
    print(f"Correctness: FAIL  max_diff={max_diff:.2e}")
    print(str(e))
    sys.exit(1)
