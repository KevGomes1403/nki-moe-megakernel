"""Correctness check: kernel_v28b vs kernel_v27d."""
import os, sys
...
kernel_ref  = load_kernel("/home/ubuntu/nki-moe/kernels/moe_fused_tkg/kernel_v27d.py", "_k27d")
kernel_cand = load_kernel("/home/ubuntu/nki-moe/kernels/moe_fused_tkg/kernel_v28b.py", "_k28b")
...
print("Running v27d reference...")
ref_out  = kernel_ref[2](*args); xm.mark_step(); ref_cpu  = ref_out.cpu().float().numpy()
print("Running v28b candidate...")
cand_out = kernel_cand[2](*args); xm.mark_step(); cand_cpu = cand_out.cpu().float().numpy()

max_diff = np.abs(ref_cpu - cand_cpu).max()
print(f"\nmax_diff = {max_diff:.4e}")
try:
    np.testing.assert_allclose(cand_cpu, ref_cpu, rtol=1e-2, atol=1e-2)
