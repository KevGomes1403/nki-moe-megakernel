...
v8b changes vs v8a — Plan E: Unified K-loop with Pre-issued Down DMA:
  Bottleneck targeted: DMA dominates at 54% (~75 µs).

  Root cause: v8a's 3-pass structure serializes:
    [all K gate/up] → [all K down DMA + matmul] → [all K accumulate]
  Pre-issuing each expert's down DMA before its gate/up matmul loop allows
  DGE and TE to execute concurrently for that expert.

  Changes:
  1. Remove inter_buf [128, I_tiles, K] and down_buf [128, H_free_shard, K]
     K-slot SBUF buffers — no longer needed.
  2. Replace 3-pass (Pass1 + Pass2 + Pass3) with a single
     `for k in nl.static_range(K)` loop.
  3. Within each k iteration:
     a. Issue gate/up 3D DMA (same AP as v8a)
     b. Issue down 3D DMA (same AP as v8a) — BEFORE gate/up matmul
     c. gate/up affine_range h1 matmul loop
     d. gate/up PSUM flush; sendrecv with pipe_id=0 for gate, pipe_id=1 for up
        (static_range k means pipe_ids don't need to be distinct across k)
     e. silu(full_gate) * full_up -> inter_f32
     f. down affine_range i_tile2 matmul loop using down_tile_3d
     g. down PSUM flush; scale by aff_bcast[:, k:k+1]
     h. Accumulate: tensor_copy if k==0, tensor_tensor add if k>0

  Expected effect: Down DMA latency hidden behind gate/up matmuls.
  Removing inter_buf/down_buf K-slot buffers also reduces SBUF pressure.
  Target: < 130 µs.
"""
...
def qwen3_moe_fused_tkg(
    ...
    # Partial sums exchanged via nisa.sendrecv (SBUF↔SBUF).
    ...
    #
    # Plan E: Unified K-loop. Single static_range(K) loop combining gate/up DMA,
    # down DMA (pre-issued), gate/up matmul, sendrecv, silu, down matmul, accumulate.
    # Removes inter_buf and down_buf K-slot buffers.
    # -----------------------------------------------------------------------
    ...
    # Change 3 (from v1b): k=0 uses tensor_copy to initialise (avoids add-to-zero).
    output_temp = nl.ndarray((_PMAX, H_free_shard, T), dtype=nl.float32, buffer=nl.sbuf)

    # Plan E: NO inter_buf or down_buf — everything is computed and accumulated
    # within a single unified K-loop iteration.
    ...
    for t in nl.static_range(T):
        ...
        # inside the K loop (which would cost 6×K instructions per token).
        ...
        # ── UNIFIED K-loop: gate/up DMA + down DMA + matmuls + accumulate ──────
        # static_range required: sendrecv pipe_ids must be issued in fixed order.
        # pipe_id=0 for ALL gate sendrecvs, pipe_id=1 for ALL up sendrecvs.
        # Since this is static_range (sequential), pipe_ids don't need to be
        # distinct across k iterations.
        for k in nl.static_range(K):
            expert_id = top8_idx.ap(pattern=[[K, 1], [1, 1]], offset=t * K + k)

            # Step a: Issue gate/up 3D DMA (same AP as v8a)
            ...
            # Step b: Issue down 3D DMA BEFORE gate/up matmul — allows DGE and TE to overlap
            ...
            #   i1: HBM stride = I0 * H = 32768 (actually I0 * H_full, stepping I_tiles rows)
            #   h:  HBM stride = 1, count = H_shard = 1024
            # offset = H_shard_start = prg_id * H_shard (within expert's row of H columns)
            # Note: H here is the FULL hidden dim 2048 (stride for the I dimension of down_w [E, I, H])
            ...
            # Step c: gate/up matmul h1 loop — DGE loads down_tile_3d concurrently
            ...
            # Step d: gate/up PSUM flush; sendrecv with simplified pipe_ids
            # pipe_id=0 for gate (all k), pipe_id=1 for up (all k)
            # Static_range K means sequential, so same pipe_ids are safe across k.
            ...
            nisa.sendrecv(src=up_sb,   dst=recv_up,
                          send_to_rank=other, recv_from_rank=other, pipe_id=1)

            # Step e: silu and multiply -> inter_f32
            ...
            # Step f: down matmul i_tile2 loop using down_tile_3d (should be fully loaded)
            ...
            # Step g: Single flush per expert (Plan 1); apply affinity scale
            ...
            # Step h: Accumulate into output_temp
            # tensor_copy for k==0 (initializes), tensor_tensor add for k>0
            if k == 0:
                nisa.tensor_copy(
