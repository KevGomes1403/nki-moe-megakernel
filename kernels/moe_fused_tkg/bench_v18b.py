"""
Benchmark for kernel_v18b — Plan D: Full K=8 DMA prefetch (gate_up + down, all experts before compute).

Restructures Stage 4 into two phases:
1. Prefetch loop (nl.affine_range(K)): issues all 24 DMAs simultaneously
2. Compute loop (nl.static_range(K)): reads from pre-loaded SBUF buffers, no DMAs
"""
...
os.environ["NEURON_RT_INSPECT_OUTPUT_DIR"] = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "output_v18b"
)
...
spec = importlib.util.spec_from_file_location(
    "_kernel", "/home/ubuntu/nki-moe/kernels/moe_fused_tkg/kernel_v18b.py"
)
...
if r:
    print(f"\n{'='*60}")
    print("  kernel_v18b — Plan D full prefetch")
    print(f"{'='*60}")
    ...
    print("    v18a (Plan C base): device_time_us=120.47, hbm_read_KiB=31760")
else:
