...
kernel_v27g — v27d with PSUM memset moved after Phase 1b DMAs
=============================================================
Based on v26g (Plan G: 2-wave, 4 buffer sets).

Changes vs v26g:
  Remove 4 redundant inter-wave down_full1 re-memsets. The DMA for each
  expert only writes to [0:I1, 0:H_shard] (rows 0:64), never touching
  rows 64:128 which were zeroed before wave 0. Zero padding persists.
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        # DMAs issued BEFORE PSUM memset so DMA engine starts immediately
        for k in nl.static_range(_K_WAVE):
        ...
        # PSUM memset for wave 1 — placed AFTER DMAs to overlap with in-flight DMA
        nisa.memset(gate_up_psum, value=0.0)
        nisa.memset(down_psum, value=0.0)
