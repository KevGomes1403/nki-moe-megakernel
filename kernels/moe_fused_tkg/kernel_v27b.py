...
kernel_v27b — Plan B: Double-Buffered 1-Expert-at-a-Time Processing
====================================================================
Based on v26g (Plan G: 2-wave, 4 buffer sets).

Changes vs v26g:
  Reduce to 2 buffer sets. Process 8 experts one at a time with ping-pong
  double buffering. Expert k uses buffer k%2. The compiler sees all 8
  unrolled iterations and can overlap expert k's compute with expert k+1's
  DMA since they use different buffers.

  SBUF per partition:
    2 x gate_up [16 x 384 x 2] = 24 KiB
    2 x down_full0 [1024 x 2]  =  4 KiB
    2 x down_full1 [1024 x 2]  =  4 KiB
    gate_t1 + up_t1             =  8 KiB
    misc                        = ~2 KiB
    Total: ~42 KiB << 224 KiB (128 KiB freed vs v26g)

  PSUM: gate_up_psum [128, 4] + down_psum [128, 8] = 12 cols (minimal)
...
def qwen3_moe_fused_tkg(
    ...
    # -----------------------------------------------------------------------
    # Stage 4: Double-Buffered 1-Expert-at-a-Time Processing
    #
    # 2 buffer sets (A=0, B=1). Expert k uses buffer k%2.
    # Compiler sees all 8 unrolled iterations via static_range and can
    # overlap expert k's compute with expert k+1's DMA (different buffers).
    #
    # PSUM: single-expert capacity (4 gate_up + 8 down = 12 cols)
    # -----------------------------------------------------------------------
    ...
    for t in nl.static_range(T):
        ...
        # ------------------------------------------------------------------
        # Allocate 2 named SBUF buffers (ping-pong)
        # ------------------------------------------------------------------
        ...
        gate_up_bufs = [gate_up_buf0, gate_up_buf1]
        ...
        down_full0_bufs = [down_full0_buf0, down_full0_buf1]
        ...
        down_full1_bufs = [down_full1_buf0, down_full1_buf1]

        # ------------------------------------------------------------------
        # Zero pad region (rows I1:I0 = 64:128) for 2 down_full1 buffers
        # (DMA only writes [0:I1, :], never touches rows 64:128, so padding
        #  persists across all 8 experts)
        # ------------------------------------------------------------------
        ...
        # Single-expert PSUM (minimal: 4 gate_up + 8 down = 12 cols)
        ...
        # ------------------------------------------------------------------
        # Process 8 experts, one at a time, double-buffered
        # ------------------------------------------------------------------
        for k in nl.static_range(K):
            b = k % 2  # buffer index (ping-pong)

            # DMA: load expert k into buffer b
            ...
            nisa.dma_copy(
                dst=down_full1_bufs[b][0:I1, 0:H_shard],
                src=down_w.ap(
            ...
            # Compute norm_weights once (overlap with expert 0's DMA)
            if k == 0:
            ...
            # PSUM memset for this expert
            nisa.memset(gate_up_psum, value=0.0)
            nisa.memset(down_psum, value=0.0)

            # Tile-1 tensor_copy (extract I1-width slices, zero-padded to I0)
            nisa.tensor_copy(
                dst=gate_t1_128[0:_PMAX, 0:H_free, 0:I1],
                src=gate_up_bufs[b][0:_PMAX, 0:H_free, nl.ds(I0, I1)],
            )
            nisa.tensor_copy(
                dst=up_t1_128[0:_PMAX, 0:H_free, 0:I1],
                src=gate_up_bufs[b][0:_PMAX, 0:H_free, nl.ds(I + I0, I1)],
            )
            ...
            for h1 in nl.affine_range(H_free):
                for i_tile in nl.static_range(I_tiles):
                    if i_tile == 0:
                        g_stat = gate_up_bufs[b][0:_PMAX, h1, nl.ds(0, I0)]
                        u_stat = gate_up_bufs[b][0:_PMAX, h1, nl.ds(I, I0)]
                    else:
            ...
            nisa.activation(silu_res, op=nl.silu, data=gate_up_psum[0:_PMAX, 0:I_tiles])
            ...
            for h1_out in nl.affine_range(H_free_shard):
                nisa.nc_matmul(
                    ...
                    stationary=down_full0_bufs[b][0:_PMAX, nl.ds(h1_out * _PMAX, _PMAX)],
                    moving=inter_bf16[0:_PMAX, 0:1],
                )
                nisa.nc_matmul(
                    dst=down_psum[0:_PMAX, h1_out:h1_out + 1],
                    stationary=down_full1_bufs[b][0:_PMAX, nl.ds(h1_out * _PMAX, _PMAX)],
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v27b with native weight layouts — no preprocessing required."""
    import torch
