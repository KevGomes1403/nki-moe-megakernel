...
kernel_v15 — Native weight layouts, coalesced DMA, no preprocessing
====================================================================
...
SBUF budget estimate (per partition lane, 224 KiB limit):
  ...
  gate_t1_128:        [_PMAX,16,128]  bf16  =   4 KB  (per expert, reused)
  up_t1_128:          [_PMAX,16,128]  bf16  =   4 KB  (per expert, reused)
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        for k in nl.affine_range(K):
            ...
            # memset only the pad half (cols I1:I0 = 64:128) to avoid zeroing the valid half.
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v15 with native weight layouts — no preprocessing required.
    ...
    if gate_up_w.dim() == 4:
        E, Hd, _, Iv = gate_up_w.shape
        gate_up_w = gate_up_w.reshape(E, Hd, 2 * Iv)
