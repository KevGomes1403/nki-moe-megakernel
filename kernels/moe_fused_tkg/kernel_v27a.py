...
kernel_v27a — Plan A: Remove redundant memsets + dual PSUM
=============================================================
Based on v26g (Plan G: 2-Wave Expert Processing, 4 Buffer Sets).

Changes vs v26g:
  1. Remove 4 redundant inter-wave down_full1 re-memsets. The wave 0 DMA
     only writes to [0:I1, 0:H_shard] = [0:64, 0:1024], never touching
     rows 64:128 which were zeroed before wave 0.
  2. Use separate PSUM regions per wave to avoid inter-wave PSUM memsets:
     - Wave 0: gate_up_psum_w0 [128, 16] and down_psum_w0 [128, 32]
     - Wave 1: gate_up_psum_w1 [128, 16] and down_psum_w1 [128, 32]
     - Total PSUM: 96 cols (well within 512 limit)
     - All 4 PSUM regions memset once before wave 0.
     - Inter-wave PSUM memsets removed.

Interface contract (no repack required):
  gate_up_w  [E=128, H=2048, 2*I=384] bf16
  down_w     [E=128, I=192,  H=2048]   bf16

SBUF Budget (per partition): same as v26g (~74 KiB << 224 KiB)

PSUM budget: gate_up_psum_w0 [128, 16] + down_psum_w0 [128, 32]
           + gate_up_psum_w1 [128, 16] + down_psum_w1 [128, 32] = 96 cols < 512 max.
"""
...
def qwen3_moe_fused_tkg(
    ...
    # -----------------------------------------------------------------------
    # Stage 4: Selective-Expert MLP — Plan A: Dual PSUM + no redundant memsets
    #
    # Wave 0: Load & compute experts 0-3 (buffer indices 0-3, PSUM w0)
    # Wave 1: Re-load & compute experts 4-7 (reusing buffer indices 0-3, PSUM w1)
    # -----------------------------------------------------------------------
    ...
    for t in nl.static_range(T):
        ...
        # (done once — wave 0 DMA only writes [0:I1, 0:H_shard], never
        #  touches rows 64:128, so padding persists through both waves)
        # ------------------------------------------------------------------
        ...
        # PSUM allocation: separate regions per wave (no inter-wave memset needed)
        ...
        # Memset all 4 PSUM regions once before wave 0
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            nisa.activation(silu_res, op=nl.silu, data=gate_up_psum_w0[0:_PMAX, gu_base:gu_base + I_tiles])

            up_sb = nl.ndarray((_PMAX, I_tiles), dtype=nl.float32, buffer=nl.sbuf)
            nisa.activation(up_sb, op=nl.copy, data=gate_up_psum_w0[0:_PMAX, gu_base + I_tiles:gu_base + 2 * I_tiles])
        ...
        # NOTE: No re-memset of down_full1 padding — wave 0 DMA only wrote
        # to [0:I1, 0:H_shard], rows 64:128 remain zeroed from initial memset.
        # NOTE: No PSUM memset — wave 1 uses separate PSUM regions (w1).
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # Gate/Up matmul (using wave 1 PSUM)
            for h1 in nl.affine_range(H_free):
                for i_tile in nl.static_range(I_tiles):
                    if i_tile == 0:
                        g_stat = gate_up_bufs[k][0:_PMAX, h1, nl.ds(0, I0)]
            ...
            # SiLU + multiply + cast (using wave 1 PSUM)
            silu_res = nl.ndarray((_PMAX, I_tiles), dtype=nl.float32, buffer=nl.sbuf)
            nisa.activation(silu_res, op=nl.silu, data=gate_up_psum_w1[0:_PMAX, gu_base:gu_base + I_tiles])

            up_sb = nl.ndarray((_PMAX, I_tiles), dtype=nl.float32, buffer=nl.sbuf)
            nisa.activation(up_sb, op=nl.copy, data=gate_up_psum_w1[0:_PMAX, gu_base + I_tiles:gu_base + 2 * I_tiles])
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v27a with native weight layouts — no preprocessing required.
