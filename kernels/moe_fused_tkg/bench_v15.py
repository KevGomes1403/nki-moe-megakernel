"""
Benchmark for kernel_v15 — native weight layouts, no preprocessing.

kernel_v15 improvements vs v13:
  gate_up: ONE coalesced DMA per expert (stride_p=384=count=384)
           vs two fragmented DMAs (stride_p=384 ≠ count=192) in v13.
  down:    ONE coalesced full-H DMA per I-tile (stride_p=2048=count=2048)
           vs fragmented H-shard DMAs in v13.
  run():   zero preprocessing cost — native [E, H, 2*I] and [E, I, H] layouts.
"""
...
spec = importlib.util.spec_from_file_location(
    "_kernel", "/home/ubuntu/nki-moe/kernels/moe_fused_tkg/kernel_v15.py"
)
...
if r:
    print(f"\n{'='*60}")
    print("  kernel_v15 — native layouts, coalesced DMA (no repack)")
    print(f"{'='*60}")
    ...
    print("    v13: fragmented gate_up DMA (2×fragmented per expert)")
    print("    v15-old: coalesced DMA but with repack inside NEFF")
else:
