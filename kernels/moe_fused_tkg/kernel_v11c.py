...
kernel_v11c — v10b + fused scalar_tensor_tensor for expert accumulation
=======================================================================

Changes from v10b:
  1. Fused expert accumulation: eliminate separate scale (tensor_scalar) and
     store-to-down_buf + Pass3 copy/add pattern. Replace with a single
     nisa.scalar_tensor_tensor call that computes output_temp += down_result * aff
     directly (fused multiply-accumulate in ONE VectorE instruction).
  2. Remove down_buf [128, 8, 8] fp32 (saves 32 KB SBUF).
  3. Remove down_result_scaled allocation (saves 4 KB SBUF).
  4. Remove Pass 3 accumulation loop (eliminates 8 tensor_copy/tensor_tensor
     scalar instructions).
  5. Add memset(output_temp, 0) before the k-loop; Pass 2 (static_range) now
     directly accumulates into output_temp via scalar_tensor_tensor.
  6. Pass 2 changed from affine_range(K) to static_range(K) because we now
     write to shared output_temp sequentially (RAW dependency across k).

v10b optimizations preserved:
  1. Router weight batching: 4 tiles per DMA (4 × 32KB = 128KB per packet).
  2. Pass 1 gate/up k-loop: affine_range(K) for DMA/matmul overlap.
  3. Batch RMSNorm loads (single 3D DMA for inp and gamma) [v4a]
  4. Pre-broadcast affinity weights into aff_bcast[128, K] [v5a]
  5. 3D DMA pattern for gate/up (full H) and down (H_shard) — Plan D adapted
...
SBUF budget estimate (per core):
  ...
  Total peak: ~118 KB — within 224 KB limit (down_buf removed saves 32 KB;
              fused_tile and down_tile don't coexist;
...
def qwen3_moe_fused_tkg(
    ...
    #
    # Change 1 (v10b): Router weight batching — 4 tiles per DMA.
    ...
    # Zeroed once before Pass 2 loop; scalar_tensor_tensor accumulates in-place.
    output_temp = nl.ndarray((_PMAX, H_free_shard, T), dtype=nl.float32, buffer=nl.sbuf)

    # K-slot buffer for inter results (no down_buf — eliminated by Plan C fusion)
    ...
    for t in nl.static_range(T):
        ...
        #
        # Change 2 (v10b): affine_range(K) instead of static_range(K).
        # The K experts are independent (distinct expert_ids, distinct fused_tile_3d
        # SBUF allocations, distinct inter_buf[:,:,k:k+1] write slots).
        # affine_range signals independence → compiler can overlap expert k+1's
        ...
        # ── PASS 2: down projection + fused scale-accumulate into output_temp ────
        # Each core loads only its H_shard columns of down weights.
        # output_temp zeroed once; scalar_tensor_tensor fuses multiply+add per k.
        # static_range(K) required: sequential RAW dependency on output_temp.
        nisa.memset(output_temp[0:_PMAX, 0:H_free_shard, t:t + 1], value=0.0)
        for k in nl.static_range(K):
            ...
            #   i1: stride = I0*H = 131072 (wait: I0*H = 128*2048 = 262144), count = I_tiles = 2
            ...
            # Fused scale + accumulate: output_temp += down_result_sb * aff_bcast[k]
            # scalar_tensor_tensor computes: dst = (data * operand0) + operand1
            # i.e.: output_temp = down_result_sb * aff_bcast[k] + output_temp  (in-place)
            nisa.scalar_tensor_tensor(
                dst=output_temp[0:_PMAX, 0:H_free_shard, t:t + 1],
                data=down_result_sb[0:_PMAX, 0:H_free_shard],
                operand0=aff_bcast[0:_PMAX, k:k + 1],
                operand1=output_temp[0:_PMAX, 0:H_free_shard, t:t + 1],
                op0=nl.multiply,
                op1=nl.add,
            )
