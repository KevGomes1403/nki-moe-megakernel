...
kernel_v12f — v10b + partition-first gate_up_w layout (eliminates strided gather DMA)
======================================================================================

Changes from v10b:
  1. gate_up_w pre-transposed to [E, _PMAX=128, H_free=16, I*2=512] in run().
     Original layout [E, H=2048, I*2=512] caused a strided gather DMA:
     partition p needed rows {p, 128+p, 256+p, …, 1920+p} — stride=128 rows×512 elems×2B
     = 131 KB between accesses → ~672 sub-packets of ~4 KB each at ~18% peak BW.
  2. New DMA .ap() pattern for gate_up_w:
     [[H_free*I*2, _PMAX], [I*2, H_free], [1, I*2]]
     Partition p's data is now a contiguous H_free*I*2=8192 element block.
     One contiguous 2 MB DMA per expert instead of strided gather.
...
v10b optimizations preserved:
  ...
  7. Router weight batching: 4 tiles per DMA (4 × 32KB = 128KB per packet) [v10b]
  8. Pass 1 gate/up k-loop: affine_range(K) for DMA/compute overlap [v10b]
...
SBUF budget estimate (per core):
  ...
  down_tile_3d:        [128, 2, 1024] bf16    = 64 KB   (H_shard, reused)
  aff_bcast:           [128, 8] fp32          =  4 KB
  router_w_wide_sb:    [128, 512] bf16        = 128 KB  (4 tiles × 128 cols, router batch)
  out_sb:              [1, 1024] bf16         =  2 KB
  Total peak: ~150 KB — within 224 KB limit (fused_tile and down_tile don't coexist;
...
try:
    import torch_xla
    _torch_xla_sync = torch_xla.sync
except (ImportError, AttributeError):
    try:
        import torch_xla.core.xla_model as _xm
        _torch_xla_sync = _xm.mark_step
    except ImportError:
        _torch_xla_sync = None
...
def qwen3_moe_fused_tkg(
    ...
    gate_up_w_t,  # [E=128, _PMAX=128, H_free=16, I*2=512] bf16 (partition-first)
    down_w,       # [E=128, I=256, H=2048] bf16
):
    """
    Fused RMSNorm + Router + TopK(K=8) + Expert MLP for Qwen3 MoE TKG.
    Invoked as: qwen3_moe_fused_tkg[2](inp, gamma, router_w, gate_up_w_t, down_w)
    ...
    gate_up_w_t must be pre-transposed to [E, _PMAX, H_free, I*2] layout
    (partition-first) before calling this kernel. See run() for the transform.
    """
    B = inp.shape[0]
    ...
    #
    # (v10b): Router weight batching — 4 tiles per DMA.
    ...
    for h_chunk in nl.affine_range(H_free // _ROUTER_BATCH):  # 4 outer iterations
        ...
        # 3D AP: P-dim strides across rows of router_w; batch-dim skips _PMAX rows;
        # E-dim is contiguous.
        # router_w HBM layout: [2048, 128] → element [row, col] at offset row*E + col.
        # P dim:           stride = E = 128,          count = _PMAX = 128
        # batch dim (F1):  stride = _PMAX*E = 16384,  count = _ROUTER_BATCH = 4
        # E dim (F2):      stride = 1,                count = E = 128
        nisa.dma_copy(
    ...
    #
    # v12f: gate_up_w_t already in [E, _PMAX, H_free, I*2] layout.
    # DMA loads partition p's contiguous H_free*I*2=8192-element block directly.
    # No reshape needed — gate_up_w_t is used directly with updated .ap() pattern.
    # -----------------------------------------------------------------------
    ...
    for t in nl.static_range(T):
        ...
        # Each core loads full H and computes full intermediate independently.
        #
        # v12f gate/up DMA: gate_up_w_t is [E, _PMAX, H_free, I*2].
        # Layout per expert e: _PMAX contiguous blocks of H_free*I*2 elements each.
        # Partition p's data: contiguous H_free*I*2=8192 elements at offset
        #   e * _PMAX * H_free * I*2 + p * H_free * I*2
        # AP pattern:
        #   P dim (outer):     stride = H_free*I*2 = 8192, count = _PMAX = 128
        #   H_free dim (mid):  stride = I*2 = 512,         count = H_free = 16
        #   Feature dim (inn): stride = 1,                 count = I*2 = 512
        # scalar_offset=expert_id selects expert (units of _PMAX*H_free*I*2 elements).
        # This is ONE contiguous 2 MB DMA per expert — no strided gather.
        #
        # affine_range(K) signals independence → compiler overlaps expert k+1's
        # gate/up 3D DMA with expert k's matmul computation.
        for k in nl.affine_range(K):
            ...
            # gate_up_w_t shape: [E=128, _PMAX=128, H_free=16, I*2=512]
            # fused_tile_3d [_PMAX, H_free, I*2] — same SBUF shape as v10b.
            # AP pattern (3D):
            #   dim 0 (p):      stride = H_free*I*2 = 8192,  count = _PMAX = 128
            #   dim 1 (h1):     stride = I*2 = 512,          count = H_free = 16
            #   dim 2 (col):    stride = 1,                  count = I*2 = 512
            # scalar_offset selects expert (in units of _PMAX*H_free*I*2 elements).
            fused_tile_3d = nl.ndarray((_PMAX, H_free, I * 2), dtype=gate_up_w_t.dtype, buffer=nl.sbuf)
            nisa.dma_copy(
                dst=fused_tile_3d,
                src=gate_up_w_t.ap(
                    pattern=[[H_free * I * 2, _PMAX], [I * 2, H_free], [1, I * 2]],
                    offset=0,
            ...
            # fused_tile_3d SBUF shape unchanged: [_PMAX, H_free, I*2]
            # Matmul accesses identical to v10b — no changes needed here.
            ...
            # No sendrecv — each core independently has the full intermediate
        ...
        # Output: H_free_shard tiles (this core's H slice only).
        for k in nl.affine_range(K):
            expert_id_k = top8_idx.ap(pattern=[[K, 1], [1, 1]], offset=t * K + k)

            # 3D DMA for down weights — H_shard only (with prg_id offset)
            # down_w shape: [E=128, I=256, H=2048] — unchanged from v10b
            # Target: down_tile_3d[i0, i1, h] = down_w[expert_id, i1*I0 + i0, prg_id*H_shard + h]
            # AP pattern (3D):
...
# Module-level cache: maps id(gate_up_w_original) → gate_up_w_transposed
_gate_up_w_transposed_cache = {}
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    ...
    gate_up_w: [E, H, 2, I] bf16  (original layout, transposed lazily on first call)
    Returns the expert MLP output after routing and accumulation.
    """
    global _gate_up_w_transposed_cache

    key = id(gate_up_w)
    if key not in _gate_up_w_transposed_cache:
        # Transpose once: [E, H, 2, I] → [E, _PMAX, H_free, I*2]
        # gate_up_w may be 4D [E, H, 2, I] or 3D [E, H, I*2]
        if gate_up_w.dim() == 4:
            E, H, two, I_val = gate_up_w.shape          # E=128, H=2048, 2, I=256
            gate_up_3d = gate_up_w.reshape(E, H, two * I_val)  # [E, H, I*2=512]
        else:
            E, H, I2_val = gate_up_w.shape
            gate_up_3d = gate_up_w

        H_free_val = H // _PMAX  # 16
        # [E, H, I*2] → [E, H_free, _PMAX, I*2] → [E, _PMAX, H_free, I*2]
        gate_up_w_t = (
            gate_up_3d
            .reshape(E, H_free_val, _PMAX, gate_up_3d.shape[-1])  # [E, H_free, _PMAX, I*2]
            .permute(0, 2, 1, 3)                                    # [E, _PMAX, H_free, I*2]
            .contiguous()
        )
        # Flush the transpose as a separate XLA execution so the NTFF for the
        # NKI kernel (captured next) does not include the DVE transpose overhead.
        if _torch_xla_sync is not None:
            _torch_xla_sync()
        _gate_up_w_transposed_cache[key] = gate_up_w_t

    gate_up_w_t = _gate_up_w_transposed_cache[key]

    outputs = qwen3_moe_fused_tkg[2](inp, gamma, router_w, gate_up_w_t, down_w)
