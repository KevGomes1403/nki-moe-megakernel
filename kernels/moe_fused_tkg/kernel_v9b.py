...
kernel_v9b — Full Gate/Up Replication, H-shard Down, No Communication
======================================================================
...
Benefits over v9a:
  - No shared-HBM workspace (partial_hbm removed)
  - No core_barrier (zero in-kernel synchronization)
  - Simpler output stage (no load-both-partials-and-add)
  - Returns single output tensor (no tuple)

Trade-off vs v8a:
  - Gate/up DMA doubles (load full H instead of H_shard): +8 tiles per expert
  - Gate/up matmul doubles: H_free=16 iterations instead of H_free_shard=8
  - Down DMA and matmul unchanged (still H_shard only)
...
def qwen3_moe_fused_tkg(
    ...
    # Broadcast top8_idx from partition 0 to all 128 partitions.
    # top8_idx has shape (T, K) so only partitions 0..T-1 hold valid expert indices.
    # Partitions T..127 are uninitialized; if used directly as scalar_offset in the
    # DGE DMA calls, those garbage uint32 values cause scalar DGE out-of-bound access.
    # For T=1 this means 127 of 128 partitions would fire OOB DMAs.
    top8_idx_bcast = nl.ndarray((_PMAX, K), dtype=nl.uint32, buffer=nl.sbuf)
    nisa.memset(top8_idx_bcast, value=0)
    nisa.tensor_copy(dst=top8_idx_bcast[0:1, 0:K], src=top8_idx[0:1, 0:K])
    for _g in nl.static_range(4):
        nisa.nc_stream_shuffle(
            dst=top8_idx_bcast[nl.ds(_g * 32, 32), 0:K],
            src=top8_idx_bcast[0:1, 0:K],
            shuffle_mask=[0] * 32,
        )
    ...
    for t in nl.static_range(T):
        ...
        for k in nl.static_range(K):
            expert_id = top8_idx_bcast.ap(pattern=[[K, 1], [1, 1]], offset=k)  # T=1, t=0
        ...
        for k in nl.affine_range(K):
            expert_id_k = top8_idx_bcast.ap(pattern=[[K, 1], [1, 1]], offset=k)  # T=1, t=0
