"""
Benchmark for kernel_v16a — Plan A: hoisted pad memsets outside k-loop.

Plan A optimisation vs v15:
  Static pad-region memsets hoisted outside the k-loop (once per token instead
  of once per expert — 8× reduction in scalar_engine memset work):
    gate_t1_128 pad zeros [cols 64:128]: 8× → 1× per token
    up_t1_128   pad zeros [cols 64:128]: 8× → 1× per token
    down_full1  pad zeros [rows 64:128]: 8× → 1× per token
  gate_up_psum and down_psum allocations also hoisted before k-loop.
"""
...
os.environ["NEURON_RT_INSPECT_OUTPUT_DIR"] = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "output_v16a"
)
...
spec = importlib.util.spec_from_file_location(
    "_kernel", "/home/ubuntu/nki-moe/kernels/moe_fused_tkg/kernel_v16a.py"
)
...
if r:
    print(f"\n{'='*60}")
    print("  kernel_v16a — Plan A: hoisted pad memsets")
    print(f"{'='*60}")
    ...
    print("    v15: pad memsets inside k-loop (3 memsets × 8 experts = 24 per token)")
    print("    v16a: pad memsets hoisted (3 memsets × 1 per token)")
else:
