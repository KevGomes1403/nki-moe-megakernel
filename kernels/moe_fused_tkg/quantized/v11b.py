...
v11b — Round 3 Plan B: output_temp batching on v10a (double-buffer baseline)
=============================================================================
Based on v10a (double-buffer Wave 1 DMA loads).

Key changes vs v10a:
  V11b: Drop the T=1 dimension from output_temp entirely (same change as v10b
        but applied to v10a instead of v9a).
       ...
       - Wave 0 output accumulation: single tensor_copy/tensor_tensor([128,8])
         replacing the per-column loop of 8× [128,1] ops
       ...
       Isolates the effect of output_temp batching without affine_range changes.
...
def qwen3_moe_fused_tkg(
    ...
    # -----------------------------------------------------------------------
    # V11b: Drop T dimension — output_temp is [128, H_free_shard=8] (T=1 always in TKG)
    ...
    for t in nl.static_range(T):
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # V11b: Fully batched output accumulation — single [128, H_free_shard=8] op
            if k == 0:
                nisa.tensor_copy(
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # V11b: Fully batched output accumulation (Wave 1 always adds — Wave 0 already wrote)
            nisa.tensor_tensor(
                dst=output_temp[0:_PMAX, 0:_H_FREE_SHARD],
                data1=output_temp[0:_PMAX, 0:_H_FREE_SHARD],
                data2=down_h_all[0:_PMAX, 0:_H_FREE_SHARD],
    ...
    # V11b: output_temp is [128, H_free_shard] — use [0:_PMAX, h1:h1+1] for [128,1] slice
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v11b kernel (Round 3 Plan B: output_temp batching on v10a).
