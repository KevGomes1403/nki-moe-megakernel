...
v11c — Round 3 Plan C: affine_range expert loads + output_temp batching
========================================================================
Based on v11a (pre-read all expert IDs + affine_range for weight loads)
and v10b (drop T dim from output_temp, batch accumulation).

Key changes vs v11a:
  V11c: Drop the T=1 dimension from output_temp entirely.
       ...
       - Wave 0 output accumulation: single tensor_copy/tensor_tensor([128,8])
         replacing per-column loop of 8× [128,1] ops
       ...
       Combines v11a's affine_range DMA pipelining with v10b's batched accumulation.

        Order (inherited from v11a):
          ...
          Phase 2a: Compute Wave 0  (batched output accumulation)
          PSUM memset
          Phase 2b: Compute Wave 1  (batched output accumulation)
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v11c kernel (Round 3 Plan C: affine_range expert loads + output_temp batching).
