...
v9a — Plan A: Batch post-matmul chain [128,1]×8 → [128,8]
============================================================
Based on v8 (H_shard Fix: full H=2048, 1024 per LNC prg_id).

Key changes vs v8:
  V9a: Batch the 3 post-matmul loops (combined_down_scale precompute, PSUM drain,
       output accumulation) from 8× [128,1] ops into single [128,8] ops per expert.
       - Allocate zero_bias_hshard [128, H_free_shard=8] once per token (shared across waves)
       - combined_down_scale precompute: single tensor_tensor([128,8]) via TensorView broadcast
       - PSUM drain: single nisa.activation([128,8])
       - Output accumulation: single tensor_copy/tensor_tensor([128,8])
       Reduces 192 [128,1] instruction-overhead-dominated calls to 24 [128,8] calls.
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # Copy affinity column to a separate [128,1] buffer, then broadcast via TensorView
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v9a kernel (Plan A: batched post-matmul chain [128,1]×8 → [128,8]).
