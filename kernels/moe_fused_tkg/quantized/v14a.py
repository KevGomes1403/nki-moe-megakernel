...
v14a — Plan A: trn3 port of v12i
=================================
Based on v12i (affine_range prefetch for tile-1 tensor_copies).

Key changes vs v12i:
  v14a: trn3-specific port of v12i.
  - Target: Trainium3 (trn3), launched with NEURON_PLATFORM_TARGET_OVERRIDE=trn3.
  - No nc_matmul_mx: the nc_matmul_mx API requires a very different 3D tensor layout
    (quadrant-based scale layout, explicit H/512-tile dimension) incompatible with
    our 2D tiling structure. Integrating it would require a full kernel rewrite.
  - No activation quantize_mx: quantize_mx produces float8_e4m3fn_x4 (x4-packed),
    which cannot be directly used as the moving tensor in our nc_matmul calls
    (those expect unpacked float8_e4m3). Folding per-block scales from quantize_mx's
    quadrant layout into our per-neuron weight scales is non-trivial.
  - v12i already avoids tensor_partition_reduce(op=nl.maximum) — only nl.add is used,
    which IS supported on trn3. So the kernel is trn3-compatible without changes.
  - This kernel establishes the definitive trn3 baseline for further MXFP8 work.
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v14a kernel (Plan A: trn3 port of v12i).
