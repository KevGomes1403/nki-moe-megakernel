...
v6c — Plan C: Merge Load+Dequant Loops per Expert to Maximize DMA/ScalarE Overlap
==================================================================================
Based on v5 (Plan D: Collapse gate_up FP8→BF16 Cast to ONE Call per Expert).

Key changes vs v5:
  V6C.1: For both Wave 0 and Wave 1, merge the three separate
          `for k in nl.static_range(_K_WAVE)` loops (load, dequant, up_scale)
          into a single loop. This enables the hardware to overlap DMA for
          expert k+1 with ScalarE dequant for expert k, since they access
          independent SBUF regions on independent engines.
  V6C.2: norm_weights + aff_bcast computation is placed BEFORE the merged
          loop so it can overlap with the first DMA in each wave.
  V6C.3: Everything else identical to v5.
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        # ------------------------------------------------------------------
        # Compute norm_weights BEFORE Wave 0 merged loop
        # (no DMA dependency — computed from top8_vals/top8_idx already in SBUF)
        # Can overlap with first DMA in the merged loop below.
        # ------------------------------------------------------------------
        ...
        # ==================================================================
        # WAVE 0: Experts 0-3
        # Merged loop: load expert k, then immediately dequant expert k.
        # Hardware can overlap DMA for expert k+1 with ScalarE dequant for k.
        # ==================================================================
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # --- Dequant expert k (ScalarE, can overlap with DMA for expert k+1) ---
            ...
            # --- Assemble up_scale (tiny, runs immediately after scale DMA) ---
            nisa.tensor_copy(
        ...
        # Phase 2a: Compute experts 0-3 (bf16 stationary, same as v5)
        for k in nl.static_range(_K_WAVE):
            gu_base = k * 2 * I_tiles
            d_base  = k * H_free_shard

            # Tile-1 tensor_copy (bf16, same as v5)
            nisa.tensor_copy(
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # --- Load expert kk (wave 1, reusing buffers 0-3) ---
            nisa.dma_copy(
                dst=eid_scratch[0:1, 0:1],
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v6c kernel with merged load+dequant loops (Plan C).
