...
# FP8 weights: stored as int8
gate_up_w_data = (rng.integers(-127, 127, (E, H, GU))).astype(np.int8)
gate_up_w = torch.tensor(gate_up_w_data).to(device)

gate_up_scales = torch.tensor(rng.random((E, GU)).astype(np.float32) * 0.01 + 0.001, dtype=torch.float32).to(device)
down_w_data = (rng.integers(-127, 127, (E, I, H))).astype(np.int8)
down_w = torch.tensor(down_w_data).to(device)
down_scales = torch.tensor(rng.random((E, H_SHARD_TP)).astype(np.float32) * 0.01 + 0.001, dtype=torch.float32).to(device)
