...
v6a — Plan A: FP8 Stationary Matmul
====================================
Based on v5 (Plan D: Collapse gate_up FP8→BF16 Cast to ONE Call per Expert).

Key changes vs v5:
  V6A.1: Eliminate all 24 large FP8→BF16 dequantization activation calls.
         Use FP8 weight tensors directly as stationary in nc_matmul.
         Removes gate_up_bufs, down_full0_bufs, down_full1_bufs (bf16 copies).
  V6A.2: gate_t1_128 and up_t1_128 changed to float8_e4m3 dtype.
  V6A.3: All matmul stationary references updated to fp8 buffers directly.
  V6A.4: Post-matmul scale/bias (gate_up_scale_bufs, down_scale_bufs) unchanged.
