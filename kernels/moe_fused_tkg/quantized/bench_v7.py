...
os.environ["NEURON_RT_INSPECT_OUTPUT_DIR"] = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "_bench_out_v7"
)
...
from kernels.moe_fused_tkg.quantized.v7 import qwen3_moe_fused_tkg
...
gate_up_scales_fp32 = (gate_up_w_fp32.abs().amax(dim=1)/240.0).clamp(min=1e-6)
gate_up_w_q = (gate_up_w_fp32/gate_up_scales_fp32.unsqueeze(1)).clamp(-240,240).to(torch.float8_e4m3fn).view(torch.int8)
down_w_fp32 = torch.randn(E,I,H)*0.1
down_scales_full = (down_w_fp32[:,:,0:H_SHARD_TP].abs().amax(dim=1)/240.0).clamp(min=1e-6)
down_w_q_shard = (down_w_fp32[:,:,0:H_SHARD_TP]/down_scales_full.unsqueeze(1)).clamp(-240,240).to(torch.float8_e4m3fn)
...
r = kernel.last_result if hasattr(kernel, 'last_result') else None
if r:
