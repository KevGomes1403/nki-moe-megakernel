...
v2 — Plan B: two-step dequant (fp8→fp32 cast + tensor_tensor scale multiply)
=============================================================================
Based on v0 (Plan A: FP8 dequant using nisa.activation with scale=).

Changes vs v0:
  B1: Dequant replaced with two-step approach:
      OLD (scalar engine): activation(bf16_dst, fp8_src, scale=[128P,1F])
      NEW step 1 (scalar engine): activation(fp32_tmp, fp8_src)  -- pure fp8->fp32 cast
      NEW step 2 (VectorE):       tensor_tensor(bf16_dst, fp32_tmp, scale_bcast, multiply)
        where scale_bcast = TensorView(scale_col).broadcast(dim=1, size=128)
  B2: One fp32 temp buffer per expert (4 total) reused across dequant iterations.

Hypothesis: splitting dequant moves the scale multiply from scalar engine to VectorE,
reducing scalar engine utilization while VectorE handles the multiply.

Interface contract: same as v0
  gate_up_w  [E=128, H=2048, 2*I=384] fp8_e4m3fn (passed as int8)
  gate_up_scales [E=128, H=2048, GU/128=3] fp32
  down_w     [E=128, I=192,  H=2048]   fp8_e4m3fn (passed as int8)
  down_scales [E=128, I=192,  H/128=16] fp32

SBUF Budget (per partition):
  4 x gate_up_fp8 [16 x 384 x 1] = 24 KiB
  4 x gate_up_bf16 [16 x 384 x 2] = 48 KiB
  4 x gate_up_scales [48 x 4] = 0.75 KiB
  4 x fp32_temp [128 x 4] = 0.5 KiB  (NEW: reused per dequant step)
  4 x down_full0 [1024 x 2]  =  8 KiB
  4 x down_full1 [1024 x 2]  =  8 KiB
  4 x down_scales [16 x 4] = 0.25 KiB
  gate_t1 + up_t1             =  8 KiB
  misc                        = ~2 KiB
  Total: ~99.5 KiB << 224 KiB
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        # Plan B: fp32 temp buffers for two-step dequant (one per expert, reused across loops)
        # These hold the fp8->fp32 intermediate before tensor_tensor scale multiply.
        # Shape [128P, 128F] = 128*4 = 512 bytes per partition per expert.
        fp32_temp_buf0 = nl.ndarray((_PMAX, _PMAX), dtype=nl.float32, buffer=nl.sbuf)
        fp32_temp_buf1 = nl.ndarray((_PMAX, _PMAX), dtype=nl.float32, buffer=nl.sbuf)
        fp32_temp_buf2 = nl.ndarray((_PMAX, _PMAX), dtype=nl.float32, buffer=nl.sbuf)
        fp32_temp_buf3 = nl.ndarray((_PMAX, _PMAX), dtype=nl.float32, buffer=nl.sbuf)
        fp32_temp_bufs = [fp32_temp_buf0, fp32_temp_buf1, fp32_temp_buf2, fp32_temp_buf3]

        # Plan B: scale_tmp buffers for TensorView broadcast — one per expert.
        # Used to hold the scale column before broadcasting for tensor_tensor.
        # TensorView.broadcast() requires a base tensor (not a slice), so we copy first.
        # Two sizes needed: [128P, 1] for gate_up/down_tile0, [64P, 1] for down_tile1.
        scale_tmp_buf0 = nl.ndarray((_PMAX, 1), dtype=nl.float32, buffer=nl.sbuf)
        scale_tmp_buf1 = nl.ndarray((_PMAX, 1), dtype=nl.float32, buffer=nl.sbuf)
        scale_tmp_buf2 = nl.ndarray((_PMAX, 1), dtype=nl.float32, buffer=nl.sbuf)
        scale_tmp_buf3 = nl.ndarray((_PMAX, 1), dtype=nl.float32, buffer=nl.sbuf)
        scale_tmp_bufs = [scale_tmp_buf0, scale_tmp_buf1, scale_tmp_buf2, scale_tmp_buf3]

        # I1-sized scale_tmp for down_tile1 (only 64 valid rows)
        scale_tmp_i1_buf0 = nl.ndarray((I1, 1), dtype=nl.float32, buffer=nl.sbuf)
        scale_tmp_i1_buf1 = nl.ndarray((I1, 1), dtype=nl.float32, buffer=nl.sbuf)
        scale_tmp_i1_buf2 = nl.ndarray((I1, 1), dtype=nl.float32, buffer=nl.sbuf)
        scale_tmp_i1_buf3 = nl.ndarray((I1, 1), dtype=nl.float32, buffer=nl.sbuf)
        scale_tmp_i1_bufs = [scale_tmp_i1_buf0, scale_tmp_i1_buf1, scale_tmp_i1_buf2, scale_tmp_i1_buf3]
        ...
        for k in nl.static_range(_K_WAVE):
            # Plan B two-step dequant for gate_up: [128P, H_free, GU_FLAT] fp8 → bf16
            # Step 1: activation(fp8→fp32, no scale) — scalar engine but pure cast
            # Step 2: tensor_tensor(fp32 * scale_bcast → bf16) — VectorE
            ...
            for h1 in nl.static_range(H_free):
                for j_blk in nl.static_range(GU_FLAT_BLOCKS):
                    ...
                    nisa.activation(
                        fp32_temp_bufs[k][0:_PMAX, 0:_PMAX],
                        op=nl.copy,
                        data=gate_up_fp8_bufs[k][0:_PMAX, h1, nl.ds(j_blk * _PMAX, _PMAX)],
                    )
                    # Step 2: fp32 * scale_bcast → bf16 (VectorE)
                    # Copy scale column [128P, 1F] into scale_tmp so TensorView has a base tensor.
                    nisa.tensor_copy(
                        scale_tmp_bufs[k][0:_PMAX, 0:1],
                        gate_up_scale_bufs[k][0:_PMAX, h1, j_blk:j_blk + 1],
                    )
                    _scale_bcast = TensorView(scale_tmp_bufs[k]).broadcast(dim=1, size=_PMAX).get_view()
                    nisa.tensor_tensor(
            ...
            # Plan B two-step dequant for down tile 0: [I0=128P, H_shard] fp8 → bf16
            ...
            for h_blk in nl.static_range(H_BLOCKS):
                ...
                # Step 2: copy scale col into scale_tmp, then broadcast multiply
                nisa.tensor_copy(
                    scale_tmp_bufs[k][0:_PMAX, 0:1],
                    down_scale0_bufs[k][0:_PMAX, h_blk:h_blk + 1],
                )
                _scale_bcast_d0 = TensorView(scale_tmp_bufs[k]).broadcast(dim=1, size=_PMAX).get_view()
                nisa.tensor_tensor(
            ...
            # Plan B two-step dequant for down tile 1: [I1=64, H_shard] fp8 → bf16
            for h_blk in nl.static_range(H_BLOCKS):
                # Step 1: fp8 → fp32 (no scale, only I1=64 rows)
                nisa.activation(
                    fp32_temp_bufs[k][0:I1, 0:_PMAX],
                    op=nl.copy,
                    data=down_full1_fp8_bufs[k][0:I1, nl.ds(h_blk * _PMAX, _PMAX)],
                )
                # Step 2: copy scale col [I1, 1] into I1-sized scale_tmp, then broadcast
                nisa.tensor_copy(
                    scale_tmp_i1_bufs[k][0:I1, 0:1],
                    down_scale1_bufs[k][0:I1, h_blk:h_blk + 1],
                )
                _scale_bcast_d1 = TensorView(scale_tmp_i1_bufs[k]).broadcast(dim=1, size=_PMAX).get_view()
                nisa.tensor_tensor(
        ...
        # Plan B two-step dequant for Wave 1 (identical to Wave 0 pattern)
        for k in nl.static_range(_K_WAVE):
            GU_FLAT_BLOCKS = _GU_FLAT // _PMAX  # = 3
            for h1 in nl.static_range(H_free):
                for j_blk in nl.static_range(GU_FLAT_BLOCKS):
            ...
            for h_blk in nl.static_range(H_BLOCKS):
                ...
                # Step 2: copy scale col [I1, 1] into I1-sized scale_tmp, then broadcast multiply
                nisa.tensor_copy(
                    scale_tmp_i1_bufs[k][0:I1, 0:1],
                    down_scale1_bufs[k][0:I1, h_blk:h_blk + 1],
                )
