"""
Benchmark for v14b (trn3-compatible W8A8 FP8, per-partition activation quantization).
"""
import os
import sys
...
os.environ["NEURON_RT_INSPECT_OUTPUT_DIR"] = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "_bench_out_v14b"
)
...
from kernels.moe_fused_tkg.quantized.v14b import qwen3_moe_fused_tkg
...
print("Running benchmark for v14b (trn3 W8A8 per-partition FP8)...")
kernel(*args)

r = kernel.last_result
if r:
    scalar_pct = r.prof.get('scalar_engine_active_time_percent', 0)
    vector_pct = r.prof.get('vector_engine_active_time_percent', 0)
    gpsimd_pct = r.prof.get('gpsimd_engine_active_time_percent', 0)
    ...
    print(f"\n--- vs v12i baseline (89.80 us) ---")
    delta = r.device_time_us - 89.80
    pct = delta / 89.80 * 100
    sign = "+" if delta >= 0 else ""
    print(f"delta = {sign}{delta:.2f} us  ({sign}{pct:.1f}%)")
    print(f"\n--- vs v14a (88.68 us) ---")
    delta14a = r.device_time_us - 88.68
    pct14a = delta14a / 88.68 * 100
    sign14a = "+" if delta14a >= 0 else ""
    print(f"delta = {sign14a}{delta14a:.2f} us  ({sign14a}{pct14a:.1f}%)")
else:
