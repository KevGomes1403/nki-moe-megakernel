...
v15a — Plan A [F2]: Direct DGE for expert weight loads (trn3)
==============================================================
Based on v14a (trn3 port of v12i with full optimization stack preserved).

Key change vs v14a:
  - Phase 0 profile showed ~15% of device time (13.4 μs / 88.06 μs) was
    spent on GpSimdE stalls serializing the ten indirect expert-weight /
    expert-scale DMAs. Those calls used `dge_mode=0` (unknown => compiler-
    selected), which the compiler implements as SWDGE (GpSimd-driven
    descriptor generation) when the src tensor view carries a runtime
    `scalar_offset`.
  - Selective dge_mode selection per DMA:
      * 6 weight DMAs (gate_up_w, down_w tile 0, down_w tile 1, each × 2 waves):
        these perform an i8 -> f8_e4m3 dtype cast at DMA time, which HWDGE
        does not support (compiler rejects with: "HWDGE requires same src/dst
        element type"). Pinned to explicit SWDGE (`dge_mode=1`).
      * 4 scale DMAs (gate_up_scales, down_scales, each × 2 waves): fp32
        -> fp32 with no cast. Explicit HWDGE (`dge_mode=2`) moves their
        descriptor generation off GpSimd entirely, to the HWDGE hardware
        block. These 4 DMAs no longer contend with the 6 weight DMAs for
        GpSimd cycles.
  - Phase 0 bench of v14a: 88.36 μs, gpsimd=44.6%.
    Result for v15a:      76.38 μs (-11.98 μs = -13.6%), gpsimd=35.0%.
  - ALL v14a optimizations are preserved: SBUF hoisting, affine_range tile-1
    prefetch, ring buffers across waves, pre-read of 8 expert IDs,
    rearranged up-scale bufs, post-matmul combined-scale precompute.
    Only the DMA descriptor generation path changes.

Implementation note on nc_matmul_mx probe:
  - nc_matmul_mx compiled successfully on trn3 with both scale shapes tested
    ([P/32, STAT_F] and [P, STAT_F]) — both variants accepted by neuronx-cc.
  - However, DMA copy of uint8 HBM data to float8_e4m3fn_x4 SBUF is rejected by
    the compiler (NCC_IBIR285): MX dtypes require data to be produced in-kernel
    (via quantize_mx or memset), not loaded from HBM in the x4 dtype.
  - To use nc_matmul_mx for weights, they would need to be stored in HBM as raw
    bytes with separate scale tensors in the quadrant layout — a new weight format
    incompatible with the existing W8 int8 interface.

Order (inherited from v12i):
  ...
  Assemble up_scale bufs (affine_range — depend on weight loads)
  Prefetch 2a (affine_range): fill gate_t1_128[k] and up_t1_128[k] for k=0..3
  Phase 2a: Compute experts 0-3 (affine_range, memset-init output_temp)
  PSUM memset
  Prefetch 2b (affine_range): fill gate_t1_128_w1[k] and up_t1_128_w1[k] for k=0..3
  Phase 2b: Compute experts 4-7 (affine_range, always-add)
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.affine_range(T):
        ...
        for k in nl.affine_range(_K_WAVE):
            ...
            # v15a [F2]: weights do i8->f8 dtype cast at DMA, which HWDGE does
            # not support; stay on SWDGE but pin it explicitly.
            nisa.dma_copy(
        ...
        for k in nl.affine_range(_K_WAVE):
            ...
            # v15a [F2]: SWDGE — weight DMA does i8->f8 cast, HWDGE unavailable.
            nisa.dma_copy(
            ...
            # Load gate_up_scales for Wave 1 — HWDGE (fp32 -> fp32, no cast)
            nisa.dma_copy(
        ...
        for k in nl.affine_range(_K_WAVE):
            ...
            # V12i: Always-add to output_temp (initialized to zero before loop)
            nisa.tensor_tensor(
        ...
        for k in nl.affine_range(_K_WAVE):
            ...
            # V12i: Fully batched output accumulation (Wave 1 always adds to running sum)
            nisa.tensor_tensor(
                dst=output_temp[0:_PMAX, 0:_H_FREE_SHARD],
                data1=output_temp[0:_PMAX, 0:_H_FREE_SHARD],
                data2=down_h_all[0:_PMAX, 0:_H_FREE_SHARD],
    ...
    # V12i: output_temp is [128, H_free_shard] — use [0:_PMAX, h1:h1+1] for [128,1] slice
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v15a kernel (Plan A [F2]: direct DGE for expert weight loads on trn3).
