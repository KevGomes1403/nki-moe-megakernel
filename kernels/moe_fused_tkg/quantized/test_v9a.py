...
from kernels.moe_fused_tkg.quantized.v9a import run as run_v9a
...
print("Running v9a...")
v9a_result = run_v9a(inp, gamma, router_w, gate_up_w_dev, gate_up_scales_dev, down_w_dev, down_scales_dev)
xm.mark_step()
v9a_np = v9a_result.cpu().float().numpy()

max_diff = np.abs(v9a_np - v8_np).max()
print(f"max_diff = {max_diff:.4e}")

try:
    np.testing.assert_allclose(v9a_np, v8_np, rtol=1e-2, atol=1e-2)
