...
v12g — Round 4 Plan G: Precompute all combined_down_scales before Phase 2a
===========================================================================
Based on v11c (affine_range expert loads + output_temp batching).

Key changes vs v11c:
  V12g: Precompute combined_down_scale for ALL 8 experts (both waves) BEFORE
        Phase 2a begins, so they overlap with in-flight Wave 1 DMA instead of
        serializing in the Phase 2a / Phase 2b compute loops.
        ...
        Order:
          ...
          [NEW] Precompute combined_down_scales — Wave 0 (4 experts)
          [NEW] Precompute combined_down_scales — Wave 1 (4 experts)
          Phase 2a: Compute Wave 0 (use precomputed cds_pre_w0[k])
          PSUM memset
          Phase 2b: Compute Wave 1 (use precomputed cds_pre_w1[k])

        The precompute requires aff_bcast to be ready and down_scale_bufs to
        be loaded (both conditions satisfied before Phase 2a in v11c).
        Wave 1 DMA is still in-flight during the precompute, so the 8 small
        VectorE ops overlap with DMA tail.

        SBUF cost: 8 × [128P, 8F] fp32 = 8 × 4096 bytes = 32 KB extra.
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        # ------------------------------------------------------------------
        # Pre-allocate all 8 expert ID scratch buffers (replaces eid_scratch / eid_scratch_w1_bufs)
        # ------------------------------------------------------------------
        ...
        # This is the key change: each eid_all_bufs[k] is exclusively used in
        # k-iteration k of the weight load loops, making those iterations
        # truly independent and allowing affine_range to pipeline them.
        # ==================================================================
        ...
        # ==================================================================
        # V12g: Precompute combined_down_scales for ALL 8 experts BEFORE Phase 2a
        #
        # This overlaps with in-flight Wave 1 DMA (issued in Phase 1b above).
        # aff_bcast and down_scale_bufs are both ready at this point:
        #   - aff_bcast was computed after Phase 1b issue
        #   - down_scale_bufs[k] were loaded in Phase 1a (Wave 0 DMA complete)
        #   - down_scale_w1_bufs[k] were loaded in Phase 1b (Wave 1 DMA in-flight/complete)
        # ==================================================================

        # Allocate precomputed combined_down_scale buffers — Wave 0
        cds_pre_w0_0 = nl.ndarray((_PMAX, _H_FREE_SHARD), dtype=nl.float32, buffer=nl.sbuf)
        cds_pre_w0_1 = nl.ndarray((_PMAX, _H_FREE_SHARD), dtype=nl.float32, buffer=nl.sbuf)
        cds_pre_w0_2 = nl.ndarray((_PMAX, _H_FREE_SHARD), dtype=nl.float32, buffer=nl.sbuf)
        cds_pre_w0_3 = nl.ndarray((_PMAX, _H_FREE_SHARD), dtype=nl.float32, buffer=nl.sbuf)
        cds_pre_w0 = [cds_pre_w0_0, cds_pre_w0_1, cds_pre_w0_2, cds_pre_w0_3]

        # Allocate precomputed combined_down_scale buffers — Wave 1
        cds_pre_w1_0 = nl.ndarray((_PMAX, _H_FREE_SHARD), dtype=nl.float32, buffer=nl.sbuf)
        cds_pre_w1_1 = nl.ndarray((_PMAX, _H_FREE_SHARD), dtype=nl.float32, buffer=nl.sbuf)
        cds_pre_w1_2 = nl.ndarray((_PMAX, _H_FREE_SHARD), dtype=nl.float32, buffer=nl.sbuf)
        cds_pre_w1_3 = nl.ndarray((_PMAX, _H_FREE_SHARD), dtype=nl.float32, buffer=nl.sbuf)
        cds_pre_w1 = [cds_pre_w1_0, cds_pre_w1_1, cds_pre_w1_2, cds_pre_w1_3]

        # Precompute Wave 0 combined_down_scales (k = 0..3)
        for k_pre in nl.static_range(_K_WAVE):
            aff_col_buf_pre = nl.ndarray((_PMAX, 1), dtype=nl.float32, buffer=nl.sbuf)
            nisa.tensor_copy(dst=aff_col_buf_pre, src=aff_bcast[0:_PMAX, k_pre:k_pre + 1])
            aff_col_view_pre = TensorView(aff_col_buf_pre).broadcast(dim=1, size=_H_FREE_SHARD)
            nisa.tensor_tensor(
                cds_pre_w0[k_pre],
                down_scale_bufs[k_pre],
                aff_col_view_pre.get_view(),
                nl.multiply,
            )

        # Precompute Wave 1 combined_down_scales (kk = 4..7)
        for k_pre in nl.static_range(_K_WAVE):
            kk_pre = k_pre + 4
            aff_col_buf_pre = nl.ndarray((_PMAX, 1), dtype=nl.float32, buffer=nl.sbuf)
            nisa.tensor_copy(dst=aff_col_buf_pre, src=aff_bcast[0:_PMAX, kk_pre:kk_pre + 1])
            aff_col_view_pre = TensorView(aff_col_buf_pre).broadcast(dim=1, size=_H_FREE_SHARD)
            nisa.tensor_tensor(
                cds_pre_w1[k_pre],
                down_scale_w1_bufs[k_pre],
                aff_col_view_pre.get_view(),
                nl.multiply,
            )
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # V12g: Use precomputed combined_down_scale (Wave 0)
            combined_down_scale = cds_pre_w0[k]
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # V12g: Use precomputed combined_down_scale (Wave 1)
            combined_down_scale = cds_pre_w1[k]
            ...
            # V11c: Fully batched output accumulation (Wave 1 always adds — Wave 0 already wrote)
            nisa.tensor_tensor(
                dst=output_temp[0:_PMAX, 0:_H_FREE_SHARD],
                data1=output_temp[0:_PMAX, 0:_H_FREE_SHARD],
                data2=down_h_all[0:_PMAX, 0:_H_FREE_SHARD],
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v12g kernel (Round 4 Plan G: Precomputed combined_down_scales).
