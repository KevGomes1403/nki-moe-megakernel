...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        # Plan A: tmp buffer for 3-step VectorE dequant (fp8 → fp32 → scale → bf16)
        # Reused across all (k, h1, j_blk) dequant iterations in both waves.
        tmp_fp32_dq = nl.ndarray((_PMAX, _PMAX), dtype=nl.float32, buffer=nl.sbuf)
        # Contiguous [128, 1] fp32 column for scale extraction; broadcast to [128, 128]
        tmp_scale_col = nl.ndarray((_PMAX, 1), dtype=nl.float32, buffer=nl.sbuf)
        tmp_scale_col_bcast = TensorView(tmp_scale_col).broadcast(dim=1, size=_PMAX)
        # For down tile1 (I1=64 valid rows): separate [64, 1] → [64, 128] broadcast
        tmp_scale_col_i1 = nl.ndarray((I1, 1), dtype=nl.float32, buffer=nl.sbuf)
        tmp_scale_col_i1_bcast = TensorView(tmp_scale_col_i1).broadcast(dim=1, size=_PMAX)
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # Plan A: 3-step VectorE pattern (cast fp8→fp32, scale, cast fp32→bf16)
            # gate_up_scale_bufs[k] is [128P, H_free, 3F] (3D); extract to contiguous
            # tmp_scale_col then broadcast [128,1]→[128,128] via TensorView
            GU_FLAT_BLOCKS = _GU_FLAT // _PMAX  # = 3
            for h1 in nl.static_range(H_free):
                for j_blk in nl.static_range(GU_FLAT_BLOCKS):
                    # Extract contiguous [128P, 1F] scale column from 3D scale buffer
                    nisa.activation(
                        tmp_scale_col[0:_PMAX, 0:1],
                        op=nl.copy,
                        data=gate_up_scale_bufs[k][0:_PMAX, h1, j_blk:j_blk + 1],
                    )
                    # Step 1: Cast fp8 → fp32 (scale=1.0, compile-time scalar)
                    nisa.activation(
                        tmp_fp32_dq[0:_PMAX, 0:_PMAX],
                        op=nl.copy,
                        data=gate_up_fp8_bufs[k][0:_PMAX, h1, nl.ds(j_blk * _PMAX, _PMAX)],
                    )
                    # Step 2: Multiply with broadcast [128,1]→[128,128]
                    nisa.tensor_tensor(
                        tmp_fp32_dq[0:_PMAX, 0:_PMAX],
                        tmp_fp32_dq[0:_PMAX, 0:_PMAX],
                        tmp_scale_col_bcast.get_view(),
                        nl.multiply,
                    )
                    # Step 3: Cast fp32 → bf16
                    nisa.activation(
                        gate_up_bufs[k][0:_PMAX, h1, nl.ds(j_blk * _PMAX, _PMAX)],
                        op=nl.copy,
                        data=tmp_fp32_dq[0:_PMAX, 0:_PMAX],
                    )

            # Dequantize down tile 0: [I0=128P, H_shard] fp8 → bf16
            # down_scale0_bufs[k] is [128P, 8F] (2D), broadcast [128,1]→[128,128]
            ...
            for h_blk in nl.static_range(H_BLOCKS):
                ...
                # Extract scale column to contiguous buffer, then broadcast
                nisa.activation(
                    tmp_scale_col[0:_PMAX, 0:1],
                    op=nl.copy,
                    data=down_scale0_bufs[k][0:_PMAX, h_blk:h_blk + 1],
                )
                # Step 2: Scale with broadcast
                nisa.tensor_tensor(
                    tmp_fp32_dq[0:_PMAX, 0:_PMAX],
                    tmp_fp32_dq[0:_PMAX, 0:_PMAX],
                    tmp_scale_col_bcast.get_view(),
            ...
            # down_scale1_bufs[k] is [128P, 8F] (2D), broadcast [64,1]→[64,128]
            for h_blk in nl.static_range(H_BLOCKS):
                # Step 1: Cast fp8 → fp32 (only valid rows 0:I1)
                nisa.activation(
                    tmp_fp32_dq[0:I1, 0:_PMAX],
                    op=nl.copy,
                    data=down_full1_fp8_bufs[k][0:I1, nl.ds(h_blk * _PMAX, _PMAX)],
                )
                # Extract scale column to dedicated [I1, 1] buffer
                nisa.activation(
                    tmp_scale_col_i1[0:I1, 0:1],
                    op=nl.copy,
                    data=down_scale1_bufs[k][0:I1, h_blk:h_blk + 1],
                )
                # Step 2: Scale with broadcast [I1,1]→[I1,128]
                nisa.tensor_tensor(
                    tmp_fp32_dq[0:I1, 0:_PMAX],
                    tmp_fp32_dq[0:I1, 0:_PMAX],
                    tmp_scale_col_i1_bcast.get_view(),
                    nl.multiply,
                )
                # Step 3: Cast fp32 → bf16
                nisa.activation(
        ...
        # Dequantize Wave 1 weights (Plan A: 3-step VectorE dequant, same as wave 0)
        for k in nl.static_range(_K_WAVE):
            GU_FLAT_BLOCKS = _GU_FLAT // _PMAX  # = 3
            for h1 in nl.static_range(H_free):
                for j_blk in nl.static_range(GU_FLAT_BLOCKS):
