...
v15g — Round 2 Plan C [G] cross-layer expert prefetch prototype (skeleton)
=========================================================================
Based on v15c (Round 1 winner at 72.88 us). Preserves ALL v15c optimizations
(SBUF hoisting, affine_range tile-1 prefetch, ring buffers across waves,
pre-read of 8 expert IDs, rearranged up-scale bufs, post-matmul combined-
scale precompute, fp8 stationary weights, pre-quantize per-partition scales,
prg_id H-sharding under LNC=2, packed gate_up HBM layout with int8-
reinterpret HWDGE, merged gate_up weight+scale DMA, etc.)

[G] cross-layer expert prefetch — reality check
------------------------------------------------
The canonical Plan [G] (Fate, arXiv 2502.12224) prefetches layer N+1's
top-K expert weights into SBUF concurrently with layer N's compute.
In the single-layer isolated kernel (v15c), there IS no layer N+1 — the
prefetch benefit is only measurable when the kernel is embedded in a
multi-layer harness (fused transformer loop or a megakernel).

Within-kernel probes ran and found the ceiling
-----------------------------------------------
Three within-kernel reordering variants were attempted to expose extra
overlap between Wave 1 DMA and Wave 0 compute:

  Attempt 1a: defer Wave 1 scale extract + up-scale assembly until
              after Wave 0 compute. Result: regressed to 78.60 us
              (+7.8%) — the VectorE work for Wave 1 scale setup
              extended the compute critical path instead of filling a
              DMA-wait gap.
  Attempt 1b: group DMAs by TYPE (all 8 gate_up HWDGE, then all 8
              down_w tile-0 SWDGE, then tile-1, then down_scales).
              Result: 74.92 us (+2.8%).
  Attempt 1c: hoist only the fat Wave 1 gate_up_packed DMAs ahead of
              Wave 0's down_w tiles. Result: 75.20 us (+3.2%).

All three reduced gpsimd_engine_pct (30.4% -> 24.3-26.8%) and
dma_active_pct (57.4% -> 49.8-53.8%), but tensor_engine_pct dropped
correspondingly. The compiler's default schedule on v15c already
effectively overlaps Wave 1 DMA with Wave 0 compute via affine_range
prefetch + HWDGE/SWDGE dual-queue parallelism. There is no within-kernel
slack left to exploit.

What v15g ships
---------------
v15g preserves v15c's DMA ordering verbatim — the file is structurally
identical to v15c. The intent is to serve as the "Plan [G] skeleton" that
a future multi-layer megakernel harness can extend by:

  1. Promoting the Wave 1 `*_w1_bufs` set to a generic "speculative
     prefetch" buffer set, indexed by a predicted top-K expert list
     rather than the current layer's top-8 IDs.
  2. Adding an explicit predictor hook (described in the Plan [G] design
     doc) that consumes the current layer's gate-input features (the
     pre-router activations) and produces a predicted top-K list for
     the next layer.
  3. Stitching the predicted next-layer DMAs into the tail of the
     current layer's compute phase (Phase 2b) so they overlap with the
     residual-accumulation / layer-norm work at the layer boundary.

Correctness: bit-exact identical to v15c (same NEFF expected under cache
reuse). Benchmark: within measurement noise of v15c (v15c: 72.29 us mean
of 3 runs; v15g: 71.65 us mean of 3 runs; delta -0.64 us, -0.89%).

Interface: IDENTICAL to v15c (same arg signature, same HBM layouts).

See `docs/plan_g_cross_layer_prefetch_design.md` for the full predictor
interface spec, required kernel-level changes, and integration plan for
moving the speculative prefetch across the layer boundary.

Key change (F3) vs v14a (preserved from v15c):
  ...
  - v14a issues 5 separate indirect DMAs per expert (gate_up_w, gate_up_scales,
    down_w tile 0, down_w tile 1, down_scales). v15c reduces this to 4 by
...
def qwen3_moe_fused_tkg(
    ...
    v15g: Plan [G] cross-layer prefetch skeleton — structurally identical
    to v15c (same DMA ordering, same arithmetic). Within-kernel reordering
    probes uniformly regressed; v15c's schedule is at the within-kernel
    ceiling. Real Plan [G] gain requires a multi-layer harness (see module
    docstring + design doc).
    ...
    for t in nl.affine_range(T):
        ...
        #
        # NOTE (v15g): Attempts to reorder these DMAs (hoist Wave 1 gate_up
        # ahead, or group DMAs by type) uniformly regressed by 2-8% —
        # v15c's ordering is already well-tuned for within-wave overlap.
        # We preserve it exactly. See the Plan [G] design doc for the
        # restructuring that would be required to measure cross-layer gain.
        ...
        # This phase represents the "within-kernel prefetch" — Wave 1 DMAs
        # are issued here so they overlap with Wave 0 compute below. In a
        # future multi-layer harness, this is where the speculative Fate-
        # style cross-layer prefetch would take over: instead of loading
        # Wave 1 of the CURRENT layer, these DMAs would preload layer N+1's
        # top-K weights into a third buffer set, based on the predictor's
        # output from the layer N gate-input features. See Plan [G] design
        # notes in the module docstring / external design doc.
        for k in nl.affine_range(_K_WAVE):
            kk = k + 4
...
def pack_gate_up(gate_up_w_i8, gate_up_scales_f32):
    """Build the v15c/v15g packed gate_up HBM tensor (same layout as v15c).
...
def run(inp, gamma, router_w, gate_up_packed_w, down_w, down_scales):
    """Run v15g kernel (Round 2 Plan C [G] within-kernel Wave 0->Wave 1 prefetch).
