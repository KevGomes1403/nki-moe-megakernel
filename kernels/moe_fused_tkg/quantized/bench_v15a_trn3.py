"""
Benchmark for v15a (Plan A [F2] — direct DGE for expert weight loads on trn3).
Mirrors bench_v14a_trn3.py so the numbers are directly comparable.
"""
import os
import sys
...
os.environ["NEURON_RT_INSPECT_OUTPUT_DIR"] = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "_bench_out_v15a_trn3"
)
...
from kernels.moe_fused_tkg.quantized.v15a import qwen3_moe_fused_tkg
...
print("Running v15a bench (trn3 harness)...")
kernel(*args)
...
if r and r.prof:
    ...
    print(f"\nv14a baseline (from Phase 0): 88.06 μs  delta={r.device_time_us - 88.06:+.2f} μs")
else:
