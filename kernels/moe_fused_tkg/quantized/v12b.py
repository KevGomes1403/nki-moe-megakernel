...
v12b — Plan B: Output-Tile Block-Wise Scale
============================================
Based on v11c (affine_range expert loads + output_temp batching).

Key changes vs v11c:
  Scale granularity reduced:
    gate_up_scales: [E, GU=384]  → [E, 4]   — one fp32 scalar per output tile
    down_scales:    [E, H=2048]  → [E, H_FREE=16] — one fp32 scalar per 128-wide H block

  Scale loading:
    Old: DMA [E,384] as [128P, 3F] per expert + 6 tensor_copy rearrangements per expert
    New: DMA [E,4] as [1P, 4F] + nc_stream_shuffle x4 → [128P, 4F], no rearrangements

    Old: DMA [E,2048] as [128P, 8F] per expert shard
    New: DMA [E,16] as [1P, 8F] + nc_stream_shuffle x4 → [128P, 8F]

  Eliminated:
    up_scale_i0_bufs / up_scale_i1_bufs (and w1 equivalents)
    All 24 tensor_copy rearrangements per kernel call
    gate_up_scale_bufs now [128P, 4F] instead of [128P, 3F]
    up col uses col 2 (i_tile=0) and col 3 (i_tile=1) directly

  Tile mapping (gate_up_scales [E, 4]):
    col 0 → gate_t0 scale (output cols 0:128)
    col 1 → gate_t1 scale (output cols 128:192, zero-padded to 128)
    col 2 → up_t0 scale   (output cols 192:320)
    col 3 → up_t1 scale   (output cols 320:384, zero-padded to 128)

  Down scale mapping (down_scales [E, 16]):
    col j → scale for H-block j (output rows j*128:(j+1)*128, full H)
    Each LNC shard (prg_id ∈ {0,1}) accesses 8 blocks at offset prg_id*8
...
def qwen3_moe_fused_tkg(
    ...
    gate_up_scales,   # [E=128, 4]                   fp32  per-output-tile (v12b)
    down_w,           # [E=128, I=192,  H=2048]       int8 (reinterpreted as fp8_e4m3fn)
    down_scales,      # [E=128, H_FREE=16]            fp32  per-H-block (v12b)
):
    """
    ...
    for t in nl.static_range(T):
        ...
        # v12b: gate_up scale buffers: [128P, 4F] — one scale per output tile
        ...
        # v12b: down scale buffers: [128P, 8F] — one per H-block in the shard
        ...
        for k in nl.affine_range(_K_WAVE):
            ...
            # v12b: Load gate_up_scales[e, :] as [1P, 4F] (16 bytes), then broadcast
            ...
            # v12b: Load down_scales[e, prg_id*8 : prg_id*8+8] as [1P, 8F], then broadcast
            # down_scales shape: [E, H_FREE=16], each LNC shard accesses 8 values at prg_id*8
        ...
        for k in nl.affine_range(_K_WAVE):
            ...
            # v12b: Load gate_up_scales for Wave 1 — compact [1P, 4F] + broadcast
            ...
            # v12b: Load down_scales for Wave 1 — compact [1P, 8F] + broadcast
        ...
        # NOTE: No up_scale rearrangement needed — columns 2 and 3 of gate_up_scale_bufs
        # are used directly for up_t0 and up_t1 respectively.
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            for i_tile in nl.static_range(I_tiles):
                # v12b: gate scale = col i_tile of [128P, 4F]
                ...
                # v12b: up scale = col 2+i_tile of [128P, 4F] — no rearrangement needed
                up_scale_col = gate_up_scale_bufs[k][0:_PMAX, 2 + i_tile:2 + i_tile + 1]
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            for i_tile in nl.static_range(I_tiles):
                ...
                # v12b: up scale = col 2+i_tile — no rearrangement (Wave 1)
                up_scale_col = gate_up_scale_w1_bufs[k][0:_PMAX, 2 + i_tile:2 + i_tile + 1]
                up_f32_scaled = nl.ndarray((_PMAX, 1), dtype=nl.float32, buffer=nl.sbuf)
                nisa.activation(
                    up_f32_scaled,
                    op=nl.copy,
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v12b kernel (Plan B: Output-Tile Block-Wise Scale).

    Accepts:
      gate_up_w:      [E, H, 2*I=384] fp8_e4m3fn (as int8)
      gate_up_scales: [E, 4]           fp32  per-output-tile (v12b)
      down_w:         [E, I=192, H=2048] fp8_e4m3fn (as int8)
      down_scales:    [E, H_FREE=16]   fp32  per-H-block (v12b)
