...
v6b — Plan B: Fuse SiLU into Scaled Gate Drain + Fuse Down-Scale with Affinity
================================================================================
Based on v5 (Plan D: Collapse gate_up FP8→BF16 Cast to ONE Call per Expert).

Key changes vs v5:
  V6b.1: Fuse gate PSUM drain+scale and SiLU into a single activation call
          (reduces 2 ScalarE calls to 1 per i_tile per expert).
  V6b.2: Precompute combined_down_scale = down_scale × affinity before h1_out loop,
          then drain PSUM with combined scale (replaces activation+tensor_scalar with
          tensor_tensor precompute + activation per h1_out per expert).

Note: fp8 native stationary nc_matmul is NOT supported by the current Neuron
compiler (BIR verification fails). This is v3's fallback approach but with
dramatically fewer scalar engine calls by collapsing all h1 rows into one cast.
...
SBUF Budget (per partition):
  4 x gate_up_fp8 [128P, 16*384] = 24 KiB
  4 x gate_up_bf16 [128P, 16*384] = 48 KiB
  ...
  4 x down_bf16_tile0 [128P, 256F] = 8 KiB each = 32 KiB
  4 x down_bf16_tile1 [64P, 256F]  = 4 KiB each = 16 KiB
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        # bf16 dequantized gate_up (same layout as v3)
        ...
        # down fp8 weight buffers (H_shard=256 per prg_id)
        ...
        # Zero-pad down_full1 rows I1:I0 (bf16 buffers)
        ...
        # ------------------------------------------------------------------
        # v4 Plan C: Batch dequantize Wave 0 weights (fp8 → bf16)
        # Replace per-tile loops with one call per h1 for gate_up,
        # and one call per buffer for down.
        # ------------------------------------------------------------------
        for k in nl.static_range(_K_WAVE):
            # v5: ONE activation call for entire gate_up buffer [128P, H_free=16, GU_FLAT=384]
            nisa.activation(
                gate_up_bufs[k][0:_PMAX, 0:H_free, 0:_GU_FLAT],
                op=nl.copy,
                data=gate_up_fp8_bufs[k][0:_PMAX, 0:H_free, 0:_GU_FLAT],
            )

            # down tile0: one activation call covering full H_shard=256
            nisa.activation(
                down_full0_bufs[k][0:_PMAX, 0:H_shard],
                op=nl.copy,
                data=down_full0_fp8_bufs[k][0:_PMAX, 0:H_shard],
            )

            # down tile1: one activation call, only I1=64 valid rows, full H_shard width
            nisa.activation(
                down_full1_bufs[k][0:I1, 0:H_shard],
                op=nl.copy,
                data=down_full1_fp8_bufs[k][0:I1, 0:H_shard],
            )
        ...
        # Phase 2a: Compute experts 0-3 (bf16 stationary, same as v3)
        for k in nl.static_range(_K_WAVE):
            gu_base = k * 2 * I_tiles
            d_base  = k * H_free_shard

            # Tile-1 tensor_copy (bf16, same as v3)
            nisa.tensor_copy(
            ...
            # Gate/Up matmul (bf16 stationary)
            ...
            for i_tile in nl.static_range(I_tiles):
                ...
                # V6b Change 1: Fuse drain+scale+SiLU into single activation call
            ...
            # V6b Change 2: Precompute combined_down_scale = down_scale * affinity
            ...
            for h1_out in nl.static_range(H_free_shard):
                nisa.tensor_tensor(
                    combined_down_scale[0:_PMAX, h1_out:h1_out + 1],
                    down_scale_bufs[k][0:_PMAX, h1_out:h1_out + 1],
                    aff_bcast[0:_PMAX, k:k + 1],
                    nl.multiply,
                )

            # Post-matmul drain with combined scale + accumulate
            for h1_out in nl.static_range(H_free_shard):
        ...
        # v5: Collapse gate_up cast to ONE call per expert for wave 1
        for k in nl.static_range(_K_WAVE):
            # v5: ONE activation call for entire gate_up buffer [128P, H_free=16, GU_FLAT=384]
            nisa.activation(
                gate_up_bufs[k][0:_PMAX, 0:H_free, 0:_GU_FLAT],
            ...
            # down tile0: one activation call for full H_shard=256
            nisa.activation(
                down_full0_bufs[k][0:_PMAX, 0:H_shard],
                op=nl.copy,
                data=down_full0_fp8_bufs[k][0:_PMAX, 0:H_shard],
            ...
            # down tile1: one activation call, only I1=64 valid rows
            nisa.activation(
                down_full1_bufs[k][0:I1, 0:H_shard],
                op=nl.copy,
                data=down_full1_fp8_bufs[k][0:I1, 0:H_shard],
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v6b kernel with fused SiLU and fused down-scale+affinity (Plan B).
