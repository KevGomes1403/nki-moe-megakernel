"""
Benchmark for v13a (W8A8 FP8 variant of v12i).
"""
import os
import sys
...
os.environ["NEURON_RT_INSPECT_OUTPUT_DIR"] = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "_bench_out_v13a"
)
...
print("Running benchmark for v13a (W8A8 FP8)...")
kernel(*args)
...
if r:
    ...
    print(f"\n--- vs v12i baseline (90.54 μs) ---")
    delta = r.device_time_us - 90.54
    pct = delta / 90.54 * 100
    sign = "+" if delta >= 0 else ""
    print(f"delta = {sign}{delta:.2f} μs  ({sign}{pct:.1f}%)")
else:
