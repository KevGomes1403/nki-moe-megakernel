...
v10b — Plan B: Drop T dim from output_temp, fully batch accumulation
=====================================================================
Based on v9a (Plan A: Batch post-matmul chain [128,1]×8 → [128,8]).

Key changes vs v9a:
  V10b: Drop the T=1 dimension from output_temp entirely.
       ...
       Eliminates remaining 16× per-expert per-column [128,1] instruction overhead.
...
def qwen3_moe_fused_tkg(
    ...
    # -----------------------------------------------------------------------
    # V10b: Drop T dimension — output_temp is [128, H_free_shard=8] (T=1 always in TKG)
    ...
    for t in nl.static_range(T):
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # V10b: Fully batched output accumulation — single [128, H_free_shard=8] op
            if k == 0:
                nisa.tensor_copy(
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # V10b: Fully batched output accumulation (Wave 1 always adds — Wave 0 already wrote)
            nisa.tensor_tensor(
                dst=output_temp[0:_PMAX, 0:_H_FREE_SHARD],
                data1=output_temp[0:_PMAX, 0:_H_FREE_SHARD],
                data2=down_h_all[0:_PMAX, 0:_H_FREE_SHARD],
    ...
    # V10b: output_temp is [128, H_free_shard] — use [0:_PMAX, h1:h1+1] for [128,1] slice
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v10b kernel (Plan B: Drop T dim from output_temp, fully batch accumulation).
