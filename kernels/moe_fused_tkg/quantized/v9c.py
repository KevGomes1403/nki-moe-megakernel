...
v9c — Plan C: Fuse inter fp32→bf16 cast into gate_silu and up_scaled allocations
==================================================================================
Based on v8 (H_shard Fix: full H=2048, 1024 per LNC prg_id).

Key changes vs v8:
  V9c: Eliminate the extra fp32→bf16 ScalarE cast (nisa.activation inter_f32→inter_bf16)
       by making gate_silu and up_f32_scaled directly bf16 (inp.dtype).
       inter_bf16 is now allocated as inp.dtype directly from tensor_tensor output.
       Removes 1 ScalarE call per expert × 8 experts = 8 ScalarE calls eliminated.
       Also frees inter_f32 buffer (2 fp32 SBUF slots: [128,2] × 2 waves).
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # V9c: gate_silu and up_f32_scaled are now bf16; inter_bf16 directly from tensor_tensor
            ...
            for i_tile in nl.static_range(I_tiles):
                ...
                # V9c: output dtype is bf16 — ScalarE computes in fp32 and casts at output
                gate_silu = nl.ndarray((_PMAX, 1), dtype=inp.dtype, buffer=nl.sbuf)
                nisa.activation(
                    gate_silu,
                    op=nl.silu,
                ...
                up_f32_scaled = nl.ndarray((_PMAX, 1), dtype=inp.dtype, buffer=nl.sbuf)
                nisa.activation(
                    up_f32_scaled,
                    op=nl.copy,
                ...
                nisa.tensor_tensor(inter_bf16[0:_PMAX, i_tile:i_tile + 1], gate_silu, up_f32_scaled, nl.multiply)

            # V9c: inter_bf16 is already bf16 — no extra cast needed
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            for i_tile in nl.static_range(I_tiles):
                ...
                # V9c: output dtype is bf16 (Wave 1)
                gate_silu = nl.ndarray((_PMAX, 1), dtype=inp.dtype, buffer=nl.sbuf)
                nisa.activation(
                    gate_silu,
                    op=nl.silu,
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v9c kernel (Plan C: fuse inter fp32→bf16 cast into gate_silu/up_scaled allocations).
