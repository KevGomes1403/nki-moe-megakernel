"""
Benchmark for v14a (current trn3 best) using the trn3 skill's benchmark harness.

Phase 0 ground-truth bench:
  - trn3 harness (adds vector_engine_pct; tuned for trn3 summary keys).
  - reports every BenchmarkResult field listed in the master brief.
  - one wrap_benchmark call captures one NEFF+NTFF; the hardware run is
    deterministic within noise once the NEFF is compiled, so a single
    NTFF is representative of device behavior.
"""
import os
import sys
...
os.environ["NEURON_RT_INSPECT_OUTPUT_DIR"] = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "_bench_out_v14a_trn3"
)
...
from kernels.moe_fused_tkg.quantized.v14a import qwen3_moe_fused_tkg
...
print("Running v14a bench (trn3 harness)...")
kernel(*args)
...
if r and r.prof:
    ...
    print(f"\nv14a declared baseline (log): 88.68 μs  delta={r.device_time_us - 88.68:+.2f} μs")
else:
