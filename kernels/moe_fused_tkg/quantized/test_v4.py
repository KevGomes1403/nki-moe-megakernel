...
v4 uses fp8 directly as stationary in nc_matmul (no pre-matmul dequant).
The reference mirrors what v4 does:
  1. RMSNorm(x) * gamma
...
The fp8 stationary matmul with bf16 moving is numerically equivalent to:
  bf16_cast(fp8) then bf16 matmul — same as v3's reference.
...
v4_mod = load_kernel("/home/ubuntu/nki-moe/kernels/moe_fused_tkg/quantized/v4.py", "v4")
...
# v4 reference: fp8 stationary matmul is equivalent to bf16 cast then matmul
...
# -----------------------------------------------------------------------
# PyTorch Reference: mimics v4 exactly (fp8 stationary = bf16 cast + matmul)
# -----------------------------------------------------------------------
T = B
...
new_out = v4_mod.qwen3_moe_fused_tkg[2](
