...
v10c — Round 2 Plan C: Combine double-buffer Wave 1 DMA loads + output_temp batching
====================================================================================
Based on v10a (double-buffer Wave 1 DMA loads).

Key changes vs v10a (which itself is vs v9a):
  V10a: Move Phase 1b (Wave 1 weight loads) BEFORE Phase 2a (Wave 0 compute).
        Allocate separate Wave 1 weight buffers to avoid data hazards.
        GpSimdE can issue Wave 1 DMA descriptors while TensorE/ScalarE/VectorE
        execute Wave 0 matmuls — eliminating the "dead zone" after Wave 0 compute.

        Restructured order:
          Phase 1a: Load Wave 0 weights (k=0..3) into wave0 buffers
          Phase 1b: Load Wave 1 weights (k=0..3, kk=4..7) into dedicated w1 buffers  ← MOVED HERE
          Compute norm_weights/aff_bcast
          Phase 2a: Compute Wave 0 (Wave 1 DMAs overlap with TensorE compute)
          PSUM memset
          Phase 2b: Compute Wave 1 (data already loaded)

  V10c (additional, from v10b): Drop the T=1 dimension from output_temp entirely.
       - output_temp reshaped from [128, H_free_shard, T=1] → [128, H_free_shard=8]
       - Wave 0 output accumulation: single tensor_copy/tensor_tensor([128,8])
         replacing the per-column loop of 8x [128,1] ops
       - Wave 1 output accumulation: single tensor_tensor([128,8])
         replacing the per-column loop of 8x [128,1] ops
       - Stage 5: output_temp access changed from [0:_PMAX, h1, 0:T] → [0:_PMAX, h1:h1+1]
...
def qwen3_moe_fused_tkg(
    ...
    # -----------------------------------------------------------------------
    # V10c: Drop T dimension — output_temp is [128, H_free_shard=8] (T=1 always in TKG)
    ...
    for t in nl.static_range(T):
        ...
        # Separate eid scratch buffers for Wave 1
        eid_scratch_w1_0 = nl.ndarray((_PMAX, 1), dtype=nl.int32, buffer=nl.sbuf)
        eid_scratch_w1_1 = nl.ndarray((_PMAX, 1), dtype=nl.int32, buffer=nl.sbuf)
        eid_scratch_w1_2 = nl.ndarray((_PMAX, 1), dtype=nl.int32, buffer=nl.sbuf)
        eid_scratch_w1_3 = nl.ndarray((_PMAX, 1), dtype=nl.int32, buffer=nl.sbuf)
        eid_scratch_w1_bufs = [eid_scratch_w1_0, eid_scratch_w1_1, eid_scratch_w1_2, eid_scratch_w1_3]
        ...
        # ------------------------------------------------------------------
        # Phase 1b (MOVED BEFORE COMPUTE): Load Wave 1 experts into dedicated W1 buffers
        # GpSimdE issues these DMA descriptors; Wave 0 compute will overlap with
        # actual data transfer once TensorE starts executing Phase 2a.
        # ------------------------------------------------------------------
        for k in nl.static_range(_K_WAVE):
            kk = k + 4
            nisa.dma_copy(
                dst=eid_scratch_w1_bufs[k][0:1, 0:1],
                src=top8_idx.ap(pattern=[[K, 1], [1, 1]], offset=t * K + kk),
            )
            expert_id_w1 = eid_scratch_w1_bufs[k].ap(pattern=[[1, 1], [1, 1]], offset=0)
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # V9a: Batch combined_down_scale precompute (Wave 0: use k)
            ...
            # V9a: Batch PSUM drain — single activation([128, H_free_shard]), no scale
            ...
            # V9a: Apply combined_down_scale via tensor_tensor [128, 8]
            ...
            # V10c: Fully batched output accumulation — single [128, H_free_shard=8] op
            if k == 0:
                nisa.tensor_copy(
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # V9a: Batch combined_down_scale precompute (Wave 1: use kk)
            aff_col_buf = nl.ndarray((_PMAX, 1), dtype=nl.float32, buffer=nl.sbuf)
            ...
            # V10c: Fully batched output accumulation (Wave 1 always adds — Wave 0 already wrote)
            nisa.tensor_tensor(
                dst=output_temp[0:_PMAX, 0:_H_FREE_SHARD],
                data1=output_temp[0:_PMAX, 0:_H_FREE_SHARD],
                data2=down_h_all[0:_PMAX, 0:_H_FREE_SHARD],
    ...
    # V10c: output_temp is [128, H_free_shard] — use [0:_PMAX, h1:h1+1] for [128,1] slice
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v10c kernel (Round 2 Plan C: double-buffer Wave 1 DMA loads + output_temp batching).
