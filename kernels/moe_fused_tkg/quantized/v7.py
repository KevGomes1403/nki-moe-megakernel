...
v7 — Plan D: v6a + SiLU Fusion + Down-Scale+Affinity Fusion
=============================================================
Based on v6a (FP8 Stationary Matmul).

Key changes vs v6a:
  V7.1: Fuse gate drain+scale+SiLU into single nisa.activation call.
         Eliminates gate_f32_scaled intermediate buffer per i_tile.
  V7.2: Fuse down-scale with affinity via precomputed combined_down_scale.
         Eliminates down_h_sbuf + tensor_scalar(affinity) — replaces 2 ScalarE
         calls per h1_out with 1 activation call using combined scale.
  Changes applied to both Wave 0 and Wave 1 compute loops.
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v7 kernel (Plan D: v6a + SiLU fusion + down-scale+affinity fusion).
