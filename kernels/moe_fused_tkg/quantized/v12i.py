...
v12i — Plan I: affine_range prefetch for tile-1 tensor_copies
=============================================================
Based on v11c (affine_range expert loads + output_temp batching).

Key changes vs v11c:
  V12i: Split the gate_t1_128 / up_t1_128 tensor_copy out of the affine_range
        compute loop into a separate affine_range prefetch loop.
        - gate_t1_128 and up_t1_128 each gain a k-dimension: [_K_WAVE, 128P, H_free, I0]
        - A prefetch loop (affine_range) fills all 4 k-slots before Phase 2a/2b compute
        - The compiler can now pipeline prefetch of expert k+1's tile-1 data while
          running expert k's h1 matmul loop
        - output_temp initialized with memset before loop; `if k == 0` branch removed;
          Wave 0 and Wave 1 both always-add to output_temp

        SBUF cost of extra k-dim:
          2 * 4 * 128 * 16 * 128 bytes (fp8) = 2 MB per wave (acceptable in 24 MB)
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v12i kernel (Plan I: affine_range prefetch for tile-1 tensor_copies).
