...
v10a — Round 2 Plan A: Double-buffer Wave 1 DMA loads to overlap with Wave 0 compute
=====================================================================================
Based on v9a (batch post-matmul chain [128,1]×8 → [128,8]).
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # V9a: Output accumulation per-column using batched down_h_all
            for h1_out in nl.static_range(H_free_shard):
                if k == 0:
                    nisa.tensor_copy(
                        dst=output_temp[0:_PMAX, h1_out:h1_out + 1, t:t + 1],
                        src=down_h_all[0:_PMAX, h1_out:h1_out + 1],
                    )
                else:
                    nisa.tensor_tensor(
                        dst=output_temp[0:_PMAX, h1_out:h1_out + 1, t:t + 1],
                        data1=output_temp[0:_PMAX, h1_out:h1_out + 1, t:t + 1],
                        data2=down_h_all[0:_PMAX, h1_out:h1_out + 1],
                        op=nl.add,
                    )
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # V9a: Output accumulation per-column (Wave 1 always adds — Wave 0 already wrote)
            for h1_out in nl.static_range(H_free_shard):
                nisa.tensor_tensor(
                    dst=output_temp[0:_PMAX, h1_out:h1_out + 1, t:t + 1],
                    data1=output_temp[0:_PMAX, h1_out:h1_out + 1, t:t + 1],
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v10a kernel (Round 2 Plan A: double-buffer Wave 1 DMA loads).
