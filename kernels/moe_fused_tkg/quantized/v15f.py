...
v15f — Plan B [F3b]: Extend int8-reinterpret HWDGE to down_w DMAs
=================================================================
Based on v15c (Plan C [F3]). Inherits v14a's full optimization stack plus
v15c's merged gate_up weight+scale packing. Adds the same HWDGE-friendly
int8-reinterpret trick to the 4 down_w DMAs that were still on SWDGE.

Key change vs v15c:
  - v15c already applied the int8-reinterpret HWDGE trick to the merged
    gate_up tile: load as int8, then .view() into float8_e4m3 for matmul
    and float32 for scales — HWDGE-compatible because src/dst dtypes match.
  - v15c however kept down_w on SWDGE: the DMA src uses `dtype=nl.float8_e4m3`
    on an int8 HBM tensor, inducing an i8 -> f8 element-type cast that the
    HWDGE block does not support. This serialized 4 descriptor-gens per (t)
    on GpSimdE (baseline GpSimdE% = 30.47%, ~22 μs absolute).
  - v15f changes all 4 down_w load sites (Wave 0 tile0/tile1, Wave 1
    tile0/tile1) to: SBUF tile allocated as nl.int8, DMA without dtype
    kwarg (inherits HBM int8), dge_mode=hwdge, and at matmul consumption
    the buffer is `.view(nl.float8_e4m3)` (bit-level, zero data copy).
  - Expected: drops GpSimdE% toward 15-20%, reducing device time by 3-5 μs.
...
ALL v15c / v14a optimizations are preserved:
  - SBUF hoisting, affine_range tile-1 prefetch, ring buffers across waves,
    ...
    partition scales, prg_id H-sharding under LNC=2, merged gate_up packed
    weight+scale tile.
  - HBM layout of down_w and down_scales is UNCHANGED vs v15c/v14a. Only
    the SBUF tile dtype (int8 instead of float8_e4m3) and the DMA
    descriptor (HWDGE, no dtype cast) change.
...
def qwen3_moe_fused_tkg(
    ...
    v15f: v15c (merged gate_up HWDGE) + int8-reinterpret HWDGE on the 4
    down_w DMAs (Wave 0 tile0/tile1, Wave 1 tile0/tile1).
    ...
    for t in nl.affine_range(T):
        ...
        # down fp8 weight buffers (H_shard=1024 per prg_id) — v15f: allocate
        # as int8 so the DMA src (int8 HBM) matches dst dtype, enabling HWDGE.
        # The fp8 view is what matmul consumes (bit-level reinterpret).
        down_full0_int8_buf0 = nl.ndarray((_PMAX, H_shard), dtype=nl.int8, buffer=nl.sbuf)
        down_full0_int8_buf1 = nl.ndarray((_PMAX, H_shard), dtype=nl.int8, buffer=nl.sbuf)
        down_full0_int8_buf2 = nl.ndarray((_PMAX, H_shard), dtype=nl.int8, buffer=nl.sbuf)
        down_full0_int8_buf3 = nl.ndarray((_PMAX, H_shard), dtype=nl.int8, buffer=nl.sbuf)
        down_full0_int8_bufs = [down_full0_int8_buf0, down_full0_int8_buf1, down_full0_int8_buf2, down_full0_int8_buf3]
        down_full0_fp8_buf0_v = down_full0_int8_buf0.view(nl.float8_e4m3)
        down_full0_fp8_buf1_v = down_full0_int8_buf1.view(nl.float8_e4m3)
        down_full0_fp8_buf2_v = down_full0_int8_buf2.view(nl.float8_e4m3)
        down_full0_fp8_buf3_v = down_full0_int8_buf3.view(nl.float8_e4m3)
        down_full0_fp8_bufs = [down_full0_fp8_buf0_v, down_full0_fp8_buf1_v, down_full0_fp8_buf2_v, down_full0_fp8_buf3_v]

        down_full1_int8_buf0 = nl.ndarray((_PMAX, H_shard), dtype=nl.int8, buffer=nl.sbuf)
        down_full1_int8_buf1 = nl.ndarray((_PMAX, H_shard), dtype=nl.int8, buffer=nl.sbuf)
        down_full1_int8_buf2 = nl.ndarray((_PMAX, H_shard), dtype=nl.int8, buffer=nl.sbuf)
        down_full1_int8_buf3 = nl.ndarray((_PMAX, H_shard), dtype=nl.int8, buffer=nl.sbuf)
        down_full1_int8_bufs = [down_full1_int8_buf0, down_full1_int8_buf1, down_full1_int8_buf2, down_full1_int8_buf3]
        down_full1_fp8_buf0_v = down_full1_int8_buf0.view(nl.float8_e4m3)
        down_full1_fp8_buf1_v = down_full1_int8_buf1.view(nl.float8_e4m3)
        down_full1_fp8_buf2_v = down_full1_int8_buf2.view(nl.float8_e4m3)
        down_full1_fp8_buf3_v = down_full1_int8_buf3.view(nl.float8_e4m3)
        down_full1_fp8_bufs = [down_full1_fp8_buf0_v, down_full1_fp8_buf1_v, down_full1_fp8_buf2_v, down_full1_fp8_buf3_v]
        ...
        # Wave 1 down_w buffers — int8 storage, fp8 view for matmul (v15f)
        down_full0_int8_w1_buf0 = nl.ndarray((_PMAX, H_shard), dtype=nl.int8, buffer=nl.sbuf)
        down_full0_int8_w1_buf1 = nl.ndarray((_PMAX, H_shard), dtype=nl.int8, buffer=nl.sbuf)
        down_full0_int8_w1_buf2 = nl.ndarray((_PMAX, H_shard), dtype=nl.int8, buffer=nl.sbuf)
        down_full0_int8_w1_buf3 = nl.ndarray((_PMAX, H_shard), dtype=nl.int8, buffer=nl.sbuf)
        down_full0_int8_w1_bufs = [down_full0_int8_w1_buf0, down_full0_int8_w1_buf1, down_full0_int8_w1_buf2, down_full0_int8_w1_buf3]
        down_full0_fp8_w1_buf0_v = down_full0_int8_w1_buf0.view(nl.float8_e4m3)
        down_full0_fp8_w1_buf1_v = down_full0_int8_w1_buf1.view(nl.float8_e4m3)
        down_full0_fp8_w1_buf2_v = down_full0_int8_w1_buf2.view(nl.float8_e4m3)
        down_full0_fp8_w1_buf3_v = down_full0_int8_w1_buf3.view(nl.float8_e4m3)
        down_full0_fp8_w1_bufs = [down_full0_fp8_w1_buf0_v, down_full0_fp8_w1_buf1_v, down_full0_fp8_w1_buf2_v, down_full0_fp8_w1_buf3_v]

        down_full1_int8_w1_buf0 = nl.ndarray((_PMAX, H_shard), dtype=nl.int8, buffer=nl.sbuf)
        down_full1_int8_w1_buf1 = nl.ndarray((_PMAX, H_shard), dtype=nl.int8, buffer=nl.sbuf)
        down_full1_int8_w1_buf2 = nl.ndarray((_PMAX, H_shard), dtype=nl.int8, buffer=nl.sbuf)
        down_full1_int8_w1_buf3 = nl.ndarray((_PMAX, H_shard), dtype=nl.int8, buffer=nl.sbuf)
        down_full1_int8_w1_bufs = [down_full1_int8_w1_buf0, down_full1_int8_w1_buf1, down_full1_int8_w1_buf2, down_full1_int8_w1_buf3]
        down_full1_fp8_w1_buf0_v = down_full1_int8_w1_buf0.view(nl.float8_e4m3)
        down_full1_fp8_w1_buf1_v = down_full1_int8_w1_buf1.view(nl.float8_e4m3)
        down_full1_fp8_w1_buf2_v = down_full1_int8_w1_buf2.view(nl.float8_e4m3)
        down_full1_fp8_w1_buf3_v = down_full1_int8_w1_buf3.view(nl.float8_e4m3)
        down_full1_fp8_w1_bufs = [down_full1_fp8_w1_buf0_v, down_full1_fp8_w1_buf1_v, down_full1_fp8_w1_buf2_v, down_full1_fp8_w1_buf3_v]
        ...
        # Zero-pad down_full1 rows I1:I0 — Wave 0. v15f: buffers are int8
        # (storage) but fp8 zero-bits == int8 zero-bits, so we memset the
        # int8 alias with value=0 (integer) to match the nisa.memset kwarg
        # type requirement.
        for k_pad in nl.affine_range(4):
            nisa.memset(down_full1_int8_bufs[k_pad][nl.ds(I1, I1), 0:H_shard], value=0)

        # Zero-pad down_full1_w1 rows I1:I0 — Wave 1
        for k_pad in nl.affine_range(4):
            nisa.memset(down_full1_int8_w1_bufs[k_pad][nl.ds(I1, I1), 0:H_shard], value=0)
        ...
        for k in nl.affine_range(_K_WAVE):
            ...
            # Load down_w tile 0 (rows 0:I0=128) for this shard — v15f: int8
            # -> int8 (no dtype cast) so HWDGE accepts. Matmul consumes the
            # fp8 view (bit-level reinterpret, zero data copy).
            nisa.dma_copy(
                dst=down_full0_int8_bufs[k],
                src=down_w.ap(
            ...
            # Load down_w tile 1 (rows I0:I0+I1=64) for this shard — v15f HWDGE
            nisa.dma_copy(
                dst=down_full1_int8_bufs[k][0:I1, 0:H_shard],
                src=down_w.ap(
        ...
        for k in nl.affine_range(_K_WAVE):
            ...
            # Load down_w tile 0 for Wave 1 — v15f: int8 HWDGE
            nisa.dma_copy(
                dst=down_full0_int8_w1_bufs[k],
                src=down_w.ap(
                    pattern=[[H, I0], [1, H_shard]],
                    offset=prg_id * H_shard,
                    scalar_offset=expert_id_w1,
            ...
            # Load down_w tile 1 for Wave 1 — v15f: int8 HWDGE
            nisa.dma_copy(
                dst=down_full1_int8_w1_bufs[k][0:I1, 0:H_shard],
                src=down_w.ap(
                    pattern=[[H, I1], [1, H_shard]],
                    offset=I0 * H + prg_id * H_shard,
                    scalar_offset=expert_id_w1,
