...
v12m — Plan M: Preloaded expert scales (SBUF dynamic indexing NOT supported)
=============================================================================
Based on v12i (affine_range prefetch for tile-1 tensor_copies).

FINDING: The NKI compiler rejects nisa.tensor_copy with scalar_offset on SBUF
sources ("Dynamic Access is not allowed in the instruction"). SBUF-to-SBUF
copies with runtime expert_id as offset are not supported.

Root cause of why Plan M cannot fully work:
  - 16 indirect HBM scale DMAs (dge_mode=0) use GpSimdE to compute
    address = expert_id * stride at runtime.
  - Alternative: preload all E=128 experts' scales into SBUF, then use
    SBUF dynamic indexing to extract per-expert slice.
  - BLOCKED: NKI compiler forbids dynamic SBUF access with runtime offsets.
  - HBM direct DMA (dge_mode=3) requires statically-known offsets (no runtime
    expert_id can be used as offset).

This version (v12m) demonstrates the preload with direct DMA (which compiles
and runs correctly) but must still use indirect HBM DMAs for per-expert scale
extraction — providing no improvement over v12i, and adds small overhead from
2 extra direct DMA preloads (192KB gate_up + 512KB down = 704KB at 3TB/s ~234ns).

The preloaded SBUF buffers are allocated but their data is not used in the
compute path (the indirect DMA loads are still performed). This confirms
Plan M provides no benefit on current NKI/neuronx-cc toolchain.

Key changes vs v12i (for documentation):
  + Two direct DMA preloads added (all_gu_scales_sb, all_down_scales_sb)
  = Per-expert scale extraction still uses dge_mode=0 indirect DMAs (unchanged)
  - No reduction in GpSimdE descriptor overhead

Interface (same as v12i):
...
# Preloaded scale dims (attempted, but SBUF dynamic indexing blocked)
_GU_SCALES_FREE = _E * _GU_J_BLOCKS    # = 384 (all experts' gate_up scales)
_DOWN_SCALES_FREE = _E * _H_SHARD_BLOCKS  # = 1024 (all experts' down scales for one shard)
...
def qwen3_moe_fused_tkg(
    ...
    Plan M (v12m): Attempts to preload all expert scales via direct DMA into SBUF,
    but SBUF dynamic indexing with runtime expert_id is not allowed by the compiler.
    Falls back to v12i behavior for per-expert scale extraction.
    """
    B = inp.shape[0]
    T = B
    ...
    for t in nl.static_range(T):
        ...
        for k in nl.affine_range(_K_WAVE):
            ...
            # NOTE: dge_mode=0 (software GpSimdE) still required because expert_id is runtime.
            # Plan M attempted to use preloaded SBUF but SBUF dynamic indexing is not supported.
            nisa.dma_copy(
            ...
            # NOTE: dge_mode=0 still required (same reason as gate_up_scales above).
            nisa.dma_copy(
        ...
        # ------------------------------------------------------------------
        # Initialize output_temp to zero before Phase 2a loop.
        # ------------------------------------------------------------------
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # Fully batched output accumulation (Wave 1 always adds to running sum)
            nisa.tensor_tensor(
                dst=output_temp[0:_PMAX, 0:_H_FREE_SHARD],
                data1=output_temp[0:_PMAX, 0:_H_FREE_SHARD],
                data2=down_h_all[0:_PMAX, 0:_H_FREE_SHARD],
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v12m kernel (Plan M: attempted preloaded scales — same as v12i, documents finding).

    Accepts same interface as v12i:
