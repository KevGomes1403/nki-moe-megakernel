...
v12k — Plan L (Alternative 2): Split gate_up loading to eliminate tile-1 SBUF copies
======================================================================================
Based on v12i (affine_range prefetch for tile-1 tensor_copies).

Background:
  Plan L intended to use MXFP4 weights via nc_matmul_mx, but that instruction
  is NOT supported on trn2 (requires trn3+). Error from compiler:
  "MatmultMx is not supported on arch Trn2, must be Trn3 or greater"

  Similarly, perf_mode=double_row requires T≥2 (incompatible with TKG T=1).

Alternative 2 (this kernel):
  Instead of loading gate_up_fp8_bufs as a single [128P, H_free, GU_FLAT=384] tensor
  and then copying tile-1 slices into separate 128P-padded buffers (the affine_range
  prefetch loop in v12i), split the DMA into 4 targeted sub-loads per expert:

    gate0_fp8_bufs: [128P, H_free, I0=128] loaded from gate_up_w cols [0:I0]
    gate_t1_fp8_bufs: [128P, H_free, I0=128] loaded directly from cols [I0:I0+I1]
                      (only I1=64 rows filled; rows I1:I0 pre-zeroed)
    up0_fp8_bufs: [128P, H_free, I0=128] loaded from gate_up_w cols [I:I+I0]
    up_t1_fp8_bufs: [128P, H_free, I0=128] loaded from gate_up_w cols [I+I0:I+I0+I1]
                    (only I1=64 rows filled; rows I1:I0 pre-zeroed)

  Benefits:
  - Eliminates the affine_range prefetch loops (32 tensor_copy SBUF operations total
    across both waves) → frees scheduler for better DMA/compute overlap
  - tile-1 data goes directly from HBM to its usage buffer (no intermediate copy)

  Trade-offs:
  - DMA calls per expert: 2 (gate_up_full + scales) → 5 (gate0, gate_t1, up0, up_t1, scales)
  - GpSimdE descriptor generation: 5 → 8 per expert (×8 experts ×2 waves = 80→128)
  - SBUF bytes: slightly reduced (no separate gate_up_fp8_bufs needed)

  Same HBM bytes (384 cols = 128+64+128+64) so DMA bandwidth unchanged.

Key differences vs v12i:
  - 4 DMA sub-loads per expert (instead of 1 full load + SBUF copy)
  - No gate_up_fp8_bufs[k] (replaced by gate0_fp8_bufs[k] and up0_fp8_bufs[k])
  - No affine_range prefetch loop for tile-1 copies
  - gate_t1_fp8_bufs[k] and up_t1_fp8_bufs[k] loaded directly from HBM

Interface: Same as v12i (unchanged).
...
def qwen3_moe_fused_tkg(
    ...
    # -----------------------------------------------------------------------
    # v12k: output_temp is [128, H_free_shard=8] (T=1 always in TKG)
    output_temp = nl.ndarray((_PMAX, _H_FREE_SHARD), dtype=nl.float32, buffer=nl.sbuf)

    for t in nl.static_range(T):

        # ------------------------------------------------------------------
        # v12k: Split gate_up into separate tile-0 and tile-1 buffers.
        # Load tile-1 data directly from HBM instead of via tensor_copy.
        # This eliminates the affine_range prefetch loops for tile-1 copies.
        #
        # gate0_fp8_bufs[k]: [128P, H_free, I0=128] fp8 — gate tile-0 weights
        # gate_t1_fp8_bufs[k]: [128P, H_free, I0=128] fp8 — gate tile-1 (only I1=64 valid)
        # up0_fp8_bufs[k]: [128P, H_free, I0=128] fp8 — up tile-0 weights
        # up_t1_fp8_bufs[k]: [128P, H_free, I0=128] fp8 — up tile-1 (only I1=64 valid)
        # ------------------------------------------------------------------

        # Wave 0 — gate tile-0 buffers
        gate0_fp8_buf0 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate0_fp8_buf1 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate0_fp8_buf2 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate0_fp8_buf3 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate0_fp8_bufs = [gate0_fp8_buf0, gate0_fp8_buf1, gate0_fp8_buf2, gate0_fp8_buf3]

        # Wave 0 — gate tile-1 buffers (I0=128 allocated, only I1=64 rows filled)
        gate_t1_fp8_buf0 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate_t1_fp8_buf1 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate_t1_fp8_buf2 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate_t1_fp8_buf3 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate_t1_fp8_bufs = [gate_t1_fp8_buf0, gate_t1_fp8_buf1, gate_t1_fp8_buf2, gate_t1_fp8_buf3]

        # Wave 0 — up tile-0 buffers
        up0_fp8_buf0 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up0_fp8_buf1 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up0_fp8_buf2 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up0_fp8_buf3 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up0_fp8_bufs = [up0_fp8_buf0, up0_fp8_buf1, up0_fp8_buf2, up0_fp8_buf3]

        # Wave 0 — up tile-1 buffers (I0=128 allocated, only I1=64 rows filled)
        up_t1_fp8_buf0 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up_t1_fp8_buf1 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up_t1_fp8_buf2 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up_t1_fp8_buf3 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up_t1_fp8_bufs = [up_t1_fp8_buf0, up_t1_fp8_buf1, up_t1_fp8_buf2, up_t1_fp8_buf3]
        ...
        # ------------------------------------------------------------------
        # Wave 1 buffers (separate to avoid data hazards)
        # ------------------------------------------------------------------
        gate0_fp8_w1_buf0 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate0_fp8_w1_buf1 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate0_fp8_w1_buf2 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate0_fp8_w1_buf3 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate0_fp8_w1_bufs = [gate0_fp8_w1_buf0, gate0_fp8_w1_buf1, gate0_fp8_w1_buf2, gate0_fp8_w1_buf3]

        gate_t1_fp8_w1_buf0 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate_t1_fp8_w1_buf1 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate_t1_fp8_w1_buf2 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate_t1_fp8_w1_buf3 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate_t1_fp8_w1_bufs = [gate_t1_fp8_w1_buf0, gate_t1_fp8_w1_buf1, gate_t1_fp8_w1_buf2, gate_t1_fp8_w1_buf3]

        up0_fp8_w1_buf0 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up0_fp8_w1_buf1 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up0_fp8_w1_buf2 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up0_fp8_w1_buf3 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up0_fp8_w1_bufs = [up0_fp8_w1_buf0, up0_fp8_w1_buf1, up0_fp8_w1_buf2, up0_fp8_w1_buf3]

        up_t1_fp8_w1_buf0 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up_t1_fp8_w1_buf1 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up_t1_fp8_w1_buf2 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up_t1_fp8_w1_buf3 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up_t1_fp8_w1_bufs = [up_t1_fp8_w1_buf0, up_t1_fp8_w1_buf1, up_t1_fp8_w1_buf2, up_t1_fp8_w1_buf3]
        ...
        # v12k: Zero-pad tile-1 buffers for gate and up (both waves)
        # Only rows I1:I0 need zeroing since DMA only fills rows 0:I1
        for k_pad in nl.static_range(4):
            nisa.memset(gate_t1_fp8_bufs[k_pad][0:_PMAX, 0:H_free, nl.ds(I1, I1)], value=0.0)
            nisa.memset(up_t1_fp8_bufs[k_pad][0:_PMAX, 0:H_free, nl.ds(I1, I1)],   value=0.0)

        for k_pad in nl.static_range(4):
            nisa.memset(gate_t1_fp8_w1_bufs[k_pad][0:_PMAX, 0:H_free, nl.ds(I1, I1)], value=0.0)
            nisa.memset(up_t1_fp8_w1_bufs[k_pad][0:_PMAX, 0:H_free, nl.ds(I1, I1)],   value=0.0)
        ...
        # ==================================================================
        # Phase 1a: Load Wave 0 experts — split into 4 sub-loads per expert
        # (gate0, gate_t1, up0, up_t1) + scales + down
        # affine_range allows pipelining across k
        # ==================================================================
        for k in nl.affine_range(_K_WAVE):
            expert_id = eid_all_bufs[k].ap(pattern=[[1, 1], [1, 1]], offset=0)

            # Load gate tile-0: gate_up_w cols [0:I0=128]
            # Pattern: stride=GU_FLAT between partitions (full row stride),
            #          but only I0 elements per row
            nisa.dma_copy(
                dst=gate0_fp8_bufs[k],
                src=gate_up_w.ap(
                    pattern=[[_GU_FLAT, _PMAX], [_PMAX * _GU_FLAT, H_free], [1, I0]],
                    offset=0,
            ...
            # Load gate tile-1: gate_up_w cols [I0:I0+I1=192]
            # Only I1=64 elements per row; rows I1:I0 pre-zeroed
            nisa.dma_copy(
                dst=gate_t1_fp8_bufs[k][0:_PMAX, 0:H_free, 0:I1],
                src=gate_up_w.ap(
                    pattern=[[_GU_FLAT, _PMAX], [_PMAX * _GU_FLAT, H_free], [1, I1]],
                    offset=I0,
                    scalar_offset=expert_id,
                    indirect_dim=0,
                    dtype=nl.float8_e4m3,
            ...
            # Load up tile-0: gate_up_w cols [I:I+I0=192:320]
            nisa.dma_copy(
                dst=up0_fp8_bufs[k],
                src=gate_up_w.ap(
                    pattern=[[_GU_FLAT, _PMAX], [_PMAX * _GU_FLAT, H_free], [1, I0]],
            ...
            # Load up tile-1: gate_up_w cols [I+I0:I+I0+I1=320:384]
            # Only I1=64 elements per row; rows I1:I0 pre-zeroed
            nisa.dma_copy(
                dst=up_t1_fp8_bufs[k][0:_PMAX, 0:H_free, 0:I1],
                src=gate_up_w.ap(
                    pattern=[[_GU_FLAT, _PMAX], [_PMAX * _GU_FLAT, H_free], [1, I1]],
                    offset=I + I0,
                    scalar_offset=expert_id,
        ...
        # ==================================================================
        # Phase 1b: Load Wave 1 experts — split loads (same pattern)
        # ==================================================================
        for k in nl.affine_range(_K_WAVE):
            kk = k + 4
            expert_id_w1 = eid_all_bufs[kk].ap(pattern=[[1, 1], [1, 1]], offset=0)

            # Load gate tile-0 for Wave 1
            nisa.dma_copy(
                dst=gate0_fp8_w1_bufs[k],
                src=gate_up_w.ap(
                    pattern=[[_GU_FLAT, _PMAX], [_PMAX * _GU_FLAT, H_free], [1, I0]],
                    offset=0,
            ...
            # Load gate tile-1 for Wave 1
            nisa.dma_copy(
                dst=gate_t1_fp8_w1_bufs[k][0:_PMAX, 0:H_free, 0:I1],
                src=gate_up_w.ap(
                    pattern=[[_GU_FLAT, _PMAX], [_PMAX * _GU_FLAT, H_free], [1, I1]],
                    offset=I0,
                    scalar_offset=expert_id_w1,
            ...
            # Load up tile-0 for Wave 1
            nisa.dma_copy(
                dst=up0_fp8_w1_bufs[k],
                src=gate_up_w.ap(
                    pattern=[[_GU_FLAT, _PMAX], [_PMAX * _GU_FLAT, H_free], [1, I0]],
                    offset=I,
                    scalar_offset=expert_id_w1,
            ...
            # Load up tile-1 for Wave 1
            nisa.dma_copy(
                dst=up_t1_fp8_w1_bufs[k][0:_PMAX, 0:H_free, 0:I1],
                src=gate_up_w.ap(
                    pattern=[[_GU_FLAT, _PMAX], [_PMAX * _GU_FLAT, H_free], [1, I1]],
                    offset=I + I0,
                    scalar_offset=expert_id_w1,
        ...
        # ------------------------------------------------------------------
        # v12k: Initialize output_temp to zero before Phase 2a loop.
        # ------------------------------------------------------------------
        nisa.memset(output_temp, value=0.0)

        # Phase 2a: Compute experts 0-3
        # v12k: Use gate0_fp8_bufs/up0_fp8_bufs for tile-0
        #       Use gate_t1_fp8_bufs/up_t1_fp8_bufs for tile-1 (directly loaded)
        for k in nl.static_range(_K_WAVE):
            gu_base = k * 2 * I_tiles
            d_base  = k * H_free_shard

            # Gate/Up matmul — using directly-loaded tile buffers
            for h1 in nl.affine_range(H_free):
                for i_tile in nl.static_range(I_tiles):
                    if i_tile == 0:
                        g_stat = gate0_fp8_bufs[k][0:_PMAX, h1, 0:I0]
                        u_stat = up0_fp8_bufs[k][0:_PMAX, h1, 0:I0]
                    else:
                        g_stat = gate_t1_fp8_bufs[k][0:_PMAX, h1, 0:I0]
                        u_stat = up_t1_fp8_bufs[k][0:_PMAX, h1, 0:I0]
                    nisa.nc_matmul(
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            for h1 in nl.affine_range(H_free):
                for i_tile in nl.static_range(I_tiles):
                    if i_tile == 0:
                        g_stat = gate0_fp8_w1_bufs[k][0:_PMAX, h1, 0:I0]
                        u_stat = up0_fp8_w1_bufs[k][0:_PMAX, h1, 0:I0]
                    else:
                        g_stat = gate_t1_fp8_w1_bufs[k][0:_PMAX, h1, 0:I0]
                        u_stat = up_t1_fp8_w1_bufs[k][0:_PMAX, h1, 0:I0]
                    nisa.nc_matmul(
                        dst=gate_up_psum[0:_PMAX, gu_base + i_tile:gu_base + i_tile + 1],
                        stationary=g_stat,
                        moving=rmsnorm_normed_bf16[0:_PMAX, nl.ds(h1 * T, T)],
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v12k kernel (Plan L Alternative 2: split gate_up loading to eliminate tile-1 SBUF copies).
