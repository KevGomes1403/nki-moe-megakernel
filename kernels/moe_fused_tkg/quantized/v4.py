...
v4 — Plan C: Batch FP8→BF16 Casts (Fewer Scalar Engine Ops)
=============================================================
Based on v3 (Plan A: Per-Output-Neuron FP8 + Post-Matmul Scale).

Key changes vs v3:
  V4.1: Replace per-tile dequant activation loops with batch (per-h1) activations.
        gate_up dequant: was H_free*GU_FLAT_BLOCKS=48 calls of [128P,128F]
                         now H_free=16 calls of [128P,384F] — 3x fewer for gate_up
        down_full0 dequant: was H_BLOCKS=2 calls of [128P,128F]
                            now 1 call of [128P,256F]
        down_full1 dequant: was H_BLOCKS=2 calls of [64P,128F]
                            now 1 call of [64P,256F]
        Total per expert: 18 activation calls vs v3's 52 (2.9x reduction)
  V4.2: Everything else identical to v3 (bf16 matmul, post-matmul scale, etc.)
...
dramatically fewer scalar engine calls by batching the activation casts.
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        # ------------------------------------------------------------------
        for k in nl.static_range(_K_WAVE):
            # gate_up: one activation per h1 row, covering full GU_FLAT=384 free dim
            # Instead of 3 calls of [128P,128F] per h1 → 1 call of [128P,384F] per h1
            for h1 in nl.static_range(H_free):
                nisa.activation(
                    gate_up_bufs[k][0:_PMAX, h1, 0:_GU_FLAT],
                    op=nl.copy,
                    data=gate_up_fp8_bufs[k][0:_PMAX, h1, 0:_GU_FLAT],
                )
        ...
        # v4 Plan C: Batch dequantize wave 1 weights
        for k in nl.static_range(_K_WAVE):
            # gate_up: one activation per h1, covering GU_FLAT=384 free dim
            for h1 in nl.static_range(H_free):
                nisa.activation(
                    gate_up_bufs[k][0:_PMAX, h1, 0:_GU_FLAT],
                    op=nl.copy,
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v4 kernel with batch fp8→bf16 casts (Plan C).
