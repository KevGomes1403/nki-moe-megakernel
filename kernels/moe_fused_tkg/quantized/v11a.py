...
v11a — Round 3 Plan A: Pre-read all expert IDs + affine_range for weight load loops
====================================================================================
Based on v10a (double-buffer Wave 1 DMA loads).

Key changes vs v10a:
  V11a: Allocate 8 separate eid_scratch buffers (eid_all_bufs[0..7]).
        Pre-read all 8 expert IDs with static_range BEFORE any weight loads
        (8 tiny 4-byte DMAs, complete nearly instantly).
        Change Wave 0 and Wave 1 weight load loops from nl.static_range to
        nl.affine_range — each k-iteration references only eid_all_bufs[k]
        or eid_all_bufs[k+4], making iterations truly independent, allowing
        the compiler to pipeline GpSimdE DMA issuance across k-iterations.
        Remove old per-iteration eid_scratch / eid_scratch_w1_bufs.
        ...
        Restructured order:
          ...
          Phase 2a: Compute Wave 0
          PSUM memset
          Phase 2b: Compute Wave 1
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # Output accumulation per-column
        ...
        for k in nl.static_range(_K_WAVE):
            ...
            # Output accumulation per-column (Wave 1 always adds — Wave 0 already wrote)
            for h1_out in nl.static_range(H_free_shard):
                nisa.tensor_tensor(
                    dst=output_temp[0:_PMAX, h1_out:h1_out + 1, t:t + 1],
                    data1=output_temp[0:_PMAX, h1_out:h1_out + 1, t:t + 1],
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v11a kernel (Round 3 Plan A: pre-read all expert IDs + affine_range for weight loads).
