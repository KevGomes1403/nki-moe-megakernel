...
from kernels.moe_fused_tkg.quantized.v10c import run as run_v10c
...
print("Running v10c...")
v10c_result = run_v10c(inp, gamma, router_w, gate_up_w_dev, gate_up_scales_dev, down_w_dev, down_scales_dev)
xm.mark_step()
v10c_np = v10c_result.cpu().float().numpy()

max_diff = np.abs(v10c_np - v8_np).max()
print(f"max_diff = {max_diff:.4e}")

try:
    np.testing.assert_allclose(v10c_np, v8_np, rtol=1e-2, atol=1e-2)
