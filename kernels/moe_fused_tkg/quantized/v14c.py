...
v14c — Plan C: Single 8-Expert Wave
=====================================
Based on v14a (trn3 port of v12i).

Key changes vs v14a:
  v14c: Restructures the 2-wave (K_WAVE=4 × 2 passes) expert loop into a
  single 8-expert wave to:
  1. Eliminate 2 inter-wave PSUM memsets
  2. Widen the affine_range window from k=0..3 to k=0..7 — the compiler can
     see dependencies across all 8 experts simultaneously, potentially
     improving DMA/compute pipelining.

  Buffer renaming:
  - gate_up_fp8_bufs[0..7] (was gate_up_fp8_bufs[0..3] + gate_up_fp8_w1_bufs[0..3])
  - gate_up_scale_bufs[0..7], up_scale_i0_bufs[0..7], up_scale_i1_bufs[0..7]
  - down_full0_fp8_bufs[0..7], down_full1_fp8_bufs[0..7], down_scale_bufs[0..7]
  - gate_t1_128_bufs[0..7], up_t1_128_bufs[0..7]

  PSUM expanded:
  - gate_up_psum: [128, 8*2*2=32] float32 = 16 KB
  - down_psum:    [128, 8*8=64]  float32 = 32 KB
  - Single memset for each, no inter-wave reset.

Order:
  Pre-read 8 eid_all_bufs[0..7]
  Phase 1: Load all 8 expert weights (affine_range, k=0..7)
  Compute norm_weights/aff_bcast
  Assemble up_scale bufs (affine_range, k=0..7)
  Prefetch (affine_range, k=0..7): fill gate_t1_128[k] and up_t1_128[k]
  memset output_temp
  Phase 2: Compute all 8 experts (single affine_range, k=0..7, always-add)
...
# Single wave: all 8 experts in one pass
_K_ALL = 8
...
def qwen3_moe_fused_tkg(
    ...
    # -----------------------------------------------------------------------
    # Stage 4: Selective-Expert MLP — Single 8-Expert Wave
    # -----------------------------------------------------------------------
    # output_temp is [128, H_free_shard=8] (T=1 always in TKG)
    output_temp = nl.ndarray((_PMAX, _H_FREE_SHARD), dtype=nl.float32, buffer=nl.sbuf)

    for t in nl.affine_range(T):

        # ------------------------------------------------------------------
        # Allocate fp8 weight buffers — unified 8-expert (was 4+4 in two waves)
        # ------------------------------------------------------------------
        ...
        gate_up_fp8_buf4 = nl.ndarray((_PMAX, H_free, _GU_FLAT), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate_up_fp8_buf5 = nl.ndarray((_PMAX, H_free, _GU_FLAT), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate_up_fp8_buf6 = nl.ndarray((_PMAX, H_free, _GU_FLAT), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate_up_fp8_buf7 = nl.ndarray((_PMAX, H_free, _GU_FLAT), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate_up_fp8_bufs = [gate_up_fp8_buf0, gate_up_fp8_buf1, gate_up_fp8_buf2, gate_up_fp8_buf3,
                            gate_up_fp8_buf4, gate_up_fp8_buf5, gate_up_fp8_buf6, gate_up_fp8_buf7]
        ...
        gate_up_scale_buf4 = nl.ndarray((_PMAX, _GU_J_BLOCKS), dtype=nl.float32, buffer=nl.sbuf)
        gate_up_scale_buf5 = nl.ndarray((_PMAX, _GU_J_BLOCKS), dtype=nl.float32, buffer=nl.sbuf)
        gate_up_scale_buf6 = nl.ndarray((_PMAX, _GU_J_BLOCKS), dtype=nl.float32, buffer=nl.sbuf)
        gate_up_scale_buf7 = nl.ndarray((_PMAX, _GU_J_BLOCKS), dtype=nl.float32, buffer=nl.sbuf)
        gate_up_scale_bufs = [gate_up_scale_buf0, gate_up_scale_buf1, gate_up_scale_buf2, gate_up_scale_buf3,
                              gate_up_scale_buf4, gate_up_scale_buf5, gate_up_scale_buf6, gate_up_scale_buf7]
        ...
        up_scale_i0_buf4 = nl.ndarray((_PMAX, 1), dtype=nl.float32, buffer=nl.sbuf)
        up_scale_i0_buf5 = nl.ndarray((_PMAX, 1), dtype=nl.float32, buffer=nl.sbuf)
        up_scale_i0_buf6 = nl.ndarray((_PMAX, 1), dtype=nl.float32, buffer=nl.sbuf)
        up_scale_i0_buf7 = nl.ndarray((_PMAX, 1), dtype=nl.float32, buffer=nl.sbuf)
        up_scale_i0_bufs = [up_scale_i0_buf0, up_scale_i0_buf1, up_scale_i0_buf2, up_scale_i0_buf3,
                            up_scale_i0_buf4, up_scale_i0_buf5, up_scale_i0_buf6, up_scale_i0_buf7]
        ...
        up_scale_i1_buf4 = nl.ndarray((_PMAX, 1), dtype=nl.float32, buffer=nl.sbuf)
        up_scale_i1_buf5 = nl.ndarray((_PMAX, 1), dtype=nl.float32, buffer=nl.sbuf)
        up_scale_i1_buf6 = nl.ndarray((_PMAX, 1), dtype=nl.float32, buffer=nl.sbuf)
        up_scale_i1_buf7 = nl.ndarray((_PMAX, 1), dtype=nl.float32, buffer=nl.sbuf)
        up_scale_i1_bufs = [up_scale_i1_buf0, up_scale_i1_buf1, up_scale_i1_buf2, up_scale_i1_buf3,
                            up_scale_i1_buf4, up_scale_i1_buf5, up_scale_i1_buf6, up_scale_i1_buf7]
        ...
        down_full0_fp8_buf4 = nl.ndarray((_PMAX, H_shard), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        down_full0_fp8_buf5 = nl.ndarray((_PMAX, H_shard), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        down_full0_fp8_buf6 = nl.ndarray((_PMAX, H_shard), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        down_full0_fp8_buf7 = nl.ndarray((_PMAX, H_shard), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        down_full0_fp8_bufs = [down_full0_fp8_buf0, down_full0_fp8_buf1, down_full0_fp8_buf2, down_full0_fp8_buf3,
                               down_full0_fp8_buf4, down_full0_fp8_buf5, down_full0_fp8_buf6, down_full0_fp8_buf7]
        ...
        down_full1_fp8_buf4 = nl.ndarray((_PMAX, H_shard), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        down_full1_fp8_buf5 = nl.ndarray((_PMAX, H_shard), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        down_full1_fp8_buf6 = nl.ndarray((_PMAX, H_shard), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        down_full1_fp8_buf7 = nl.ndarray((_PMAX, H_shard), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        down_full1_fp8_bufs = [down_full1_fp8_buf0, down_full1_fp8_buf1, down_full1_fp8_buf2, down_full1_fp8_buf3,
                               down_full1_fp8_buf4, down_full1_fp8_buf5, down_full1_fp8_buf6, down_full1_fp8_buf7]
        ...
        down_scale_buf4 = nl.ndarray((_PMAX, _H_SHARD_BLOCKS), dtype=nl.float32, buffer=nl.sbuf)
        down_scale_buf5 = nl.ndarray((_PMAX, _H_SHARD_BLOCKS), dtype=nl.float32, buffer=nl.sbuf)
        down_scale_buf6 = nl.ndarray((_PMAX, _H_SHARD_BLOCKS), dtype=nl.float32, buffer=nl.sbuf)
        down_scale_buf7 = nl.ndarray((_PMAX, _H_SHARD_BLOCKS), dtype=nl.float32, buffer=nl.sbuf)
        down_scale_bufs = [down_scale_buf0, down_scale_buf1, down_scale_buf2, down_scale_buf3,
                          down_scale_buf4, down_scale_buf5, down_scale_buf6, down_scale_buf7]

        # ------------------------------------------------------------------
        # Pre-allocate all 8 expert ID scratch buffers (unchanged)
        # ------------------------------------------------------------------
        ...
        # Zero-pad down_full1 rows I1:I0 (fp8 buffers) — all 8 experts
        for k_pad in nl.affine_range(_K_ALL):
            nisa.memset(down_full1_fp8_bufs[k_pad][nl.ds(I1, I1), 0:H_shard], value=0.0)

        # ------------------------------------------------------------------
        # gate_t1_128 and up_t1_128 with k-dimension for affine_range prefetch
        # Unified 8 experts: [_K_ALL=8, _PMAX=128, H_free=16, I0=128]
        # SBUF cost: 2 * 8 * 128 * 16 * 128 = 4 MB (fp8) — same total as v14a 4+4
        # ------------------------------------------------------------------
        ...
        gate_t1_128_k4 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate_t1_128_k5 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate_t1_128_k6 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate_t1_128_k7 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        gate_t1_128_bufs = [gate_t1_128_k0, gate_t1_128_k1, gate_t1_128_k2, gate_t1_128_k3,
                            gate_t1_128_k4, gate_t1_128_k5, gate_t1_128_k6, gate_t1_128_k7]
        ...
        up_t1_128_k4 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up_t1_128_k5 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up_t1_128_k6 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up_t1_128_k7 = nl.ndarray((_PMAX, H_free, I0), dtype=nl.float8_e4m3, buffer=nl.sbuf)
        up_t1_128_bufs = [up_t1_128_k0, up_t1_128_k1, up_t1_128_k2, up_t1_128_k3,
                          up_t1_128_k4, up_t1_128_k5, up_t1_128_k6, up_t1_128_k7]

        # Zero-pad tile-1 I1..I0 region — all 8 experts unified
        for k_pad in nl.affine_range(_K_ALL):
        ...
        # ==================================================================
        # Phase 1: Load all 8 experts — single affine_range(8)
        # (was: Phase 1a for k=0..3, Phase 1b for k=0..3 with kk=k+4)
        # Compiler can pipeline all 8 expert DMAs simultaneously.
        # ==================================================================
        for k in nl.affine_range(_K_ALL):
        ...
        # Assemble up scale rearranged buffers — all 8 experts (single affine_range)
        for k in nl.affine_range(_K_ALL):
            nisa.tensor_copy(
        ...
        # Expand PSUM to hold all 8 experts in a single wave
        # gate_up_psum: [128, 8*2*2=32] float32 = 16 KB
        # down_psum:    [128, 8*8=64]   float32 = 32 KB
        # Total = 48 KB — well within 2 MB PSUM limit
        gate_up_psum = nl.ndarray((_PMAX, _K_ALL * 2 * I_tiles), dtype=nl.float32, buffer=nl.psum)
        down_psum    = nl.ndarray((_PMAX, _K_ALL * H_free_shard), dtype=nl.float32, buffer=nl.psum)
        nisa.memset(gate_up_psum, value=0.0)  # single memset (was 2x in v14a)
        nisa.memset(down_psum, value=0.0)     # single memset (was 2x in v14a)

        # ==================================================================
        # Plan C: single affine_range prefetch loop for all 8 experts
        # (was: separate Wave 0 loop k=0..3, Wave 1 loop k=0..3)
        # ==================================================================
        for k in nl.affine_range(_K_ALL):
            nisa.tensor_copy(
        ...
        # ------------------------------------------------------------------
        # Initialize output_temp to zero before single unified compute loop.
        # ------------------------------------------------------------------
        nisa.memset(output_temp, value=0.0)

        # ==================================================================
        # Phase 2: Compute all 8 experts — single affine_range(8)
        # (was: Phase 2a k=0..3 + PSUM memset + Phase 2b k=0..3)
        # ==================================================================
        for k in nl.affine_range(_K_ALL):
            gu_base = k * 2 * I_tiles
            ...
            # Batch combined_down_scale precompute (k goes 0..7, covers all 8 experts)
            ...
            # Always-add to output_temp (initialized to zero before loop)
            nisa.tensor_tensor(
    ...
    # output_temp is [128, H_free_shard] — use [0:_PMAX, h1:h1+1] for [128,1] slice
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v14c kernel (Plan C: single 8-expert wave).
