...
v12h — Plan H: Double PSUM, eliminate between-wave memset
==========================================================
Based on v11c (affine_range expert loads + output_temp batching).

Key changes vs v11c:
  V12h: Allocate PSUM large enough for all 8 experts simultaneously.
       - gate_up_psum: [128P, 2*K_WAVE*2*I_tiles=32F] (was [128P, K_WAVE*2*I_tiles=16F])
       - down_psum:    [128P, 2*K_WAVE*H_free_shard=64F] (was [128P, K_WAVE*H_free_shard=32F])
       - Wave 0 (Phase 2a) uses cols 0:16 / 0:32 (same as before)
       - Wave 1 (Phase 2b) uses cols 16:32 / 32:64 (offset by K_WAVE stride)
       - The two nisa.memset calls between Phase 2a and Phase 2b are removed.
       - A single pair of memset calls at PSUM allocation covers all 8 experts.
       This eliminates the serialization barrier between Phase 2a and Phase 2b,
       allowing the compiler to freely schedule Wave 1 compute.

        Order (inherited from v11c):
          ...
          Phase 2a: Compute Wave 0  (batched output accumulation, cols 0:16/0:32)
          Phase 2b: Compute Wave 1  (batched output accumulation, cols 16:32/32:64)
          [No PSUM memset between 2a and 2b]
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        # ==================================================================
        # Plan H: Allocate PSUM for ALL 8 experts (doubled free dimension).
        # Wave 0 uses cols 0:(_K_WAVE*2*I_tiles) for gate_up and 0:(_K_WAVE*H_free_shard) for down.
        # Wave 1 uses cols (_K_WAVE*2*I_tiles):(2*_K_WAVE*2*I_tiles) and
        #                   (_K_WAVE*H_free_shard):(2*_K_WAVE*H_free_shard).
        # Only ONE pair of memsets needed — at the top — no between-wave serialization.
        # gate_up_psum: [128P, 32F]  (was 16F)
        # down_psum:    [128P, 64F]  (was 32F)
        # Both are within the 512F PSUM free-dimension limit.
        # ==================================================================
        gate_up_psum = nl.ndarray((_PMAX, 2 * _K_WAVE * 2 * I_tiles), dtype=nl.float32, buffer=nl.psum)
        down_psum    = nl.ndarray((_PMAX, 2 * _K_WAVE * H_free_shard), dtype=nl.float32, buffer=nl.psum)
        ...
        # Wave 0 uses PSUM cols 0:(_K_WAVE*2*I_tiles) / 0:(_K_WAVE*H_free_shard) — same as v11c.
        for k in nl.static_range(_K_WAVE):
            gu_base = k * 2 * I_tiles
        ...
        # Plan H: NO PSUM memset here — Wave 1 uses different PSUM columns.
        # Wave 1 gate_up uses cols (_K_WAVE*2*I_tiles) : (2*_K_WAVE*2*I_tiles)
        # Wave 1 down uses cols (_K_WAVE*H_free_shard) : (2*_K_WAVE*H_free_shard)
        # ==================================================================

        # Phase 2b: Compute experts 4-7 (using w1 buffers)
        for k in nl.static_range(_K_WAVE):
            kk = k + 4
            # Plan H: Wave 1 uses upper half of the doubled PSUM
            gu_base_w1 = (_K_WAVE + k) * 2 * I_tiles   # cols 16, 20, 24, 28
            d_base_w1  = (_K_WAVE + k) * H_free_shard  # cols 32, 40, 48, 56
            ...
            for h1 in nl.affine_range(H_free):
                for i_tile in nl.static_range(I_tiles):
                    if i_tile == 0:
                        ...
                        u_stat = up_t1_128[0:_PMAX, h1, 0:I0]
                    nisa.nc_matmul(
                        dst=gate_up_psum[0:_PMAX, gu_base_w1 + i_tile:gu_base_w1 + i_tile + 1],
                        stationary=g_stat,
                        moving=rmsnorm_normed_bf16[0:_PMAX, nl.ds(h1 * T, T)],
                    )
                    nisa.nc_matmul(
                        dst=gate_up_psum[0:_PMAX, gu_base_w1 + I_tiles + i_tile:gu_base_w1 + I_tiles + i_tile + 1],
                        stationary=u_stat,
                        moving=rmsnorm_normed_bf16[0:_PMAX, nl.ds(h1 * T, T)],
                    )
            ...
            for i_tile in nl.static_range(I_tiles):
                ...
                nisa.activation(
                    gate_silu,
                    op=nl.silu,
                    data=gate_up_psum[0:_PMAX, gu_base_w1 + i_tile:gu_base_w1 + i_tile + 1],
                    scale=gate_scale_col,
                    bias=zero_bias,
                )
                ...
                nisa.activation(
                    up_f32_scaled,
                    op=nl.copy,
                    data=gate_up_psum[0:_PMAX, gu_base_w1 + I_tiles + i_tile:gu_base_w1 + I_tiles + i_tile + 1],
                    scale=up_scale_col,
                    bias=zero_bias,
                )
            ...
            # Down matmul (fp8 stationary) — using w1 buffers, writing to Wave 1 PSUM cols
            for h1_out in nl.affine_range(H_free_shard):
                nisa.nc_matmul(
                    dst=down_psum[0:_PMAX, d_base_w1 + h1_out:d_base_w1 + h1_out + 1],
            ...
            # Batch PSUM drain — single activation([128, H_free_shard]) from Wave 1 cols
            down_h_raw = nl.ndarray((_PMAX, _H_FREE_SHARD), dtype=nl.float32, buffer=nl.sbuf)
            nisa.activation(
                down_h_raw,
                op=nl.copy,
                data=down_psum[0:_PMAX, d_base_w1:d_base_w1 + _H_FREE_SHARD],
            )
...
def run(inp, gamma, router_w, gate_up_w, gate_up_scales, down_w, down_scales):
    """Run v12h kernel (Plan H: Double PSUM, eliminate between-wave memset).
