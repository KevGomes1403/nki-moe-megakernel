...
from kernels.moe_fused_tkg.quantized.v11b import run as run_v11b
...
print("Running v11b...")
v11b_result = run_v11b(inp, gamma, router_w, gate_up_w_dev, gate_up_scales_dev, down_w_dev, down_scales_dev)
xm.mark_step()
v11b_np = v11b_result.cpu().float().numpy()

max_diff = np.abs(v11b_np - v8_np).max()
print(f"max_diff = {max_diff:.4e}")

try:
    np.testing.assert_allclose(v11b_np, v8_np, rtol=1e-2, atol=1e-2)
