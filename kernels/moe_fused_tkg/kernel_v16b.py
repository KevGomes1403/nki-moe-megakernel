...
kernel_v16b — Plan B: per-h-tile 2D DMA in h1 loop
====================================================================
...
DMA coalescing strategy:
  gate_up:  H_free=16 individual 2D indirect DMAs per expert, one per h-tile, issued
              INSIDE the h1 loop. Each DMA loads one h-tile [_PMAX=128, _GU_FLAT=384]
              with a simpler 2D pattern [[_GU_FLAT, _PMAX], [1, _GU_FLAT]], eliminating
              the 3D DMA decomposition overhead of v15.
            The offset h1 * _PMAX * _GU_FLAT selects the correct H-block within the expert.
            DMA for h1+1 can overlap with matmul for h1 via nl.affine_range(H_free).
            Tile-1 buffers gate_t1/up_t1 are [_PMAX, I0] (per h-tile), smaller than v15's
              [_PMAX, H_free, I0], reducing SBUF pressure.
...
SBUF budget estimate (per partition lane, 224 KiB limit):
  ...
  gate_up_htile:      [_PMAX, 384]    bf16  =  96 KB  (per h-tile, reused each h1)
  gate_t1:            [_PMAX, 128]    bf16  =  32 KB  (per h-tile, reused each h1)
  up_t1:              [_PMAX, 128]    bf16  =  32 KB  (per h-tile, reused each h1)
  down_full0:         [_PMAX, 2048]   bf16  =   4 KB  (per expert, reused)
  down_full1:         [_PMAX, 2048]   bf16  =   4 KB  (per expert, reused)
...
def qwen3_moe_fused_tkg(
    ...
    #   DMA: H_free=16 individual 2D indirect DMAs per expert, one per h-tile.
    #   Each DMA loads [_PMAX=128, _GU_FLAT=384] with pattern [[_GU_FLAT, _PMAX], [1, _GU_FLAT]].
    #   Offset h1 * _PMAX * _GU_FLAT selects the H-block within the expert.
    #   tile 0 (K=128): gate cols 0:128, up cols 192:320 — taken directly from htile.
    #   tile 1 (K=128): gate cols 128:192 (64 valid), up cols 320:384 (64 valid),
    #     each zero-padded to 128 cols via SBUF tensor_copy into gate_t1/up_t1.
    #
    ...
    for t in nl.static_range(T):
        ...
        for k in nl.affine_range(K):
            ...
            # Same coalesced pattern; only 64 rows loaded; rows 64:128 zeroed first.
            # offset = I0 * H = 128 * 2048 = 262144 (skip past tile-0 I rows)
            # ----------------------------------------------------------------
            down_full1 = nl.ndarray((_PMAX, H), dtype=down_w.dtype, buffer=nl.sbuf)
            # Zero rows I1:I0 = 64:128 (the pad region; rows 0:64 are overwritten by DMA)
            ...
            # Each h1 loop iteration:
            #   1. Issues a 2D indirect DMA for h1-th h-tile [_PMAX, _GU_FLAT=384]
            #   2. Prepares gate_t1 and up_t1 per-h-tile [_PMAX, I0] zero-padded buffers
            #   3. Runs 4 nc_matmul calls accumulating into gate_up_psum
            ...
            for h1 in nl.affine_range(H_free):
                # 2D indirect DMA: load one h-tile [_PMAX, _GU_FLAT=384] for this h1
                # Pattern [[_GU_FLAT, _PMAX], [1, _GU_FLAT]]: 2D, simpler than v15's 3D.
                # Offset h1 * _PMAX * _GU_FLAT selects the h1-th H-block within the expert.
                gate_up_htile = nl.ndarray((_PMAX, _GU_FLAT), dtype=gate_up_w.dtype, buffer=nl.sbuf)
                nisa.dma_copy(
                    dst=gate_up_htile,
                    src=gate_up_w.ap(
                        pattern=[[_GU_FLAT, _PMAX], [1, _GU_FLAT]],
                        offset=h1 * _PMAX * _GU_FLAT,
                        scalar_offset=expert_id,
                        indirect_dim=0,
                    ),
                    dge_mode=0,
                ...
                # Tile-0: direct slices from htile (no copy needed)
                g_stat_t0 = gate_up_htile[0:_PMAX, nl.ds(0, I0)]      # gate cols 0:128
                u_stat_t0 = gate_up_htile[0:_PMAX, nl.ds(I, I0)]       # up cols 192:320

                # Tile-1: copy valid half into pre-zeroed [_PMAX, I0] buffer
                # gate_t1 and up_t1 are [_PMAX, I0] — smaller than v15's [_PMAX, H_free, I0]
                gate_t1 = nl.ndarray((_PMAX, I0), dtype=gate_up_w.dtype, buffer=nl.sbuf)
                up_t1   = nl.ndarray((_PMAX, I0), dtype=gate_up_w.dtype, buffer=nl.sbuf)
                # Zero the pad half (cols I1:I0 = 64:128)
                nisa.memset(gate_t1[0:_PMAX, nl.ds(I1, I1)], value=0.0)
                nisa.memset(up_t1[0:_PMAX, nl.ds(I1, I1)],   value=0.0)
                # Copy valid half from htile
                # gate tile 1: cols I0:I = 128:192 in the flat combined buffer
                nisa.tensor_copy(dst=gate_t1[0:_PMAX, 0:I1], src=gate_up_htile[0:_PMAX, nl.ds(I0, I1)])
                # up tile 1: cols I+I0:I+I = 320:384 in the flat combined buffer
                nisa.tensor_copy(dst=up_t1[0:_PMAX, 0:I1],   src=gate_up_htile[0:_PMAX, nl.ds(I + I0, I1)])

                # Gate/up matmul accumulation
                moving = rmsnorm_normed_bf16[0:_PMAX, nl.ds(h1 * T, T)]
                nisa.nc_matmul(
                    dst=gate_up_psum[0:_PMAX, 0:1],
                    stationary=g_stat_t0,
                    moving=moving,
                )
                nisa.nc_matmul(
                    dst=gate_up_psum[0:_PMAX, 1:2],
                    stationary=gate_t1[0:_PMAX, 0:I0],
                    moving=moving,
                )
                nisa.nc_matmul(
                    dst=gate_up_psum[0:_PMAX, 2:3],
                    stationary=u_stat_t0,
                    moving=moving,
                )
                nisa.nc_matmul(
                    dst=gate_up_psum[0:_PMAX, 3:4],
                    stationary=up_t1[0:_PMAX, 0:I0],
                    moving=moving,
                )
            ...
            for h1_out in nl.affine_range(H_free_shard):
                ...
                # Tile 1: I rows 128:192 (64 valid, rows 64:128 zero from memset)
                nisa.nc_matmul(
                    dst=down_psum[0:_PMAX, h1_out:h1_out + 1],
...
def run(inp, gamma, router_w, gate_up_w, down_w):
    """Run kernel_v16b with native weight layouts — no preprocessing required.
    ...
    # Accept [E, H, 2, I] from qwen_fused_moe_tkg.py — reshape to [E, H, 2*I] at zero cost.
    # If I > _I (e.g. zero-padded I=256), slice to the valid _I=192 columns first.
    if gate_up_w.dim() == 4:
        E, Hd, two, Iv = gate_up_w.shape
        if Iv > _I:
            gate_up_w = gate_up_w[:, :, :, :_I]   # slice zero-pad away: [E, H, 2, _I]
        gate_up_w = gate_up_w.reshape(E, Hd, 2 * _I)

    # Accept [E, H, 2*I_padded] flat with I_padded > _I — interleaved not expected, skip
    assert gate_up_w.shape == (
        _E, _H, _GU_FLAT
    ), f"gate_up_w shape {gate_up_w.shape} != ({_E}, {_H}, {_GU_FLAT})"

    # down_w may also be zero-padded: [E, I_padded, H] → slice to [E, _I, H]
