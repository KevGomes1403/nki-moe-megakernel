...
kernel_v17b — Plan B (double-buffer gate_up prefetch) on top of v17a
======================================================================
...
SBUF budget estimate (per partition lane, 224 KiB limit):
  ...
  gate_up_buf_a/b:    [_PMAX,16,384]   bf16 =  12 KB each (2× for double-buffer, 12 KB extra)
  ...
  Total peak:        ~146 KB  — comfortable within 224 KiB (12 KB extra for the second buffer).
"""
...
def qwen3_moe_fused_tkg(
    ...
    for t in nl.static_range(T):
        ...
        down_full0 = nl.ndarray((_PMAX, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        down_full1 = nl.ndarray((_PMAX, H_shard), dtype=down_w.dtype, buffer=nl.sbuf)
        nisa.memset(down_full1[nl.ds(I1, I1), 0:H_shard], value=0.0)
        ...
        # ----------------------------------------------------------------
        # Plan B: Double-buffer gate_up prefetch.
        # Two separate 3D SBUF buffers (ping/pong) — each same shape as the
        # original gate_up_tile so dynamic DMA dim-count constraints are met.
        # Before the compute loop, we pre-issue the DMA for k=0 into buf_a (slot 0).
        # In each compute iteration k:
        #   1. Issue the DMA for k+1 into the next ping-pong buffer (prefetch next)
        #   2. Compute using the current buffer
        # This hides gate_up DMA latency behind the previous iteration's compute
        # (gate/up matmul + down DMA + down matmul).
        # The K=8 full-prefetch (all 8 tiles) was attempted first but caused
        # SBUF overflow (compiler saw 8×1.5 MB as a single unpartitioned block).
        # ----------------------------------------------------------------
        gate_up_buf_a = nl.ndarray((_PMAX, H_free, _GU_FLAT), dtype=gate_up_w.dtype, buffer=nl.sbuf)
        gate_up_buf_b = nl.ndarray((_PMAX, H_free, _GU_FLAT), dtype=gate_up_w.dtype, buffer=nl.sbuf)
        # Map slot index to buffer: even k → buf_a, odd k → buf_b
        _gate_up_bufs = [gate_up_buf_a, gate_up_buf_b]

        # Prime: issue DMA for k=0 into slot 0 (buf_a) before entering the compute loop
        ...
        # Single-pass K-loop with double-buffering.
        # nl.static_range so we can select the correct buffer at compile time.
        for k in nl.static_range(K):
            cur_buf  = _gate_up_bufs[k % 2]
            next_buf = _gate_up_bufs[(k + 1) % 2]

            # Issue prefetch DMA for k+1 into next buffer (no-op for last iteration)
            if k + 1 < K:
            ...
            # ----------------------------------------------------------------
            # Plan C: issue down DMAs immediately at the top of the compute loop.
            # (gate_up DMA for current k was pre-issued; k+1 just issued above)
            # down_full0 and down_full1 have NO dependency on gate_up_tile data —
            # they only need expert_id (already known). By issuing here, the DMA
            # engine pipelines the k+1 gate_up prefetch + down0 + down1 together,
            # allowing them to complete during the gate/up matmul.
            # ----------------------------------------------------------------
            ...
            #          count_p = I0 = 128, col stride = 1, count_col = H_shard = 1024
            ...
            nisa.dma_copy(
                dst=down_full1[0:I1, 0:H_shard],       # only write to rows 0:64
                src=down_w.ap(
                    pattern=[[H, I1], [1, H_shard]],
                    offset=I0 * H + prg_id * H_shard,   # skip I0=128 rows, then shard
                    scalar_offset=expert_id,
                    indirect_dim=0,
                ),
                dge_mode=0,
            ...
            for h1 in nl.affine_range(H_free):
                for i_tile in nl.static_range(I_tiles):
                    if i_tile == 0:
                        # Tile 0: K=128 cols from the current double-buffer
                        g_stat = cur_buf[0:_PMAX, h1, nl.ds(0, I0)]        # gate cols 0:128
                        u_stat = cur_buf[0:_PMAX, h1, nl.ds(I, I0)]         # up   cols 192:320
                    else:
            ...
            for h1_out in nl.affine_range(H_free_shard):
                # down_full0/1 now only hold H_shard=1024 cols (this core's shard).
                ...
                nisa.nc_matmul(
                    ...
                    stationary=down_full0[0:_PMAX, nl.ds(h1_out * _PMAX, _PMAX)],
                    moving=inter_bf16[0:_PMAX, 0:1],
                )
                # Tile 1: I rows 128:192 (64 valid, rows 64:128 zero from pre-k-loop memset)
                nisa.nc_matmul(
                    dst=down_psum[0:_PMAX, h1_out:h1_out + 1],
                    stationary=down_full1[0:_PMAX, nl.ds(h1_out * _PMAX, _PMAX)],
