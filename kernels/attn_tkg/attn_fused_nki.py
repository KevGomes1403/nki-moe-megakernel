...
def qwen3_attn_tkg_fused_oproj_v13bc_kv_norm_hoisted_weights(
    ...
    # Cross-partition sum: CustomRMSNorm lowers the 128-lane sum as a PE matmul
    # against an all-ones tile, broadcasting the scalar back across partitions.
    ...
    for s_t in nl.affine_range(NUM_S_TILES):
        ...
        # Baseline lowers exp_prior.sum(dim=-1) as a PE reduction. Use matmul
        # against an all-ones column instead of scalar tensor_reduce so the
        # denominator accumulation order matches for long prior contexts.
        tile_sum_psum = nl.zeros((GQA, 1), dtype=nl.float32, buffer=nl.psum)
        nisa.nc_matmul(tile_sum_psum, stationary=score2_exp, moving=rms_ones[0:PMAX, 0:1])
        tile_sum_gqa = sbm.alloc_stack((GQA, 1), nl.float32, name=f"tile_sum_gqa_{s_t}")
        nisa.tensor_copy(tile_sum_gqa, tile_sum_psum)
    ...
    # The active attention path consumes the live V projection, while the cache
    # update separately rounds V to bf16. Keep V active in f32 here; otherwise
    # pos>0 cases differ by one bf16 ulp in the final attention vector.
    ...
    nisa.tensor_tensor(v_act_weighted_fp32, v_act_packed, softmax_act_bf16, op=nl.multiply)
    ...
    if n_prgs == 1:
        ...
        res_sbuf_0 = sbm.alloc_stack((1, F_MAX), nl.float32, name="res_sbuf_0")
        res_peer_0 = sbm.alloc_stack((1, F_MAX), nl.float32, name="res_peer_0")
        res_sbuf_1 = sbm.alloc_stack((1, F_MAX), nl.float32, name="res_sbuf_1")
        res_peer_1 = sbm.alloc_stack((1, F_MAX), nl.float32, name="res_peer_1")
        res_sbuf_2 = sbm.alloc_stack((1, F_MAX), nl.float32, name="res_sbuf_2")
        res_peer_2 = sbm.alloc_stack((1, F_MAX), nl.float32, name="res_peer_2")
        res_sbuf_3 = sbm.alloc_stack((1, F_MAX), nl.float32, name="res_sbuf_3")
        res_peer_3 = sbm.alloc_stack((1, F_MAX), nl.float32, name="res_peer_3")
        ...
        if prg_id == 0:
            for head in [0, 1, 2, 3]:
                ...
                nisa.nc_matmul(res_psum_2, stationary=attn_out[0:PMAX, head:head+1], moving=wo_sbuf[head][0:PMAX, 2*F_MAX:3*F_MAX])
                nisa.nc_matmul(res_psum_3, stationary=attn_out[0:PMAX, head:head+1], moving=wo_sbuf[head][0:PMAX, 3*F_MAX:4*F_MAX])
            nisa.tensor_copy(res_sbuf_0, res_psum_0)
            nisa.tensor_copy(res_sbuf_1, res_psum_1)
            nisa.tensor_copy(res_sbuf_2, res_psum_2)
            nisa.tensor_copy(res_sbuf_3, res_psum_3)
        else:
            nisa.memset(res_sbuf_0, value=0.0)
            nisa.memset(res_sbuf_1, value=0.0)
            nisa.memset(res_sbuf_2, value=0.0)
            nisa.memset(res_sbuf_3, value=0.0)

        # Send the heads 0..3 prefix to program 1. Program 1 then accumulates
        # heads 4..7 into that same fp32 accumulator, matching the baseline
        # RowParallelLinear reduction order over the 1024-wide input.
        nisa.sendrecv(src=res_sbuf_0, dst=res_peer_0, send_to_rank=peer_id, recv_from_rank=peer_id, pipe_id=0)
        nisa.sendrecv(src=res_sbuf_1, dst=res_peer_1, send_to_rank=peer_id, recv_from_rank=peer_id, pipe_id=1)
        nisa.sendrecv(src=res_sbuf_2, dst=res_peer_2, send_to_rank=peer_id, recv_from_rank=peer_id, pipe_id=2)
        nisa.sendrecv(src=res_sbuf_3, dst=res_peer_3, send_to_rank=peer_id, recv_from_rank=peer_id, pipe_id=3)

        if prg_id == 1:
            nisa.tensor_copy(res_psum_0, res_peer_0)
            nisa.tensor_copy(res_psum_1, res_peer_1)
            nisa.tensor_copy(res_psum_2, res_peer_2)
            nisa.tensor_copy(res_psum_3, res_peer_3)
            for head in [4, 5, 6, 7]:
                nisa.nc_matmul(res_psum_0, stationary=attn_out[0:PMAX, head:head+1], moving=wo_sbuf[head][0:PMAX, 0*F_MAX:1*F_MAX])
                nisa.nc_matmul(res_psum_1, stationary=attn_out[0:PMAX, head:head+1], moving=wo_sbuf[head][0:PMAX, 1*F_MAX:2*F_MAX])
                nisa.nc_matmul(res_psum_2, stationary=attn_out[0:PMAX, head:head+1], moving=wo_sbuf[head][0:PMAX, 2*F_MAX:3*F_MAX])
        ...
        # Return the final sequential accumulator to program 0 so both programs
        # write the same bf16 output tile.
        nisa.sendrecv(src=res_sbuf_0, dst=res_peer_0, send_to_rank=peer_id, recv_from_rank=peer_id, pipe_id=0)
        nisa.sendrecv(src=res_sbuf_1, dst=res_peer_1, send_to_rank=peer_id, recv_from_rank=peer_id, pipe_id=1)
        nisa.sendrecv(src=res_sbuf_2, dst=res_peer_2, send_to_rank=peer_id, recv_from_rank=peer_id, pipe_id=2)
        nisa.sendrecv(src=res_sbuf_3, dst=res_peer_3, send_to_rank=peer_id, recv_from_rank=peer_id, pipe_id=3)
        ...
        if prg_id == 0:
            nisa.tensor_copy(res_sbuf_0, res_peer_0)
            nisa.tensor_copy(res_sbuf_1, res_peer_1)
            nisa.tensor_copy(res_sbuf_2, res_peer_2)
            nisa.tensor_copy(res_sbuf_3, res_peer_3)

    # Transpose each 128-column chunk — 4 blocks x 4 chunks = 16 columns
    if n_prgs > 1:
        nisa.tensor_copy(out_sb_tmp, res_sbuf_0)
    else:
    ...
    if n_prgs > 1:
        nisa.tensor_copy(out_sb_tmp, res_sbuf_1)
    else:
    ...
    if n_prgs > 1:
        nisa.tensor_copy(out_sb_tmp, res_sbuf_2)
    else:
    ...
    if n_prgs > 1:
        nisa.tensor_copy(out_sb_tmp, res_sbuf_3)
    else:
