...
os.environ["NEURON_RT_INSPECT_OUTPUT_DIR"] = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "_bench_out_v15a"
)
...
from v15a_peer_split_kv import qwen3_attn_tkg_fused_oproj_v15a_peer_split_kv

PMAX = 128

@nki.jit
def v15a_wrapper(hidden_states, Wq, Wk, Wv, Wo, q_norm_weight, k_norm_weight,
    ...
    out_sb = qwen3_attn_tkg_fused_oproj_v15a_peer_split_kv(
...
v15a_wrapper = wrap_benchmark(v15a_wrapper[2], warmup=5, iters=50)
...
v15a_wrapper(hidden,Wq,Wk,Wv,Wo,qnw,knw,gpan,K_cache,V_cache,cos,sin,pos,output)
xm.mark_step()

r_res = v15a_wrapper.last_result
if r_res:
