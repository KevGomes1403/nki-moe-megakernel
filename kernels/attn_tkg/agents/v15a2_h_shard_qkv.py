"""
v15a2_h_shard_qkv: H-shard the QKV K-contraction (hidden dim) across LNC=2.

Follows nkilib's transformer_tkg H1_shard pattern:
  - NUM_H_TILES=16 split into 8 per core (H_TILES_SHARD=8).
  - prg 0 owns h-tiles [0..7], prg 1 owns [8..15].
  - Wq/Wk/Wv/gamma_pre_attn DMAs load only the owned stripe.
  - Pre-attn RMSNorm sq-sum is partial; all-reduced via sendrecv+add.
  - QKV matmul PSUMs are partial; all-reduced via sendrecv+add.
  - Q still head-sharded (prg 0 owns heads 0..3, prg 1 owns 4..7).
  - All downstream stages (KV RMSNorm/RoPE, flash decode, Wo head-shard,
    final o-proj sendrecv, KV scatter split) are unchanged.

Under n_prgs==1 the kernel falls back to the full-16-tile path, identical
to v14b_kv_norm_pretransposed.

All v14b optimizations are preserved:
  - Pre-transposed Wq/Wk/Wv + dma_copy
  - Early K/V cache DMA hoist
  - PSUM bank interleaving in Q-proj
  - Wo head-sharded hoist with dma_copy
  - In-place KV scatter split across prgs
"""
...
def qwen3_attn_tkg_fused_oproj_v15a2(
    hidden_sb,        # [PMAX, num_h_tiles] = [128, 16]  bf16  SBUF (full)
    ...
    if n_prgs == 1:
        ...
        H_TILES_SHARD = NUM_H_TILES  # 16
        h_tile_start = 0
    elif prg_id == 0:
        owned_heads = [0, 1, 2, 3]
        H_TILES_SHARD = NUM_H_TILES // 2  # 8
        h_tile_start = 0
    else:
        owned_heads = [4, 5, 6, 7]
        H_TILES_SHARD = NUM_H_TILES // 2  # 8
        h_tile_start = NUM_H_TILES // 2  # 8

    H_SHARD = H_TILES_SHARD * d  # 1024 when sharded, 2048 when not

    # =========================================================================
    # EARLY K/V CACHE DMA (unchanged)
    # =========================================================================
    k_cache_tiles_hbm = []
    v_cache_tiles_hbm = []
    ...
    # =========================================================================
    # Wk / Wv DMA — only owned h-tile stripe
    # Wk_pt[p, h_t*d+f_d] for h_t in [h_tile_start, h_tile_start+H_TILES_SHARD)
    # =========================================================================
    wk_sb = sbm.alloc_stack((PMAX, H_SHARD), nl.bfloat16, name="wk_sb")
    nisa.dma_copy(
        dst=wk_sb,
        src=Wk[0:PMAX, h_tile_start * d:(h_tile_start + H_TILES_SHARD) * d],
        dge_mode=nisa.dge_mode.hwdge,
    )

    wv_sb = sbm.alloc_stack((PMAX, H_SHARD), nl.bfloat16, name="wv_sb")
    nisa.dma_copy(
        dst=wv_sb,
        src=Wv[0:PMAX, h_tile_start * d:(h_tile_start + H_TILES_SHARD) * d],
        dge_mode=nisa.dge_mode.hwdge,
    )
    ...
    # gamma_pre_attn — only owned stripe of H_TILES_SHARD columns
    gpan_bf16 = sbm.alloc_stack((PMAX, H_TILES_SHARD), nl.bfloat16, name="gpan_bf16")
    nisa.dma_copy(
        dst=gpan_bf16,
        src=gamma_pre_attn.reshape((H, 1)).ap(
            pattern=[[1, PMAX], [PMAX, H_TILES_SHARD]],
            offset=h_tile_start * PMAX,
        ),
        dge_mode=nisa.dge_mode.hwdge,
    )
    gpan_f32 = sbm.alloc_stack((PMAX, H_TILES_SHARD), nl.float32, name="gpan_f32")
    ...
    # =========================================================================
    # h_all_shard: owned slab of post-norm hidden state, [PMAX, H_TILES_SHARD]
    # =========================================================================
    h_all = sbm.alloc_stack((PMAX, H_TILES_SHARD), nl.bfloat16, name="h_all")

    # =====================================================================
    # PRE-ATTENTION RMSNORM (partial-sum over owned h-tiles, then AR)
    # =====================================================================
    sbm.open_scope("pre_attn_norm")

    # Owned slab of hidden_sb
    h_f32 = sbm.alloc_stack((PMAX, H_TILES_SHARD), nl.float32, name="h_f32")
    nisa.tensor_copy(h_f32, hidden_sb[0:PMAX, h_tile_start:h_tile_start + H_TILES_SHARD])

    h_sq = sbm.alloc_stack((PMAX, H_TILES_SHARD), nl.float32, name="h_sq")
    nisa.tensor_tensor(h_sq, h_f32, h_f32, op=nl.multiply)

    # Partial sum over owned tiles
    h_sq_sum_partial = sbm.alloc_stack((PMAX, 1), nl.float32, name="h_sq_sum_partial")
    nisa.tensor_reduce(dst=h_sq_sum_partial, op=nl.add, data=h_sq, axis=1)

    # All-reduce partial across cores (sendrecv + add)
    if n_prgs > 1:
        h_sq_sum_peer = sbm.alloc_stack((PMAX, 1), nl.float32, name="h_sq_sum_peer")
        peer_id = 1 - prg_id
        nisa.sendrecv(
            src=h_sq_sum_partial,
            dst=h_sq_sum_peer,
        ...
        nisa.tensor_tensor(h_sq_sum, h_sq_sum_partial, h_sq_sum_peer, op=nl.add)
    else:
        h_sq_sum = h_sq_sum_partial
    ...
    # Broadcast rsqrt to H_TILES_SHARD columns
    h_rms_T_psum = nl.ndarray((H_TILES_SHARD, PMAX), nl.float32, buffer=nl.psum)
    nisa.nc_transpose(h_rms_T_psum, h_rms_inv.ap([[1, PMAX], [0, H_TILES_SHARD]], offset=0))
    h_rms_T = sbm.alloc_stack((H_TILES_SHARD, PMAX), nl.float32, name="h_rms_T")
    nisa.tensor_copy(h_rms_T, h_rms_T_psum)
    h_rms_expanded_psum = nl.ndarray((PMAX, H_TILES_SHARD), nl.float32, buffer=nl.psum)
    nisa.nc_transpose(h_rms_expanded_psum, h_rms_T)
    h_rms_expanded = sbm.alloc_stack((PMAX, H_TILES_SHARD), nl.float32, name="h_rms_expanded")
    nisa.tensor_copy(h_rms_expanded, h_rms_expanded_psum)

    h_normed = sbm.alloc_stack((PMAX, H_TILES_SHARD), nl.float32, name="h_normed")
    ...
    # =========================================================================
    # Wq LOAD — only owned h-tile stripe per owned head
    # =========================================================================
    wq_head_sb = [None] * HQ_TP_CONST
    for q_h in owned_heads:
        w = sbm.alloc_stack((PMAX, H_SHARD), nl.bfloat16, name=f"wq_head_{q_h}")
        nisa.dma_copy(
            dst=w,
            src=Wq[q_h * PMAX:(q_h + 1) * PMAX,
                   h_tile_start * d:(h_tile_start + H_TILES_SHARD) * d],
            dge_mode=nisa.dge_mode.hwdge,
        )
        wq_head_sb[q_h] = w
    ...
    # =========================================================================
    # KV PROJ SCOPE — partial matmul over owned h-tiles, then AR
    # =========================================================================
    sbm.open_scope("kv_proj")

    k_psum = nl.zeros((PMAX, B), dtype=nl.float32, buffer=nl.psum)
    for h_t in nl.affine_range(H_TILES_SHARD):
        nisa.nc_matmul(k_psum,
                       stationary=wk_sb[0:PMAX, h_t*d:(h_t+1)*d],
                       moving=h_all[0:PMAX, h_t:h_t+1])

    k_vec_local = sbm.alloc_stack((PMAX, B), nl.float32, name="k_vec_local")
    nisa.tensor_copy(k_vec_local, k_psum)

    # AR partial K
    if n_prgs > 1:
        k_vec_local_bf16 = sbm.alloc_stack((PMAX, B), nl.bfloat16, name="k_vec_local_bf16")
        nisa.tensor_copy(k_vec_local_bf16, k_vec_local)
        k_vec_peer_bf16 = sbm.alloc_stack((PMAX, B), nl.bfloat16, name="k_vec_peer_bf16")
        peer_id = 1 - prg_id
        nisa.sendrecv(
            src=k_vec_local_bf16,
            dst=k_vec_peer_bf16,
            send_to_rank=peer_id,
            recv_from_rank=peer_id,
            pipe_id=2,
        )
        k_vec_peer = sbm.alloc_stack((PMAX, B), nl.float32, name="k_vec_peer")
        nisa.tensor_copy(k_vec_peer, k_vec_peer_bf16)
        k_vec = sbm.alloc_stack((PMAX, B), nl.float32, name="k_vec")
        nisa.tensor_tensor(k_vec, k_vec_local, k_vec_peer, op=nl.add)
    else:
        k_vec = k_vec_local
    ...
    # AR partial V
    if n_prgs > 1:
        v_local_bf16 = sbm.alloc_stack((PMAX, B), nl.bfloat16, name="v_local_bf16")
        nisa.tensor_copy(v_local_bf16, v_local)
        v_peer_bf16 = sbm.alloc_stack((PMAX, B), nl.bfloat16, name="v_peer_bf16")
        peer_id = 1 - prg_id
        nisa.sendrecv(
            src=v_local_bf16,
            dst=v_peer_bf16,
        ...
        nisa.tensor_copy(v_peer, v_peer_bf16)
        nisa.tensor_tensor(v_active, v_local, v_peer, op=nl.add)
    else:
        nisa.tensor_copy(v_active, v_local)
    ...
    # =========================================================================
    # Q PROJ SCOPE — partial matmul over owned h-tiles, then AR (pack-Q)
    # =========================================================================
    ...
    # Pack each core's partials into [PMAX, GQA], only OWNED columns populated (other cols = 0)
    q_packed_local = sbm.alloc_stack((PMAX, GQA), nl.float32, name="q_packed_local")
    nisa.memset(q_packed_local, value=0.0)
    for i in range(len(owned_heads)):
        q_h = owned_heads[i]
        nisa.tensor_copy(q_packed_local[0:PMAX, q_h:q_h + 1], q_psums[i])

    # AR packed Q
    if n_prgs > 1:
        q_packed_local_bf16 = sbm.alloc_stack((PMAX, GQA), nl.bfloat16, name="q_packed_local_bf16")
        nisa.tensor_copy(q_packed_local_bf16, q_packed_local)
        q_packed_peer_bf16 = sbm.alloc_stack((PMAX, GQA), nl.bfloat16, name="q_packed_peer_bf16")
        peer_id = 1 - prg_id
        nisa.sendrecv(
            src=q_packed_local_bf16,
            dst=q_packed_peer_bf16,
            send_to_rank=peer_id,
            recv_from_rank=peer_id,
            pipe_id=4,
        )
        q_packed_peer = sbm.alloc_stack((PMAX, GQA), nl.float32, name="q_packed_peer")
        nisa.tensor_copy(q_packed_peer, q_packed_peer_bf16)
        q_packed_f32 = sbm.alloc_stack((PMAX, GQA), nl.float32, name="q_packed_f32")
        nisa.tensor_tensor(q_packed_f32, q_packed_local, q_packed_peer, op=nl.add)
    else:
        q_packed_f32 = q_packed_local
    ...
    # =========================================================================
    # WO WEIGHT HOISTING — head-sharded, unchanged
    # =========================================================================
    for head in owned_heads:
        nisa.dma_copy(
            dst=wo_sbuf[head],
    ...
    # =========================================================================
    # TWO-PASS FLASH DECODE (unchanged)
    # =========================================================================
    ...
    # Cross-core all-reduce for o-proj (pipe_id=0, matches baseline)
    if n_prgs > 1:
    ...
    # KV CACHE IN-PLACE SCATTER (unchanged split)
