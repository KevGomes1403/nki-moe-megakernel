"""
v14_planD: v13_BC + LNC=2 Wo sharding (Plan D).

Changes from v13_BC:
  - core_id = nl.program_id(0): each of 2 physical cores gets id 0 or 1.
  - H_half = H_wo // 2 = 1024: each core handles half the output columns.
  - Wo loading: each core loads [PMAX, H_half] per head instead of [PMAX, H_wo].
    Core 0: Wo[:, 0:1024],    Core 1: Wo[:, 1024:2048].
  - O-projection: 2 PSUMs per core (instead of 4) covering H_half columns.
  - Output stores: use ap() with dynamic core_id offset (same as v13a).
    Core 0: writes output[:, 0:H_half], Core 1: writes output[:, H_half:H_wo].
  - Both cores write k_rope_out and v_out (idempotent — same value).
  - Kernel launched with qwen3_attn_tkg_fused_oproj_v14d[2](...).

Reduces per-core HBM reads by ~2MB (4MB Wo → 2MB Wo per core).

All v13_BC optimizations preserved:
  hw_dge on all dma_copy, pre-broadcast mask tiles (Change C), Plan B K-cache
  contiguous load + PE transpose + saved_scores, all static shape constants.
"""
...
@nki.jit
def qwen3_attn_tkg_fused_oproj_v14d(
    ...
    core_id = nl.program_id(0)   # 0 or 1 under NEURON_LOGICAL_NC_CONFIG=2
    ...
    # Output H: LNC=2 sharding — each core writes half of H_wo output columns.
    ...
    for head in nl.affine_range(HQ_TP_CONST):
        # Each core loads only H_half=1024 columns instead of H_wo=2048
        wo_tile = nl.ndarray((PMAX, H_half), dtype=nl.bfloat16, buffer=nl.sbuf,
                             name=f"wo_tile_h{head}")
        nisa.dma_copy(
            dst=wo_tile,
            src=Wo_reshaped.ap(
                pattern=[[H_wo, PMAX], [1, H_half]],   # partition: stride H_wo, free: H_half contiguous cols
                offset=head * PMAX * H_wo + core_id * H_half,  # core_id selects first or second half
            ),
            dge_mode=nisa.dge_mode.hwdge,
        )
    ...
    # =========================================================================
    # FUSED OUTPUT PROJECTION — LNC=2 sharding (Plan D).
    # Each core handles H_half=1024 output columns (2 blocks of F_MAX=512).
