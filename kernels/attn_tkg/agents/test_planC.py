"""
CPU-simulator correctness test for v14b_kv_norm_planABC.

Uses nki.simulate (no hardware required). NKI_PRECISE_FP=1 enables accurate bf16
simulation so the kernel's bf16 round-trips match the hardware exactly.

Reference: float32 ground truth. The KV cache is bf16 storage (new_k/new_v are cast
to bf16 on write and read back as float32), which is the correct model of hardware
behavior — NOT an artifact of mimicking the kernel.

Tests positions: [0, 1, 63, 64, 127, 128, 129, 255, 320]
Tolerance: rtol=1e-2, atol=1e-2.

Run:
    NKI_PRECISE_FP=1 NEURON_PLATFORM_TARGET_OVERRIDE=trn3 \\
        python test_planC.py
"""
...
from v14b_kv_norm_planABC import qwen3_attn_tkg_fused_oproj_v13bc_kv_norm
...
# ---------------------------------------------------------------------------
# Wrapper kernel — @nki.jit, takes all HBM inputs as numpy arrays
# ---------------------------------------------------------------------------
@nki.jit
def v14b_kv_norm_planABC_wrapper(
...
# ---------------------------------------------------------------------------
# Reference implementation — float32 ground truth
# ---------------------------------------------------------------------------
def pytorch_attn_ref(
    ...
    """
    Float32 ground-truth reference for the fused attention kernel.

    The KV cache is bf16 storage: new_k and new_v are cast to bf16 on write and
    read back as float32. This is the correct model of hardware behavior (the cache
    IS bf16), not an artifact of mimicking the kernel.

    Everything else — projections, norms, RoPE, Q, scores, attention — is float32.
    """
    ...
    # Pre-attention RMSNorm — float32 throughout
    ...
    # K projection + RMSNorm + RoPE — float32
    k = x @ Wk.float().t()
    k = k * (k.pow(2).mean(-1, keepdim=True) + EPS).rsqrt() * k_norm_weight.float()
    k_rot = torch.cat([-k[:, d // 2:], k[:, :d // 2]], dim=-1)
    k = k * cos_f + k_rot * sin_f
    # KV cache is bf16 storage: quantise on write, read back as float32
    new_k = k.bfloat16()

    # V projection — float32; cast to bf16 for cache
    v = x @ Wv.float().t()
    new_v = v.bfloat16()
    ...
    # Q projection + RMSNorm + RoPE + scale — float32
    ...
    q_scaled = (q * cos_q + q_rot * sin_q) / (d ** 0.5)  # float32

    # Flash-decode attention — read cache as float32
...
def run_one(pos_val, weights, hidden_rng_seed):
    ...
    result_np, K_out_np, V_out_np = nki.simulate(v14b_kv_norm_planABC_wrapper)(
        hidden_np, Wq_np, Wk_np, Wv_np, Wo_np,
        qnw_np, knw_np, gpan_np,
        K_cache_sim, V_cache_sim, cos_np, sin_np, pos_np, output_np,
    )

    # Convert numpy outputs to float32 for comparison
