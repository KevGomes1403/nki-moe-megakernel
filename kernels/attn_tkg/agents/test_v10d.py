"""
Correctness + benchmark harness for v10d.
Tests: (a) partial-fill case (valid_seq_len < S_prior), (b) full cache (valid_seq_len == S_prior).

v10d removes attn_mask parameter; mask is generated on-chip from K-cache zero-detection.
"""
import os, sys, math
os.environ["NEURON_PLATFORM_TARGET_OVERRIDE"] = "trn2"
os.environ["NEURON_RT_INSPECT_ENABLE"] = "1"
os.environ["NEURON_RT_INSPECT_DEVICE_PROFILE"] = "1"
os.environ["NEURON_RT_INSPECT_OUTPUT_DIR"] = os.path.join(os.path.dirname(os.path.abspath(__file__)), "_bench_out_v10d")
...
def run_test(valid_seq_len, S_prior=640, B=1, H=2048, d=128, Hq_tp=8, Hkv_tp=1):
    print(f"\n{'='*70}")
    print(f"v10d Test  valid_seq_len={valid_seq_len}  S_prior={S_prior}")
    print(f"{'='*70}")
    ...
    # PyTorch reference (slices to valid_seq_len — no attn_mask needed)
    ...
    # v10d: no attn_mask argument
    nki_out_dev, nki_k_rope_dev, nki_v_dev = qwen3_attn_tkg_fused_oproj_v10d[2](
...
if __name__ == "__main__":
    # Test 1: partial fill (valid_seq_len = 100, S_prior = 640) — primary mask correctness case
    # run_test(valid_seq_len=100)
    # Test 2: full cache (valid_seq_len = S_prior) — all positions valid, mask ≈ 0 everywhere
    run_test(valid_seq_len=640)
    run_test(S_prior=4096, valid_seq_len=700)

    # Benchmark
    print("\n" + "="*70)
    print("Benchmarking v10d...")
    print("="*70)
    ...
    # v10d: no attn_mask argument
    kernel_bench = wrap_benchmark(qwen3_attn_tkg_fused_oproj_v10d[2], warmup=5, iters=50)
    kernel_bench(hidden_states, Wq, Wk, Wv, Wo, q_norm, k_norm,
                 K_cache, V_cache, cos, sin_t, position_ids)
