"""
Correctness test for v14_planE (fused mask computation) vs v12e (baseline).
"""
import os
import sys
...
print("Running v14e[2] under test...")
try:
    out14e, k14e, v14e_out = qwen3_attn_tkg_fused_oproj_v14e[2](*inputs)
    xm.mark_step()
...
out14e_cpu = out14e.cpu().float()
k12e_cpu = k12e.cpu().float()
k14e_cpu = k14e.cpu().float()
v12e_cpu = v12e_out.cpu().float()
v14e_cpu = v14e_out.cpu().float()

print(f"\nout shape: v12e={out12e_cpu.shape}, v14e={out14e_cpu.shape}")

# Compute max diffs
out_maxdiff = (out12e_cpu - out14e_cpu).abs().max().item()
k_maxdiff = (k12e_cpu - k14e_cpu).abs().max().item()
v_maxdiff = (v12e_cpu - v14e_cpu).abs().max().item()

print(f"\nMax abs diff — output: {out_maxdiff:.3e}, k_rope_out: {k_maxdiff:.3e}, v_out: {v_maxdiff:.3e}")

rtol, atol = 1e-3, 1e-3
pass_out = torch.allclose(out12e_cpu, out14e_cpu, rtol=rtol, atol=atol)
pass_k   = torch.allclose(k12e_cpu,   k14e_cpu,   rtol=rtol, atol=atol)
pass_v   = torch.allclose(v12e_cpu,   v14e_cpu,   rtol=rtol, atol=atol)
...
if pass_out and pass_k and pass_v:
    ...
    if not pass_out:
        H_wo = 2048
        first_half_max = (out12e_cpu[..., :H_wo//2] - out14e_cpu[..., :H_wo//2]).abs().max().item()
        second_half_max = (out12e_cpu[..., H_wo//2:] - out14e_cpu[..., H_wo//2:]).abs().max().item()
