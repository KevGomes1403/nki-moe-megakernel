"""
v10a: Combined Plan A+B optimizations on top of v9_optimized.

Changes from v9_optimized:
  Plan A — Static shape specialization:
    Replace all runtime-derived nl.affine_range bounds (num_s_tiles, num_h_tiles,
    Hq_tp) with compile-time integer constants (NUM_S_TILES=5, NUM_H_TILES=16,
    HQ_TP_CONST=8). This eliminates dynamic DMA descriptors (98% dynamic packets
    in v9_optimized profiling) by letting the compiler emit static DMA addresses.

  Plan B — K-cache contiguous load + PE transpose:
    Replace strided ap([[1,PMAX],[d,PMAX]]) transpose-load (128 scatter segments
    per tile × 5 tiles = 640 strided DMA packets at 50% fill ratio) with:
      1. Contiguous dma_copy of each [PMAX, PMAX] K-tile (1 wide 32KB packet)
      2. nisa.nc_transpose to reorder into the [d, S_tile] layout the matmul needs
    V-cache loading unchanged (already contiguous per-partition).

Preserved unchanged from v9_optimized:
