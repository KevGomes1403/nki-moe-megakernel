"""
Correctness + benchmark harness for v11b (Plan B — Deferred Wo Loading).

Compares v11b output against v10e (reference kernel) to isolate the
scheduling change (deferred Wo loading) from any algorithmic differences.

Test configurations:
  - pos=320, S_prior=640  (primary — half-filled cache)
  - pos=640               (full cache)
  - pos=128               (nearly empty — first tile only)

Tolerance: rtol=1e-3, atol=1e-3 (no algorithmic change, should be bit-identical)
"""
import os
import sys
...
os.environ["NEURON_RT_INSPECT_OUTPUT_DIR"] = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "_bench_out_v11b"
)
...
from v11b import qwen3_attn_tkg_fused_oproj_v11b
...
def run_correctness_test(pos, S_prior=640, label=""):
    """
    Compare v11b output vs v10e output (v10e = reference).
    Since only scheduling changes, outputs should be bit-identical.
    """
    print(f"\n{'='*70}")
    print(f"v11b Correctness (vs v10e ref)  {label}  pos={pos}  S_prior={S_prior}")
    print(f"{'='*70}")
    ...
    # v10e reference kernel
    ref_out_dev, ref_k_rope_dev, ref_v_dev = qwen3_attn_tkg_fused_oproj_v10e[2](
    ...
    ref_k_rope = ref_k_rope_dev.cpu()
    ref_v = ref_v_dev.cpu()

    # v11b kernel
    nki_out_dev, nki_k_rope_dev, nki_v_dev = qwen3_attn_tkg_fused_oproj_v11b[2](
        hidden_states.to(device), Wq.to(device), Wk.to(device), Wv.to(device),
        Wo_kernel.to(device), q_norm_weight.to(device), k_norm_weight.to(device),
        K_cache.to(device), V_cache.to(device),
        cos_at_pos.to(device), sin_at_pos.to(device),
    ...
    np.testing.assert_allclose(
        nki_out.float().numpy(), ref_out.float().numpy(), rtol=1e-3, atol=1e-3,
        err_msg="v11b output mismatch vs v10e reference"
    )
    np.testing.assert_allclose(
        nki_k_rope.float().numpy(), ref_k_rope.float().numpy(), rtol=1e-3, atol=1e-3,
        err_msg="v11b k_rope mismatch vs v10e reference"
    )
    np.testing.assert_allclose(
        nki_v.float().numpy(), ref_v.float().numpy(), rtol=1e-3, atol=1e-3,
        err_msg="v11b v mismatch vs v10e reference"
    )
...
def run_benchmark(pos, S_prior=640):
    print(f"\n{'='*70}")
    print(f"Benchmarking v11b  pos={pos}  S_prior={S_prior}")
    print(f"{'='*70}")
    device = xm.xla_device()
    ...
    bench_fn = wrap_benchmark(qwen3_attn_tkg_fused_oproj_v11b, warmup=5, iters=50)
...
if __name__ == "__main__":
    # ── Correctness: v11b vs v10e ─────────────────────────────────────────────
    md_320 = run_correctness_test(pos=320, S_prior=640, label="half-filled cache")
    md_640 = run_correctness_test(pos=640, S_prior=640, label="full cache")
    md_128 = run_correctness_test(pos=128, S_prior=640, label="one tile valid")
    ...
    # ── Benchmark: v11b at pos=320, S_prior=640 ───────────────────────────────
    r = run_benchmark(pos=320, S_prior=640)

    print(f"\n{'='*70}")
    print(f"BENCHMARK SUMMARY  (v11b, pos=320, S_prior=640)")
    print(f"{'='*70}")
    if r:
        print(f"  device_time_us       = {r.device_time_us:.2f}")
        print(f"  tensor_engine_pct    = {r.tensor_engine_pct:.1f}%")
        ...
        print("  WARNING: benchmark result not available")
