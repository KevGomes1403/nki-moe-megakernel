"""
v13_planB: Add hw_dge to all nisa.dma_copy calls (Plan B — Hardware DGE).

Changes from v12e:
  Plan B — Hardware DGE for all dma_copy calls:
    - Add dge_mode=nisa.dge_mode.hwdge to every nisa.dma_copy(...) call.
    - hw_dge causes descriptor generation to happen in hardware (~600ns) and
      overlap with ScalarE compute pipeline, rather than paying GpSimdE
      descriptor generation serially (~600ns x ~34 calls = ~20us overhead).
    - This is a performance hint only; no semantic change.
    - Expected: dma_active_pct decreases, device_time_us decreases.

All v12e optimizations are preserved unchanged:
  v12e causal masking boundary fix, Plan D deferred Wo DMA overlapping flash
  decode, Wq pre-load (8×512KB DMA), early Wv DMA, Plan A static shape
  constants, Plan B K-cache contiguous load + PE transpose, Q-proj
  psum→q_packed_f32 direct copy, O-proj head-outer/h_blk-inner loop order,
...
@nki.jit
def qwen3_attn_tkg_fused_oproj_v13b(
