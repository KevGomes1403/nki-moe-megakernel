"""
v10b: Extends v10a by also returning k_rope_out and v_out HBM tensors for KV cache update.

Changes from v10a:
  - Allocates k_rope_out [d, B] bf16 HBM tensor for the new token's K after RMSNorm+RoPE
  - Allocates v_out [d, B] bf16 HBM tensor for the new token's V (no RoPE)
  - Stores k_rope (SBUF f32) to k_rope_out after RoPE computation
  - Stores v_active (SBUF f32) to v_out after V projection
  - Returns (output, k_rope_out, v_out) instead of output alone

Shape notes for callers integrating with NeuronAttentionBase:
  - k_rope_out: shape [d, B] = [128, 1] bf16.
    Caller reshapes to [B, 1, d, 1] then permutes to [B, 1, 1, d] before writing
    to K_cache at position S_prior (matching nkilib k_out convention).
  - v_out: shape [d, B] = [128, 1] bf16.
    Caller reshapes to [B, 1, 1, d] before writing to V_cache at position S_prior.

Preserved unchanged from v10a:
...
@nki.jit
def qwen3_attn_tkg_fused_oproj_v10b(
...
# =============================================================================
# Test harness (v10b: also verifies k_rope_out and v_out)
# =============================================================================
if __name__ == "__main__":
    ...
    def pytorch_reference(
        ...
        # K_rope_new and V_new: compute using NKI-equivalent formula.
        # NKI nc_matmul computes stationary.T @ moving, so the projection formula is:
        #   k_nki[j] = sum_{h_t,c} W[c, h_t*d+j] * hidden[c+h_t*d]
        # This is equivalent to:
        #   hidden_r = hidden[0,0,:].reshape(num_h_tiles, d)  # [16, 128]
        #   W_perm = W.reshape(d, num_h_tiles, d).permute(1,0,2).reshape(num_h_tiles*d, d)
        #   k_nki = hidden_r.reshape(1, num_h_tiles*d) @ W_perm  # [1, 128]
        num_h_tiles = H // d
        hs_1d = hidden_states[0, 0, :].float()  # [H=2048]
        hidden_r = hs_1d.reshape(1, num_h_tiles * d)  # [1, 2048]

        Wk_perm = Wk.float().reshape(d, num_h_tiles, d).permute(1, 0, 2).reshape(num_h_tiles * d, d)
        k_nki_vec = (hidden_r @ Wk_perm).squeeze(0)  # [d=128]
        # RMSNorm on k_nki_vec
        var_k = k_nki_vec.pow(2).mean() + 1e-6
        k_nki_normed = k_nki_vec * torch.rsqrt(var_k) * k_norm_weight.float()
        # RoPE on k_nki_normed
        half_d = d // 2
        cos_1d = cos_at_pos[0].float()  # [d]
        sin_1d = sin_at_pos[0].float()  # [d]
        rot_k_nki = torch.cat([-k_nki_normed[half_d:], k_nki_normed[:half_d]], dim=0)
        k_rope_nki_vec = k_nki_normed * cos_1d + rot_k_nki * sin_1d  # [d=128]

        Wv_perm = Wv.float().reshape(d, num_h_tiles, d).permute(1, 0, 2).reshape(num_h_tiles * d, d)
        v_nki_vec = (hidden_r @ Wv_perm).squeeze(0)  # [d=128]

        # Shape [B, d] = [1, 128] bf16 to match kernel output layout
        K_rope_new = k_rope_nki_vec.to(torch.bfloat16).reshape(B, d)
        V_new = v_nki_vec.to(torch.bfloat16).reshape(B, d)
        return output.to(torch.bfloat16), K_rope_new, V_new
    ...
    def run_test(B=1, S_prior=640, H=2048, d=128, Hq_tp=8, Hkv_tp=1):
        ...
        print(f"  ref_k_rope shape: {ref_k_rope.shape}")
        print(f"  ref_v shape: {ref_v.shape}")
        ...
        print("\n[3/4] Running NKI kernel (v10b)...")
        nki_out_dev, nki_k_rope_dev, nki_v_dev = qwen3_attn_tkg_fused_oproj_v10b[2](
        ...
        print(f"  nki_k_rope shape: {nki_k_rope.shape}")
        print(f"  nki_v shape: {nki_v.shape}")

        print("\n[4/4] Comparing results...")
        ref_f = ref_out.float()
        nki_f = nki_out.float()
        diff_out = (ref_f - nki_f).abs()
        ...
        import numpy as np
        try:
            np.testing.assert_allclose(nki_f.numpy(), ref_f.numpy(), rtol=1e-2, atol=1e-2)
            np.testing.assert_allclose(nki_k_rope.float().numpy(), ref_k_rope.float().numpy(), rtol=1e-2, atol=1e-2)
            np.testing.assert_allclose(nki_v.float().numpy(), ref_v.float().numpy(), rtol=1e-2, atol=1e-2)
            passed = True
        ...
        if max_diff < threshold and passed:
            ...
            print(f"{'='*70}")
            print(f"\nRef output sample:  {ref_f[0, 0, :8].tolist()}")
            print(f"NKI output sample:  {nki_f[0, 0, :8].tolist()}")
            return False
