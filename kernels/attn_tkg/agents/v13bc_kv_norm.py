...
PSUM_BANK_SIZE = 512  # fp32 elements per bank; 4 banks available on trn3
...
def qwen3_attn_tkg_fused_oproj_v13bc_kv_norm(
    ...
    # =========================================================================
    # Plan C target 2: Hoist wo_sbuf allocations + DMAs here (right after Wq loads).
    # wo_sbuf only depends on inputs; issuing early lets DMA overlap with the
    # entire pre_attn_norm + KV projection + Q projection + KV norm/RoPE chain.
    # =========================================================================
    ...
    # =========================================================================
    # WO WEIGHT HOISTING — moved earlier (see Plan C target 2 above)
    # =========================================================================
    ...
    for s_t in nl.affine_range(NUM_S_TILES):
        ...
        # Plan C target 4: fuse PSUM→SBUF copy into the mask add (tensor_tensor reads PSUM directly).
        mask_gqa = mask_gqa_tiles[s_t]
        nisa.tensor_tensor(score_sb_masked, score_psum, mask_gqa, op=nl.add)
    ...
    for s_t in nl.affine_range(NUM_S_TILES):
        ...
        # Plan C target 4: fuse PSUM->SBUF copy into the accumulation add.
        nisa.tensor_tensor(sum_acc, sum_acc, tile_sum_psum, op=nl.add)
        ...
        # Plan C target 4: fuse PSUM->SBUF copy into the accumulation add.
        nisa.tensor_tensor(v_acc, v_acc, v_weighted_psum, op=nl.add)
