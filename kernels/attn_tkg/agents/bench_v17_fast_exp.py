"""
Trn3 correctness + benchmark for v17_fast_exp (Plan B [X1]).

Mirrors bench_v13bc_trn3.py structure:
  - NEURON_PLATFORM_TARGET_OVERRIDE=trn3
  - LNC=2
  - trn3 skill's benchmark.py via wrap_benchmark
  - full BenchmarkResult fields reported

Correctness is checked against the PyTorch float32 reference in
bench_v13bc_sbm_tiled.py (reused). Tolerances: rtol=1e-2, atol=2e-2.
"""

import os
import sys

os.environ["NEURON_PLATFORM_TARGET_OVERRIDE"] = "trn3"
os.environ["NEURON_LOGICAL_NC_CONFIG"] = "2"
os.environ["NEURON_RT_INSPECT_ENABLE"] = "1"
os.environ["NEURON_RT_INSPECT_DEVICE_PROFILE"] = "1"
os.environ["NEURON_RT_INSPECT_OUTPUT_DIR"] = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "_bench_out_v17_fast_exp_trn3"
)

import shutil
_HERE = os.path.dirname(os.path.abspath(__file__))
shutil.copy(
    "/home/ubuntu/nki-moe/.claude/skills/nki-kernel-optimizer-trn3/scripts/benchmark.py",
    os.path.join(_HERE, "benchmark.py"),
)

sys.path.insert(0, "/home/ubuntu/nki-moe")
sys.path.insert(0, _HERE)

import math
import numpy as np
import torch
import nki
import nki.language as nl
import nki.isa as nisa
from nkilib.core.utils.allocator import SbufManager

from v17_fast_exp import qwen3_attn_tkg_fused_oproj_v13bc
from benchmark import wrap_benchmark

# ---------------------------------------------------------------------------
# Shape constants (identical to bench_v13bc_trn3.py / bench_v13bc_sbm_tiled.py)
# ---------------------------------------------------------------------------
B = 1
H = 2048
d = 128
Hq_tp = 8
GQA = 8
S_prior = 640
H_wo = 2048
PMAX = 128
NUM_OUT_COLS = H_wo // PMAX  # 16


@nki.jit
def attn_kernel_wrapper(
    hidden_states, Wq, Wk, Wv, Wo,
    q_norm_weight, k_norm_weight,
    K_cache, V_cache, cos, sin, position_ids,
):
    sbm = SbufManager(0, nl.tile_size.total_available_sbuf_size, use_auto_alloc=True)
    sbm.open_scope("wrapper_outer")

    out_sb, k_rope_out, v_out = qwen3_attn_tkg_fused_oproj_v13bc(
        hidden_states, Wq, Wk, Wv, Wo,
        q_norm_weight, k_norm_weight,
        K_cache, V_cache, cos, sin, position_ids,
        out_sb=None,
        sbm=sbm,
    )

    output_hbm = nl.ndarray((128, 16), dtype=nl.bfloat16, buffer=nl.shared_hbm)
    nisa.dma_copy(dst=output_hbm, src=out_sb, dge_mode=nisa.dge_mode.hwdge)

    sbm.close_scope()  # wrapper_outer
    sbm.close_scope()  # attn_outer (opened inside sub-function)

    return output_hbm, k_rope_out, v_out


# ---------------------------------------------------------------------------
# Reference (PyTorch float32) — copied from bench_v13bc_sbm_tiled.py
# ---------------------------------------------------------------------------
def reference_attn(hidden_states, Wq, Wk, Wv, Wo, q_norm_weight, k_norm_weight,
                   K_cache, V_cache, cos, sin, position_ids):
    B_r, _, H_r = hidden_states.shape
    d_r = 128; Hq_tp_r = 8; GQA_r = 8; S_prior_r = K_cache.shape[2]
    h = hidden_states.reshape(H_r, B_r).float()

    q = Wq.float() @ h
    k = Wk.float() @ h
    v = Wv.float() @ h

    k_ms = (k**2).mean(dim=0, keepdim=True) + 1e-6
    k = k * k_ms.rsqrt() * k_norm_weight.reshape(d_r, 1).float()

    half_d = d_r // 2
    cos_v = cos.reshape(d_r, B_r).float()
    sin_v = sin.reshape(d_r, B_r).float()
    rot_k = torch.cat([-k[half_d:], k[:half_d]], dim=0)
    k_rope = k * cos_v + rot_k * sin_v

    q_heads = q.reshape(Hq_tp_r, d_r, B_r)
    q_ms = (q_heads**2).mean(dim=1, keepdim=True) + 1e-6
    q_heads = q_heads * q_ms.rsqrt() * q_norm_weight.reshape(1, d_r, 1).float()

    cos_h = cos_v.unsqueeze(0).expand(Hq_tp_r, -1, -1)
    sin_h = sin_v.unsqueeze(0).expand(Hq_tp_r, -1, -1)
    rot_q = torch.cat([-q_heads[:, half_d:], q_heads[:, :half_d]], dim=1)
    q_rope = q_heads * cos_h + rot_q * sin_h
    q_scaled = q_rope / math.sqrt(d_r)

    pos = int(position_ids[0, 0])
    K_all = torch.cat([K_cache.reshape(S_prior_r, d_r).float().T, k_rope], dim=1)
    V_all = torch.cat([V_cache.reshape(S_prior_r, d_r).float().T, v], dim=1)

    attn_out = []
    for h_idx in range(Hq_tp_r):
        q_h = q_scaled[h_idx]
        scores = (K_all.T @ q_h)
        mask = torch.zeros(S_prior_r + 1, 1)
        mask[pos:S_prior_r] = -1e9
        scores = scores + mask
        weights = torch.softmax(scores, dim=0)
        out_h = V_all @ weights
        attn_out.append(out_h)

    attn_packed = torch.stack(attn_out, dim=1).reshape(d_r, Hq_tp_r)
    attn_flat = attn_packed.T.reshape(1, Hq_tp_r * d_r)
    out = attn_flat @ Wo.float()
    return out.reshape(B_r, 1, H_r)


def make_inputs_pair(seed=42):
    """Build CPU copies plus xla-device copies for the kernel."""
    torch.manual_seed(seed)
    device = "xla"

    hidden_states_cpu = torch.randn(B, 1, H, dtype=torch.bfloat16)
    Wq_cpu = torch.randn(Hq_tp * d, H, dtype=torch.bfloat16) * 0.02
    Wk_cpu = torch.randn(d, H, dtype=torch.bfloat16) * 0.02
    Wv_cpu = torch.randn(d, H, dtype=torch.bfloat16) * 0.02
    Wo_cpu = torch.randn(Hq_tp * d, H_wo, dtype=torch.bfloat16) * 0.02
    q_norm_weight_cpu = torch.ones(d, dtype=torch.bfloat16)
    k_norm_weight_cpu = torch.ones(d, dtype=torch.bfloat16)
    K_cache_cpu = torch.randn(B, 1, S_prior, d, dtype=torch.bfloat16) * 0.1
    V_cache_cpu = torch.randn(B, 1, S_prior, d, dtype=torch.bfloat16) * 0.1
    cos_cpu = torch.cos(torch.linspace(0, 1, d)).reshape(B, d).to(torch.bfloat16)
    sin_cpu = torch.sin(torch.linspace(0, 1, d)).reshape(B, d).to(torch.bfloat16)
    position_ids_cpu = torch.tensor([[S_prior // 2]], dtype=torch.int32)

    cpu = (hidden_states_cpu, Wq_cpu, Wk_cpu, Wv_cpu, Wo_cpu,
           q_norm_weight_cpu, k_norm_weight_cpu,
           K_cache_cpu, V_cache_cpu, cos_cpu, sin_cpu, position_ids_cpu)
    xla = tuple(t.to(device) for t in cpu)
    return cpu, xla


def main():
    cpu, xla = make_inputs_pair()

    # ---------- Correctness ----------
    print("Running v17_fast_exp kernel for correctness...")
    output_hbm, k_rope_out, v_out = attn_kernel_wrapper(*xla)

    result = output_hbm.cpu().float().numpy()  # [128, 16]

    print("Computing reference...")
    ref = reference_attn(*cpu).float().numpy()

    print(f"Result shape: {result.shape}, ref shape: {ref.shape}")
    result_flat = result.flatten(order="F")   # column-major: [p, j] -> j*128+p
    ref_flat = ref.reshape(-1)

    max_abs_err = float(np.abs(result_flat - ref_flat).max())
    mean_abs_err = float(np.abs(result_flat - ref_flat).mean())
    print(f"Max abs error: {max_abs_err:.6e}")
    print(f"Mean abs error: {mean_abs_err:.6e}")

    np.testing.assert_allclose(result_flat, ref_flat, rtol=1e-2, atol=2e-2)
    print("CORRECTNESS PASSED (rtol=1e-2, atol=2e-2)")

    # ---------- Benchmark ----------
    print("\nRunning v17_fast_exp benchmark (trn3)...")
    bench = wrap_benchmark(attn_kernel_wrapper, warmup=5, iters=50)
    bench(*xla)

    r = bench.last_result
    if r and r.prof:
        prof = r.prof
        def pct(key): return prof.get(key, 0) * 100
        print("\n=== v17_fast_exp Attention Benchmark (trn3) ===")
        print(f"device_time_us       = {r.device_time_us:.2f}")
        print(f"tensor_engine_pct    = {pct('tensor_engine_active_time_percent'):.2f}%")
        print(f"vector_engine_pct    = {pct('vector_engine_active_time_percent'):.2f}%")
        print(f"scalar_engine_pct    = {pct('scalar_engine_active_time_percent'):.2f}%")
        print(f"gpsimd_engine_pct    = {pct('gpsimd_engine_active_time_percent'):.2f}%")
        print(f"dma_active_pct       = {pct('dma_active_time_percent'):.2f}%")
        print(f"spill_bytes          = {r.spill_bytes}")
        print(f"mfu_estimated_pct    = {prof.get('mfu_estimated_percent', 0):.4f}%")
        print(f"mbu_estimated_pct    = {prof.get('mbu_estimated_percent', 0):.4f}%")
        print(f"mm_arithmetic_intensity = {prof.get('mm_arithmetic_intensity', 0):.3f}")
        print(f"hbm_read_KiB         = {prof.get('hbm_read_bytes', 0)/1024:.1f}")
        print(f"hbm_write_KiB        = {prof.get('hbm_write_bytes', 0)/1024:.1f}")
        print(f"cc_op_count          = {prof.get('cc_op_count', 0)}")
        print(f"cc_op_active_time_us = {prof.get('cc_op_active_time', 0)*1e6:.2f}")
        print(f"max_abs_err          = {max_abs_err:.6e}")
    else:
        print("No profile data")


if __name__ == "__main__":
    main()
