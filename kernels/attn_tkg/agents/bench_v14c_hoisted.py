"""
bench_v14c_hoisted.py — Benchmark v14c in hoisted mode (all *_f32_sb provided).
"""
import os, sys
...
os.environ["NEURON_RT_INSPECT_OUTPUT_DIR"] = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "_bench_out_v14c_hoisted"
)
...
def v14c_hoisted_wrapper(hidden_states, Wq, Wk, Wv, Wo, q_norm_weight, k_norm_weight,
    ...
    # Pre-load all 5 layer-invariant constants outside the kernel
...
v14c_hoisted_wrapper = wrap_benchmark(v14c_hoisted_wrapper[2], warmup=5, iters=50)
...
v14c_hoisted_wrapper(hidden, Wq, Wk, Wv, Wo, qnw, knw, gpan,
                     K_cache, V_cache, cos, sin, pos, output)
xm.mark_step()

r_res = v14c_hoisted_wrapper.last_result
if r_res:
