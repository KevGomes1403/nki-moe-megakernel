"""
v14_planG: v13_BC + use nisa.dma_transpose for K-cache loading.

Changes from v13_BC:
  Plan G: Replace 3-op K-cache load sequence (dma_copy + nc_transpose + tensor_copy)
    with a single nisa.dma_transpose call.
    - Eliminates 10 TensorE instructions (2 nc_transpose x 5 tiles).
    - DMA transpose loads HBM->SBUF with simultaneous transpose:
      dst[feature, token] = K_cache[tile_start+token, feature]
    - This produces identical layout to the old nc_transpose path.
...
@nki.jit
def qwen3_attn_tkg_fused_oproj_v14g(
    ...
    # Plan G: reshape to 4D for dma_transpose (hardware requires 4D src).
    K_cache_4d = K_cache.reshape((S_prior, 1, 1, d))  # [S_prior, 1, 1, 128]
    ...
    for s_t in nl.affine_range(NUM_S_TILES):
        # Plan G: single dma_transpose replaces (dma_copy + nc_transpose + tensor_copy).
        # dma_transpose requires 4D src. Use K_cache_4d[s_t*PMAX:(s_t+1)*PMAX, 0:1, 0:1, :].
        # Permutation [3,1,2,0]: dst[d, 0, 0, s] = src[s, 0, 0, d] = K_cache[tile+s, d].
        # dst.reshape([PMAX, PMAX]) gives k_ct[feature, token] — identical to nc_transpose path.
