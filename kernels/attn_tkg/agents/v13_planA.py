"""
v13_planA: Hoist all Wo DMAs to kernel prologue (Plan A).

Changes from v12e:
  Plan A — Wo DMA hoisted to prologue:
    - Wo_reshaped reshape and the entire wo_sbuf DMA loop are moved to immediately
      after the wq_heads DMA loop (kernel prologue), instead of just before flash
      decode (Plan D position in v12d/v12e).
    - All 16x512KB weight DMAs (Wq + Wo = 8MB) are now issued at the very start
      of the kernel, allowing the DMA engine to pipeline all 16 transfers in
      parallel with k-proj, v-proj, q-proj, RMSNorm, RoPE, and flash decode compute.
    - No compute changes — correctness is identical to v12e (bit-for-bit).
    - Expected effect: better DMA/compute overlap -> lower device_time_us.

All v12e optimizations are preserved:
  v12e causal masking fix, Plan D loop order (head-outer/h_blk-inner o-proj),
  Wq pre-load (8x512KB DMA), early Wv DMA, Plan A static shape constants,
  Plan B K-cache contiguous load + PE transpose, Q-proj psum->q_packed_f32 direct
  copy, all tp_broadcast patterns, Plan B saved scores, hidden tile hoisting,
...
def qwen3_attn_tkg_fused_oproj_v13a(
    ...
    v13_planA changes (extends v12e):
      - Wo_reshaped reshape and wo_sbuf DMA loop moved to kernel prologue (after wq_heads
        loop), so all 16x512KB weight DMAs (Wq + Wo = 8MB) are issued at the very start.
      - No compute changes; correctness is bit-identical to v12e.
    """
    # --- Dimensions ---
    ...
    # =========================================================================
    # WO WEIGHT RESHAPE + SBUF HOISTING (Plan A: moved to prologue)
    #
    ...
    #
    # Plan A change: the wo_sbuf DMA loop is issued HERE (immediately after wq_heads),
    # so all 16x512KB weight DMAs (Wq + Wo = 8MB) are in flight from the very start.
    # The DMA engine can pipeline all 16 transfers in parallel with k-proj, v-proj,
    # q-proj, RMSNorm, RoPE, and flash decode compute.
    #
    ...
    for head in nl.affine_range(HQ_TP_CONST):
        ...
        nisa.dma_copy(
            ...
            src=Wo_reshaped.ap(
                ...
                offset=head * PMAX * H_wo,             # skip head * [128 x 2048] elements
            ),
        )
