...
os.environ["NEURON_RT_INSPECT_OUTPUT_DIR"] = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "_bench_out_planABC"
)
...
v14b_wrapper = wrap_benchmark(v14b_wrapper, warmup=5, iters=50)
...
hidden = r(B,1,H).to(device); Wq = r(Hq_tp*d,H).to(device); Wk = r(d,H).to(device)
...
v14b_wrapper(hidden,Wq,Wk,Wv,Wo,qnw,knw,gpan,K_cache,V_cache,cos,sin,pos,output)
xm.mark_step()

r_res = v14b_wrapper.last_result
if r_res:
