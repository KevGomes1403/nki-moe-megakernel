"""
Correctness test for v14c_kv_norm_hoisted.

Tests two modes:
  Mode 1 — Back-compat:  all *_f32_sb kwargs = None  →  must match PyTorch reference
  Mode 2 — Hoisted:      caller pre-loads qnw/knw/cos/sin/gpan into SBUF,
                         passes them in  →  must produce identical results

Wq, Wk, Wv are tile-transposed before being passed to the kernel (same as v14b).
Tolerance: rtol=1e-2, atol=1e-2 (fp32-promoted reference).

Run:
    NKI_PRECISE_FP=1 NEURON_PLATFORM_TARGET_OVERRIDE=trn3 \\
        python test_v14c_sim.py
"""
...
from v14c_kv_norm_hoisted import qwen3_attn_tkg_fused_oproj_v13bc_kv_norm_hoisted
...
def tile_transpose_weight(W_np, n_heads, d, n_tiles):
    """
    W_np: [n_heads*d, n_tiles*d] float32/bf16 numpy array
    Returns: same shape, tile-transposed layout
      W_pt[h*d+p, t*d+f] = W[h*d+f, t*d+p]
    """
    W_f32 = np.array(W_np, dtype=np.float32)
    W_pt_f32 = (W_f32.reshape(n_heads, d, n_tiles, d)
                      .transpose(0, 3, 2, 1)  # permute(0,3,2,1)
...
# ---------------------------------------------------------------------------
# Mode 1 — Back-compat wrapper: all *_f32_sb = None
# ---------------------------------------------------------------------------
@nki.jit
def v14c_backcompat_wrapper(
    ...
    # All *_f32_sb = None → back-compat mode (identical to v14b)
    out_sb = qwen3_attn_tkg_fused_oproj_v13bc_kv_norm_hoisted(
        ...
        sbm=sbm,
        # All pre-loaded tensors = None → full HBM load path
        qnw_f32_sb=None,
        knw_f32_sb=None,
        cos_f32_sb=None,
        sin_f32_sb=None,
...
# ---------------------------------------------------------------------------
# Mode 2 — Hoisted wrapper: caller pre-loads all 5 constants into SBUF
# ---------------------------------------------------------------------------
@nki.jit
def v14c_hoisted_wrapper(
    hidden_states, Wq, Wk, Wv, Wo,
    q_norm_weight, k_norm_weight, gamma_pre_attn,
    K_cache, V_cache, cos, sin, position_ids, output,
):
    ...
    # -------------------------------------------------------------------------
    # Pre-load the 5 constants — exactly the same code that was inside v14b,
    # now done in the caller scope before the kernel call.
    # -------------------------------------------------------------------------
    cos_col = cos.reshape((PMAX, B))
    sin_col = sin.reshape((PMAX, B))

    # q_norm_weight → qnw_f32_sb
    ...
    # -------------------------------------------------------------------------
    # Call v14c with all 5 pre-loaded tensors — the kernel skips those DMAs
    # -------------------------------------------------------------------------
    out_sb = qwen3_attn_tkg_fused_oproj_v13bc_kv_norm_hoisted(
        hidden_sb=hidden_sb,
        Wq=Wq, Wk=Wk, Wv=Wv, Wo=Wo,
        q_norm_weight=q_norm_weight,   # ignored when *_f32_sb provided
        k_norm_weight=k_norm_weight,   # ignored
        gamma_pre_attn=gamma_pre_attn, # ignored
        K_cache=K_cache, V_cache=V_cache,
        cos=cos, sin=sin,              # ignored
        position_ids=position_ids,
        sbm=sbm,
        # Pre-loaded f32 SBUF tensors — kernel uses these directly
        qnw_f32_sb=qnw_f32_h,
        knw_f32_sb=knw_f32_h,
        cos_f32_sb=cos_f32_h,
        sin_f32_sb=sin_f32_h,
        gpan_f32_sb=gpan_f32_h,
    )
...
# ---------------------------------------------------------------------------
# Reference (uses original untransposed weights)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
def run_one(pos_val, weights, hidden_rng_seed, mode="backcompat"):
    """
    mode: "backcompat" | "hoisted"
    Returns result dict with pass/fail flags and max_diff values.
    """
    ...
    # Pre-transpose Wq, Wk, Wv for the kernel (same as v14b)
    ...
    # Choose kernel wrapper
    if mode == "backcompat":
        wrapper = nki.simulate(v14c_backcompat_wrapper)
    else:
        wrapper = nki.simulate(v14c_hoisted_wrapper)
...
# Main
# ---------------------------------------------------------------------------
def run_suite(mode, weights, positions):
    import logging
    logging.getLogger("SBM").setLevel(logging.WARNING)

    results = []
    for i, p in enumerate(positions):
        print(f"\n=== [{mode}] position_ids={p} ===", flush=True)
        res = run_one(pos_val=p, weights=weights, hidden_rng_seed=100 + i, mode=mode)
        results.append(res)
        ok_parts = [
...
def print_summary(results, mode, atol=1e-2):
    print(f"\n\n======== SUMMARY [{mode}] ========")
    ...
    n = len(results)
    print(f"\n{n - fail}/{n} positions PASS  (atol={atol})  [{mode}]")
    return fail
...
def main():
    ...
    # ---- Mode 1: back-compat (all *_f32_sb = None) ----
    print("\n" + "="*60)
    print("  Mode 1: Back-compat (all *_f32_sb = None)")
    print("="*60)
    results_bc = run_suite("backcompat", weights, positions)
    fail_bc = print_summary(results_bc, "backcompat")
    total_fail += fail_bc

    # ---- Mode 2: hoisted (all 5 constants pre-loaded) ----
    print("\n" + "="*60)
    print("  Mode 2: Hoisted (all *_f32_sb provided)")
    print("="*60)
    results_h = run_suite("hoisted", weights, positions)
    fail_h = print_summary(results_h, "hoisted")
    total_fail += fail_h

    print("\n" + "="*60)
    print(f"  OVERALL: {'PASS' if total_fail == 0 else 'FAIL'}")
    print(f"  Back-compat failures: {fail_bc}/{len(positions)}")
    print(f"  Hoisted failures:     {fail_h}/{len(positions)}")
    print("="*60)
