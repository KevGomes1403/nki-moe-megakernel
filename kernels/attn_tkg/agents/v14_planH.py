"""
v14_planH: v13_BC with K and V cache hoisting merged into a single affine_range loop.

Changes from v13_BC:
  Plan H: Merge K-cache and V-cache hoisting into one loop.
    - In v13_BC, K-cache and V-cache are loaded in separate affine_range loops.
    - In v14_planH, both are loaded in a single loop.
    - V-cache DMA is issued BEFORE K-cache DMA and nc_transpose compute, so
      the DMA engine has maximum time to prefetch the V tile while K transpose runs.
    - This gives the compiler the opportunity to pipeline K and V DMA loads for
      the same tile together, potentially improving DMA throughput.

All v13_BC optimizations are preserved unchanged.
"""
...
os.environ["NEURON_RT_INSPECT_OUTPUT_DIR"]= "./_bench_out_v14h"

os.environ["NEURON_PLATFORM_TARGET_OVERRIDE"] = "trn2"
os.environ["NEURON_LOGICAL_NC_CONFIG"] = "2"

@nki.jit
def qwen3_attn_tkg_fused_oproj_v14h(
    ...
    # =========================================================================
    # =========================================================================
    # Plan H: Merged K-cache + V-cache hoisting in a single affine_range loop.
    # V-cache DMA is issued FIRST (before K DMA and nc_transpose) so the DMA
    # engine can prefetch the V tile while K transpose runs on TensorE.
    # This gives the compiler the opportunity to pipeline K and V DMAs together.
    # =========================================================================
    k_cache_tiles = []
    ...
    for s_t in nl.affine_range(NUM_S_TILES):
        # V-cache DMA issued FIRST — gives max time to prefetch while K transpose runs
