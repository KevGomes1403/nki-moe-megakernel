"""
v10c: Extends v10b by adding KV cache zero-padding mask support.

Changes from v10b:
  - Adds attn_mask parameter [S_prior, 1] bf16 — additive score bias:
      0.0 for valid positions, -1e9 for zero-padded positions.
  - In Pass 1, loads a per-tile slice of attn_mask, broadcasts it to
    [PMAX, GQA], and adds it to score_sb before saving/using the score.
  - This ensures padded KV cache positions (K=0 → score=0 → exp(0)=1)
    are masked to -1e9 so exp(-1e9 - max) ≈ 0, eliminating denominator
    inflation when valid_seq_len < S_prior.
  - The active (new) token score is NOT masked — new tokens are always valid.
  - All existing optimizations from v10b are preserved unchanged.

Parameters:
  attn_mask: [S_prior, 1] bf16 — additive score bias tensor.
    Build as: mask[i] = 0.0 if i < valid_seq_len else -1e9

Preserved unchanged from v10b:
...
@nki.jit
def qwen3_attn_tkg_fused_oproj_v10c(
    ...
    attn_mask,       # [S_prior, 1]     bf16 — additive score bias: 0.0 valid, -1e9 padded
):
    """
    Fused QKV + RMSNorm + RoPE + flash decode + output projection with KV cache mask.
    ...
    attn_mask [S_prior, 1] bf16: additive bias applied to KV-cache scores before
    softmax. Set to 0.0 for valid positions and -1e9 for zero-padded positions to
    prevent inflated softmax denominators when valid_seq_len < S_prior.
    """
    # --- Dimensions ---
    ...
    attn_mask_2d = attn_mask.reshape((S_prior, 1))   # HBM [S_prior, 1] bf16
    ...
    for s_t in nl.affine_range(NUM_S_TILES):
        ...
        # Load per-tile additive mask [PMAX, 1] bf16 from HBM
        mask_tile_bf16 = nl.ndarray((PMAX, 1), dtype=nl.bfloat16, buffer=nl.sbuf, name=f"mask_tile_bf16_{s_t}")
        nisa.dma_copy(dst=mask_tile_bf16, src=attn_mask_2d[s_t * PMAX:(s_t + 1) * PMAX, :])
        mask_tile_f32 = nl.ndarray((PMAX, 1), dtype=nl.float32, buffer=nl.sbuf, name=f"mask_tile_f32_{s_t}")
        nisa.tensor_copy(mask_tile_f32, mask_tile_bf16)
