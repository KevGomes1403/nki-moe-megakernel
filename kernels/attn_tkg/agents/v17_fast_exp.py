"""
v17_fast_exp: Plan B [X1] variant of v13bc_sbm_tiled — nisa.exponential in softmax.

Only change from v13bc_sbm_tiled:
  - The two softmax exp() call sites in Pass 2 now use nisa.exponential (Vector Engine
    fast-exp, 4x throughput on trn3/NeuronCore-v4) instead of
    nisa.activation(..., op=nl.exp).
  - Because the per-head max varies along the GQA free dim (not the partition dim),
    we cannot use the max_value= broadcast form (nisa.exponential requires max_value
    of shape (src.shape[0], 1) which would broadcast across the free dim). We keep
    the explicit `score + neg_max` subtract and pass max_value=0.0 (the scalar
    default). nisa.exponential still gives the 4x VectorE throughput win vs
    nisa.activation(op=nl.exp).
  - All other optimizations (SBUF hoisting, DMA optimizations, tiled output layout,
    Wq/Wo hoisting, kv_hoist scope, two-pass flash decode structure) are unchanged.
  - Exported symbol name preserved: qwen3_attn_tkg_fused_oproj_v13bc (for bench import).
"""
...
def qwen3_attn_tkg_fused_oproj_v13bc(
    ...
    out_sb=None,     # optional [PMAX, H_wo//PMAX] = [128, 16] bf16 sbuf tensor; alloc_stack'd if None
    ...
    for s_t in nl.affine_range(NUM_S_TILES):
        ...
        # Plan B [X1]: fast exp on VectorE (4x throughput vs nisa.activation op=nl.exp).
        # max_value omitted (defaults to 0.0) — per-head max is already pre-subtracted
        # into score2_shifted since it varies along the GQA free dim.
