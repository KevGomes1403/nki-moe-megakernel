"""
Correctness test for v14b_kv_norm_pretransposed.

Wq, Wk, Wv are tile-transposed before being passed to the kernel:
  W_pt = W.reshape(n_heads, d, n_tiles, d).permute(0,3,2,1).reshape(n_heads*d, H)

The PyTorch reference always uses the original (non-transposed) weights.
Tolerance: rtol=1e-2, atol=1e-2.

Run:
    NKI_PRECISE_FP=1 NEURON_PLATFORM_TARGET_OVERRIDE=trn3 \\
        python test_v14b_kv_norm_pretransposed_sim.py
"""
...
from v15b_oproj_collapse import qwen3_attn_tkg_fused_oproj_v15b_oproj_collapse as qwen3_attn_tkg_fused_oproj_v13bc_kv_norm_pretransposed
...
def tile_transpose_weight(W_np, n_heads, d, n_tiles):
    ...
    """
    # Work in float32 numpy throughout; cast to bfloat16 at the end
...
# ---------------------------------------------------------------------------
@nki.jit
def v14b_pretransposed_wrapper(
    ...
    out_sb = qwen3_attn_tkg_fused_oproj_v13bc_kv_norm_pretransposed(
        hidden_sb=hidden_sb,
...
def run_one(pos_val, weights, hidden_rng_seed):
    ...
    # Pre-transpose Wq, Wk, Wv for the kernel
    ...
    result_np, K_out_np, V_out_np = nki.simulate(v14b_pretransposed_wrapper)(
