"""
Correctness test for v14d_kv_norm_hoisted_weights — back-compat mode.

Test A: invoke v14d with ALL new kwargs (wk_sb, wv_sb, wq_heads_sb, wo_heads_sb)
left at default None.  Output must match v14c output / PyTorch reference within
kernel-level tolerance: atol=1e-2, rtol=1e-2 (fp32-promoted reference).

Run:
    NKI_PRECISE_FP=1 NEURON_PLATFORM_TARGET_OVERRIDE=trn2 \\
        python test_v14d_backcompat_sim.py
"""
...
# ---------------------------------------------------------------------------
# Back-compat wrapper: all new weight kwargs = None
# ---------------------------------------------------------------------------
@nki.jit
def v14d_backcompat_wrapper(
    ...
    # All kwargs = None — back-compat mode
    out_sb = v14d_kernel(
        hidden_sb=hidden_sb,
        ...
        # New v14d weight kwargs — all None for back-compat
...
def run_one(pos_val, weights, hidden_rng_seed):
    ...
    wrapper = nki.simulate(v14d_backcompat_wrapper)
...
def main():
    ...
    print("\n" + "="*60)
    print("  Test A: v14d back-compat mode (all weight kwargs = None)")
    print("="*60)
