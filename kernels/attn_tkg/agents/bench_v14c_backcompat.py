"""
bench_v14c_backcompat.py — Benchmark v14c in back-compat mode (all *_f32_sb=None).
Run this script first, then bench_v14c_hoisted.py, then bench_v14b_ref.py.
"""
import os, sys
...
os.environ["NEURON_RT_INSPECT_OUTPUT_DIR"] = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "_bench_out_v14c_backcompat"
)
...
v14c_backcompat_wrapper = wrap_benchmark(v14c_backcompat_wrapper[2], warmup=5, iters=50)
...
v14c_backcompat_wrapper(hidden, Wq, Wk, Wv, Wo, qnw, knw, gpan,
                        K_cache, V_cache, cos, sin, pos, output)
xm.mark_step()

r_res = v14c_backcompat_wrapper.last_result
if r_res:
