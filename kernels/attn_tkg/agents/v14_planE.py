"""
v14_planE: v13_BC + fused mask computation (Plan E).

Changes from v13_BC:
  Plan E: Fuse mask computation to reduce ScalarE instructions.
    - Per-tile mask computation reduced from 6 ops to 4 ops (saves 10 ops total for 5 tiles).
    - Fusion 1: Fold the +1.0 shift (Op 3) into Op 1 (neg_threshold).
      neg_threshold_shifted = pos_f32 * (-1.0) + (tile_start + 1.0)
      delta_eps = par_index + neg_threshold_shifted  (delta and delta_eps steps merged)
    - Fusion 2: Combine clamp (Op 5) + scale (Op 6) into a single 2-op tensor_scalar.
      mask_tile_f32 = min(relu_delta, 1.0) * (-1e9)
    - Removed intermediate buffers: neg_threshold, delta, clamped.
    - Net: 6 ops → 4 ops per tile × 5 tiles = 10 fewer ScalarE instructions.

All v13_BC optimizations preserved unchanged.
"""
...
@nki.jit
def qwen3_attn_tkg_fused_oproj_v14e(
    ...
    for s_t in nl.affine_range(NUM_S_TILES):
        ...
        # Plan E Fusion 1: Fold the +1.0 boundary shift (Op 3) into Op 1.
        # neg_threshold_shifted = pos_f32 * (-1.0) + (tile_start + 1.0)
        # delta_eps[p] = p + neg_threshold_shifted = p + (tile_start + 1 - pos)
        #   p < pos-tile_start (valid):  delta_eps < 1 → relu≤0 if <1, but +1 shift means:
        #   delta=-1 (last valid): neg_thresh_shifted = tile_start+1-pos → delta_eps=0 → relu=0 ✓
        #   delta= 0 (boundary):  delta_eps=1 → relu=1 → mask=-1e9 ✓
        #   delta>0  (future):    delta_eps≥2 → relu≥2 → clamp=1 → mask=-1e9 ✓
        # Eliminates separate delta and delta_eps tensor_scalar ops (saves 1 ScalarE per tile).
        neg_threshold_shifted = nl.ndarray((1, 1), dtype=nl.float32, buffer=nl.sbuf, name=f"neg_thresh_shifted_{s_t}")
        nisa.tensor_scalar(neg_threshold_shifted, pos_f32,
                           op0=nl.multiply, operand0=-1.0,
                           op1=nl.add, operand1=float(tile_start + 1))

        # Op 2a: broadcast neg_threshold_shifted [1,1] → [PMAX,1] using nc_transpose pattern.
        neg_thresh_psum = nl.ndarray((PMAX, 1), dtype=nl.float32, buffer=nl.psum, name=f"neg_thresh_psum_{s_t}")
        nisa.nc_transpose(neg_thresh_psum, neg_threshold_shifted.ap([[1, 1], [0, PMAX]], offset=0))
        ...
        # Op 2b (fused): delta_eps = par_index + neg_thresh_shifted (already includes +1 shift)
        delta_eps = nl.ndarray((PMAX, 1), dtype=nl.float32, buffer=nl.sbuf, name=f"delta_eps_{s_t}")
        nisa.tensor_tensor(delta_eps, par_index_f32, neg_thresh_sb, op=nl.add)

        # Op 4: relu (input is already +1 shifted, so same semantics as before)
        ...
        # Plan E Fusion 2: Combine clamp [0,1] (Op 5) + scale to -1e9 (Op 6) into one
        # two-op tensor_scalar, eliminating the intermediate clamped buffer.
        mask_tile_f32 = nl.ndarray((PMAX, 1), dtype=nl.float32, buffer=nl.sbuf, name=f"mask_tile_f32_{s_t}")
        nisa.tensor_scalar(mask_tile_f32, relu_delta, op0=nl.minimum, operand0=1.0, op1=nl.multiply, operand1=-1e9)
