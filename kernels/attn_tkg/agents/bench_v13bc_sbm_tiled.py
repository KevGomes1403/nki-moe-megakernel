"""
Correctness + benchmark harness for qwen3_attn_tkg_fused_oproj_v13bc tiled output variant.
Output layout: [128, 16] = [PMAX, H_wo//PMAX] bf16 in SBUF, DMA'd to HBM as [128, 16].
Element mapping: out_hbm[p, j] = linear_out[j * 128 + p]  (column-major).
"""
...
def attn_kernel_wrapper(
    ...
    # out_sb is now [PMAX, H_wo//PMAX] = [128, 16] in SBUF
...
def main():
    ...
    # Correctness check — remap [128, 16] column-major to flat [2048] for comparison
    ...
    # result is [128, 16]; column-major flatten gives linear_out ordering
    result_flat = result.flatten(order='F')   # [p, j] -> j*128+p ordering
    ref_flat = ref.reshape(-1)                # [1, 1, 2048] -> [2048]

    max_abs_err = np.abs(result_flat - ref_flat).max()
    mean_abs_err = np.abs(result_flat - ref_flat).mean()
