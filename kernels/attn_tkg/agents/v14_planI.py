"""
v14_planI: v13_BC + stride-0 DMA broadcast for cos_gqa, sin_gqa, and qnw_gqa.

Changes from v13_BC:
  Plan I: Eliminate double-nc_transpose broadcast sequences for cos_gqa, sin_gqa,
  and qnw_gqa by loading [PMAX, GQA] directly from HBM using stride-0 in the free
  dimension.

  Old (6 instructions per tensor): dma_copy + tensor_copy + nc_transpose + tensor_copy
    + nc_transpose + tensor_copy = 6 instructions x 3 tensors = 18 instructions total
    (for Q RoPE broadcast sequences only).

  New (2 instructions per tensor): dma_copy with ap([[1,PMAX],[0,GQA]]) from HBM
    + tensor_copy for f32 cast = 2 x 3 = 6 instructions total.

  Net savings: 12 instructions eliminated.

  cos_f32 and sin_f32 [PMAX,1] are preserved for K RoPE (unchanged).
  qnw_sb [PMAX,1] is preserved but qnw_gqa now loaded directly via stride-0 DMA.
"""
...
@nki.jit
def qwen3_attn_tkg_fused_oproj_v14i(
    ...
    # Plan I: stride-0 DMA broadcast qnw [PMAX,1] -> [PMAX,GQA] directly from HBM.
    # ap([[1,PMAX],[0,GQA]]): partition stride=1 (PMAX elements), free stride=0 (repeat GQA times).
    qnw_gqa_bf16 = nl.ndarray((PMAX, GQA), dtype=nl.bfloat16, buffer=nl.sbuf, name="qnw_gqa_bf16")
    nisa.dma_copy(dst=qnw_gqa_bf16, src=q_norm_weight.reshape((PMAX, 1)).ap([[1, PMAX], [0, GQA]], offset=0), dge_mode=nisa.dge_mode.hwdge)
    qnw_gqa = nl.ndarray((PMAX, GQA), dtype=nl.float32, buffer=nl.sbuf, name="qnw_gqa")
    nisa.tensor_copy(qnw_gqa, qnw_gqa_bf16)
    ...
    # =========================================================================
    # Plan I: stride-0 DMA broadcast cos/sin [PMAX,1] -> [PMAX,GQA] directly from HBM.
    # ap([[1,PMAX],[0,GQA]]): partition stride=1 (PMAX elements), free stride=0 (repeat GQA times).
    # cos_col = cos.reshape((PMAX, B)) = [128, 1]; stride-0 in free broadcasts to [PMAX, GQA].
    cos_gqa_bf16 = nl.ndarray((PMAX, GQA), dtype=nl.bfloat16, buffer=nl.sbuf, name="cos_gqa_bf16")
    nisa.dma_copy(dst=cos_gqa_bf16, src=cos_col.ap([[1, PMAX], [0, GQA]], offset=0), dge_mode=nisa.dge_mode.hwdge)
    cos_gqa = nl.ndarray((PMAX, GQA), dtype=nl.float32, buffer=nl.sbuf, name="cos_gqa")
    nisa.tensor_copy(cos_gqa, cos_gqa_bf16)

    sin_gqa_bf16 = nl.ndarray((PMAX, GQA), dtype=nl.bfloat16, buffer=nl.sbuf, name="sin_gqa_bf16")
    nisa.dma_copy(dst=sin_gqa_bf16, src=sin_col.ap([[1, PMAX], [0, GQA]], offset=0), dge_mode=nisa.dge_mode.hwdge)
    sin_gqa = nl.ndarray((PMAX, GQA), dtype=nl.float32, buffer=nl.sbuf, name="sin_gqa")
    nisa.tensor_copy(sin_gqa, sin_gqa_bf16)
