"""
v13_BC: Round 2 baseline combining Plan B (hw_dge on all dma_copy) and
Plan C (pre-broadcast mask tiles during K-cache hoisting).

Changes from v12e:
  Change B: hw_dge on all dma_copy calls.
    - Add dge_mode=nisa.dge_mode.hwdge to every nisa.dma_copy() call.
    - Enables hardware data-gathering engine for all DMA transfers.

  Change C: Pre-broadcast mask [PMAX,1] -> [PMAX,GQA] during K-cache hoisting loop.
    - The 4-instruction broadcast block in Pass 1 is moved into the hoisting loop,
      so mask_gqa is computed once during hoisting and reused in Pass 1.
    - Pass 1 replaces the 4-instruction broadcast with a single assignment:
