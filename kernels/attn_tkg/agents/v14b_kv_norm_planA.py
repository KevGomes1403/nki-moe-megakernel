"""
v14b_kv_norm: v14a_kv_norm with corrected active K/V quantization alignment.

Key fixes over v14a:
  1. Active K score uses bf16-round-tripped k_rope instead of float32 k_rope.
     The reference writes new_k = k_rope.bfloat16() to cache and reads float32 back.
     Using raw k_rope (float32) missed this quantization, causing ~4 ULP error.
  2. Active V uses bf16-round-tripped v_active instead of float32 v_active.
     Same reason: reference writes new_v = v.bfloat16() to cache.

Also includes LNC=2 sharding (Q/Wo split across cores, KV on both cores,
cross-core all-reduce on out_sb via sendrecv).
...
def qwen3_attn_tkg_fused_oproj_v13bc_kv_norm(
    ...
    # Move reshapes to attn_outer level for early K/V DMA
    K_cache_2d = K_cache.reshape((S_prior, d))
    V_cache_2d = V_cache.reshape((S_prior, d))

    # =========================================================================
    # EARLY K/V CACHE DMA — issued before projections to overlap with Wq/Wo loads
    # dma_transpose for K (transposes during load, free DMA engine)
    # dma_copy with AP stride for V (no transpose needed for V in nc_matmul)
    # =========================================================================
    k_cache_tiles_hbm = []
    v_cache_tiles_hbm = []
    ...
    # =========================================================================
    # LNC weight sharding setup
    # In LNC=2 mode, each core loads only 4 of the 8 Q heads (halving Wq/Wo DMA).
    # Core 0 -> heads [0, 4), Core 1 -> heads [4, 8). Cross-core sum via nccl.all_reduce on out_sb.
    # In LNC=1 mode, one core does all 8 heads (Plan A fallback).
    # =========================================================================
    n_prgs = nl.num_programs(0)
    prg_id = nl.program_id(0)

    if n_prgs == 1:
        owned_heads = [0, 1, 2, 3, 4, 5, 6, 7]      # all heads on single core
    elif prg_id == 0:
        owned_heads = [0, 1, 2, 3]                   # core 0: heads 0..3
    else:
        owned_heads = [4, 5, 6, 7]                   # core 1: heads 4..7
    ...
    # LNC sharding: only owned heads are loaded on each core. Non-owned slots left as None.
    ...
    # FIX 1: bf16-round-tripped k_rope for active K score (matches cache quantization)
    k_rope_bf16_f32 = sbm.alloc_stack((PMAX, B), nl.float32, name="k_rope_bf16_f32")

    # FIX 2: bf16-round-tripped v for active V contribution (matches cache quantization)
    v_from_bf16 = sbm.alloc_stack((PMAX, B), nl.float32, name="v_from_bf16")

    # wo_sbuf tiles (attn_outer level) — LNC-sharded: only owned heads get allocations
    ...
    # FIX 1: Round-trip k_rope through bf16 to match the cache quantization path.
    # The reference does new_k = k_rope.bfloat16() then reads float32 from cache.
    ...
    # FIX 2: Round-trip v through bf16 to match the cache quantization path.
    # The reference does new_v = v.bfloat16() then reads float32 from cache.
    ...
    # LNC sharded: each core only loads Wo rows for its owned Q heads.
    # =========================================================================
    for head in owned_heads:
        nisa.dma_copy(
            dst=wo_sbuf[head],
    ...
    # =========================================================================
    # K_cache_2d and V_cache_2d already reshaped at attn_outer level above.

    # FIX 1: Active position score uses bf16-round-tripped k_rope to match cache quantization.
    # k_rope_bf16_f32 = float32(k_rope_bf16) was computed in kv_proj scope above.
    ...
    for s_t in nl.affine_range(NUM_S_TILES):
        # k_ct and v_ct already DMA-loaded at attn_outer level
    ...
    # FIX 2: Active V broadcast uses bf16-round-tripped v_from_bf16 instead of float32 v_active.
    # v_from_bf16 = float32(v_bf16) was computed in kv_proj scope above.
    ...
    # LNC sharded: accumulate partial o_proj over this core's 4 (or 8 in LNC=1) owned heads.
    # Non-owned heads' attn_out columns are zero anyway (q zero-init + skipped matmul).
    for head in owned_heads:
    ...
    # =========================================================================
    # CROSS-CORE ALL-REDUCE on out_sb (LNC=2 only) via SBUF sendrecv.
    # Each core's out_sb holds the partial sum over its 4 owned heads; summing across
    # cores yields the full 8-head o_proj result on both cores. sendrecv is a
    # pure-SBUF cross-core primitive — no HBM round-trip.
    # Pattern matches nkilib/experimental/moe/forward/bwmm_shard_on_H.py.
    # =========================================================================
    if n_prgs > 1:
