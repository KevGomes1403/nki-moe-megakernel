"""
v14c_kv_norm_hoisted: v14b_kv_norm_pretransposed with optional pre-loaded SBUF constants.

Key difference over v14b:
  Five "tiny" HBM loads that sit on the critical path before the first matmul —
  q_norm_weight, k_norm_weight, cos, sin, gamma_pre_attn — can now be passed in
  as already-loaded f32 SBUF tensors.  When a *_f32_sb kwarg is not None, the
  corresponding bf16 DMA + f32 cast is skipped and the caller-owned tensor is
  used directly.

  This is the building block for the multi-layer megakernel: call once outside
  the layer loop for the layer-invariant tensors (cos/sin), or stack across all
  layers for qnw/knw/gpan, then feed the pre-loaded slices to each layer call.

Calling conventions
-------------------
  - Back-compat mode (all *_f32_sb = None): identical to v14b — the DMA+cast
    happens inside the function, allocated via sbm.alloc_stack.
  - Hoisted mode (some or all *_f32_sb provided): caller is responsible for
    the bf16→f32 cast and for keeping the tensors live.  v14c will NOT pop
    (close_scope) them.

SBUF allocation
---------------
  All allocations still go through the caller-supplied sbm (BufferManager in
  manual/auto mode via SbufManager).  When the caller passes a pre-loaded
  tensor it must also have been allocated from the same sbm instance.
"""
...
def qwen3_attn_tkg_fused_oproj_v13bc_kv_norm_hoisted(
    ...
    # NEW: pre-loaded SBUF tensors (f32).  When provided, skip the DMA + bf16→f32 cast.
