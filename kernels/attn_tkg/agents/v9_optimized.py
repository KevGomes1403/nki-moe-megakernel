"""
v9_optimized: Two structural SBUF optimizations on top of v9_broadcasts.

Changes from v9_broadcasts:
  Change 1: Q Projection — one head at a time (reduces peak SBUF from ~4MB to ~512KB)
    Instead of hoisting all 8 wq_head[128,2048] tiles simultaneously (~4MB),
    each head's weight is loaded, matmul computed, and result packed into
    q_packed_f32[:, q_h] sequentially. Fuses psum→q_vec→q_packed_f32 into a
    single tensor_copy per head, skipping the intermediate q_vec buffer.

  Change 2: O-proj — head-outer, h_blk-inner with 4 pre-allocated PSUMs
    Pre-allocate all 4 output PSUMs (res_psum_0..3) before the head loop.
    The head loop iterates over all 8 heads and for each head accumulates
    all 4 output blocks into the corresponding pre-allocated PSUM. This
    fully consumes each head's wo_sbuf before moving to the next head
    (better SBUF access locality). After the head loop, all 4 output blocks
    are stored with tensor_copy + dma_copy.

Preserved unchanged from v9_broadcasts:
  tp_broadcast pattern for all GQA=8 broadcast loops, v8 Wo contiguous DMA,
  Plan B cached scores, SBUF hoisting, hidden tile hoisting, packed Q
  RMSNorm+RoPE, packed K/V cache hoisting, all name= strings unique.
...
if __name__ == "__main__":
    ...
    def run_test(B=1, S_prior=640, H=2048, d=128, Hq_tp=8, Hkv_tp=1):
        ...
        print("\n[3/4] Running NKI kernel (v9_optimized)...")
