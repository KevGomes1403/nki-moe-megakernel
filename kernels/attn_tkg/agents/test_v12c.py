...
from v12c import qwen3_attn_tkg_fused_oproj_v12c
...
xm.mark_step()
new_out, new_k, new_v = qwen3_attn_tkg_fused_oproj_v12c(*inputs)
xm.mark_step()
ro, rk, rv = ref_out.float().cpu().numpy(), ref_k.float().cpu().numpy(), ref_v.float().cpu().numpy()
no, nk, nv = new_out.float().cpu().numpy(), new_k.float().cpu().numpy(), new_v.float().cpu().numpy()
...
print(f"max_diff={np.abs(no-ro).max():.2e}  PASS")
