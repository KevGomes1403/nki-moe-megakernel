...
def qwen3_attn_tkg_fused_oproj_v15b_oproj_collapse(
    ...
    # v15b: collapse per-column transpose chain — write chunk_T_psum directly into out_sb,
    # removing the col_tmp intermediate (-16 tensor_copys vs v14b).
    nisa.tensor_copy(out_sb_tmp, res_psum_0)
    nisa.nc_transpose(chunk_T_psum, out_sb_tmp[0:1, 0*PMAX:1*PMAX])
    nisa.tensor_copy(out_sb[0:PMAX, 0:1], chunk_T_psum)
    nisa.nc_transpose(chunk_T_psum, out_sb_tmp[0:1, 1*PMAX:2*PMAX])
    nisa.tensor_copy(out_sb[0:PMAX, 1:2], chunk_T_psum)
    nisa.nc_transpose(chunk_T_psum, out_sb_tmp[0:1, 2*PMAX:3*PMAX])
    nisa.tensor_copy(out_sb[0:PMAX, 2:3], chunk_T_psum)
    nisa.nc_transpose(chunk_T_psum, out_sb_tmp[0:1, 3*PMAX:4*PMAX])
    nisa.tensor_copy(out_sb[0:PMAX, 3:4], chunk_T_psum)

    nisa.tensor_copy(out_sb_tmp, res_psum_1)
    nisa.nc_transpose(chunk_T_psum, out_sb_tmp[0:1, 0*PMAX:1*PMAX])
    nisa.tensor_copy(out_sb[0:PMAX, 4:5], chunk_T_psum)
    nisa.nc_transpose(chunk_T_psum, out_sb_tmp[0:1, 1*PMAX:2*PMAX])
    nisa.tensor_copy(out_sb[0:PMAX, 5:6], chunk_T_psum)
    nisa.nc_transpose(chunk_T_psum, out_sb_tmp[0:1, 2*PMAX:3*PMAX])
    nisa.tensor_copy(out_sb[0:PMAX, 6:7], chunk_T_psum)
    nisa.nc_transpose(chunk_T_psum, out_sb_tmp[0:1, 3*PMAX:4*PMAX])
    nisa.tensor_copy(out_sb[0:PMAX, 7:8], chunk_T_psum)

    nisa.tensor_copy(out_sb_tmp, res_psum_2)
    nisa.nc_transpose(chunk_T_psum, out_sb_tmp[0:1, 0*PMAX:1*PMAX])
    nisa.tensor_copy(out_sb[0:PMAX, 8:9], chunk_T_psum)
    nisa.nc_transpose(chunk_T_psum, out_sb_tmp[0:1, 1*PMAX:2*PMAX])
    nisa.tensor_copy(out_sb[0:PMAX, 9:10], chunk_T_psum)
    nisa.nc_transpose(chunk_T_psum, out_sb_tmp[0:1, 2*PMAX:3*PMAX])
    nisa.tensor_copy(out_sb[0:PMAX, 10:11], chunk_T_psum)
    nisa.nc_transpose(chunk_T_psum, out_sb_tmp[0:1, 3*PMAX:4*PMAX])
    nisa.tensor_copy(out_sb[0:PMAX, 11:12], chunk_T_psum)

    nisa.tensor_copy(out_sb_tmp, res_psum_3)
    nisa.nc_transpose(chunk_T_psum, out_sb_tmp[0:1, 0*PMAX:1*PMAX])
    nisa.tensor_copy(out_sb[0:PMAX, 12:13], chunk_T_psum)
    nisa.nc_transpose(chunk_T_psum, out_sb_tmp[0:1, 1*PMAX:2*PMAX])
    nisa.tensor_copy(out_sb[0:PMAX, 13:14], chunk_T_psum)
    nisa.nc_transpose(chunk_T_psum, out_sb_tmp[0:1, 2*PMAX:3*PMAX])
    nisa.tensor_copy(out_sb[0:PMAX, 14:15], chunk_T_psum)
    nisa.nc_transpose(chunk_T_psum, out_sb_tmp[0:1, 3*PMAX:4*PMAX])
    nisa.tensor_copy(out_sb[0:PMAX, 15:16], chunk_T_psum)
