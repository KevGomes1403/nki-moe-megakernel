"""
v14b_kv_norm_pretransposed: v14b_kv_norm_planABC with pre-tile-transposed Wq/Wk/Wv.

Key difference over planABC:
  Wq, Wk, Wv are stored in tile-transposed HBM layout (as in qwen_with_nki.py).
  This lets every weight load use plain dma_copy instead of dma_transpose,
  freeing the DMA engine from transpose bookkeeping.

Pre-transposed layout (same shape, different byte order):
  Wq_pt[q_h*128 + p, h_t*128 + f_d] = Wq[q_h*128 + f_d, h_t*128 + p]
  Produced by: W.reshape(nh, d, nh_tiles, d).permute(0,3,2,1).reshape(nh*d, H)

  Same pattern for Wk and Wv (nh=1 for both in this config).
  Wo is unchanged — plain .T.contiguous() transpose, no tile-transpose needed.
"""
...
def qwen3_attn_tkg_v15c_broadcast_elim(
    ...
    This allows all weight loads to use dma_copy instead of dma_transpose.
    """
    ...
    # =========================================================================
    # EARLY Wk/Wv DMA HOIST — dma_copy (pre-transposed layout, no transpose needed)
    ...
    # =========================================================================
    # Wq LOAD — dma_copy (pre-transposed layout)
    ...
    # v15c: free-dim broadcast via stride-0 .ap (eliminates 2x nc_transpose + 2x tensor_copy)
    q_normed2 = sbm.alloc_stack((PMAX, GQA), nl.float32, name="q_normed2")
    nisa.tensor_tensor(q_normed2, q_normed, qnw_sb.ap([[1, PMAX], [0, GQA]], offset=0), op=nl.multiply)
    ...
    # v15c: free-dim broadcast cos_f32 [PMAX,1] -> [PMAX,GQA] inline
    nisa.tensor_tensor(q_cos, q_normed2, cos_f32.ap([[1, PMAX], [0, GQA]], offset=0), op=nl.multiply)
    # v15c: free-dim broadcast sin_f32 [PMAX,1] -> [PMAX,GQA] inline
    nisa.tensor_tensor(q_sin_part, rot_q, sin_f32.ap([[1, PMAX], [0, GQA]], offset=0), op=nl.multiply)
    ...
    # =========================================================================
    # WO WEIGHT HOISTING — dma_copy with AP (plain .T layout, unchanged)
    # =========================================================================
    for head in owned_heads:
        nisa.dma_copy(
            dst=wo_sbuf[head],
    ...
    # =========================================================================
    # v15c: free-dim broadcast k_rope_bf16_f32 [PMAX,B=1] -> [PMAX,GQA] inline
    kq_elem = sbm.alloc_stack((PMAX, GQA), nl.float32, name="kq_elem")
    nisa.tensor_tensor(kq_elem, k_rope_bf16_f32.ap([[1, PMAX], [0, GQA]], offset=0), q_bf16, op=nl.multiply)
    ...
    for s_t in nl.affine_range(NUM_S_TILES):
        ...
        # v15c: keep mask_tile_f32 as [PMAX,1]; broadcast inline in pass1 via stride-0 .ap
        sbm.close_scope()  # tile_{s_t}

        k_cache_tiles.append(k_ct)
        mask_gqa_tiles.append(mask_tile_f32)
    ...
    for s_t in nl.affine_range(NUM_S_TILES):
        ...
        # v15c: free-dim broadcast mask_tile_f32 [PMAX,1] -> [PMAX,GQA] inline
        nisa.tensor_tensor(score_sb_masked, score_sb,
                           mask_gqa_tiles[s_t].ap([[1, PMAX], [0, GQA]], offset=0),
                           op=nl.add)
    ...
    # v15c: free-dim broadcast v_from_bf16 [PMAX,B=1] -> [PMAX,GQA] inline
    v_act_weighted = sbm.alloc_stack((PMAX, GQA), nl.float32, name="v_act_weighted")
    nisa.tensor_tensor(v_act_weighted,
                       v_from_bf16.ap([[1, PMAX], [0, GQA]], offset=0),
                       score_act_exp, op=nl.multiply)
    ...
    # Transpose each 128-column chunk — 4 blocks × 4 chunks = 16 columns
