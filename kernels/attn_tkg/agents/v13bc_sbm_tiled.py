"""
v13bc_sbm_tiled: Sub-function version of qwen3_attn_tkg_fused_oproj_v13bc with tiled output.

Changes from v13bc_sbm:
  - Output out_sb layout changed from [1, H_wo] to [PMAX, H_wo//PMAX] = [128, 16] bf16.
  - The o_proj scope replaces the flat copy with 16 nc_transpose operations (4 blocks x 4
    chunks each), transposing each [1, 128] slice of PSUM into a [128, 1] column of out_sb.
  - No HBM roundtrips in the output path.
  - Element mapping: out_sb[p, j] = linear_out[j * 128 + p]  (column-major).
  - All other optimizations (SBUF hoisting, DMA optimizations) unchanged.
"""
