...
def _find_kernel_artifacts(
    ...
    if inspect_dir:
        ...
        if new_neffs:
            ...
            # Strip "neff_" prefix and ".neff" suffix; also strip trailing "_vnc_N"
            # so the pattern matches the NTFF which uses the base hash without vnc suffix.
            neff_hash = (
                os.path.basename(profile_neff)
                .replace("neff_", "")
                .replace(".neff", "")
            )
            # Remove trailing _vnc_N suffix (e.g. "_vnc_1" -> "")
            import re as _re
