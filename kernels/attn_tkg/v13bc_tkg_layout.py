"""
v13bc_tkg_layout: Modified copy of v13bc_sbm where the o_proj output is written
directly into TKG layout (PMAX=128, H_wo//PMAX=16) instead of flat (1, H_wo=2048).

Changes from v13bc_sbm:
  - out_sb shape changed from (1, H_wo=2048) to (PMAX=128, H_wo//PMAX=16).
  - out_sb_tmp eliminated — no longer needed.
  - Each PSUM chunk is scattered row-by-row into out_sb via a static_range loop.
  - All other optimizations (SBUF hoisting, DMA patterns, scope structure) preserved.
"""
...
def qwen3_attn_tkg_fused_oproj_v13bc(
    ...
    out_sb=None,     # optional [PMAX=128, H_wo//PMAX=16] bf16 sbuf tensor in TKG layout; alloc_stack'd if None
    ...
    Returns (out_sb, k_rope_out, v_out):
      out_sb:     [PMAX=128, H_wo//PMAX=16] bf16 in SBUF — TKG layout; valid until caller closes attn_outer scope
    ...
    # TKG layout constants
    H1_tkg = H_wo // PMAX   # = 16:  number of output elements per partition row

    # Allocate or use caller-provided out_sb.
    # NOTE: We allocate as (1, H_wo) in SBUF because PSUM output is always at partition 0
    # and tensor_copy to partition p > 0 is illegal on trn2. The TKG layout shape
    # (PMAX=128, H_wo//PMAX=16) is presented to the caller via a reshape on return.
    # This is a zero-copy view — same underlying SBUF bytes, different shape interpretation.
    if out_sb is None:
        out_sb_flat = sbm.alloc_stack((1, H_wo), nl.bfloat16, name="out_sb")
    else:
        out_sb_flat = out_sb

    out_sb_tmp = sbm.alloc_stack((1, F_MAX), nl.bfloat16, name="out_sb_tmp")

    # Pre-allocate all 4 output PSUMs (shape [1, F_MAX=512] each)
    ...
    # Copy each PSUM block → out_sb_tmp → out_sb_flat slice (no DMA to HBM)
    nisa.tensor_copy(out_sb_tmp, res_psum_0)
    nisa.tensor_copy(out_sb_flat[0:1, 0*F_MAX:1*F_MAX], out_sb_tmp[0:1, 0:F_MAX])
    nisa.tensor_copy(out_sb_tmp, res_psum_1)
    nisa.tensor_copy(out_sb_flat[0:1, 1*F_MAX:2*F_MAX], out_sb_tmp[0:1, 0:F_MAX])
    nisa.tensor_copy(out_sb_tmp, res_psum_2)
    nisa.tensor_copy(out_sb_flat[0:1, 2*F_MAX:3*F_MAX], out_sb_tmp[0:1, 0:F_MAX])
    nisa.tensor_copy(out_sb_tmp, res_psum_3)
    nisa.tensor_copy(out_sb_flat[0:1, 3*F_MAX:4*F_MAX], out_sb_tmp[0:1, 0:F_MAX])

    # Return out_sb as TKG layout (PMAX=128, H_wo//PMAX=16) via reshape.
    # The reshape is a zero-copy shape metadata change — no instructions emitted.
    # A TKG consumer can index out_sb[p, c] instead of out_sb[0, p*H1_tkg + c].
    out_sb = out_sb_flat.reshape((PMAX, H1_tkg))
