"""
Optimized MoE kernel — v7c: Coalesce Hidden State DMA.

Built on v6b (Coalesce Down Weight DMAs).

Key optimization over v6b:
  - Hidden state DMA coalescing: instead of loading num_h_tiles separate
    [P, 1] tiles per token (16 DMAs for H=2048), we load a single [P, num_h_tiles]
    tensor in ONE DMA, then slice [P, h_t:h_t+1] for each h_t's matmul.
  - This reduces hidden state DMAs from num_h_tiles to 1 per token
    (e.g. 16 → 1 for H=2048).

Hidden state DMA strategy:
  - Old (v6b): for each h_t, loads [P, 1] tile via separate DMA
  - New (v7c): loads entire [P, num_h_tiles] in ONE DMA, then slices
    [P, h_t:h_t+1] for each matmul.
  - Access pattern mapping:
      Original: h_tiles_sb[h_t] partition p = flat[t*H + h_t*128 + p]
      Coalesced: h_full[p, h_t]              = flat[t*H + p*1 + h_t*128]
                                              = flat[t*H + p + h_t*128]
      These are identical, so h_full[:, h_t:h_t+1] == h_tiles_sb[h_t]. ✓
...
def nki_moe_v7c(
    ...
    """
    Sparse Token-Parallel MoE kernel (v7c) with coalesced hidden state,
    gate+up weight, and down weight DMAs.

    Hidden:  loads full [P, num_h_tiles] per token in a single DMA (new in v7c).
    Gate+up: loads full [P, two_I] row per h_t in a single DMA (from v5a/v6b).
    ...
    for t in range(T):
        ...
        # -------------------------------------------------------------------
        # v7c: Coalesced hidden state load — single [P, num_h_tiles] DMA.
        #
        # Each partition p gets elements {p, p+128, p+256, ..., p+15*128}
        # from the token's row, i.e. the same data as the 16 separate [P, 1]
        # tiles combined.
        #
        # Pattern: stride 1 between partitions (P=128 partitions), stride P
        # between free-dim elements (num_h_tiles elements per partition).
        # This replaces 16 separate [P, 1] DMAs with 1 [P, 16] DMA (4KB total).
        # -------------------------------------------------------------------
        h_full = nl.ndarray((P, num_h_tiles), dtype=hidden_states.dtype, buffer=nl.sbuf)
        nisa.dma_copy(
            dst=h_full,
            src=hidden_states.ap(
                pattern=[[1, P], [P, num_h_tiles]],   # P=128 partitions, num_h_tiles free elements
                offset=t * H,                          # start of token t's row
            ),
        )
        ...
        for k in range(K):
            ...
            for h_t in range(num_h_tiles):
                # v7c: slice [P, 1] from coalesced [P, num_h_tiles] buffer
                # h_full[:, h_t:h_t+1] gives the same data as the old h_tiles_sb[h_t]
                h_tile = h_full[0:P, h_t:h_t+1]
            ...
            # ---------------------------------------------------------------
            # Stage 2: Down projection  (UNCHANGED from v6b — coalesced DMA)
            ...
            # h_t/i_t loop, then slice [P, P] for each matmul.
            # ---------------------------------------------------------------
            ...
            # Same loop structure as v6b, but matmul slices from pre-loaded rows.
...
def test_nki_moe_v7c_qwen3_t1():
    """Qwen3 shapes, T=1: H=2048, I=256, E=128, K=8."""
    import torch_xla.core.xla_model as xm

    print("\n" + "=" * 70)
    print("nki_moe_v7c — Qwen3 T=1 (H=2048, I=256, E=128, K=8)")
    print("=" * 70)
    ...
    print("[2/3] NKI v7c kernel on Trainium...")
    nki_out = nki_moe_v7c(
        hidden.to(device),
        gu_w.to(device),
        d_w.to(device),
...
def test_nki_moe_v7c():
    """Run all v7c correctness tests."""
    results = []
    results.append(("qwen3 T=1 (H=2048,I=256,E=128,K=8)", test_nki_moe_v7c_qwen3_t1()))
    ...
    print("=" * 70)
    if all_ok:
        print("All v7c tests PASSED.")
    else:
        print("Some v7c tests FAILED.")
    return all_ok
