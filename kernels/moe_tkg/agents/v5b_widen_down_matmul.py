"""
Optimized MoE kernel — v5b: Widen Down Projection Moving Dimension.

Based on v5 (Token-Parallel Sparse Execution).

Key optimization over v5:
  - Batch 4 h_tiles (or fewer for small H) per nc_matmul in the down projection,
    loading [P, H_PER_GROUP*P] weight blocks and producing [1, H_PER_GROUP*P] results.
    This uses up to 4x more of the Tensor Engine's moving-dimension capacity (max 512).
  - Gate+Up projection is unchanged from v5.
...
Algorithm per token t:
  ...
  2. For k in range(K) [sequential — output accumulation]:
     ...
     b. Gate+Up matmul → gu_psum_tiles (memset before each k).  [unchanged]
     c. PSUM→SBUF, SiLU, multiply, cast to bf16.                [unchanged]
     d. Down matmul → down_psum_groups (widened: [1, H_PER_GROUP*P]).
     e. PSUM→SBUF, scale by rw_k, accumulate into out_accum (widened).
  3. Cast out_accum → bf16, write to HBM output[t, :] (widened store).
...
def nki_moe_v5b(
    ...
    """
    Sparse Token-Parallel MoE kernel (v5b) with widened down projection.

    Down projection batches H_PER_GROUP h_tiles per nc_matmul to fill
    more of the Tensor Engine's 512-wide moving dimension.
    """
    ...
    # --- Plan B: Batch multiple h_tiles per down-projection matmul ---
    # H_PER_GROUP = 4 means [P, 4*P]=[128,512] moving blocks (hw max).
    # For small H where num_h_tiles < 4, fall back to num_h_tiles groups of size 1 or 2.
    ...
    #   down_psum_groups[h_g]: [1, H_PER_GROUP*P] float32 psum  (widened)
    # =========================================================================
    # PSUM tile for gate+up: one [P, 1] tile per gu column block  [UNCHANGED]
    gu_psum_tiles = []
    ...
    # PSUM tiles for down: one [1, H_PER_GROUP*P] tile per h-group (WIDENED from v5)
    ...
    for t in range(T):
        ...
        # Output accumulator for this token: num_h_groups × [1, H_PER_GROUP*P] f32 SBUF
        # WIDENED from v5: each group covers H_PER_GROUP h_tiles.
        ...
        for k in range(K):
            ...
            # ---------------------------------------------------------------
            # Stage 1: Gate+Up projection  [UNCHANGED from v5]
            ...
            # ---------------------------------------------------------------
            # Stage 2: Down projection  [WIDENED — Plan B]
            #   act[1, I] @ down_w[I, H] → [1, H]
            #   nc_matmul layout:  dst += stationary.T @ moving
            #     stationary = act_tile  [P_i, 1]               (intermediate tile)
            #     moving     = dw_block  [P_i, H_PER_GROUP*P]   (wider weight block)
            #     result     = [1, H_PER_GROUP*P]                (batched h_tiles)
            #
            #   Instead of 16 separate [P,P] matmuls, we do 4 groups of
            #   [P, 4*P]=[128, 512] matmuls, filling the Tensor Engine's
            #   512-wide moving dimension capacity.
            # ---------------------------------------------------------------

            # h_g groups are independent → affine_range; i_t sequential (PSUM acc dep)
            for h_g in nl.affine_range(num_h_groups):
                # Base H offset for this group of h_tiles
                ...
                for i_t in range(num_i_tiles):
                    ...
                    # Load [P, H_PER_GROUP*P] weight block spanning H_PER_GROUP h_tiles
                    # from down_weights[eid, i_off:i_off+P, h_base:h_base+H_PER_GROUP*P]
                    dw_block = nl.ndarray(
                        (P, H_PER_GROUP * P), dtype=down_weights.dtype, buffer=nl.sbuf
                    )
                    nisa.dma_copy(
                        dst=dw_block,
                        src=down_weights.ap(
                            # Pattern: stride H along partition dim, H_PER_GROUP*P contiguous in free dim
                            pattern=[[H, P], [1, H_PER_GROUP * P]],
                            offset=i_off * H + h_base,
                            scalar_offset=eid_offset,
                            indirect_dim=0,
                        ),
                    )

                    # Wider matmul: stationary=[P, 1], moving=[P, H_PER_GROUP*P],
                    # result=[1, H_PER_GROUP*P] accumulated into down_psum_groups[h_g]
                    nisa.nc_matmul(
        ...
        # WIDENED: each group stores H_PER_GROUP*P elements in one DMA.
        # -------------------------------------------------------------------
        for h_g in nl.affine_range(num_h_groups):
            h_base   = h_g * H_PER_GROUP * P
            out_cast = nl.ndarray(
...
def test_nki_moe_v5b_small():
    """Small correctness test: T=1, H=256, I=256, E=8, K=2."""
    import torch_xla.core.xla_model as xm

    print("=" * 70)
    print("nki_moe_v5b — Small shapes (T=1, H=256, I=256, E=8, K=2)")
    print("=" * 70)
    ...
    print(f"  num_h_tiles={H // 128}, H_PER_GROUP={min(4, H // 128)}")
    ...
    print("[2/3] NKI v5b kernel on Trainium...")
    nki_out = nki_moe_v5b(
        hidden.to(device),
        gu_w.to(device),
        d_w.to(device),
    ...
    if max_diff < threshold:
        print("\n" + "=" * 70)
        print("PASS — nki_moe_v5b small test.")
        print("=" * 70)
        return True
    else:
...
def test_nki_moe_v5b_qwen3_t1():
    """Qwen3 shapes, T=1: H=2048, I=256, E=128, K=8."""
    import torch_xla.core.xla_model as xm

    print("\n" + "=" * 70)
    print("nki_moe_v5b — Qwen3 T=1 seed=999 (H=2048, I=256, E=128, K=8)")
    print("=" * 70)
...
def test_nki_moe_v5b():
    """Run all v5b correctness tests."""
    results = []
    # results.append(("small (T=1,H=256,I=256,E=8,K=2)",  test_nki_moe_v5b_small()))
    results.append(("qwen3 T=1 seed=999 (H=2048,I=256,E=128,K=8)", test_nki_moe_v5b_qwen3_t1()))
    ...
    print("=" * 70)
    if all_ok:
        print("All v5b tests PASSED.")
    else:
        print("Some v5b tests FAILED.")
    return all_ok
