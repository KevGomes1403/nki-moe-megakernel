...
v7a architecture (v6a + score-softmax decoupling):
    ...
    for kvh in nl.affine_range(Hkv_tp):
        ...
        for gi in nl.affine_range(gqa):
            ...
            for qsi in nl.affine_range(num_s_tiles):
                ...
                # Broadcast neg_max [PMAX,1] -> [PMAX, 4*PMAX] for wide tile subtraction
                ...
                # Wide: shift scores by -max for numerical stability, then exponentiate
                ...
                # Initialize row_sum with the sum of exp'd wide scores
                ...
                # Wide MM2: accumulate exp-weighted V for first BATCH tiles into single PSUM
                atacc_psum = nl.zeros((PMAX, PMAX), dtype=nl.float32, buffer=nl.psum,
                                      name=f"p2_atp_kv{kvh}_g{gi}_s{qsi}")
                for i in range(BATCH):
                    # Extract the i-th [PMAX, PMAX] slice of exp'd scores, cast to bf16 for matmul
                    ...
                    # Transpose exp'd scores to [S_k, S_q] for matmul (stationary position)
                ...
                # Broadcast neg_max [PMAX,1] -> [PMAX,PMAX] for narrow tail tiles
                ...
                # Process each tail tile: shift, exp, accumulate sum and V
                for ti in range(len(masked_tail_tiles)):
                    sm_t = masked_tail_tiles[ti]
...
if __name__ == "__main__":
    ...
    def run_test(B, S, H=2048, d=128, Hq_tp=8, Hkv_tp=4):
        print(f"\n{'=' * 70}")
        print(f"qwen3_attn_cte_fused v7a Test: B={B}, S={S}, H={H}, d={d}")
