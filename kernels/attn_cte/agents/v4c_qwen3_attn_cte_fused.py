...
Plan A optimization (Phase Fusion):
  ...
  v4c architecture (v4a + 512-wide K matmul + PSUM-accumulated MM2):
    ...
    Flash attention rescaling (exp/max/sum) operates on [PMAX, 512] tiles -> 4x fewer iterations.
    SBUF extra: K_wide [128,512] bf16 = 128KB; sexp_w [128,512] f32 = 256KB.
...
def qwen3_attn_cte_fused(
    ...
    for kvh in nl.affine_range(Hkv_tp):
        ...
        # v4c: flash attention inner loop restructured as:
        #   - full-batch loop: processes 4 K/V tiles per iteration via K_wide [d, 512]
        #   - tail loop: processes remaining tiles (< 4) one at a time (v4a body)
        # -----------------------------------------------------------------
        for gi in nl.affine_range(gqa):
            ...
            for qsi in nl.affine_range(num_s_tiles):
                ...
                # Flash attention state -- [PMAX, PMAX]
                ...
                # ============================================================
                # v4c: Full-batch loop -- process 4 K tiles at once
                # Uses K_wide [d=128, K_WIDE=512] for a single wider MM1.
                # ============================================================
                for ki_b in range(num_full_batches):   # Python range -- sequential (carried flash-attn state)
                    batch_start = ki_b * 4
                    kso_batch = batch_start * PMAX      # start k-position for this batch

                    # ---- MM1: build K_wide [d=128, K_WIDE=512] from 4 pre-transposed K tiles ----
                    ...
                    nisa.nc_matmul(scp_w, stationary=qTs, moving=K_wide)   # [S_q=128, 512]
                    ...
                    # ---- Causal mask on [PMAX, K_WIDE] via affine_select ----
                    sm_w = nl.ndarray((PMAX, K_WIDE), dtype=nl.float32, buffer=nl.sbuf,
                                       name=f"p2_smw_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    nisa.affine_select(
                        dst=sm_w,
                        on_true_tile=scs_w,
                    ...
                    # ---- tile_max over [PMAX, K_WIDE] -> [PMAX, 1] ----
                    tmax1_w = nl.ndarray((PMAX, 1), dtype=nl.float32, buffer=nl.sbuf,
                                          name=f"p2_tmax1w_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    nisa.tensor_reduce(dst=tmax1_w, op=nl.maximum, data=sm_w, axis=1)

                    # Broadcast [PMAX,1] -> [PMAX,PMAX] for running-max arithmetic
                    tmax_w = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                         name=f"p2_tmaxw_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    nisa.dma_copy(dst=tmax_w, src=tmax1_w.ap(pattern=[[1, PMAX], [0, PMAX]], offset=0))

                    # new_max = max(rmax, tmax_w)
                    nmax_w = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                         name=f"p2_nmaxw_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    nisa.tensor_tensor(nmax_w, rmax, tmax_w, op=nl.maximum)

                    # neg_new_max [PMAX,PMAX] for rmax rescale
                    nnmax_w = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                          name=f"p2_nnmaxw_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    nisa.tensor_scalar(nnmax_w, nmax_w, op0=nl.multiply, operand0=-1.0)

                    # alpha = exp(rmax + nnmax_w) = exp(rmax - new_max) [PMAX,PMAX]
                    aarg_w = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                         name=f"p2_aargw_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    nisa.tensor_tensor(aarg_w, rmax, nnmax_w, op=nl.add)
                    alp_w = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                        name=f"p2_alpw_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    nisa.activation(alp_w, op=nl.exp, data=aarg_w)

                    # Rescale running accumulators
                    nrs_w = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                        name=f"p2_nrsw_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    nisa.tensor_tensor(nrs_w, rsum, alp_w, op=nl.multiply)
                    nacc_w = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                         name=f"p2_naccw_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    nisa.tensor_tensor(nacc_w, atacc, alp_w, op=nl.multiply)

                    # ---- score_exp on [PMAX, K_WIDE] ----
                    # Get per-row new_max as [PMAX, 1] via reduce, then negate for shift
                    # (nmax_w has identical columns, so reduce gives the same values)
                    nmax1_w = nl.ndarray((PMAX, 1), dtype=nl.float32, buffer=nl.sbuf,
                                          name=f"p2_nmax1w_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    nisa.tensor_reduce(dst=nmax1_w, op=nl.maximum, data=nmax_w, axis=1)
                    neg_nmax1 = nl.ndarray((PMAX, 1), dtype=nl.float32, buffer=nl.sbuf,
                                            name=f"p2_nnmax1_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    nisa.tensor_scalar(neg_nmax1, nmax1_w, op0=nl.multiply, operand0=-1.0)

                    # Broadcast [PMAX,1] -> [PMAX, K_WIDE] for score shift
                    nnmax_wide = nl.ndarray((PMAX, K_WIDE), dtype=nl.float32, buffer=nl.sbuf,
                                             name=f"p2_nnmaxwide_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    nisa.dma_copy(dst=nnmax_wide, src=neg_nmax1.ap(pattern=[[1, PMAX], [0, K_WIDE]], offset=0))

                    sshift_w = nl.ndarray((PMAX, K_WIDE), dtype=nl.float32, buffer=nl.sbuf,
                                           name=f"p2_sshiftw_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    nisa.tensor_tensor(sshift_w, sm_w, nnmax_wide, op=nl.add)
                    sexp_w = nl.ndarray((PMAX, K_WIDE), dtype=nl.float32, buffer=nl.sbuf,
                                         name=f"p2_sexpw_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    nisa.activation(sexp_w, op=nl.exp, data=sshift_w)

                    # tile_sum over [PMAX, K_WIDE] -> [PMAX,1] -> broadcast to [PMAX,PMAX]
                    tsum1_w = nl.ndarray((PMAX, 1), dtype=nl.float32, buffer=nl.sbuf,
                                          name=f"p2_tsum1w_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    nisa.tensor_reduce(dst=tsum1_w, op=nl.add, data=sexp_w, axis=1)
                    tsum_w = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                         name=f"p2_tsumw_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    nisa.dma_copy(dst=tsum_w, src=tsum1_w.ap(pattern=[[1, PMAX], [0, PMAX]], offset=0))

                    # Update running sum
                    nrs2_w = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                         name=f"p2_nrs2w_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    nisa.tensor_tensor(nrs2_w, nrs_w, tsum_w, op=nl.add)

                    # ---- MM2: accumulate 4 V tiles in one PSUM ----
                    # sexp_w is [PMAX, K_WIDE]; for each V tile: take 128-wide slice, transpose, matmul
                    vcp_w = nl.zeros((PMAX, PMAX), dtype=nl.float32, buffer=nl.psum,
                                      name=f"p2_vcpw_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    for mm2_i in range(4):     # Python range for unrolling (compile-time constant index)
                        # Extract sexp slice for this V tile: [PMAX, PMAX]
                        sexp_sl_f = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                                name=f"p2_sexpslf_kv{kvh}_g{gi}_s{qsi}_b{ki_b}_m{mm2_i}")
                        nisa.tensor_copy(sexp_sl_f, sexp_w[0:PMAX, mm2_i*PMAX:(mm2_i+1)*PMAX])
                        sexp_sl_b = nl.ndarray((PMAX, PMAX), dtype=nl.bfloat16, buffer=nl.sbuf,
                                                name=f"p2_sexpslb_kv{kvh}_g{gi}_s{qsi}_b{ki_b}_m{mm2_i}")
                        nisa.tensor_copy(sexp_sl_b, sexp_sl_f)
                        sexp_sl_Tp = nl.ndarray((PMAX, PMAX), dtype=nl.bfloat16, buffer=nl.psum,
                                                 name=f"p2_sexpslTp_kv{kvh}_g{gi}_s{qsi}_b{ki_b}_m{mm2_i}")
                        nisa.nc_transpose(sexp_sl_Tp, sexp_sl_b)
                        sexp_sl_Ts = nl.ndarray((PMAX, PMAX), dtype=nl.bfloat16, buffer=nl.sbuf,
                                                 name=f"p2_sexpslTs_kv{kvh}_g{gi}_s{qsi}_b{ki_b}_m{mm2_i}")
                        nisa.tensor_copy(sexp_sl_Ts, sexp_sl_Tp)
                        # Accumulate into the same PSUM -- nc_matmul ADDs to existing PSUM content
                        nisa.nc_matmul(vcp_w, stationary=sexp_sl_Ts, moving=V_tiles[batch_start + mm2_i])

                    vc_w = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                       name=f"p2_vcw_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    nisa.tensor_copy(vc_w, vcp_w)
                    nacc2_w = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                          name=f"p2_nacc2w_kv{kvh}_g{gi}_s{qsi}_b{ki_b}")
                    nisa.tensor_tensor(nacc2_w, nacc_w, vc_w, op=nl.add)

                    # Update running state
                    nisa.tensor_copy(rmax, nmax_w)
                    nisa.tensor_copy(rsum, nrs2_w)
                    nisa.tensor_copy(atacc, nacc2_w)

                # ============================================================
                # v4c: Tail loop -- process remaining ki_tail tiles (< 4)
                # Uses the standard 128-wide path from v4a's ki loop.
                # ============================================================
                for ki in range(ki_tail):   # Python range -- sequential
                    ...
                    tmax1 = nl.ndarray((PMAX, 1), dtype=nl.float32, buffer=nl.sbuf,
                                       name=f"p2_tmax1_tail_kv{kvh}_g{gi}_s{qsi}_k{ki}")
                    ...
                    tmax = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                      name=f"p2_tmax_tail_kv{kvh}_g{gi}_s{qsi}_k{ki}")
                    ...
                    nmax = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                      name=f"p2_nmax_tail_kv{kvh}_g{gi}_s{qsi}_k{ki}")
                    ...
                    nnmax = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                       name=f"p2_nnmax_tail_kv{kvh}_g{gi}_s{qsi}_k{ki}")
...
if __name__ == "__main__":
    ...
    def run_test(B, S, H=2048, d=128, Hq_tp=8, Hkv_tp=4):
        print(f"\n{'=' * 70}")
        print(f"qwen3_attn_cte_fused v4c Test: B={B}, S={S}, H={H}, d={d}")
