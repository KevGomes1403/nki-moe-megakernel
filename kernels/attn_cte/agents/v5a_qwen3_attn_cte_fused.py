...
def qwen3_attn_cte_fused(
    ...
    for kvh in nl.affine_range(Hkv_tp):
        ...
        # v5a Change 3: rmax/rsum are [PMAX, 1] (one scalar per token row).
        # v5a Change 4: 4x-wide batched ki loop with tail for remaining tiles.
        # -----------------------------------------------------------------
        BATCH = 4
        ...
        for gi in nl.affine_range(gqa):
            ...
            for qsi in nl.affine_range(num_s_tiles):
                ...
                # Q projection — uses hidden_tiles[qsi][ht] from SBUF
                qp = nl.zeros((PMAX, PMAX), dtype=nl.float32, buffer=nl.psum,
                              name=f"p2_qp_kv{kvh}_g{gi}_s{qsi}")
                for ht in nl.affine_range(num_h_tiles):
                    hqTs = hidden_tiles[qsi][ht]  # from SBUF — no HBM DMA
                ...
                # Flash attention
                # v5a Change 3: rmax/rsum are [PMAX, 1] scalars (one per token row).
                ...
                # v5a Change 4: 4x-wide batched ki loop.
                # Batch loop: groups of BATCH=4 K tiles into a single wide matmul.
                for ki_batch in range(num_full_batches):
                    kso_batch = ki_batch * BATCH * PMAX  # base sequence offset for this batch

                    # Assemble K_wide [PMAX, BATCH*PMAX] from 4 pre-transposed K tiles
                    K_wide = nl.ndarray((PMAX, BATCH * PMAX), dtype=nl.bfloat16, buffer=nl.sbuf,
                                        name=f"p2_Kwide_kv{kvh}_g{gi}_s{qsi}_b{ki_batch}")
                    for i in range(BATCH):
                        nisa.tensor_copy(K_wide[0:PMAX, i*PMAX:(i+1)*PMAX],
                                         K_tiles[ki_batch * BATCH + i])

                    # MM1: Q × K_wide → score_wide [PMAX, BATCH*PMAX]
                    scp_wide = nl.zeros((PMAX, BATCH * PMAX), dtype=nl.float32, buffer=nl.psum,
                                        name=f"p2_scpw_kv{kvh}_g{gi}_s{qsi}_b{ki_batch}")
                    nisa.nc_matmul(scp_wide, stationary=qTs, moving=K_wide)
                    scs_wide = nl.ndarray((PMAX, BATCH * PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                          name=f"p2_scsw_kv{kvh}_g{gi}_s{qsi}_b{ki_batch}")
                    nisa.tensor_copy(scs_wide, scp_wide)

                    # Causal mask via affine_select on wide tile
                    # predicate: (qso + q_idx) >= (kso_batch + k_col)
                    sm_wide = nl.ndarray((PMAX, BATCH * PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                         name=f"p2_smw_kv{kvh}_g{gi}_s{qsi}_b{ki_batch}")
                    nisa.affine_select(
                        dst=sm_wide,
                        on_true_tile=scs_wide,
                        on_false_value=-3.4e38,
                        pattern=[[-1, BATCH * PMAX]],
                        offset=qso - kso_batch,
                        channel_multiplier=1,
                        cmp_op=nl.greater_equal,
                    )

                    # tile_max: reduce over all BATCH*PMAX key positions → [PMAX, 1]
                    ...
                    # BROADCAST 1: alp1 [PMAX, 1] → alp_bc [PMAX, PMAX] for atacc rescaling
                    ...
                    nisa.dma_copy(dst=alp_bc,
                                  src=alp1.ap(pattern=[[1, PMAX], [0, PMAX]], offset=0)) # Profiler: Causes the  most idle time
                    ...
                    # BROADCAST 2: nnmax1 [PMAX, 1] → nnmax_wide [PMAX, BATCH*PMAX] for score shift
                    nnmax_wide = nl.ndarray((PMAX, BATCH * PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                            name=f"p2_nnmaxw_kv{kvh}_g{gi}_s{qsi}_b{ki_batch}")
                    nisa.dma_copy(dst=nnmax_wide,
                                  src=nnmax1.ap(pattern=[[1, PMAX], [0, BATCH * PMAX]], offset=0)) # Profiler: Causes idle time

                    # score_exp_wide = exp(sm_wide + nnmax_wide) = exp(sm_wide - new_max)
                    sshift_wide = nl.ndarray((PMAX, BATCH * PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                             name=f"p2_sshiftw_kv{kvh}_g{gi}_s{qsi}_b{ki_batch}")
                    ...
                    sexp_wide = nl.ndarray((PMAX, BATCH * PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                           name=f"p2_sexpw_kv{kvh}_g{gi}_s{qsi}_b{ki_batch}")
                    ...
                    # MM2: accumulate BATCH V tiles into single PSUM — no intermediate SBUF copies
                ...
                # Tail loop: handles remaining tiles (num_s_tiles % BATCH tiles)
                for ki_tail in range(num_full_batches * BATCH, num_s_tiles):
                    kso = ki_tail * PMAX

                    # K tile is pre-transposed [d, S_k] — no nc_transpose needed.
                    kTs2 = K_tiles[ki_tail]

                    # MM1: score[S_q, S_k]
                    scp = nl.zeros((PMAX, PMAX), dtype=nl.float32, buffer=nl.psum,
                                   name=f"p2_scp_kv{kvh}_g{gi}_s{qsi}_kt{ki_tail}")
                    nisa.nc_matmul(scp, stationary=qTs, moving=kTs2)
                    scs = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                     name=f"p2_scs_kv{kvh}_g{gi}_s{qsi}_kt{ki_tail}")
                    ...
                    sm = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                    name=f"p2_sm_kv{kvh}_g{gi}_s{qsi}_kt{ki_tail}")
                    nisa.affine_select(
                        dst=sm,
                        on_true_tile=scs,
                        on_false_value=-3.4e38,
                    ...
                    # BROADCAST: alp1 [PMAX, 1] → alp_bc [PMAX, PMAX] for atacc rescaling
                    alp_bc = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                        name=f"p2_alpbc_kv{kvh}_g{gi}_s{qsi}_kt{ki_tail}")
                    nisa.dma_copy(dst=alp_bc,
                                  src=alp1.ap(pattern=[[1, PMAX], [0, PMAX]], offset=0)) # Causes idle time
                ...
                # v5a Change 5: Normalization adapted for [PMAX, 1] rsum.
                # Compute inv as [PMAX, 1] then broadcast to [PMAX, PMAX] for final multiply.
                ...
                # Broadcast inv1 [PMAX, 1] → inv [PMAX, PMAX]
...
if __name__ == "__main__":
    ...
    def run_test(B, S, H=2048, d=128, Hq_tp=8, Hkv_tp=4):
        print(f"\n{'=' * 70}")
        print(f"qwen3_attn_cte_fused v5a Test: B={B}, S={S}, H={H}, d={d}")
