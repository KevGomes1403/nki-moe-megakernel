"""
Fused NKI attention CTE kernel for Qwen3 — Plan B optimization.
...
Plan B change (Phase 1 only):
  Load hidden_2d[si, ht] ONCE per (si, ht) and share it across ALL KV heads
  for both K and V projections. This eliminates the redundant V hidden loads
  (320 tiles × 32KB ≈ 10.2 MB saved) at the cost of 8 simultaneous PSUMs
  (4 K + 4 V = 512 KB total) held during the ht loop.
"""
...
def qwen3_attn_cte_fused(
    ...
    # PHASE 1: Project K/V + Norm + RoPE -> HBM scratch
    #
    # Plan B restructuring: within each (si, kh) pair, load hidden_2d[si, ht]
    # ONCE and accumulate into BOTH kp (K projection) and vp (V projection)
    # PSUMs. This eliminates the redundant V hidden loads that existed in the
    # original kernel, where K and V each had their own hidden-tile load loops.
    #
    # Loop structure (original):
    #   for si: for kh: for ht: [load hidden, load Wk, matmul kp]
    #                   for ht: [load hidden, load Wv, matmul vp]   <- redundant load
    #
    # Loop structure (Plan B):
    #   for si:
    #     load cos/sin ONCE (shared across all kh)
    #     for kh:
    #       kp = zeros; vp = zeros
    #       for ht:
    #         hb = load hidden ONCE  # SHARE: one load serves both K and V
    #         hTs = transpose(hb)
    #         load Wk, matmul → kp
    #         load Wv, matmul → vp
    #       [RMSNorm + RoPE + store K using kp]
    #       [store V using vp]
    #
    # V hidden loads: num_h_tiles × Hkv_tp = 16×4 = 64 → 0 (eliminated).
    # Net saving: 64 hidden tile loads × 32KB = 2.0MB.
    # PSUM cost: 2 PSUMs (1 K + 1 V) per kh iteration — same as original.
    # =========================================================================
    for si in nl.affine_range(num_s_tiles):
        # affine_range on the outer si loop enables DMA-compute overlap across tiles
        s_off = si * PMAX

        # SHARE: load cos/sin ONCE per sequence tile.
        # In the original kernel these were loaded inside the kh loop;
        # since they depend only on si, hoisting them here avoids Hkv_tp redundant
        # loads per si tile (4 × 2 loads = 8 DMA ops saved per si iteration).
        ...
        for kh in nl.affine_range(Hkv_tp):
            # One K PSUM and one V PSUM for this head — only 2 PSUMs live at once
            # (same as original). The accumulation across ht requires range() for ht.
            ...
            # range() (not affine_range): kp and vp accumulate across ht iterations
            for ht in range(num_h_tiles):
                ho = ht * PMAX

                # SHARE: load hidden tile ONCE — serves both K and V projections
                # for this (si, kh, ht). In the original, this tile was loaded
                # separately in the K loop and again in the V loop.
                hb = nl.ndarray((PMAX, PMAX), dtype=nl.bfloat16, buffer=nl.sbuf,
                                name=f"p1_h_s{si}_h{kh}_t{ht}")
                nisa.dma_copy(dst=hb, src=hidden_2d[s_off:s_off+PMAX, ho:ho+PMAX])

                # Transpose hidden to [H_tile, S] layout for nc_matmul
                hTp = nl.ndarray((PMAX, PMAX), dtype=nl.bfloat16, buffer=nl.psum,
                                 name=f"p1_hTp_s{si}_h{kh}_t{ht}")
                nisa.nc_transpose(hTp, hb)
                hTs = nl.ndarray((PMAX, PMAX), dtype=nl.bfloat16, buffer=nl.sbuf,
                                 name=f"p1_hTs_s{si}_h{kh}_t{ht}")
                nisa.tensor_copy(hTs, hTp)

                # K weight for this head + H-tile
                wkb = nl.ndarray((PMAX, PMAX), dtype=nl.bfloat16, buffer=nl.sbuf,
                                 name=f"p1_wk_s{si}_h{kh}_t{ht}")
                nisa.dma_copy(dst=wkb, src=Wk[kh*d:kh*d+PMAX, ho:ho+PMAX])
                wkTp = nl.ndarray((PMAX, PMAX), dtype=nl.bfloat16, buffer=nl.psum,
                                  name=f"p1_wkTp_s{si}_h{kh}_t{ht}")
                nisa.nc_transpose(wkTp, wkb)
                wkTs = nl.ndarray((PMAX, PMAX), dtype=nl.bfloat16, buffer=nl.sbuf,
                                  name=f"p1_wkTs_s{si}_h{kh}_t{ht}")
                nisa.tensor_copy(wkTs, wkTp)
                # Accumulate K: kp += hTs^T @ Wk^T  (same hidden tile shared below)
                nisa.nc_matmul(kp, stationary=hTs, moving=wkTs)

                # V weight for this head + H-tile
                ...
                # Accumulate V: vp += hTs^T @ Wv^T  (reuses same hTs — no reload)
                nisa.nc_matmul(vp, stationary=hTs, moving=wvTs)

            # kp and vp are now fully accumulated for all H tiles.
            ...
            # K RoPE
            # cos_f / sin_f are loaded once per si above — shared across all kh
            ...
            # V: drain vp to SBUF and store — no norm or RoPE needed for V
            # SHARE: vp was accumulated using the same hidden tiles as K above
    ...
    # PHASE 2 IS UNCHANGED from the original kernel.
    # =========================================================================
