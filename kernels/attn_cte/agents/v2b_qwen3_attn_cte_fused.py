...
Plan B optimization (on top of v1c/Plan C):
  Change 1 — Pre-transposed weight layout:
    Wq: [H, Hq_tp*d]  = [2048, 1024] bf16 (transposed from original)
    Wk: [H, Hkv_tp*d] = [2048,  512] bf16 (transposed from original)
    Wv: [H, Hkv_tp*d] = [2048,  512] bf16 (transposed from original)
    Eliminates nc_transpose + tensor_copy steps for every weight tile in Phase 1.

  Change 2 — Hoist hidden transpose and Wq outside inner loops in Phase 2:
    OLD: for kvh -> for gi -> for qsi -> [load hqTs, load wqTs per ht] + Q proj
    NEW: for qsi -> [compute hqTs once per ht] -> for kvh -> for gi -> [load wqTs per ht] + Q proj
    Saves 560 hidden-tile loads and 512 Wq-tile loads per compile unit.

  Change 3 — Eliminate nc_transpose for Wk/Wv in Phase 1:
    Pre-transposed layout means weight tiles arrive as [PMAX_h, PMAX_d] = [K, free_dim]
    which is already the correct orientation for nc_matmul moving operand.
    Replaces: dma_copy -> nc_transpose -> tensor_copy -> nc_matmul
    With:     dma_copy -> nc_matmul   (2 steps fewer per weight tile)

  Plan C broadcast optimization (carried over from v1c):
    Uses SBUF .ap() pattern to broadcast [PMAX,1] -> [PMAX,PMAX] without HBM round-trips.
"""
...
def qwen3_attn_cte_fused(
    ...
    # Wq is [H, Hq_out] (transposed), so Hq_out is dim 1
    Hq_out = Wq.shape[1]
    Hkv_out = Wk.shape[1]
    d = PMAX
    Hq_tp = Hq_out // d
    ...
    # =========================================================================
    # PHASE 1: Project K/V + Norm + RoPE -> HBM scratch
    # Change 3: Weight tiles loaded directly in [PMAX_h, PMAX_d] layout (pre-transposed),
    #           so nc_transpose + tensor_copy steps are eliminated for Wk/Wv.
    # =========================================================================
    for si in nl.affine_range(num_s_tiles):
        ...
        for kh in nl.affine_range(Hkv_tp):
            ...
            for ht in range(num_h_tiles):
                ho = ht * PMAX
                # Hidden tile: load [S_tile, H_tile], transpose to [H_tile, S_tile] for matmul
                ...
                # Change 3: Pre-transposed Wk[H, Hkv_out] -> load [ho:ho+PMAX, kh*d:kh*d+PMAX]
                # = [PMAX_h, PMAX_d] which is already [K, free_dim] for nc_matmul moving operand.
                # No nc_transpose needed!
                wkTs = nl.ndarray((PMAX, PMAX), dtype=nl.bfloat16, buffer=nl.sbuf,
                                  name=f"p1_wkTs_s{si}_h{kh}_t{ht}")
                nisa.dma_copy(dst=wkTs, src=Wk[ho:ho+PMAX, kh*d:kh*d+PMAX])
                nisa.nc_matmul(kp, stationary=hTs, moving=wkTs)
            ...
            for ht in range(num_h_tiles):
                ...
                # Change 3: Pre-transposed Wv[H, Hkv_out] -> load [ho:ho+PMAX, kh*d:kh*d+PMAX]
                wvTs = nl.ndarray((PMAX, PMAX), dtype=nl.bfloat16, buffer=nl.sbuf,
                                  name=f"p1_wvTs_s{si}_h{kh}_t{ht}")
                nisa.dma_copy(dst=wvTs, src=Wv[ho:ho+PMAX, kh*d:kh*d+PMAX])
    ...
    # Change 2: Loop order restructured to hoist hidden transpose and Wq loads.
    #
    # NEW loop order:
    #   for qsi -> [compute hqTs_tiles once per ht, invariant to kvh/gi]
    #           -> for kvh -> for gi
    #                      -> [load wqTs_tiles per ht, pre-transposed, no nc_transpose]
    #                      -> Q proj + norm + RoPE + flash attn
    #
    # Hidden savings: 640 loads (kvh*gqa*qsi*ht) -> 80 loads (qsi*ht)  = 560 saved
    # Wq savings:     640 loads (kvh*gqa*qsi*ht) -> 128 loads (kvh*gqa*ht) = 512 saved
    #
    ...
    for qsi in nl.affine_range(num_s_tiles):
        ...
        # Change 2: Hoist hidden state transpose outside (kvh, gi) loop.
        # hqTs_tiles[ht] is [PMAX_H_tile, PMAX_S_tile] = transposed hidden for Q projection.
        # Depends only on (qso, ht), NOT on (kvh, gi) -> hoist completely outside.
        hqTs_tiles = []
        for ht in range(num_h_tiles):
            ho = ht * PMAX
            hqb = nl.ndarray((PMAX, PMAX), dtype=nl.bfloat16, buffer=nl.sbuf,
                             name=f"p2_hq_s{qsi}_t{ht}")
            nisa.dma_copy(dst=hqb, src=hidden_2d[qso:qso+PMAX, ho:ho+PMAX])
            hqTp = nl.ndarray((PMAX, PMAX), dtype=nl.bfloat16, buffer=nl.psum,
                              name=f"p2_hqTp_s{qsi}_t{ht}")
            nisa.nc_transpose(hqTp, hqb)
            hqTs = nl.ndarray((PMAX, PMAX), dtype=nl.bfloat16, buffer=nl.sbuf,
                              name=f"p2_hqTs_s{qsi}_t{ht}")
            nisa.tensor_copy(hqTs, hqTp)
            hqTs_tiles.append(hqTs)

        # Also hoist cos/sin for this Q sequence tile (invariant to kvh/gi)
        ...
        for kvh in nl.affine_range(Hkv_tp):
            for gi in nl.affine_range(gqa):
                ...
                # Change 1+2: Load Wq tiles using pre-transposed layout [H, Hq_out].
                # Tile: Wq[ho:ho+PMAX, qh*d:qh*d+PMAX] = [PMAX_h, PMAX_d].
                # Already [K, free_dim] for nc_matmul moving operand -> no nc_transpose.
                # Hoisted outside qsi (depends only on qh=kvh*gqa+gi, not qsi).
                wqTs_tiles = []
                for ht in range(num_h_tiles):
                    ho = ht * PMAX
                    ...
                    wqTs_tiles.append(wqTs)

                # Q projection using hoisted tiles
                qp = nl.zeros((PMAX, PMAX), dtype=nl.float32, buffer=nl.psum,
                              name=f"p2_qp_kv{kvh}_g{gi}_s{qsi}")
                for ht in range(num_h_tiles):
                    nisa.nc_matmul(qp, stationary=hqTs_tiles[ht], moving=wqTs_tiles[ht])
                ...
                # Q RoPE (uses hoisted cosqf/sinqf)
...
if __name__ == "__main__":
    ...
    def pytorch_reference(hidden_states, Wq, Wk, Wv, q_norm_weight, k_norm_weight,
        ...
        """
        Reference implementation using original (non-transposed) weight shapes:
          Wq: [Hq_tp*d, H]  = [1024, 2048]
          Wk: [Hkv_tp*d, H] = [512, 2048]
          Wv: [Hkv_tp*d, H] = [512, 2048]
        """
    ...
    def run_test(B, S, H=2048, d=128, Hq_tp=8, Hkv_tp=4):
        print(f"\n{'=' * 70}")
        print(f"v2b_qwen3_attn_cte_fused Test: B={B}, S={S}, H={H}, d={d}")
        ...
        # Create weights in original layout [out_features, in_features]
        ...
        print("\n[3/4] Running NKI kernel (v2b — pre-transposed weights + hidden hoist)...")
        # Pass transposed weights to kernel: Wq.T = [H, Hq_out], Wk.T = [H, Hkv_out], etc.
        nki_out_dev = qwen3_attn_cte_fused(
            hidden_dev, Wq_dev.T.contiguous(), Wk_dev.T.contiguous(), Wv_dev.T.contiguous(),
            qnw_dev, knw_dev,
