...
Plan B optimization (Hidden State Preload):
  Preloads ALL hidden state tiles into SBUF before the main loops, eliminating
  redundant HBM DMA loads for hidden_2d.

  v2a cost for hidden_2d loads:
    Phase 1 K-proj: 4 kvh * 5 si * 16 ht = 320 loads
    Phase 1 V-proj: 4 kvh * 5 si * 16 ht = 320 loads (DUPLICATE of K-proj)
    Phase 2 Q-proj: 4 kvh * 2 gi * 5 qsi * 16 ht = 640 loads
    Total: 1280 HBM DMA loads for hidden tiles

  v3b eliminates all but num_s_tiles * num_h_tiles = 80 loads:
    SBUF cost: 5 * 16 * 32KB = 2.56MB (persists for entire kernel)
    hidden_tiles[si][ht] is loaded once, reused across all kvh and gi iterations.

  Sub-change 5: Phase 1 si loop converted from range() to nl.affine_range()
    Each si iteration writes K_tiles[si]/V_tiles[si] (distinct SBUF tensors).
    With hidden DMA removed from Phase 1, affine_range allows compiler to pipeline
    PSUM initialization and matmul instructions across si iterations.
...
def qwen3_attn_cte_fused(
    ...
    # =========================================================================
    # PLAN B Sub-change 1: Preload ALL hidden tiles into SBUF before main loops.
    # hidden_tiles[si][ht] = [PMAX, PMAX] bf16 SBUF tile
    # SBUF cost: num_s_tiles × num_h_tiles × 32KB = 5 × 16 × 32KB = 2.56MB
    # Each (si, ht) pair is independent. Use range() for both loops — nl.ndarray
    # allocations inside loops with a Python list require range() for indexing.
    # Use explicit nested loops to build the list to avoid list-comprehension
    # tracing issues in the NKI JIT compiler.
    # =========================================================================
    hidden_tiles = []
    for si in range(num_s_tiles):
        ...
        for ht in range(num_h_tiles):
            s_off = si * PMAX
            ho = ht * PMAX
            h_tile = nl.ndarray((PMAX, PMAX), dtype=nl.bfloat16, buffer=nl.sbuf,
                                name=f"htile_s{si}_t{ht}")
            nisa.dma_copy(dst=h_tile, src=hidden_2d[s_off:s_off + PMAX, ho:ho + PMAX])
            row.append(h_tile)
    ...
    for kvh in nl.affine_range(Hkv_tp):
        ...
        # PLAN B Sub-change 5: Use nl.affine_range for si loop.
        # Each si iteration writes to K_tiles[si] and V_tiles[si] (distinct SBUF
        # tensors). With hidden DMA eliminated, affine_range lets the compiler
        # pipeline PSUM initialization and matmul across si iterations.
        # -----------------------------------------------------------------
        K_tiles = []  # Python list of SBUF tensors: K_tiles[si] = krbf [PMAX, PMAX] bf16
        ...
        for si in nl.affine_range(num_s_tiles):
            ...
            for ht in range(num_h_tiles):
                ho = ht * PMAX
                # PLAN B Sub-change 2: Use preloaded hidden tile — no HBM DMA
                hb = hidden_tiles[si][ht]
            ...
            # PLAN A: Store K tile in SBUF (append to Python list), NOT to HBM scratch.
            # krbf stays in SBUF; Phase 2 will use K_tiles[ki] directly.
            ...
            for ht in range(num_h_tiles):
                ho = ht * PMAX
                # PLAN B Sub-change 3: Use preloaded hidden tile — same tile as K-proj
                hvb = hidden_tiles[si][ht]
        ...
        for gi in nl.affine_range(gqa):
            ...
            for qsi in nl.affine_range(num_s_tiles):
                ...
                for ht in range(num_h_tiles):
                    ho = ht * PMAX
                    # PLAN B Sub-change 4: Use preloaded hidden tile — no HBM DMA
                    hqb = hidden_tiles[qsi][ht]
                ...
                for ki in range(num_s_tiles):
                    ...
                    # PLAN A: K tile already in SBUF — no HBM DMA needed.
                    # K_tiles[ki] = krbf from Phase 1 si=ki iteration (same kvh).
                    # kt is already bf16 in SBUF; proceed directly to nc_transpose.
                    kt = K_tiles[ki]
