...
v7b architecture (v6a + narrow broadcast softmax — Plan B softmax):
  ...
  DMA broadcast budget per qsi (v6a -> v7b):
    v6a: 1x wide [PMAX,4*PMAX] + 1x narrow [PMAX,PMAX] = 2 broadcasts
    v7b: 1x narrow [PMAX,PMAX] = 1 broadcast (2x fewer, 5x less total DMA volume)
"""
    ...
    for kvh in nl.affine_range(Hkv_tp):
        ...
        for gi in nl.affine_range(gqa):
            ...
            for qsi in nl.affine_range(num_s_tiles):
                ...
                # This is the ONLY broadcast per qsi iteration — reused for both wide and tail.
                ...
                for ti in range(len(masked_tail_tiles)):
                    ...
                    # Accumulate tail tile's exp sum into running row_sum
...
if __name__ == "__main__":
    ...
    def run_test(B, S, H=2048, d=128, Hq_tp=8, Hkv_tp=4):
        print(f"\n{'=' * 70}")
        print(f"qwen3_attn_cte_fused v7b Test: B={B}, S={S}, H={H}, d={d}")
