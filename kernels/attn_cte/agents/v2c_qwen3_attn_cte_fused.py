...
Plan C2 optimizations (on top of v1c):
  1. Attention mask preload: all num_s_tiles x num_s_tiles mask tiles preloaded
     into SBUF before Phase 1. Eliminates 175 redundant 64KB HBM mask reads in
     Phase 2 flash attention (only 25 unique tiles needed, loaded 200 times in v1c).
  2. cos/sin deduplication: Phase 2 loop reordered to qsi -> (kvh -> gi) so
     cos/sin Q tiles are loaded once per qsi (5 times) instead of once per
     (kvh, gi, qsi) = 40 times. Saves 70 x 32KB = 2.24MB wasted HBM reads.
"""
...
def qwen3_attn_cte_fused(
    ...
    # =========================================================================
    # PLAN C2 Change 1: Preload all attention mask tiles into SBUF before Phase 1.
    # num_s_tiles x num_s_tiles = 25 tiles of [PMAX, PMAX] float32 = 1.6MB total.
    # Eliminates 175 redundant HBM reads in Phase 2 (200 loads -> 25 preloads).
    # =========================================================================
    mk_pre = []
    for qsi in range(num_s_tiles):
        ...
        for ki in range(num_s_tiles):
            qso_mk = qsi * PMAX
            kso_mk = ki * PMAX
            mk_tile = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                 name=f"mk_pre_q{qsi}_k{ki}")
            nisa.dma_copy(dst=mk_tile, src=attn_mask[qso_mk:qso_mk+PMAX, kso_mk:kso_mk+PMAX])
            mk_row.append(mk_tile)
        mk_pre.append(mk_row)
    ...
    for si in nl.affine_range(num_s_tiles):
        ...
        for kh in nl.affine_range(Hkv_tp):
            # K projection
            ...
            for gi in nl.affine_range(gqa):
                ...
                for ki in range(num_s_tiles):
                    ...
                    kt = nl.ndarray((PMAX, PMAX), dtype=nl.bfloat16, buffer=nl.sbuf,
                                    name=f"p2_kt_s{qsi}_kv{kvh}_g{gi}_k{ki}")
                    ...
                    nmax = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                      name=f"p2_nmax_s{qsi}_kv{kvh}_g{gi}_k{ki}")
                    ...
                    nnmax = nl.ndarray((PMAX, PMAX), dtype=nl.float32, buffer=nl.sbuf,
                                       name=f"p2_nnmax_s{qsi}_kv{kvh}_g{gi}_k{ki}")
