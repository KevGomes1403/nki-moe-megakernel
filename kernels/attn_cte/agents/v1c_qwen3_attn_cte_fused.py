...
Plan C optimization:
  Eliminates HBM broadcast staging for [PMAX,1] -> [PMAX,PMAX] broadcasts.
  Instead uses nc_matmul outer-product: [PMAX,1] x [1,PMAX] -> [PMAX,PMAX].
  This avoids ~800 tiny 512-byte DMA round-trips in flash attention (tmax/tsum),
  reducing DMA Packet Time which was dominating over DMA Transfer Time.
"""
    ...
    for si in nl.affine_range(num_s_tiles):
        ...
        for kh in nl.affine_range(Hkv_tp):
            ...
            for ht in range(num_h_tiles):
                ...
                nisa.nc_matmul(vp, stationary=hvTs, moving=wvTs)                 # Big gap between nisa.nc_transpose and this
