...
v4b architecture (v3a + hidden preload + merged K/V projection):
  v3a: weight and cos/sin hoisting (see v3a docstring)
  v3b addition: preload all 80 hidden tiles [si][ht] into SBUF before loops.
    SBUF cost: 5 * 16 * 32KB = 2.56MB
    DMA eliminated: 1,280 hidden HBM loads -> 80 loads (1200 eliminated)
      Phase 1 K+V: was 320+320=640 loads, now 0 (use hidden_tiles from SBUF)
      Phase 2 Q:   was 640 loads, now 0 (use hidden_tiles from SBUF)
  Merged K+V ht loop: share hidden tile for both K and V matmul per (kvh, si, ht).
...
def qwen3_attn_cte_fused(
    ...
    # =========================================================================
    # v3b HOISTING: Preload all hidden state tiles into SBUF once.
    # Eliminates 1,280 HBM DMA loads from the hot path:
    #   Phase 1 K-proj: 4*5*16=320 loads, V-proj: 320 loads, Phase 2 Q-proj: 640 loads
    # SBUF cost: 5 si * 16 ht * [128,128] bf16 = 2.56MB (persists entire kernel)
    ...
    # =========================================================================
    hidden_tiles = []  # hidden_tiles[si][ht] = [S, H] bf16 in SBUF (raw, not pre-transposed)
    ...
    for kvh in nl.affine_range(Hkv_tp):
        ...
        # Use nl.affine_range for si -- K_tiles[si] and V_tiles[si] are distinct
        # SBUF tensors; hidden DMA is eliminated so affine_range is safe here.
        # -----------------------------------------------------------------
        ...
        for si in nl.affine_range(num_s_tiles):
            ...
            # v4b: Merged K+V projection -- share hidden tile for both matmuls.
            ...
            for ht in nl.affine_range(num_h_tiles):
                # v4b: Use preloaded hidden tile from SBUF -- no HBM DMA.
        ...
        for gi in nl.affine_range(gqa):
            ...
            for qsi in nl.affine_range(num_s_tiles):
                ...
                # Q projection
                # v4b: Use preloaded hidden tiles from SBUF -- no HBM DMA.
                ...
                for ht in nl.affine_range(num_h_tiles):
                    hqb = hidden_tiles[qsi][ht]  # from SBUF -- no HBM DMA
...
if __name__ == "__main__":
    ...
    def run_test(B, S, H=2048, d=128, Hq_tp=8, Hkv_tp=4):
        print(f"\n{'=' * 70}")
        print(f"qwen3_attn_cte_fused v4b Test: B={B}, S={S}, H={H}, d={d}")
