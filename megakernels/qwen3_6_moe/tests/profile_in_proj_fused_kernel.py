"""Emit a NEFF for the FULL front-half DeltaNet TKG kernel (input RMSNorm + 4-way projection +
conv + gated-delta recurrence + gated per-head RMSNorm), SBUF-resident in one launch, for profiling.

Runs ONE config the way the model invokes it per rank so neuron-explorer can capture/replay the
kernel's NEFF. Reference math is NOT computed. Inputs mirror tests/test_deltanet_in_proj_fused_kernel.py
(all f32, per-rank decode/verify shapes for the value-head-sharded entrypoints):
...
def main():
    ...
    if mode == "decode":
        fn = deltanet_in_proj_fused_tkg_fwd
    else:
        fn = deltanet_in_proj_fused_tkg_fwd_state
    ...
    print(f"[profile_in_proj_fused] emitted NEFF for {mode} T={T} ({dt}) -> {_OUTDIR}")
