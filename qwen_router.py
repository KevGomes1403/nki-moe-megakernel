# coding=utf-8
"""
Qwen3 MoE model that swaps ONLY the TKG router with the routertopk_nki kernel.

CTE path (past_key_value is None):
    Identical to qwen.py — post_attention_layernorm + standard NxDI MoE
    (router + expert_mlps) via initialize_moe_module(...).

TKG path (past_key_value is not None):
    Identical to qwen.py except RouterTopK is replaced by the
    routertopk_hbm NKI kernel (kernels/moe_fused_tkg/routertopk_nki.py).
    The expert MLPs are still NxDI's ExpertMLPsV2.

Top-K affinity layout:
    routertopk_hbm returns (top_k_idx, top_k_vals) where top_k_vals are
    softmax probabilities at the chosen experts (bf16). To call NxDI's
    expert_mlps unchanged we scatter them back into a full [T, E]
    expert_affinities tensor (zeros at non-top-K positions). NxDI's
    get_expert_affinities_masked masks by expert_index, so the missing
    fp32 softmax values at non-top-K positions are dropped anyway —
    the result is identical to feeding the full softmax distribution.
"""
...
# Hidden dim and expert count are pinned by the NKI router kernel
# (kernels/moe_fused_tkg/routertopk_nki.py uses _H=2048, _E=128, _K=8).
_MOE_H = 2048
_MOE_E = 128
_MOE_K = 8
...
# ---------------------------------------------------------------------------
# Decoder layer — identical to qwen.py except TKG router uses routertopk_nki.
# ---------------------------------------------------------------------------

class NeuronQwen3MoeDecoderLayerRouter(nn.Module):
    """
    CTE (past_key_value is None): identical to qwen.py — standard
    post_attention_layernorm + self.mlp(...).

    TKG (past_key_value is not None): post_attention_layernorm →
    routertopk_hbm (NKI) for top-K affinities → NxDI ExpertMLPsV2 →
    TP all-reduce. Same structure as qwen.py's MoE._forward_compute_bound
    but with the NxDI RouterTopK forward replaced by the NKI kernel.
    """
    ...
    def __init__(self, config: Qwen3MoeInferenceConfig, layer_idx: int):
        ...
        # NxDI default MoE module — used as-is for CTE; for TKG we keep its
        # expert_mlps and replace only the router with the NKI kernel.
    ...
    def _tkg_moe_with_nki_router(self, hidden_states, padding_mask):
        """RMSNorm → NKI router → NxDI expert_mlps → TP all-reduce."""
        norm_hidden = self.post_attention_layernorm(hidden_states)
        orig_shape = norm_hidden.shape
        seq_len = orig_shape[1]

        norm_hidden_2d = norm_hidden.reshape(-1, _MOE_H)

        # NxDI RouterTopK stores linear_router.weight as [E, H] in router
        # dtype (fp32 for Qwen3). The NKI kernel expects [H, E] bf16.
        router_w = self.mlp.router.linear_router.weight  # [E, H] fp32
        router_w_bf16 = router_w.t().to(torch.bfloat16).contiguous()  # [H, E] bf16
        inp_bf16 = norm_hidden_2d.to(torch.bfloat16).contiguous()

        # routertopk_hbm: [T, H] bf16, [H, E] bf16 -> [T, K] uint32, [T, K] bf16
        # top_k_vals are the bf16-rounded fp32 softmax probabilities at the
        # K chosen experts (NOT L1-normalized; expert_mlps does that when
        # neuron_config.normalize_top_k_affinities=True).
        top_k_idx_u32, top_k_vals = routertopk_hbm(inp_bf16, router_w_bf16)

        expert_index = top_k_idx_u32.to(torch.long)

        # Scatter top-K softmax probs into a full [T, E] expert_affinities
        # tensor (zeros at non-top-K positions). NxDI's
        # get_expert_affinities_masked masks by expert_index, so non-top-K
        # values are dropped — equivalent to feeding the full softmax.
        T = norm_hidden_2d.shape[0]
        expert_affinities = torch.zeros(
            T, _MOE_E, dtype=norm_hidden_2d.dtype, device=norm_hidden_2d.device,
        )
        expert_affinities = expert_affinities.scatter(
            1, expert_index, top_k_vals.to(norm_hidden_2d.dtype)
        )

        output = self.mlp.expert_mlps(
            hidden_states=norm_hidden_2d,
        ...
        # Delayed all-reduce — NxDI's MoE._forward_compute_bound does this
        # at the end of its non-SP path; we do it here since we bypassed it.
    ...
    def forward(
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            # TKG: NxDI default everywhere except RouterTopK, which is
            # replaced by the routertopk_hbm NKI kernel.
            hidden_states = self._tkg_moe_with_nki_router(hidden_states, padding_mask)
        else:
            # CTE: identical to qwen.py
...
class NeuronQwen3MoeModelRouter(NeuronBaseModel):
    ...
    def init_model(self, config: Qwen3MoeInferenceConfig):
        ...
        self.layers = nn.ModuleList(
            [
                NeuronQwen3MoeDecoderLayerRouter(config, layer_idx)
...
class NeuronQwen3MoeForCausalLMRouter(NeuronBaseForCausalLM):
    """Qwen3 MoE model that swaps only the TKG RouterTopK with routertopk_hbm."""

    _model_cls = NeuronQwen3MoeModelRouter
...
NeuronQwen3MoeForCausalLM = NeuronQwen3MoeForCausalLMRouter
...
def generate(model_path, traced_model_path, skip_compile=False):
    ...
    if not skip_compile:
        ...
        model = NeuronQwen3MoeForCausalLMRouter(model_path, config)
        model.compile(traced_model_path)
        tokenizer.save_pretrained(traced_model_path)

    print("\nLoading model from compiled checkpoint...")
    model = NeuronQwen3MoeForCausalLMRouter(traced_model_path)
