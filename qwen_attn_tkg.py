...
class Qwen3V14DAttnNeuronConfig(MoENeuronConfig):
    ...
    def __init__(self, **kwargs):
        ...
        # Match main_new.py: enable on-device sampling from the neuron config
        # kwargs instead of hardcoding sampler values here.
...
def _mask_v14d_current_token_from_prior(attn, attention_mask, position_ids, K_cache):
    """Hide the just-written cache slot from compute_for_token_gen's prior path."""

    s_prior = attention_mask.shape[-1]
    k_seq_dim = 3 if attn.k_cache_transposed else 2
    k_cache_len = K_cache.shape[k_seq_dim]
    current_idx = position_ids.to(torch.long)

    if attn.neuron_config.padding_side == "left" and k_cache_len != s_prior:
        current_idx = current_idx - (k_cache_len - s_prior)

    current_idx = current_idx.reshape(-1, 1, 1, 1)
    current_idx = current_idx.expand(
        attention_mask.shape[0],
        attention_mask.shape[1],
        attention_mask.shape[2],
        1,
    )
    current_value = torch.zeros_like(
        torch.ops.aten.slice(attention_mask, dim=-1, start=0, end=1)
    )
    return torch.scatter(attention_mask, dim=-1, index=current_idx, src=current_value)
...
def _attention_tkg_v14d(
    ...
    prior_attention_mask = _mask_v14d_current_token_from_prior(
        attn,
        attention_mask,
        position_ids,
        K_prior,
    )
    K_attn, V_attn = _slice_v14d_cache_for_baseline_attention(
        attn,
        K_prior,
        V_prior,
