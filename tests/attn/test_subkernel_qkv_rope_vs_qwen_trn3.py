"""
Hardware sweep for the SBUF QKV projection + RoPE + K/V write subkernel.

The input is an already-normalized hidden state, matching the handoff from the
RMSNorm subkernel.

Run:
  python tests/attn/test_subkernel_qkv_rope_vs_qwen_trn3.py
"""
...
from subkernel_common import KernelQkvRopeStageModule, RefQkvRopeStageModule, make_tp1_config
...
def _stats(expected, actual):
    labels = ("Q", "K_active", "V_active", "K_cache", "V_cache")
    return {
        label: (act.detach().cpu().float() - exp.detach().cpu().float()).abs().max().item()
...
def _check_weights(ref_weight_store, kern_weight_store) -> None:
    for key in WEIGHT_KEYS:
        diff = (ref_weight_store[key].float() - kern_weight_store[key].float()).abs().max().item()
        print(f"  {key:<16} max_abs_diff={diff:.6g}", flush=True)
        if diff != 0.0:
...
def main() -> None:
    ...
    cfg = make_tp1_config()
    example_normed = torch.zeros(B, 1, H, dtype=torch.bfloat16)
    ...
    example_inputs = [(example_normed, example_pos, example_K, example_V)]
    ...
    workdir_ctx = (
        ...
        else tempfile.TemporaryDirectory(prefix="subkernel_qkv_rope_")
    )
    with workdir_ctx as workdir:
        ...
        for weight_scale in args.weight_scales:
            print(f"\n{'=' * 72}", flush=True)
            print(f"qkv_rope weight_scale={weight_scale:g}", flush=True)
            print(f"{'=' * 72}", flush=True)

            ref_weight_store = {}
            ref_traced = build_module(
                module_cls=RefQkvRopeStageModule,
            ...
            kern_traced = build_module(
                module_cls=KernelQkvRopeStageModule,
                example_inputs=example_inputs,
                module_init_kwargs={
                    "config": cfg,
                    "seed": args.seed,
            ...
            for case in cases:
                normed, _, _, position_ids, K_cache, V_cache = make_sample(case)
                expected = ref_traced(normed, position_ids, K_cache.clone(), V_cache.clone())
                actual = kern_traced(normed, position_ids, K_cache.clone(), V_cache.clone())
                ...
                stats = _stats(expected, actual)
                if args.print_failures and (not pass_strict or not pass_loose):
                    verdict = "FAIL" if not pass_loose else "STRICT"
                    print(
                        f"  {format_case(case)} "
                        f"Q={stats['Q']:.3e} K={stats['K_active']:.3e} V={stats['V_active']:.3e} "
                        f"Kc={stats['K_cache']:.3e} Vc={stats['V_cache']:.3e} {verdict}",
                        flush=True,
                    )
                if (case["sample_idx"] + 1) % 32 == 0 or case is cases[-1]:
                    print(
                        f"  {format_case(case)} "
                        f"Q={stats['Q']:.3e} K={stats['K_active']:.3e} V={stats['V_active']:.3e} "
                        f"Kc={stats['K_cache']:.3e} Vc={stats['V_cache']:.3e}",
                        flush=True,
                    )
