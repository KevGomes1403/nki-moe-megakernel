# coding=utf-8
"""
Qwen3 MOE model for NXD inference with v6b NKI kernel for TKG MoE path.

Copy of qwen.py that replaces the NxDI MoEFusedTKG module with
MoEV6bModule (RouterTopK + v6b coalesced-DMA NKI kernel) for the token
generation MoE path. Context encoding still uses initialize_moe_module.
"""
...
class Qwen3MoeInferenceConfig(InferenceConfig):
    ...
    def maybe_pad_intermediate(self):
        ...
        # For the v6b NKI kernel: I_TP must be divisible by MOE_TKG_MK_INTERMEDIATE_PER_TP (128).
        # Pad if needed so enable_moe_fused_nki_kernel() will activate the kernel path.
        # MoENeuronConfig defaults moe_fused_nki_kernel_enabled=None; treat None as True (enabled).
        if getattr(self.neuron_config, "moe_fused_nki_kernel_enabled", None) is not False:
            I_TP_now = self.moe_intermediate_size // moe_tp_degree
            if I_TP_now % MOE_TKG_MK_INTERMEDIATE_PER_TP != 0:
                padded_I_TP = math.ceil(I_TP_now / MOE_TKG_MK_INTERMEDIATE_PER_TP) * MOE_TKG_MK_INTERMEDIATE_PER_TP
                padded_moe_intermediate_size = padded_I_TP * moe_tp_degree
                extra_pad = padded_moe_intermediate_size - self.moe_intermediate_size
                self.moe_intermediate_pad_size = getattr(self, "moe_intermediate_pad_size", 0) + extra_pad
                self.moe_intermediate_size = padded_moe_intermediate_size
    ...
    def enable_moe_fused_nki_kernel(self):
        ...
        if getattr(self.neuron_config, "moe_fused_nki_kernel_enabled", None) is not False and I_TP % MOE_TKG_MK_INTERMEDIATE_PER_TP == 0:
...
class NeuronQwen3MoeDecoderLayer(nn.Module):
    ...
    def __init__(self, config: Qwen3MoeInferenceConfig, layer_idx: int):
        ...
        # Use v6b kernel only for TKG (is_prefill_stage=False).
        # CTE always uses initialize_moe_module regardless of moe_fused_nki_kernel_enabled.
        is_prefill = getattr(config.neuron_config, "is_prefill_stage", False)
        self.use_v6b_kernel = self.moe_fused_nki_kernel_enabled and not is_prefill
        self.normalize_top_k = getattr(config.neuron_config, "normalize_top_k_affinities", False)

        # Always use initialize_moe_module for weight storage: this gives properly
        # TP-sharded expert weights via ExpertMLPsV2. For TKG we bypass its forward
        # and call the v6b NKI kernel directly with the sharded weights.
        if self.use_v6b_kernel:
            logger.debug("Using v6b NKI kernel for MoE TKG path (weights via initialize_moe_module)")
        else:
            logger.debug("Using initialize_moe_module forward for MoE (CTE or kernel disabled)")
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.use_v6b_kernel:
            # TKG path: RouterTopK routing + v6b NKI kernel for expert FFN.
            # Weights come from initialize_moe_module's ExpertMLPsV2 (TP-sharded).
            original_shape = hidden_states.shape
            hidden_flat = hidden_states.reshape(-1, self.hidden_size) if hidden_states.dim() == 3 else hidden_states

            router_logits, expert_affinities, expert_index = self.mlp.router(hidden_flat)
            routing_weights_k = torch.gather(expert_affinities, 1, expert_index)
            if self.normalize_top_k:
                routing_weights_k = F.normalize(routing_weights_k.float(), p=1.0, dim=1)

            # Pass full [E, H, 2I] and [E, I, H] weight tensors directly to the
            # v8a kernel.  v8a uses DGE (indirect_dim=0) with a per-k scratch
            # buffer loaded from HBM at offset=0, avoiding the absolute-SBUF-
            # address baking bug that broke v7a in E2E compilation.
            gate_up_w = self.mlp.expert_mlps.mlp_op.gate_up_proj.weight.data  # [E, H, 2I]
            down_w = self.mlp.expert_mlps.mlp_op.down_proj.weight.data        # [E, I, H]

            hidden_states = nki_moe_v8a_qwen3(
