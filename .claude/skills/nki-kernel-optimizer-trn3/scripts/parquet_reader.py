...
Usage:
    ...
    python parquet_reader.py parquet_files/profiles/global/my-kernel@latest/

trn3-specific sections:
  - VectorE utilization is reported prominently — high VectorE with low
    TensorE indicates quantize_mx is the bottleneck.
  - SBUF spill annotation references 32 MiB SBUF (trn3 has 4 MiB more than trn2).
"""
...
def _print_summary(parquet_dir: str) -> None:
    ...
    tensor_s = float(r.get("tensor_engine_active_time", 0))
    vector_s = float(r.get("vector_engine_active_time", 0))
    for label, key, note in [
        ("tensor_engine", "tensor_engine_active_time",  ""),
        # VectorE annotation: high % on trn3 = quantize_mx bottleneck
        ("vector_engine", "vector_engine_active_time",  "  ← high = quantize-bound (trn3)"),
    ...
    # trn3 bottleneck classification hint
    if total_s > 0:
        print(f"\n  ── Bottleneck Hint (trn3) ───────────────────────")
        te_pct = tensor_s / total_s * 100
        ve_pct = vector_s / total_s * 100
        dma_s  = float(r.get("dma_active_time", 0))
        dma_pct = dma_s / total_s * 100
        spill = float(r.get("spill_save_bytes", 0)) + float(r.get("spill_reload_bytes", 0))
        if spill > 0:
            print(f"  SPILL-BOUND      spill={_fmt_bytes(spill)}  (SBUF=32MiB on trn3 — reduce tile size)")
        elif te_pct >= 90:
            print(f"  COMPUTE-BOUND    tensor_engine={te_pct:.1f}%  → if BF16, migrate to nc_matmul_mx for 4× throughput")
        elif ve_pct >= 40 and te_pct < 50:
            print(f"  QUANTIZE-BOUND   vector_engine={ve_pct:.1f}%  → move weights to offline quantization or pipeline quantize_mx")
        elif dma_pct >= 60 and te_pct < 40:
            print(f"  MEMORY-BOUND     dma_active={dma_pct:.1f}%  → use MXFP8 weights (halves HBM bytes) or larger tiles")
        else:
            print(f"  STALL/MIXED      te={te_pct:.1f}%  ve={ve_pct:.1f}%  dma={dma_pct:.1f}%  → check dependency chain or scheduling")
    ...
    spill_save   = r.get("spill_save_bytes", 0)
    spill_reload = r.get("spill_reload_bytes", 0)
    spill_note = "  ← SBUF overflow (32MiB limit on trn3)" if (spill_save + spill_reload) > 0 else ""
    print(f"  spill_save          = {spill_save} bytes{spill_note}")
    print(f"  spill_reload        = {spill_reload} bytes")
...
def _print_active_time(parquet_dir: str) -> None:
    ...
    for eng, val in by_engine.items():
        note = "  ← quantize_mx (trn3)" if "vector" in str(eng).lower() and val * scale > 10 else ""
        print(f"  {str(eng):<20} {val * scale:.2f} {unit}{note}")
...
def _print_hbm_usage(parquet_dir: str) -> None:
    ...
    nc_idx = df.get("neuroncore_idx", [0]).iloc[0] if "neuroncore_idx" in df.columns else 0
    print(f"\n  ── HBM Usage by Type (NC{nc_idx}) ─────────────────")
...
def print_profile(parquet_dir: str) -> None:
    ...
    print(f"\n{sep}")
    print(f"  Profile: {label}  [trn3 / NeuronCore-v4]")
