...
#
# ===========================================================================
# VENDORED from nkilib core/attention/attention_tkg.py
#   Source: nkilib (no version metadata) bundled with NKI 0.4.0 /
#           neuronx-cc 2.25.3371.0, installed 2026-06-12 under
#           aws_neuronx_venv_pytorch_2_9_nxd_inference.
#   Patched for head_dim=256 (Qwen3.6 GQA TKG, one TP shard).
#
# WHY: nkilib's TKG attention caps d_head at 128 because head_dim sits on the
# SBUF/PSUM partition axis and the PE array enforces contraction-K<=128,
# stationary-free-M<=128, partition<=128. This vendored copy lifts that cap to
# d_head=256 by tiling head_dim into D_TILES=ceil(d/128) partition tiles, and
# applying the tiling ONLY at the head_dim-on-partition sites:
#   (a) MM1 QK^T (head_dim is the contraction): split-K, the D_TILES partials
#       PSUM-accumulate into one scores tile.
#   (b) MM2 P.V (head_dim is the stationary-free -> output partition): D_TILES
#       stationary V-slices produce D_TILES PSUM halves [128, s_active_qh].
#   (store) the d-on-partition output store loops over D_TILES.
#
...
# tp_broadcast) are imported directly from the INSTALLED nkilib below, i.e.
# they are the unmodified AWS implementations -- only this file is patched.
# ===========================================================================
...
VENDORED + PATCHED for head_dim=256. See banner above. The patch tiles head_dim
into D_TILES = ceil(d_head / 128) partition tiles at the two matmul sites and the
output store; softmax and all other AWS logic are unmodified.
"""
...
# Unchanged AWS helpers: imported from installed nkilib (NOT re-vendored).
...
from nkilib.core.attention.gen_mask_tkg import gen_mask_tkg

# Vendored alongside this file (attention_tkg_utils.py is an unmodified copy).
...
# --- D256 PATCH (gate): head_dim tile width. AWS pinned d_head <= 128 (one
# partition tile). We now tile head_dim into ceil(d_head / _D_HEAD_TILE) partition
# tiles, so the only requirement is d_head % _D_HEAD_TILE == 0. ---
_D_HEAD_TILE = 128
_MAX_D_HEAD = 128
_MAX_S_PRIOR_ACCURATE_ROPE = 2**17


def _d_tiles(d_head: int) -> int:
    """Number of 128-wide partition tiles spanning head_dim (1 when d_head <= 128)."""
    return div_ceil(d_head, _D_HEAD_TILE)
...
def attention_tkg(
    ...
    atp = _compute_tile_params(
        cfg, TC, q, k_prior, v_prior, k_active, v_active, active_blocks_table
    )
    ...
    _, batch_tile_size = uses_batch_tiling(
        atp.bs_per_nc, cfg.q_head, cfg.s_active, atp.fa_tile_s_prior
    )
    ...
    for batch_tile_idx in range(num_batch_tiles):
        ...
        for fa_tile_idx in range(atp.num_fa_tiles):
            ...
            _compute_qk_matmul(
                k_prior, DBG_TENSORS, atp, cfg, TC, sbm, bufs, fa_ctx, btc
            )

            # Step 2. Cascaded max reduce of KQ^T (includes FA running max update)
            _cascaded_max_reduce(
                sink, DBG_TENSORS, atp, cfg, TC, sbm, bufs, fa_ctx, btc
            )
            ...
            _compute_pv_matmul_and_store(
                v_prior, v_active, out, atp, cfg, TC, sbm, bufs, fa_ctx, btc
            )
...
OOB_MODE_SKIP = (
    nisa.oob_mode.skip
)  # FIXME: needs to be instantiated externally from kernel
...
def _compute_fa_tile_context(
    fa_tile_idx: int, atp: AttnTileParams, TC: TileConstants
) -> FATileContext:
...
def _update_atp_for_batch_tile(atp: AttnTileParams, tile_bs: int, TC: TileConstants):
    ...
    atp.bs = tile_bs
    atp.s_active_bqh = (
        atp.bs * atp.s_active_qh
    )  # flattened dim of [bs, q_heads, s_active] for this batch tile
    ...
    atp.batch_interleave_degree = min(
        atp.bs, TC.psum_b_max
    )  # PSUM bank interleaving across batches
...
def _compute_tile_params(
    ...
    # --- D256 PATCH (gate): allow head_dim a multiple of the 128 partition tile
    # width (e.g. 256). head_dim is tiled into D_TILES partition tiles at the two
    # matmul sites and the output store; everything else is AWS-verbatim. ---
    kernel_assert(
        0 < cfg.d_head and cfg.d_head % _D_HEAD_TILE == 0,
        f"Unsupported d_head. Got d_head={cfg.d_head}, must be a positive multiple of {_D_HEAD_TILE}.",
    )
    ...
    atp.bs_full = cfg.bs
    atp.bs_per_nc = (
        atp.bs_full // atp.bs_n_prgs
    )  # full per-NC batch size before batch tiling
    atp.s_prior = (
        cfg.curr_sprior // atp.sprior_n_prgs
    )  # shard prior seqlen onto each prg
    atp.s_active_qh = cfg.s_active * cfg.q_head  # flattened dim of [q_heads, s_active]
    atp.n_sprior_tile = div_ceil(
        atp.s_prior, TC.p_max
    )  # total number of p_max-tiles across full s_prior
    ...
    use_fa, fa_tile_size = uses_flash_attention(
        cfg.enable_fa_s_prior_tiling, atp.s_prior
    )
    atp.use_fa = use_fa
    if atp.use_fa:
        ...
        last_tile_s_prior = (
            atp.s_prior % fa_tile_size
            if atp.s_prior % fa_tile_size != 0
            else fa_tile_size
        )
        kernel_assert(
...
def _setup_block_kv_cache(
    ...
    block_len, blk_cache_resize_factor = (
        resize_cache_block_len_for_attention_tkg_kernel(
    ...
    if (block_len < 8) or (
        block_len <= 16 and cfg.d_head <= 16 and atp.bs < 4 and atp.n_prgs == 2
    ):
...
def _setup_debug_tensors(
    DBG_TENSORS, atp: AttnTileParams, TC: TileConstants, bufs: AttnInternalBuffers
):
    ...
    bufs.DBG_QK_MAX = DBG_TENSORS[1].reshape(
        (atp.bs_n_prgs, atp.n_bsq_tiles, atp.s_active_bqh_tile)
    )
    ...
    bufs.DBG_EXP_SUM = DBG_TENSORS[3].reshape(
        (atp.bs_n_prgs, atp.n_bsq_tiles, atp.s_active_bqh_tile)
    )
    if atp.is_block_kv:
        ...
        kernel_assert(
            bufs.DBG_ACTIVE_TABLE.shape[1]
            == full_num_folds_per_batch * atp.sprior_n_prgs,
...
def _allocate_fa_buffers(
    atp: AttnTileParams, cfg: AttnTKGConfig, sbm: SbufManager, bufs: AttnInternalBuffers
):
    ...
    bufs.fa_running_output = sbm.alloc_stack(
        (cfg.d_head, atp.s_active_bqh), dtype=atp.inter_type, buffer=nl.sbuf
    )


def _fa_update_correction_factor(
    atp: AttnTileParams,
    fa_correction_factor: nl.ndarray,
    curr_running_max: nl.ndarray,
    ...
    for i_bsq_tile in range(atp.n_bsq_tiles):
        nisa.activation(
            ...
            bias=(curr_running_max if atp.max_negated else prev_running_max)[
                :, i_bsq_tile
            ],
            scale=-1.0,
        )
...
def _fa_gather_and_compute_global_running_max(
    ...
    kernel_assert(
        not atp.max_negated,
        "Unexpected atp.max_negated=True when deferring softmax gather",
    )
    ...
    if atp.softmax_final_reduction_sink_idx is not None:
        ...
        nisa.tensor_tensor(
            ...
            data2=bufs.qk_max_buf[
                : atp.s_active_bqh_tile, nl.ds(sink_offset, atp.n_bsq_tiles)
            ],
            op=nl.maximum,
        )
    ...
    remote_running_max = sbm.alloc_stack(
        bufs.fa_running_max.shape, dtype=atp.inter_type, buffer=nl.sbuf
    )
    ...
    _fa_update_correction_factor(
        atp, bufs.fa_correction_factor, bufs.fa_running_max, prev_running_max
    )
...
def _fa_update_running_max(
    ...
    if fa_ctx.fa_tile_idx == 0:
        ...
        prev_running_max = sbm.alloc_stack(
            bufs.fa_running_max.shape, dtype=bufs.fa_running_max.dtype
        )
...
def _fa_gather_and_compute_global_running_sum(
    ...
    remote_running_sum = sbm.alloc_stack(
        bufs.fa_running_sum.shape, dtype=atp.inter_type, buffer=nl.sbuf
    )
    ...
    if atp.softmax_final_reduction_sink_idx is not None:
        ...
        for i_bsq_tile in range(atp.n_bsq_tiles):
            sink_tile = bufs.qk_max_buf[
                : atp.s_active_bqh_tile, sink_offset + i_bsq_tile
            ]
            nisa.activation(
                dst=sink_tile,
                op=nl.exp,
...
def _fa_accumulate_output(
    ...
    if fa_ctx.fa_tile_idx == 0:
        ...
        fa_correction_factor_bc = sbm.alloc_stack(
            (cfg.d_head, atp.s_active_bqh), dtype=atp.inter_type, buffer=nl.sbuf
        )
        _s_active_bqh_tile_transpose_broadcast(
            bufs.fa_correction_factor, fa_correction_factor_bc, atp, TC
        )
...
def _fa_finalize_and_store(
    ...
    fa_sum_recip_bc = sbm.alloc_stack(
        (cfg.d_head, atp.s_active_bqh), dtype=atp.inter_type, buffer=nl.sbuf
    )
    _s_active_bqh_tile_transpose_broadcast(
        bufs.fa_running_sum, fa_sum_recip_bc, atp, TC
    )
    ...
    exp_v_sendrecv_gpsimd = (
        ...
        and atp.bs * atp.s_active_qh <= 128
        and cfg.d_head % 16 == 0
    )
    _gather_and_store_output(
        out, bufs.fa_running_output, exp_v_sendrecv_gpsimd, atp, cfg, sbm, btc
    )
...
def _load_and_broadcast_pos_ids(pos_ids, atp, cfg, TC, sbm, name):
    ...
    """
    pos_ids_sb = sbm.alloc_stack(
        (TC.p_max, atp.bs_per_nc * cfg.s_active), dtype=pos_ids.dtype
    )
    pos_ids = pos_ids.reshape([atp.bs_n_prgs, atp.bs_per_nc * cfg.s_active])

    sbm.open_scope()
    pos_ids_loaded = sbm.alloc_stack(
        (1, atp.bs_per_nc * cfg.s_active), dtype=pos_ids.dtype, align=4
    )
    nisa.dma_copy(
        pos_ids_loaded, pos_ids[atp.bs_prg_id, :], dge_mode=dge_mode.none, name=name
    )
...
def _load_position_ids(
    ...
    if rope_pos_ids is None:
        ...
        bufs.pos_ids_sb = _load_and_broadcast_pos_ids(
            rope_pos_ids, atp, cfg, TC, sbm, "rope_pos_ids_load"
        )

    bufs.start_pos_sb = None
    if start_pos_ids is not None:
        bufs.start_pos_sb = _load_and_broadcast_pos_ids(
            start_pos_ids, atp, cfg, TC, sbm, "start_pos_ids_load"
        )
...
def _load_mask(
    ...
    if not cfg.use_pos_id:
        ...
        mask = mask.reshape(
            (atp.sprior_n_prgs, atp.s_prior, atp.bs_n_prgs, full_s_active_bqh)
        )
        ...
        mask_hbm_view = (
            ...
            .slice(
                dim=0, start=fa_tile_offset, end=fa_tile_offset + fa_ctx.tile_s_prior
            )
        ...
        if cfg.strided_mm1 and not atp.is_block_kv:
            ...
            mask_hbm_view = mask_hbm_view.reshape_dim(
                0, [fa_tile_n_sprior, TC.p_max]
            ).permute([1, 0, 2])

        mask_sb_view = TensorView(bufs.mask_sb).reshape_dim(
            1, [fa_tile_n_sprior, atp.s_active_bqh]
        )
        nisa.dma_copy(
        ...
        bufs.mask_sb = bufs.mask_sb.reshape(
            (TC.p_max, fa_tile_n_sprior, atp.bs, cfg.q_head, cfg.s_active)
        )
        ...
        load_active_mask = (
            atp.sprior_prg_id == atp.sprior_n_prgs - 1
        ) and fa_ctx.is_last_fa_tile
        ...
        if bufs.start_pos_sb is not None:
            start_pos_tile = bufs.start_pos_sb[
                :, nl.ds(pos_ids_offset, atp.bs * cfg.s_active)
            ]
...
def _perform_rope(
    ...
    # Load inv_freqs
    sbm.open_scope()
    inv_freqs_sb = sbm.alloc_stack(
        inv_freqs.shape, dtype=inv_freqs.dtype, buffer=nl.sbuf
    )
    nisa.dma_copy(
        inv_freqs_sb, inv_freqs, dge_mode=dge_mode.none, name="inv_freqs_load"
    )
    ...
    if cfg.k_out_in_sb or (atp.sprior_prg_id == atp.sprior_n_prgs - 1):
        ...
        if not cfg.k_out_in_sb and k_out is not None:
            k_active_sb_view = TensorView(bufs.k_active_sb).reshape_dim(
                1, [cfg.bs, cfg.s_active]
            )
    ...
    nisa.activation(
        bufs.q_sb, op=nl.copy, data=bufs.q_sb, scale=1 / math.sqrt(cfg.d_head)
    )
    sbm.close_scope()
...
def _compute_qk_matmul(
    ...
    if atp.is_block_kv:
        ...
        if not atp.use_dma_transpose:
            sbuf_usage_per_batch += (
                ...
                * atp.block_len
                * cfg.d_head
                * sizeinbytes(atp.k_prior_load_type)
            )
    elif cfg.tp_k_prior and atp.is_fp8_kv:
        sbuf_usage_per_batch += (
            TC.psum_b_max * cfg.d_head * sizeinbytes(atp.k_prior_load_type)
        )
    ...
    per_batch_interleave_degree = math.floor(
        float(TC.psum_b_max) / batch_interleave_degree_safe
    )
    # --- D256 PATCH (a): head_dim is the MM1 contraction; tile it on partition. ---
    d_tiles = _d_tiles(cfg.d_head)
    ...
    for i_b in range(atp.bs):
        ...
        # --- D256 PATCH (a): k_sb is [128, D_TILES, s_prior] -- head_dim split
        # across D_TILES partition tiles. AWS used [d_head, s_prior]. ---
        k_sb = sbm.alloc_stack(
            (TC.p_max, d_tiles, fa_tile_s_prior),
            dtype=k_prior.dtype,
            buffer=nl.sbuf,
            align=32,
        )
        if atp.is_block_kv:
            ...
            for i_fold_rel in range(num_folds_this_tile):
                ...
                if atp.use_dma_transpose:
                    ...
                    cur_blks_u32 = nl.ndarray(
                        (TC.p_max, 1), dtype=nl.uint32, buffer=nl.sbuf
                    )
                    ...
                    nisa.dma_transpose(
                        dst=(
                            TensorView(k_sb)
                            .reshape_dim(
                                1, (num_folds_this_tile, atp.block_len, TC.p_max)
                            )
                            .select(1, i_fold_rel)
                            .expand_dim(1)
                            .get_view()
                        ...
                        for tp_j_in_grp in range(transpose_grp_size):
                            ...
                            tp_psum = nl.ndarray(
                                ...
                                if sbm.is_auto_alloc()
                                    ...
                                    0,
                                    (tp_j_in_grp % per_batch_interleave_degree)
                                    * TC.psum_f_max_bytes,
                                ),
                            )
                            nisa.nc_transpose(
            ...
            # --- D256 PATCH (a): load each head_dim tile into its own partition tile
            # of k_sb. k_prior_view is [d, fa_tile_s_prior]; slice d into D_TILES
            # 128-row chunks -> k_sb[:, dt, :]. AWS loaded the whole [d, *] at once. ---
            s_prior_pos = atp.sprior_prg_id * atp.s_prior + fa_tile_offset
            for dt in range(d_tiles):
                d_lo = dt * TC.p_max
                d_sz = min(TC.p_max, cfg.d_head - d_lo)
                k_prior_view = (
                    TensorView(k_prior)
                    .select(0, btc.global_batch_offset + i_b)
                    .squeeze_dim(0)
                    .slice(0, start=d_lo, end=d_lo + d_sz)
                ...
                nisa.dma_copy(
                    ...
                    dge_mode=dge_mode.none,
                    name=f"{sbm.get_name_prefix()}k_prior_flat_load_transposed_fa{fa_ctx.fa_tile_idx}_b{i_b}_d{dt}_bt{btc.batch_tile_idx}",
                )
        else:
            kernel_assert(
            ...
            if atp.is_fp8_kv:
                ...
                for tp_grp_i in range(fa_tile_n_sprior):
                    ...
                    s_prior_pos = (
                        atp.sprior_prg_id * atp.s_prior + fa_tile_offset + tile_start
                    )
                    k_prior_view = (
                        TensorView(k_prior)
                        .select(0, btc.global_batch_offset + i_b)
                    ...
                    nisa.tensor_copy(
                        k_sb[:, nl.ds(tile_start, tile_size)], tp_psum[:, :tile_size]
                    )
        ...
        if atp.sprior_prg_id == atp.sprior_n_prgs - 1 and is_last_fa_tile:
            if atp.is_block_kv:
                ...
                extra_covered = (
                    num_blks_covering_s_active * atp.block_len - cfg.s_active
                )
                ...
                if extra_covered > 0:
                    if atp.block_len > extra_covered:
                        ...
                        k_sb_view = TensorView(k_sb).slice(
                            1, start=start, end=end, step=TC.p_max
                        )
                    ...
                    if num_blks_covering_s_active > 1:
                        ...
                        k_sb_view = (
                            ...
                            .slice(
                                1,
                                start=k_sb_start_1,
                                end=k_sb_start_1 + num_blks_covering_s_active - 1,
                            )
                            .slice(
                                2, start=k_sb_start_2, end=k_sb_start_2 + atp.block_len
                            )
                        )
                        ...
                        k_active_view = (
                            ...
                            .slice(
                                ...
                                end=k_active_start_1
                                + (num_blks_covering_s_active - 1) * atp.block_len,
                            )
                            .reshape_dim(
                                1, [num_blks_covering_s_active - 1, atp.block_len]
                            )
                        )
                    ...
                    k_sb_view = (
                        ...
                        .slice(
                            1,
                            start=k_sb_start_1,
                            end=k_sb_start_1 + num_blks_covering_s_active,
                        )
                ...
                # --- D256 PATCH (a): append k_active into the s_prior tail per
                # head_dim tile. k_sb and k_active_sb both carry head_dim on the
                # D_TILES axis. AWS did one copy over the whole [d_head, *]. ---
                for dt in range(d_tiles):
                    d_sz = min(TC.p_max, cfg.d_head - dt * TC.p_max)
                    nisa.tensor_copy(
                        k_sb[
                            0:d_sz, dt, fa_tile_s_prior - cfg.s_active : fa_tile_s_prior
                        ],
                        bufs.k_active_sb[
                            0:d_sz,
                            dt,
                            nl.ds(
                                (btc.global_batch_offset + i_b) * cfg.s_active,
                                cfg.s_active,
                            ),
                        ],
                        engine=nisa.scalar_engine,
        ...
        for i_mm1_grp in range(div_ceil(fa_tile_s_prior, mm1_grp_sz)):
            ...
            for i_mm1 in range(n_mm1_per_grp):
                if cfg.strided_mm1:  # optionally use strided read to K s.t. MM2 can also be strided with sequential read to V
                ...
                # --- D256 PATCH (a): split-K over the D_TILES head_dim tiles. Each
                # partial product PSUM-accumulates into the SAME qk_psum_view
                # (hardware accumulation), so the scores are summed over the full
                # head_dim. k_tile (stationary) and q_sb (moving) are both sliced on
                # the head_dim partition tile dt. AWS issued one [d_head]-wide
                # matmul here; the surrounding tiling/masking is unchanged. ---
                for dt in range(d_tiles):
                    d_sz = min(TC.p_max, cfg.d_head - dt * TC.p_max)
                    if cfg.strided_mm1:
                        k_tile = (
                            ...
                            .slice(0, start=0, end=d_sz)
                            .get_view()
                        )
                    else:
                        k_tile = k_sb[0:d_sz, dt, nl.ds(k_tile_offset, num_acc)]
            ...
            num_acc_cpy = min(
                n_mm1_per_grp, fa_tile_s_prior // TC.p_max - i_mm1_grp * n_mm1_per_grp
            )
            ...
            qk_psum_view = (
                TensorView(qk_psum)
                .reshape_dim(1, [n_mm1_per_grp, atp.s_active_qh])
                .slice(1, start=0, end=num_acc_cpy)
            )
    ...
    if DBG_TENSORS:
        if cfg.strided_mm1 and (atp.use_fa or atp.bs != atp.bs_per_nc):
            ...
            bqh_offset = btc.tile_batch_offset * atp.s_active_qh
            dbg_zero = sbm.alloc_stack(
                (TC.p_max, 1), dtype=bufs.qk.dtype, buffer=nl.sbuf
            )
...
def _cascaded_max_reduce(
    ...
    bufs.qk_max = sbm.alloc_stack(
        (TC.p_max, atp.s_active_bqh), dtype=atp.inter_type, buffer=nl.sbuf, align=4
    )
    ...
    atp.softmax_final_reduction_length = (
        1 + (atp.sprior_n_prgs > 1 and not atp.use_fa) + should_prep_sink
    )
    atp.softmax_final_reduction_local_idx = (
        0  # The reduction result from local qk goes to 1st entry.
    )
    ...
    if atp.s_active_bqh_remainder > 0:
        _transpose_max_psum(
            atp.n_bsq_full_tiles, atp.s_active_bqh_remainder, atp, TC, bufs, sbm
        )
    sbm.close_scope()
    ...
    if atp.sprior_n_prgs > 1 and atp.sync_softmax_per_fa_tile:
        ...
        nisa.sendrecv(
            ...
            recv_from_rank=(1 - atp.sprior_prg_id),
            pipe_id=0,
            dma_engine=dma_engine.gpsimd_dma
            if cfg.use_gpsimd_sb2sb
            else dma_engine.dma,
        )
    ...
    if atp.s_active_bqh_remainder > 0:
        _transpose_broadcast_max(
            atp.n_bsq_full_tiles, atp.s_active_bqh_remainder, atp, TC, sbm, bufs
        )
    ...
    if DBG_TENSORS and not atp.use_fa and atp.bs == atp.bs_per_nc:
        ...
        qk_max_dbg = sbm.alloc_stack(
            (atp.n_bsq_tiles, atp.s_active_bqh_tile), dtype=bufs.qk_max_buf.dtype
        )
        nisa.nc_transpose(
            qk_max_dbg_psum,
            bufs.qk_max_buf[
        ...
        if not atp.max_negated:
            nisa.tensor_scalar(
                qk_max_dbg, qk_max_dbg_psum, op0=nl.multiply, operand0=-1
            )
        else:
        ...
        if atp.n_bsq_full_tiles > 0 and atp.s_active_bqh_remainder > 0:
            zeros = sbm.alloc_stack(
                (1, atp.s_active_bqh_tile - atp.s_active_bqh_remainder),
                dtype=bufs.qk_max_buf.dtype,
            )
            nisa.memset(zeros, 0)
            nisa.dma_copy(
        ...
        dbg_qk_max_view = TensorView(bufs.DBG_QK_MAX).select(0, atp.bs_prg_id)
        dbg_zero = sbm.alloc_stack(
            (dbg_qk_max_view.shape[0], 1), dtype=bufs.qk_max_buf.dtype, buffer=nl.sbuf
        )
        nisa.memset(dbg_zero, 0.0)
        nisa.dma_copy(
            dbg_qk_max_view.get_view(),
...
def _transpose_max_psum(
    ...
    qk_max_psum = nl.ndarray(
        ...
        address=None
        if sbm.is_auto_alloc()
        else (0, (index % TC.psum_b_max) * TC.psum_f_max_bytes),
    )
    nisa.nc_transpose(
        qk_max_psum,
...
def _compute_exp_qk(
    ...
    if nisa.get_nc_version() >= nisa.nc_version.gen4:
        for i_s_prior in range(fa_tile_n_sprior):
            ...
            nisa.activation(
                qk_io_type_view.get_view(), op=nl.exp, data=qk_view.get_view()
            )
    else:
        qk_max_view = (
            TensorView(bufs.qk_max).expand_dim(1).broadcast(1, fa_tile_n_sprior)
        )
    ...
    if (
        DBG_TENSORS
        and not atp.use_fa
        and not (cfg.strided_mm1 and atp.bs != atp.bs_per_nc)
    ):
        ...
        nisa.dma_copy(
            ...
            bufs.qk_io_type.reshape(
                (TC.p_max, 1, fa_tile_n_sprior, 1, atp.s_active_bqh)
            ),
            dge_mode=dge_mode.none,
        ...
        dbg_zero = sbm.alloc_stack(
            (TC.p_max, 1), dtype=bufs.qk_io_type.dtype, buffer=nl.sbuf
        )
        nisa.memset(dbg_zero, 0.0)
        dbg_zero_bc = (
            TensorView(dbg_zero)
            .reshape_dim(1, [1, 1, 1, 1])
            .broadcast(2, atp.n_sprior_tile)
            .broadcast(4, full_bqh)
        )
        nisa.dma_copy(
...
def _cascaded_sum_reduction(
    ...
    if not atp.use_fa:
        ...
        bufs.exp_sum_recip = sbm.alloc_stack(
            (TC.p_max, atp.s_active_bqh), dtype=atp.inter_type, buffer=nl.sbuf
        )

    sbm.open_scope()
    for i_bsq_tile in range(atp.n_bsq_full_tiles):
        _tile_sum_reduction(
            i_bsq_tile, atp.s_active_bqh_tile, fa_tile_n_sprior, atp, TC, bufs, sbm
        )
    sbm.close_scope()
    ...
    if atp.s_active_bqh_remainder > 0:
        sbm.open_scope()
        _tile_sum_reduction(
            atp.n_bsq_full_tiles,
            atp.s_active_bqh_remainder,
            fa_tile_n_sprior,
            atp,
            TC,
            bufs,
    ...
    if DBG_TENSORS and fa_ctx.is_last_fa_tile and atp.bs == atp.bs_per_nc:
        ...
        exp_sum_dbg = sbm.alloc_stack(
            (atp.n_bsq_tiles, atp.s_active_bqh_tile), dtype=bufs.exp_sum.dtype
        )
        nisa.nc_transpose(
            exp_sum_dbg_psum,
        ...
        dbg_exp_sum_view = TensorView(bufs.DBG_EXP_SUM).select(0, atp.bs_prg_id)
        dbg_zero = sbm.alloc_stack(
            (dbg_exp_sum_view.shape[0], 1), dtype=bufs.exp_sum.dtype, buffer=nl.sbuf
        )
        nisa.memset(dbg_zero, 0.0)
        nisa.dma_copy(
...
def _tile_sum_reduction(
    ...
    for i_exp_reduce in range(tile_n_sprior):
        sum_reduce_psum_view = TensorView(sum_reduce_psum).slice(
            1, start=i_exp_reduce, end=i_exp_reduce + 1
        )
...
def _s_active_bqh_tile_transpose_broadcast(
    src, dst, atp: AttnTileParams, TC: TileConstants
):
    ...
    """
    for i_bsq_tile in range(atp.n_bsq_full_tiles):
        _column_tile_transpose(
            src, dst, i_bsq_tile, atp.s_active_bqh_tile, atp.s_active_bqh_tile, TC
        )
    if atp.s_active_bqh_remainder > 0:
        _column_tile_transpose(
            src,
...
def _compute_pv_matmul_and_store(
    ...
    # --- D256 PATCH (b): head_dim is the MM2 stationary-free dim that lands on the
    # output partition. exp_v carries head_dim on the D_TILES axis as
    # [128, D_TILES, bs, s_active_qh]. The 128-wide partition already satisfies the
    # gpsimd extended-instruction 16-partition alignment, so the AWS
    # pad_partitions_for_ext_inst(d_head) padding is unnecessary here. AWS shape was
    # [d_head, bs, s_active_qh]. ---
    d_tiles = _d_tiles(cfg.d_head)
    bufs.exp_v = sbm.alloc_stack(
        (TC.p_max, d_tiles, atp.bs, atp.s_active_qh),
        dtype=atp.inter_type,
        buffer=nl.sbuf,
    )

    batch_interleave_degree_safe = _get_safe_batch_interleave_degree(
        cfg.d_head * fa_tile_n_sprior * sizeinbytes(v_prior.dtype),
        atp.batch_interleave_degree,
        sbm,
    )
    ...
    for i_b in range(atp.bs):
        ...
        if atp.is_block_kv:
            ...
            v_prior_view = (
                ...
                .slice(
                    0,
                    start=s_prior_pos,
                    end=s_prior_pos + (TC.p_max * fa_tile_n_sprior),
                )
            ...
            if atp.is_block_kv:
                ...
                if extra_covered > 0:
                    if atp.block_len > extra_covered:
                        v_sb_view = (
                            ...
                            .slice(
                                0,
                                start=v_sb_partition_base,
                                end=v_sb_partition_base + 1,
                            )
                        ...
                        v_active_reshaped_view = (
                            ...
                            .slice(
                                0,
                                start=v_active_reshaped_batch_pos,
                                end=v_active_reshaped_batch_pos + 1,
                            )
                    ...
                    if num_blks_covering_s_active > 1:
                        v_sb_view = (
                            ...
                            .slice(
                                0,
                                start=v_sb_partition_base + 1,
                                end=v_sb_partition_base + num_blks_covering_s_active,
                            )
                            .reshape_dim(1, [fa_tile_n_sprior, cfg.d_head])
                            .slice(
                                1,
                        ...
                        v_active_reshaped_view = (
                            ...
                            .slice(
                                ...
                                + atp.block_len * (num_blks_covering_s_active - 1),
                            )
                            .reshape_dim(
                                0, [(num_blks_covering_s_active - 1), atp.block_len]
                            )
                        )
                        nisa.dma_copy(
                            dst=v_sb_view.get_view(),
                    ...
                    v_active_reshaped_view = (
                        ...
                        .reshape_dim(0, [cfg.s_active, cfg.d_head])
                        .slice(
                            0, start=0, end=atp.block_len * num_blks_covering_s_active
                        )
                ...
                if load1_nrows > 0:
                    ...
                    v_sb_view = (
                        ...
                        .slice(
                            1,
                            start=fa_tile_n_sprior - load1_nrows,
                            end=fa_tile_n_sprior,
                        )
                    )
                    ...
                    v_active_view = (
                        TensorView(v_active)
                        .select(0, btc.global_batch_offset + i_b)
                        .slice(1, start=0, end=load1_nrows)
                    )
                ...
                if load2_nrows > 0:
                    ...
                    v_sb_view = (
                        ...
                        .slice(
                            0,
                            start=load2_pidx,
                            end=load2_pidx + load2_nrows // fa_tile_n_sprior,
                        )
                        .reshape_dim(1, [fa_tile_n_sprior, cfg.d_head])
                    )
                    ...
                    v_active_view = (
                        ...
                        .reshape_dim(
                            0, [load2_nrows // fa_tile_n_sprior, fa_tile_n_sprior]
                        )
                    )
        ...
        # --- D256 PATCH (b): head_dim is the stationary-free dim that lands on the
        # output partition. Allocate D_TILES PSUM halves [128, D_TILES, s_active_qh];
        # half dt reduces over s_prior from V's dt head_dim slice. AWS used one
        # [d_head, s_active_qh] PSUM. ---
        exp_v_psum = nl.ndarray(
            (TC.p_max, d_tiles, atp.s_active_qh),
            dtype=nl.float32,
            buffer=nl.psum,
            address=None
            if sbm.is_auto_alloc()
        ...
        for i_t in range(fa_tile_n_sprior):
            ...
            qk_io_type_view = (
                ...
                .slice(
                    1,
                    start=batch_s_active_qh_pos,
                    end=batch_s_active_qh_pos + atp.s_active_qh,
                )
            )
            # --- D256 PATCH (b): one matmul per head_dim tile; the stationary is the
            # dt 128-wide slice of V (head_dim on free -> output partition). Each half
            # reduces over s_prior independently (accumulating across i_t). ---
            for dt in range(d_tiles):
                d_lo = dt * TC.p_max
                d_sz = min(TC.p_max, cfg.d_head - d_lo)
                v_sb_view = (
                    ...
                    .reshape_dim(1, [fa_tile_n_sprior, cfg.d_head])
                    .select(1, i_t)
                    .slice(1, start=d_lo, end=d_lo + d_sz)
                )
                nisa.nc_matmul(
                    exp_v_psum[0:d_sz, dt, :],
        ...
        # --- D256 PATCH (b): copy/normalize each head_dim tile half. exp_sum_recip
        # is [128, s_active_bqh] broadcast-identical across partitions, so every
        # head_dim half multiplies by the SAME recip sliced to 128 partitions
        # (AWS sliced [0:d_head], which would exceed 128 for d_head=256). ---
        for dt in range(d_tiles):
            d_lo = dt * TC.p_max
            d_sz = min(TC.p_max, cfg.d_head - d_lo)
            exp_v_view = bufs.exp_v[0:d_sz, dt, i_b, :]
            if atp.use_fa:
                # For FA: just copy without multiplying by recip(sum)
                nisa.tensor_copy(exp_v_view, exp_v_psum[0:d_sz, dt, :])
            else:
                exp_sum_recip_view = (
    ...
    if atp.use_fa:
        ...
        _gather_and_store_output(
            out, bufs.exp_v, exp_v_sendrecv_gpsimd, atp, cfg, sbm, btc
        )
...
def _gather_and_store_output(
    ...
    Args:
        ...
        res: Result tensor in SBUF. --- D256 PATCH (store): res == bufs.exp_v with
            shape [128, D_TILES, bs, s_active_qh] (head_dim on the D_TILES axis);
            AWS used [d_head, s_active_bqh].
        btc: Batch tile context for correct output offset
    """
    # --- D256 PATCH (store): head_dim is tiled across the D_TILES axis. The
    # d_head>128 patch covers the out_in_sb store + s_prior-sharded gather (our
    # config). The HBM store and the batch-sharded sendrecv below keep head_dim on
    # the partition axis and are NOT d256-patched -- they stay dormant because for
    # d_head>128 we require out_in_sb and bs (so bs_n_prgs==1). ---
    d_tiles = _d_tiles(cfg.d_head)
    kernel_assert(
        cfg.d_head <= _D_HEAD_TILE or (cfg.out_in_sb and atp.bs_n_prgs == 1),
        "head_dim>128 requires out_in_sb=True and no batch sharding (bs_n_prgs==1).",
    )
    sbm.open_scope()
    # Gather and add partial results from other NC if sprior is sharded.
    # res partition is 128 (head_dim is on the free D_TILES axis), so the sendrecv
    # and the add operate on the whole [128, D_TILES*bs*s_active_qh] tile unchanged.
    ...
    # Store to output
    if cfg.out_in_sb:
        # --- D256 PATCH (store): out is [128, D_TILES, B*H*s_active]; copy each
        # head_dim tile half. The source res[:, dt, :, :] keeps the D_TILES gap in
        # its partition stride, so we copy it as a 3D [d_sz, bs, s_active_qh] tile
        # into a matching 3D view of the out slice -- a free-axis reshape on the
        # strided source would mis-derive the partition step. AWS did one
        # [d_head]-wide copy. ---
        out_offset = btc.global_batch_offset * atp.s_active_qh
        for dt in range(d_tiles):
            d_sz = min(_D_HEAD_TILE, cfg.d_head - dt * _D_HEAD_TILE)
            out_view = (
                TensorView(out)
                .select(1, dt)
                .slice(1, start=out_offset, end=out_offset + atp.s_active_bqh)
                .reshape_dim(1, [atp.bs, atp.s_active_qh])
                .slice(0, start=0, end=d_sz)
            )
            nisa.tensor_copy(out_view.get_view(), src=res[0:d_sz, dt, :, :])
        if atp.bs_n_prgs > 1:
            dst_bs_offset = (
                (1 - atp.bs_prg_id) * atp.bs_per_nc + btc.tile_batch_offset
            ) * atp.s_active_qh
            nisa.sendrecv(
...
def _apply_rope(
    ...
    x = (
        x_inp
        if cfg.qk_in_sb
        else sbm.alloc_stack((d_head, x_f), dtype=nl.float32, buffer=nl.sbuf)
    )
    ...
    if not cfg.qk_in_sb:
        ...
        nisa.dma_copy(
            ...
            x_inp,
            dge_mode=dge_mode.none,
            name=f"rope_x_inp_load_{name_suffix}",
        )
    ...
    cos_view = (
        TensorView(cos)
        .reshape_dim(1, [bs, s_active])
        .expand_dim(2)
        .broadcast(2, size=n_head)
    )
    nisa.tensor_tensor(
        dst=x_cos,
    ...
    nisa.tensor_scalar(
        rotated_x[: d_head // 2, :], x[d_head // 2 :, :], op0=nl.multiply, operand0=-1.0
    )
...
def _rope(inv_freqs, pos_ids, bs: int, s_a: int, d_head: int, sbm: SbufManager):
    ...
    kernel_assert(
        d_head % 64 == 0, f"RoPE expects head dim ({d_head}) to be divisible by 64"
    )
    d_head_half = d_head // 2

    # Create outputs
    cos = sbm.alloc_stack(
        (d_head, bs * s_a), dtype=nl.float32, buffer=nl.sbuf, name="name_cos"
    )
    sin = sbm.alloc_stack(
        (d_head, bs * s_a), dtype=nl.float32, buffer=nl.sbuf, name="sin_rope"
    )

    # Compute freqs = dot(inv_freqs, pos_ids), can be simplified to elem-wise multiply
    emb = sbm.alloc_stack(
        (d_head_half, bs * s_a), dtype=nl.float32, buffer=nl.sbuf, name="emb_rope"
    )
    nisa.tensor_scalar(
        emb, pos_ids[0:d_head_half, :], op0=nl.multiply, operand0=inv_freqs
    )
    ...
    emb4sin = sbm.alloc_stack(
        (d_head, bs * s_a), dtype=nl.float32, buffer=nl.sbuf, name="eb4sin_rope"
    )
    ...
    nisa.tensor_copy(
        emb4sin[d_head_half : d_head_half + d_head_half, :], emb4sin[0:d_head_half, :]
    )
    ...
    emb4cos = sbm.alloc_stack(
        (d_head, bs * s_a), dtype=nl.float32, buffer=nl.sbuf, name="emb4cos_rope"
    )
    nisa.tensor_scalar(
        emb4cos[0:d_head_half, :], emb, op0=nl.add, operand0=1.5 * math.pi
    )
    ...
    nisa.tensor_copy(
        emb4cos[d_head_half : d_head_half + d_head_half, :], emb4cos[0:d_head_half, :]
    )
...
def _prep_sink(
    ...
    sink_repeated = sbm.alloc_stack(
        (1, atp.s_active_bqh), buffer=nl.sbuf, dtype=sink_hbm.dtype
    )
    sink_repeated_view = TensorView(sink_repeated).reshape_dim(
        1, [atp.bs, cfg.q_head, cfg.s_active]
    )
    sink_sb_view = (
        TensorView(sink_sb)
        .expand_dim(2)
        .broadcast(2, size=cfg.s_active)
        .expand_dim(1)
        .broadcast(1, size=atp.bs)
    )
    nisa.tensor_copy(
    ...
    # And transpose.
    sink_tp_repeated = sbm.alloc_stack(
        (atp.s_active_bqh_tile, atp.n_bsq_tiles), buffer=nl.sbuf, dtype=sink_hbm.dtype
    )
  ...
  """
    if resize_factor == 1:
        ...
        active_blk_table_tv = (
            ...
            .reshape_dim(
                1, [n_prgs, full_num_folds, partition_resize]
            )  # [B, n_prgs, folds, 128 / folds]
            .slice(
                0, batch_start, batch_start + batch_size
            )  # [b, n_prgs, folds, 128 / folds]
            ...
            .broadcast(
                1, resize_factor
            )  # [128 / folds, resize_factor, num_folds_this_tile, b]
        )
        ...
        for batch_offset in range(0, batch_size, batch_chunk):
            ...
            active_blk_pre_reshape = sbm.alloc_stack(
                (cur_batch_chunk_sz, orig_blks_this_tile),
                dtype=active_blk_table.dtype,
                buffer=nl.sbuf,
            )
            active_blk_table_slice = (
                ...
                .slice(
                    0,
                    start=batch_start + batch_offset,
                    end=batch_start + batch_offset + cur_batch_chunk_sz,
                )
                .slice(
                    1,
                    start=prg_id * blks_per_prg + orig_blk_start,
                    end=prg_id * blks_per_prg + orig_blk_end,
                )
            )
            nisa.dma_copy(
            ...
            reshape_arange = sbm.alloc_stack(
                (cur_batch_chunk_sz, resize_factor),
                dtype=active_blk_table.dtype,
                buffer=nl.sbuf,
            )
            ...
            active_blk_reshaped = sbm.alloc_stack(
                (cur_batch_chunk_sz, orig_blks_this_tile, resize_factor),
                dtype=nl.float32,
                buffer=nl.sbuf,
            )
            active_blk_pre_reshape_view = (
                TensorView(active_blk_pre_reshape)
                .expand_dim(2)
                .broadcast(2, size=resize_factor)
            )
            reshape_arange_view = (
                TensorView(reshape_arange)
                .expand_dim(1)
                .broadcast(1, size=orig_blks_this_tile)
            )
            ...
            active_blk_reshaped = active_blk_reshaped.reshape(
                (cur_batch_chunk_sz, num_folds_this_tile * TC.p_max)
            )
            ...
            for fold_rel_idx in range(num_folds_this_tile):
                active_blk_transposed = nl.ndarray(
                    ...
                    address=None
                    if sbm.is_auto_alloc()
                    else (0, (fold_rel_idx % TC.psum_b_max) * TC.psum_f_max_bytes),
                )
                nisa.nc_transpose(
                ...
                nisa.tensor_copy(
                    active_blk_table_sb[
                        :,
                        nl.ds(
                            fold_rel_idx * batch_size + batch_offset, cur_batch_chunk_sz
                        ),
                    ],
    ...
    if bufs.DBG_ACTIVE_TABLE is not None:
        dbg_fold_offset = (
            atp.sprior_prg_id * (atp.s_prior // (atp.block_len * TC.p_max)) + fold_start
        )
        bufs.active_blocks_sb = active_blk_table_sb.reshape(
            (TC.p_max, num_folds_this_tile, batch_size)
        )
        nisa.dma_copy(
...
def _get_safe_batch_interleave_degree(
    space_needed_per_batch: int, max_batch_interleave_degree: int, sbm: SbufManager
):
    """
    ...
    kernel_assert(
        max_batch_interleave_degree > 0,
        "batch_interleave_degree must be greater than 0",
    )
...
def pad_partitions_for_ext_inst(partitions):
    ...
    return (
        (partitions + PARTITIONS_PER_GPSIMD_CORE - 1)
        // PARTITIONS_PER_GPSIMD_CORE
        * PARTITIONS_PER_GPSIMD_CORE
    )
...
def _clamp_max_to_finite(dst: nl.ndarray, src: nl.ndarray, max_negated: bool = False):
    ...
    """
    op, bound = (
        (nl.minimum, _MAX_FLOAT32) if max_negated else (nl.maximum, _MIN_FLOAT32)
    )
